$.widget("bw.bwWorkflowEditor_NewTenant", {
    options: {
        ...
        */
        ...
        /*
        ===========================================================
        This is the bwWorkflowEditor_NewTenant.js jQuery Widget. 
        ===========================================================
        ...
        */
        ...
        bwEnabledRequestTypes: ['Budget Request'],
    ...
    _create: function (assignmentRowChanged_ElementId) {
        this.element.addClass("bwWorkflowEditor_NewTenant");
        ...
        try {
            ...
            if (this.options.CurrentWorkflow != null) {
                ...
                var promise = this.loadWorkflowsAndCurrentWorkflow(); // This is the default.
                promise.then(function (results) {
                    try {

                        if (results.status != 'SUCCESS') {

                            console.log('Error in bwWorkflowEditor_NewTenant._create(). ' + results.status + ': ' + results.message);
                            displayAlertDialog('Error in bwWorkflowEditor_NewTenant._create(). ' + results.status + ': ' + results.message);
                        ...
                        console.log('Exception in bwWorkflowEditor_NewTenant._create().loadWorkflowsAndCurrentWorkflow(): ' + e.message + ', ' + e.stack);
                        thiz.element.html('Exception in bwWorkflowEditor_NewTenant._create().loadWorkflowsAndCurrentWorkflow(): ' + e.message + ', ' + e.stack);
                    }
                });
            }
            ...
            var html = '';
            html += '<span style="font-size:24pt;color:red;">bwWorkflowEditor_NewTenant: CANNOT RENDER THE WORKFLOW MATRIX</span>';
            html += '<br />';
            html += '<span style="">Exception in bwWorkflowEditor_NewTenant.Create(): ' + e.message + ', ' + e.stack + '</span>';
    ...
    displayEditWorkflowJsonDialog: function (bwWorkflowId) {
        try {
            console.log('In bwWorkflowEditor_NewTenant.js.displayEditWorkflowJsonDialog(). bwWorkflowId: ' + bwWorkflowId);
            alert('THE METHOD displayEditWorkflowJsonDialog() SHOULD BE REPLACED WITH THEONE FROM bwWorkflowEditor.js ??? <<<< In bwWorkflowEditor_NewTenant.js.displayEditWorkflowJsonDialog(). bwWorkflowId: ' + bwWorkflowId);
            // THIS WORKS JUST SHUT OFF FOR THE MOMENT 5-12-2020
            var thiz = this;
            $("#bwWorkflowEditor_NewTenant_divDisplayJsonDialog").dialog({
                modal: true,
                resizable: false,
                ...
                open: function () {
                    try {
                        $('.ui-widget-overlay').bind('click', function () {
                            $('#bwWorkflowEditor_NewTenant_divDisplayJsonDialog').dialog('close');
                        });

                        document.getElementById('bwWorkflowEditor_NewTenant_spanDisplayJsonDialogTitle').innerHTML = 'Edit Workflow JSON';
                        document.getElementById('bwWorkflowEditor_NewTenant_spanDisplayJsonDialogContent').innerHTML = 'Workflow: ' + bwWorkflowId;

                        document.getElementById('bwWorkflowEditor_NewTenant_btnRunDiagnostics').innerHTML = '<span style="font-size:30pt;">⚠ Save/Update Workflow JSON</span>';

                        $('#bwWorkflowEditor_NewTenant_btnRunDiagnostics').bind('click', function () {
                            console.log('In displayEditWorkflowJsonDialog.bwWorkflowEditor_NewTenant_btnRunDiagnostics.click().');
                            alert('In displayEditWorkflowJsonDialog.bwWorkflowEditor_NewTenant_btnRunDiagnostics.click().');

                            // This is where we validate the JSON and save it back to the database, activating the new workflow JSON/Definition.
                            var bwWorkflowJson = document.getElementById('bwWorkflowEditor_NewTenant_txtDisplayJsonDialogJSON').value;
                            var validJson = true;
                            try {
                            ...
                            if (validJson != true) {
                                ...
                                //alert('VALID JSON');
                                if (confirm("Are you certain you wish to activate this workflow? Save a copy in notepad in case something goes wrong!")) {
                                    ...
                                    $.ajax({
                                        url: thiz.options.operationUriPrefix + "odata/publishworkflow2_NewTenant", // ???????????????????????????????????????????????????????????????????????????????????????????????????
                                        type: "Post",
                                        data: json,
                                        headers: {
                                        ...
                                        try {
                                            //debugger;
                                            if (result == 'SUCCESS') {
                                                console.log('In bwWorkflowEditor_NewTenant.js.displayEditWorkflowJsonDialog().post: Successfully updated DB. result: ' + JSON.stringify(result)); // using (' + JSON.stringify(json) + ').');
                                                alert('In bwWorkflowEditor_NewTenant.js.displayEditWorkflowJsonDialog().post: Successfully updated DB.');
                                            } else {
                                            ...
                                            //thiz.options.CurrentWorkflow.DraftWorkflow = JSON.parse(JSON.stringify(thiz.options.CurrentWorkflow.DraftWorkflow)); 
                                            //thiz.checkIfWeHaveToDisplayThePublishChangesButton();

                                        } catch (e) {
                                            console.log('Exception in bwWorkflowEditor_NewTenant.js.displayEditWorkflowJsonDialog().xx.update: ' + e.message + ', ' + e.stack);
                                            displayAlertDialog('Exception in bwWorkflowEditor_NewTenant.js.displayEditWorkflowJsonDialog().xx.update: ' + e.message + ', ' + e.stack);
                                        }
                                        ...
                                        console.log('Fail in bwWorkflowEditor_NewTenant.js.displayEditWorkflowJsonDialog().xx.update: ' + JSON.stringify(data) + ', json: ' + JSON.stringify(json));
                                        displayAlertDialog('Fail in bwWorkflowEditor_NewTenant.js.displayEditWorkflowJsonDialog().xx.update: ' + msg); //+ error.message.value + ' ' + error.innererror.message);
                        ...
                        // 1-5-2022 WE DONT HAVE TO REACH OUT use this.options.Workflows <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<


                        //debugger; // bwWorkflowId
                        ...
                        for (var i = 0; i < thiz.options.Workflows.length; i++) {
                            if (bwWorkflowId == thiz.options.Workflows[i].bwWorkflowId) {
                                ...
                                $('#bwWorkflowEditor_NewTenant_txtDisplayJsonDialogJSON').empty();
                                ...
                                $('#bwWorkflowEditor_NewTenant_txtDisplayJsonDialogJSON').val(JSON.stringify(json, null, 2));
                                //$('#txtDisplayJsonDialogJSON').append(JSON.stringify(json)); // 1-6-2022

                                //$('#txtDisplayJsonDialogJSON').append(thiz.options.Workflows[i].bwWorkflowJson);
                                break;
                            }
                        }
                        ...
                        //$.ajax({
                        //    url: thiz.options.operationUriPrefix + "odata/WorkflowConfiguration_NewTenant/" + bwWorkflowId,
                        //    dataType: "json",
                        ...
                        //    }
                        //    console.log('Exception in bwWorkflowEditor_NewTenant.js.displayEditWorkflowJsonDialog().xx.Get: ' + JSON.stringify(data));
                        //    displayAlertDialog('Exception in bwWorkflowEditor_NewTenant.js.displayEditWorkflowJsonDialog().xx.Get: ' + msg); //+ error.message.value + ' ' + error.innererror.message);
                ...
                close: function () {
                    $('#bwWorkflowEditor_NewTenant_divDisplayJsonDialog').dialog('destroy');
                }
            });
    ...
    loadWorkflowsAndCurrentWorkflow: function (bwRequestTypeId) { // 12-20-2022 just added parameter "bwRequestTypeId"
        try {
            console.log('In bwWorkflowEditor_NewTenant.js.loadWorkflowsAndCurrentWorkflow().');
            //alert('In bwWorkflowEditor_NewTenant.js.loadWorkflowsAndCurrentWorkflow(). This needs work!!!');
            var thiz = this;

            //
            // THIS IS THE ADMIN NEW TENANT FUNCTIONALITY. 6-1-2020
            //
            return new Promise(function (resolve, reject) {
                ...
                $.ajax({
                    url: thiz.options.operationUriPrefix + "_bw/NewTenantWorkflowsSettings",
                    type: "POST",
                    data: data,
                    headers: {
                        "Accept": "application/json; odata=verbose"
                    ...
                    try {
                        //debugger;
                        console.log('In bwWorkflowEditor_NewTenant.js.loadWorkflowsAndCurrentWorkflow(). We dont set the CurrentWorkflow here....');

                        if (results.status != 'SUCCESS') {

                            //alert('xcx23489-2 results: ' + JSON.stringify(results));

                            resolve(results);

                        } else {

                            thiz.options.CurrentWorkflow = null; // Reset this here.

                            thiz.options.bwEnabledRequestTypes = results.RequestTypes;
                            thiz.options.Workflows = results.Workflows; // Loads this so all workflows are available at any time without going back to the server.

                            //displayAlertDialog('xcx23489 results.RequestTypes: ' + JSON.stringify(results.RequestTypes) + ', results.Workflows: ' + JSON.stringify(results.Workflows));

                            var msg = 'In loadWorkflowsAndCurrentWorkflow(). Successfully populated thiz.options.bwEnabledRequestTypes and thiz.options.Workflows.';
                            console.log(msg);
                        ...
                        var msg = 'Exception in bwWorkflowEditor_NewTenant.js.loadWorkflowsAndCurrentWorkflow():2: ' + e.message + ', ' + e.stack;
                    ...
                    var msg = 'Error in bwWorkflowEditor_NewTenant.js.loadWorkflowsAndCurrentWorkflow(): ' + JSON.stringify(data);
                    displayAlertDialog(msg);
            ...
            var msg = 'Exception in bwWorkflowEditor_NewTenant.js.loadWorkflowsAndCurrentWorkflow(): ' + e.message + ', ' + e.stack;
            displayAlertDialog(msg);
    ...
    _destroy: function () {
        this.element
            .removeClass("bwWorkflowEditor_NewTenant")
            .text("");
    },
    ...
    renderWorkflowEditor1: function (assignmentRowChanged_ElementId) {
        try {
            ...
            html += '<div style="display:none;" id="bwWorkflowEditor_NewTenant_divDisplayJsonDialog">';
            html += '  <table style="width:100%;">';
            html += '    <tr>';
            html += '      <td style="width:90%;">';
            html += '        <span id="bwWorkflowEditor_NewTenant_spanDisplayJsonDialogTitle" style="color: #3f3f3f;font-size:30pt;font-weight:bold;">[bwWorkflowEditor_NewTenant_spanDisplayJsonDialogTitle]</span>';
            html += '      </td>';
            html += '      <td style="width:9%;"></td>';
            html += '      <td style="width:1%;cursor:pointer;vertical-align:top;">';
            html += '        <span class="dialogXButton" style="font-family: \'Segoe UI Light\',\'Segoe UI\',\'Segoe\',Tahoma,Helvetica,Arial,sans-serif;font-size:30pt;font-weight:bold;" onclick="$(\'#bwWorkflowEditor_NewTenant_divDisplayJsonDialog\').dialog(\'close\');">X</span>';
            html += '      </td>';
            html += '    </tr>';
            html += '  </table>';
            html += '  <br /><br />';
            ...
            html += '  <span id="bwWorkflowEditor_NewTenant_spanDisplayJsonDialogContent" style="font-family: \'Segoe UI Light\',\'Segoe UI\',\'Segoe\',Tahoma,Helvetica,Arial,sans-serif;color: #3f3f3f;font-size:20pt;">[bwWorkflowEditor_NewTenant_spanDisplayJsonDialogContent]</span>';
            html += '  <br />';
            html += '  <br />';
            // Copy to clipboard.
            ...
            html += '  <textarea id="bwWorkflowEditor_NewTenant_txtDisplayJsonDialogJSON" rows="30" cols="120" style="padding-top:4px;font-size:8pt;"></textarea>';
            ...
            html += '  <br />';
            html += '  <br />';
            html += '  <div id="bwWorkflowEditor_NewTenant_btnRunDiagnostics" class="divDialogButton" title="Click here to run diagnostics...">';
            html += '   Run Diagnostics...';
            html += '  </div>';
            html += '  <br /><br />';
            html += '  <div class="divDialogButton" onclick="$(\'#bwWorkflowEditor_NewTenant_divDisplayJsonDialog\').dialog(\'close\');">';
            html += '   Close';
            html += '  </div>';
            html += '  <br />';
            html += '  <br />';
            ...
            html += '  <input type="button" value="Create your new role now!" style="height:30pt;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'createANewRole\');" />';
            html += '       </td>';
            html += '     </tr>';
            //html += '     <tr><td>&nbsp;</td></tr>';
            //html += '     <tr>';
            ...
            //html += '  <br /><br />';
            //html += '  <input type="button" value="SAVE" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'roleMultiPickerDialog_RenderResults\');" />';
            //html += '  <br /><br />';
            //html += '</div>';
            ...
            html += '  <br /><br />';
            html += '  <input type="button" value="SAVE" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'orgMultiPickerDialog_RenderResults\');" />';
            html += '  <br /><br />';
            html += '</div>';
            ...
            html += '  <br /><br />';
            html += '  <input type="button" value="SAVE" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'projectTypeMultiPickerDialog_RenderResults\');" />';
            html += '  <br /><br />';
            html += '</div>';
            ...
            html += '  <br /><br />';
            html += '  <input type="button" value="SAVE" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'pillarMultiPickerDialog_RenderResults\');" />';
            html += '  <br /><br />';
            html += '</div>';
            ...
            html += '    This workflow has been activated and will immediately be included in new tenant/workflow apps for new sign ups. ';
            html += '    <br />';
            html += '    <br />';
            html += '    <br />';
            ...
            html += '   <br /><br />';
            // Preview button.
            //html += '   <div class="divSignInButton" style="width: 90%; text-align: center; line-height: 1.1em; font-weight: bold;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayEmailPreviewInDialog\');">';
            ...
            //        html += ' &nbsp;&nbsp;&nbsp;';
            //        //html += ' and the email that it sends...&nbsp;';
            ...
            if (thiz.options.displayWorkflowPicker) {
                ...
                //    html += '       <td>';
                //    html += '           <h2>';
                //    html += '           Workflows Editor: <span style="color:#95b1d3;">Configure your workflows and email for this organization...&nbsp;<span class="spanButton context-menu-workflowstep2" id="xcx" style="height:20px;width:150px;"> ... </span></span>'; // Velvet Morning is #95b1d3. This was the pantone color of the day for December 9, 2019! :D
                //    html += '           </h2>';
                //    html += '       </td>';
                //    html += '       <td style="text-align:right;">';
                //    html += '           <span class="printButton" title="print" onclick="cmdPrintForm();">&#x1f5a8;</span>';
                ...
                if (thiz.options.bwEnabledRequestTypes) {
                    ...
                    html += '               <span style="color:black;font-family: \'Segoe UI Light\',\'Segoe UI\',\'Segoe\',Tahoma,Helvetica,Arial,sans-serif;color: #262626;font-size: 35pt;font-weight:bold;">';
                    html += 'Configure the workflow for ';
                    var requestTypes = thiz.options.bwEnabledRequestTypes;
                    html += '<span style="font_weight:bold;color:grey;"><strong>';
                    html += '   <select id="selectRequestTypeDropDown" onchange="$(\'.bwWorkflowEditor\').bwWorkflowEditor(\'WorkflowRequestTypeDropDown_Onchange\', \'selectWorkflowRequestTypeDropDown\');" class="selectHomePageWorkflowAppDropDown" style=\'display:inline;border-color: whitesmoke; color: grey; font-family: "Segoe UI Light","Segoe UI","Segoe",Tahoma,Helvetica,Arial,sans-serif; font-size: 1em; font-weight: bold; cursor: pointer;  margin-bottom:15px;\'>'; // was .5em
                    for (var i = 0; i < requestTypes.length; i++) {
                        var selected = '';
                    ...
                    //var requestTypes = thiz.options.bwEnabledRequestTypes;
                    //alert('xcx123123 requestTypes: ' + JSON.stringify(requestTypes));
                    //if (requestTypes.length == 1) { // If there is only one, don't display as a drop down, just as plain text.
                    //    html += '<span style="font_weight:bold;color:grey;">Active workflow for: </span>';
                    //} else { // There is more than 1, so we have to display as a drop down.
                    //    html += '<span style="font_weight:bold;color:grey;">Active workflow for: ';
                    ...
                    //if (requestTypes.length == 1) { // If there is only one, don't display as a drop down, just as plain text.
                    //    debugger;
                    //    html += '<span style="font_weight:bold;color:black;"><strong>' + requestTypes[0].SingletonName + '</strong></span>'; // bwRequestTypeId
                    //} else { // There is more than 1, so we have to display as a drop down.
                    //    html += '<span style="font_weight:bold;color:black;"><strong>';
                    //    //debugger; // WE SHOPULD ONLY COME HERE ONCE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    //    html += '   <select id="selectWorkflowRequestTypeDropDown" onchange="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'WorkflowRequestTypeDropDown_Onchange\', \'selectWorkflowRequestTypeDropDown\');" style=\'border-color: whitesmoke; color: grey; font-family: "Segoe UI Light","Segoe UI","Segoe",Tahoma,Helvetica,Arial,sans-serif; font-size: 14pt; font-weight: bold; cursor: pointer;\'>'; // was .5em
                    //    //html += '<option value="' + 'all' + '" selected >' + 'All request types' + '</option>';
                    //    //debugger;
                    //    for (var i = 0; i < requestTypes.length; i++) {
                    //        if (requestTypes[i].bwRequestTypeId == thiz.options.LastSelectedRequestType.bwRequestTypeId) { //bwLastSelectedRequestType) { // Selected
                    //            html += '<option value="' + requestTypes[i].bwRequestTypeId + '" selected >' + requestTypes[i].SingletonName + '</option>';
                    //        } else { // Not selected
                    ...
                    html += '<span style="font_weight:bold;color:grey;">Active workflow for: NONE FOUND XCX12345 In bwWorkflowEditor_NewTenant.js.renderWorkflowEditor1(). thiz.options.bwEnabledRequestTypes: ' + thiz.options.bwEnabledRequestTypes + '</span>';
                }
                ...
                // We need to decide if we show the checkbox. If there are more than 1 active workflows, then disable/hide this checkbox!!!!!!!!!!!!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                if (thiz.options.MultipleWorkflowsEnabled == false) {
                    html += '&nbsp;&nbsp;<span style="font-weight:normal;font-style:italic;color:grey;white-space:nowrap;"><input id="WorkflowForAllRequestTypesCheckbox" type="checkbox" checked="checked" onchange="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'WorkflowForAllRequestTypesCheckbox_Onchange\');" />All request types inherit this workflow</span>';
                } else {
                    //html += '&nbsp;&nbsp;<span style="font-weight:normal;font-style:italic;color:grey;white-space:nowrap;">Each request type has it\'s own workflow</span>';
                }

                html += '<span title="Workflow(s) maintenance..." style="width:200px;padding:5px 10px 5px 10px;margin:0 0 0 10px;white-space:nowrap;vertical-align:middle;border:1px solid lightblue;cursor:pointer;font-weight:normal;font-size:10pt;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayWorkflowsConfigurationDialog\');"><span style="display:inline-block;"> ⚙ </span></span>';
            ...
            console.log('EXCEPTION IN renderWorkflowEditor1: ' + e.message + ', ' + e.stack);
        }
    },
        ...
        try {
            ...
            if (!bwRequestTypeId) { //if (!bwRequestTypeId || (bwRequestTypeId == null)) {
                ...
                if (!car) {
                    ...
                    var html = '';
                    html += '<span style="color:tomato;">Error: No workflow was found for this Request Type. Create one by customizing this default workflow, and clicking the Publish button. xcx4236.</span>';
                    ...
                    bwWorkflowJson["bwRequestTypeId"] = bwRequestTypeId;
                    //
                    ...
                    //thiz.options.Workflows.push(xx); // 
                ...
                //var weFoundTheWorkflow = true;
                //if (!(car && car.bwWorkflowJson)) { //.Workflow) {

                //    if (car.Workflow) {
                //        car["bwWorkflowJson"] = JSON.stringify(car.Workflow);
                //        weFoundTheWorkflow = true;
                //    } else {
                //        debugger;
                //        displayAlertDialog('NO WORKFLOW JSON. CANNOT PROCEED. xcx32456-5-2');




                //        //
                //        // THIS IS A PROBLEM. WE SHOULD NEVER GET HERE?????????????? 1-4-2022 It is like the car get partially loaded... no Worfklow or bwWorkflowJson attribute...
                //        //
                //        console.log('THIS IS A PROBLEM. WE SHOULD NEVER GET HERE?????????????? 1-4-2022 It is like the car get partially loaded... no Worfklow or bwWorkflowJson attribute...');
                ...
                //if (weFoundTheWorkflow == true) {

                //var car = this.options.CurrentWorkflow; ////JSON.parse(this.options.CurrentWorkflow.bwWorkflowJson);
                debugger;
                ...
                $.ajax({
                    url: this.options.operationUriPrefix + "odata/Roles", // Get the eCarWorkflow json/definition from the database.
                    dataType: "json",
                    contentType: "application/json",
                    type: "Get",
    ...
    //renderWorkflowEditor2: function (assignmentRowChanged_ElementId) {
    //    try {
    //        console.log('In renderWorkflowEditor2().');
    //        var thiz = this;
    ...
    //        var bwRequestTypeId, bwRequestType, bwWorkflowId;
    //        $('#selectRequestTypeDropDown').find('option:selected').each(function (index, element) {

    //            debugger;
    //            bwRequestTypeId = element.value;

    //            for (var i = 0; i < thiz.options.bwEnabledRequestTypes.length; i++) {
    //                if (bwRequestTypeId == thiz.options.bwEnabledRequestTypes[i].bwRequestTypeId) {
    //                    bwRequestType = thiz.options.bwEnabledRequestTypes[i].SingletonName;
    //                    break;
    //                }
    //            }
    ...
    //                    bwWorkflowId = thiz.options.Workflows[i].bwWorkflowId;
    //                }
    //            }
    ...
    //            console.log('Error in bwWorkflowEditor_NewTenant.js.renderWorkflowEditor2(). Invalid value(s). bwRequestTypeId: ' + bwRequestTypeId);
    //            displayAlertDialog('Error in bwWorkflowEditor_NewTenant.js.renderWorkflowEditor2(). Invalid value(s). bwRequestTypeId: ' + bwRequestTypeId);
    ...
    //                    //alert('xcx1112-1 bwRequestTypeId: ' + bwRequestTypeId + ', thiz.options.Workflows[i]: ' + JSON.stringify(thiz.options.Workflows[i]));
    //                    if ((thiz.options.Workflows[i].bwRequestTypeId == bwRequestTypeId)) { // && thiz.options.Workflows[i].bwWorkflowActive == true) { //if (bwRequestTypeId == this.options.Workflows[i].bwRequestTypeId) {
    //                        //alert('xcx1112-2');
    //                        if (thiz.options.Workflows[i].bwWorkflowJson) {
    //                            //alert('xcx1112-3');
    //                            var bwWorkflowJson = thiz.options.Workflows[i].bwWorkflowJson;
    ...
    //            }).done(function (result) {
    //                try {
    //                    //console.log('In Get[odata/Roles]. result: ' + JSON.stringify(result));
    //                    var html = '';
    //                    var roles;
    //                    if (result) {
    //                        roles = result.value;
    ...
    //                        html += '                   <span id="spanExpandOrCollapseRightSliderPaneButton" style="cursor:pointer;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'expandOrCollapseSliderPanel\');">';
    //                        html += '                       <span title="Edit email template(s)..." style="width:200px;padding:5px 10px 5px 10px;margin:0 0 0 20px;white-space:nowrap;vertical-align:top;border:1px solid lightblue;cursor:pointer;font-weight:normal;font-size:20pt;">';
    //                        html += '                           <span style="display:inline-block;">+ ⚙✉</span>';
    //                        html += '                       </span>';
    //                        html += '                       <span id="divEmailEditorHorizontalSliderPane"></span>';
    ...
    //                        html += '                       <span title="Expand or Collapse ALL..." onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'expandOrCollapseAllWorkflowSteps\');" style="width:200px;padding:5px 10px 5px 10px;margin:0 0 0 20px;white-space:nowrap;vertical-align:top;border:1px solid lightblue;cursor:pointer;font-weight:normal;font-size:20pt;">';
    //                        html += '                           <span style="display:inline-block;">+/-</span>';
    //                        html += '                       </span>';
    ...
    //                        //if (!car.DraftWorkflow) {

    //                        //    html += '    NO WORKFLOW JSON. CANNOT PROCEED. xcx32456-6';
    //                        //    html += '  </td>';
    //                        //    html += '</tr>';

    //                        //} else {
    //                        //debugger;
    //                        html += '    <input type="button" style="padding:5px 10px 5px 10px;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'addNewWorkflowStep\', \'' + (car.DraftWorkflow.Steps.Step.length - 1) + '\');" value="Add a Step" title="bwWorkflowEditor_NewTenant.addNewWorkflowStep(). Doesn\'t work right yet..." />';
    ...
    //                                    newStepName = 'Admin';
    //                                } else {
    //                                    newStepName = car.DraftWorkflow.Steps.Step[i]["@FriendlyName"];
    //                                }
    ...
    //                                // Display the header row for this step.
    //                                //debugger;
    //                                html += '<tr id="stepheaderrow_' + i + '" style="border:0px;cursor:pointer;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'expandOrCollapseWorkflowStep\', \'' + 'stepname_' + i + '\', \'' + 'stepimage_' + i + '\', \'steprow-' + stepName.toLowerCase() + '_' + i + '_' + '0' + '\');">';
    //                                html += '  <td colspan="11" class="stepheadercell context-menu-workflowstep" style="font-weight:bold;padding:10px;background-color:' + cellColor + ';" title="Right-click to edit the step name..." >';
    //                                html += '    <table style="width:100%;">';
    //                                html += '      <tr>';
    //                                html += '        <td colspan="2" >';
    ...
    //                                        if (car.DraftWorkflow.Steps.Step[i]["@Timeout"]) {
    //                                            // This decides whether to display the timeout in minutes or days.
    //                                            var timeoutMinutes = car.DraftWorkflow.Steps.Step[i]["@Timeout"];
    //                                            var timeoutDays;
    //                                            if (timeoutMinutes > 720) {
    //                                                var minutesInASingleDay = 1440;
    //                                                timeoutDays = timeoutMinutes / minutesInASingleDay;
    ...
    //                                            }
    //                                        }
    //                                        if (car.DraftWorkflow.Steps.Step[i]["@TimeoutUnits"]) {
    //                                            timeoutUnits = car.DraftWorkflow.Steps.Step[i]["@TimeoutUnits"];
    //                                        }
    //                                    } catch (e) {
    //                                        console.log('In bwWorkflowEditor_NewTenant.js(). Exception trying to get the timeout: ' + e.message + ', ' + e.stack);
    //                                    }
    //                                    html += '<span style="white-space:nowrap;font-size:11pt;">';
    //                                    html += 'Consensus timeout:&nbsp;';
    //                                    html += '<input type="text" id="textTimeout" style="width:25px;padding:5px 5px 5px 5px;" value="' + timeout + '" onkeyup="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'saveCollaborationTimeout\', \'' + 'textTimeout' + '\');" />';
    //                                    html += '&nbsp;';
    //                                    html += '<select id="selectTimeoutUnits" style="padding:5px 5px 5px 5px;" onchange="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'saveCollaborationTimeout\', \'' + 'textTimeout' + '\');">';
    //                                    if (timeoutUnits == 'Days') html += '<option value="Days" selected >Days</option>';
    //                                    else html += '<option value="Days">Days</option>';
    //                                    if (timeoutUnits == 'Hours') html += '<option value="Hours" selected >Hours</option>';
    //                                    else html += '<option value="Hours">Hours</option>';
    ...
    //                                } else {
    //                                    html += '&nbsp;&nbsp;&nbsp;&nbsp;';
    //                                    html += '          <input type="button" class="buttonAddNewAssignmentRow" style="white-space:nowrap;padding:5px 10px 5px 10px;" value="✚ Add a Role/Participant" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'addARoleParticipant\', \'' + 'steprow-inform_' + i + '_' + '0' + '\');" />';
    ...
    //                                if (stepName == 'Create' || stepName == 'Revise') { // || stepName == 'Admin') {
    //                                    if (car.DraftWorkflow.Steps.Step[i].Assign) {
    //                                        // THERE WILL ONLY EVER BE 1 node here.
    ...
    //                                        if (thiz.options.displayRoleIdColumn) {
    //                                            html += '<td style="background-color:' + cellColor + ';" class="roleid steprowcell" bwRoleId="' + car.DraftWorkflow.Steps.Step[i].Assign["@Role"] + '" bwOldValue="' + car.DraftWorkflow.Steps.Step[i].Assign["@Role"] + '">' + car.DraftWorkflow.Steps.Step[i].Assign["@Role"] + '</td>';
    //                                        }

    //                                        var roleName = car.DraftWorkflow.Steps.Step[i].Assign["@Role"]; //["@RoleName"];
    //                                        //debugger; // AllowRequestModifications xcx1
    //                                        var allowRequestModifications = car.DraftWorkflow.Steps.Step[i].Assign["@AllowRequestModifications"];
    //                                        html += '<td style="background-color:' + cellColor + ';" class="rolename steprowcell" onMouseOver="this.style.backgroundColor=\'lightgoldenrodyellow\';this.style.color=\'red\';" onMouseOut="this.style.backgroundColor=\'' + cellColor + '\';this.style.color=\'black\';" bwStepname="' + car.DraftWorkflow.Steps.Step[i]["@Name"] + '" bwRoleId="' + car.DraftWorkflow.Steps.Step[i].Assign["@Role"] + '" bwOldValue="' + roleName + '" bwAllowRequestModifications="' + allowRequestModifications + '">';
    //                                        //html += ' <img style="width:18px;height:18px;cursor:pointer;vertical-align:text-bottom;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayPeoplePickerDialog\', \'' + 'txtApprover2FriendlyName' + '\', \'' + 'txtApprover2Id' + '\', \'' + 'txtApprover2Email' + '\');" src="images/addressbook-icon18x18.png">';
    //                                        //html += '&nbsp;' + roleName;
    //                                        html += 'Role: ' + roleName;
    ...
    //                                        html += '<td class="steprowcell">';
    //                                        html += ' <span id="rolecategory-assign_' + i + '_' + j + '" class="rolecategory" bwStepname="' + car.DraftWorkflow.Steps.Step[i]["@Name"] + '" bwRoleId="' + car.DraftWorkflow.Steps.Step[i].Assign["@Role"] + '" bwOldValue="' + car.DraftWorkflow.Steps.Step[i].Assign["@RoleCategory"] + '" style="cursor:cell;" title="Double-click to edit the role category...">';
    //                                        html += car.DraftWorkflow.Steps.Step[i].Assign["@RoleCategory"];
    //                                        html += ' </span>';
    //                                        html += '</td>';

    //                                        // Task(s)/Action(s)
    //                                        debugger;
    //                                        var task = car.DraftWorkflow.Steps.Step[i].Assign.Action["@Name"]; // There is only 1, "Submit".
    //                                        if (!task) task = '';
    //                                        else task = '•&nbsp;' + task;
    //                                        if (stepName == 'Create' || stepName == 'Revise') {
    //                                            html += '<td colspan="3" class="tasks steprowcell" bwStepname="' + car.DraftWorkflow.Steps.Step[i]["@Name"] + '" bwRoleId="' + car.DraftWorkflow.Steps.Step[i].Assign["@Role"] + '" >' + task + '</td>';
    //                                        } else {
    //                                            // Admin needs an edit button so that this role can be assigned to someone else.
    //                                            var xid = 'steprow-admin_' + i + '_' + '0';
    //                                            html += '<td class="tasks steprowcell" bwStepname="' + car.DraftWorkflow.Steps.Step[i]["@Name"] + '" bwRoleId="' + car.DraftWorkflow.Steps.Step[i].Assign["@Role"] + '" >' + task + '</td>';
    //                                            html += '<td></td>';
    //                                            html += '<td class="steprowbuttons steprowcell" style="background-color:' + cellColor + ';width:80px;text-align:right;padding-right:15px;" >';
    //                                            html += '  <input style="padding:5px 10px 5px 10px;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'renderEditSteprow\', \'' + xid + '\');" type="button" value="⚙ Edit">';
    //                                            html += '</td>';
    //                                        }
    ...
    //                                    // Display Inform roles.
    //                                    if (car.DraftWorkflow.Steps.Step[i].OnStart && car.DraftWorkflow.Steps.Step[i].OnStart.Inform) {
    //                                        if (car.DraftWorkflow.Steps.Step[i].OnStart.Inform.length > 0) {
    //                                            for (var j = 0; j < car.DraftWorkflow.Steps.Step[i].OnStart.Inform.length; j++) {
    //                                                var xid = 'steprow-inform_' + i + '_' + j;
    //                                                var additionalRowClass = '';
    //                                                //debugger;
    //                                                if (assignmentRowChanged_ElementId) { // Todd added 11-6-19 5-18pm ast
    ...
    //                                                    }
    //                                                }
    //                                                html += '<tr id="steprow-inform_' + i + '_' + j + '" class="steprow' + additionalRowClass + ' steprow_' + i + '" style="cursor:cell;display:none;" title="Double-click on this row to edit this role assignment..." >';
    //                                                //html += '  <td class="steprowbuttons steprowcell" style="background-color:' + cellColor + ';width:95px;" >';
    //                                                //html += '    <input style="padding:5px 10px 5px 10px;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'renderEditSteprow\', \'' + xid + '\');" type="button" value="Edit">';
    //                                                //html += '  </td>';
    //                                                html += '  <td style="width:30px;"></td>';
    ...
    //                                                if (thiz.options.displayRoleIdColumn) {
    //                                                    if (car.DraftWorkflow.Steps.Step[i].OnStart.Inform[j]) {
    //                                                        if (car.DraftWorkflow.Steps.Step[i].OnStart.Inform[j]["@Role"]) {
    //                                                            html += '<td style="background-color:' + cellColor + ';" class="roleid steprowcell" bwRoleId="' + car.DraftWorkflow.Steps.Step[i].OnStart.Inform[j]["@Role"] + '" bwOldValue="' + car.DraftWorkflow.Steps.Step[i].OnStart.Inform[j]["@Role"] + '">' + car.DraftWorkflow.Steps.Step[i].OnStart.Inform[j]["@Role"] + '</td>';
    //                                                        } else {
    //                                                            html += '<td style="background-color:' + cellColor + ';" class="roleid steprowcell" bwRoleId="' + car.DraftWorkflow.Steps.Step[i].OnStart.Inform[j]["@Role"] + '" bwOldValue="' + car.DraftWorkflow.Steps.Step[i].OnStart.Inform[j]["@Role"] + '">' + car.DraftWorkflow.Steps.Step[i].OnStart.Inform[j]["@IdField"] + '</td>';
    //                                                        }
    //                                                    }
    //                                                }
    ...
    //                                                var allowRequestModifications;
    //                                                if (car.DraftWorkflow.Steps.Step[i].OnStart.Inform[j]) {
    //                                                    roleName = car.DraftWorkflow.Steps.Step[i].OnStart.Inform[j]["@RoleName"];
    //                                                    //debugger; // AllowRequestModifications xcx2
    //                                                    allowRequestModifications = car.DraftWorkflow.Steps.Step[i].OnStart.Inform[j]["@AllowRequestModifications"];
    //                                                }

    //                                                html += '<td style="background-color:' + cellColor + ';" class="rolename steprowcell" onMouseOver="this.style.backgroundColor=\'lightgoldenrodyellow\';this.style.color=\'red\';" onMouseOut="this.style.backgroundColor=\'' + cellColor + '\';this.style.color=\'black\';" bwStepname="' + car.DraftWorkflow.Steps.Step[i]["@Name"] + '" bwRoleId="' + car.DraftWorkflow.Steps.Step[i].OnStart.Inform[j]["@Role"] + '" bwOldValue="' + roleName + '" bwAllowRequestModifications="' + allowRequestModifications + '">';
    ...
    //                                                html += '<td class="steprowcell">';
    //                                                html += ' <span id="rolecategory-inform_' + i + '_' + j + '" class="rolecategory" bwStepname="' + car.DraftWorkflow.Steps.Step[i]["@Name"] + '" bwRoleId="' + car.DraftWorkflow.Steps.Step[i].OnStart.Inform[j]["@Role"] + '" bwOldValue="Inform" style="cursor:cell;" title="Double-click to edit the role category...">Informed</span>';
    //                                                html += '</td>';

    //                                                html += '<td></td>';

    //                                                if (car.DraftWorkflow.Steps.Step[i].OnStart.Inform[j]["@Cond"]) {
    //                                                    html += '<td class="cond steprowcell" bwStepname = "' + car.DraftWorkflow.Steps.Step[i]["@Name"] + '" bwRoleId = "' + car.DraftWorkflow.Steps.Step[i].OnStart.Inform[j]["@Role"] + '" bwOldValue = "' + car.DraftWorkflow.Steps.Step[i].OnStart.Inform[j]["@Cond"] + '">';
    //                                                    //html += car.DraftWorkflow.Steps.Step[i].OnStart.Inform[j]["@Cond"];
    //                                                    var cond1 = car.DraftWorkflow.Steps.Step[i].OnStart.Inform[j]["@Cond"];
    //                                                    html += thiz.renderTheCondReadOnly(cond1);
    ...
    //                                    // Display Assign roles.
    //                                    if (car.DraftWorkflow.Steps.Step[i].Assign) {
    //                                        if (car.DraftWorkflow.Steps.Step[i].Assign.length > 0) {
    //                                            for (var j = 0; j < car.DraftWorkflow.Steps.Step[i].Assign.length; j++) {
    //                                                var xid = 'steprow-assign_' + i + '_' + j;
    //                                                var additionalRowClass = '';
    //                                                if (assignmentRowChanged_ElementId == xid) {
    //                                                    additionalRowClass = ' activeEditRow';
    //                                                }
    //                                                html += '<tr id="steprow-assign_' + i + '_' + j + '" class="steprow' + additionalRowClass + ' steprow_' + i + '" style="cursor:cell;display:none;" title="Double-click on this row to edit this role assignment..." >';
    ...
    //                                                if (thiz.options.displayRoleIdColumn) {
    //                                                    html += '<td style="background-color:' + cellColor + ';" class="roleid steprowcell" bwRoleId="' + car.DraftWorkflow.Steps.Step[i].Assign[j]["@Role"] + '" bwOldValue="' + car.DraftWorkflow.Steps.Step[i].Assign[j]["@Role"] + '">' + car.DraftWorkflow.Steps.Step[i].Assign[j]["@Role"] + '</td>';
    //                                                }

    //                                                // Todd changed 10-13-19 7-32am ast
    //                                                var roleName = '';
    //                                                var allowRequestModifications;
    //                                                if (car.DraftWorkflow.Steps.Step[i].Assign[j]) {
    //                                                    roleName = car.DraftWorkflow.Steps.Step[i].Assign[j]["@RoleName"];
    //                                                    //debugger; // AllowRequestModifications xcx3
    //                                                    allowRequestModifications = car.DraftWorkflow.Steps.Step[i].Assign[j]["@AllowRequestModifications"];
    //                                                }

    //                                                html += '<td style="background-color:' + cellColor + ';" class="rolename steprowcell" onMouseOver="this.style.backgroundColor=\'lightgoldenrodyellow\';this.style.color=\'red\';" onMouseOut="this.style.backgroundColor=\'' + cellColor + '\';this.style.color=\'black\';" bwStepname="' + car.DraftWorkflow.Steps.Step[i]["@Name"] + '" bwRoleId="' + car.DraftWorkflow.Steps.Step[i].Assign[j]["@Role"] + '" bwOldValue="' + roleName + '" bwAllowRequestModifications="' + allowRequestModifications + '">';
    //                                                //html += ' <img style="width:18px;height:18px;cursor:pointer;vertical-align:text-bottom;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayPeoplePickerDialog\', \'' + 'txtApprover2FriendlyName' + '\', \'' + 'txtApprover2Id' + '\', \'' + 'txtApprover2Email' + '\');" src="images/addressbook-icon18x18.png">';
    //                                                //html += '&nbsp;' + roleName;
    //                                                html += roleName;
    //                                                html += '<br />';
    ...
    //                                                // FIX THIS!! 7-7-2020
    //                                                html += '<span style="font-size:15pt;color:green;font-weight:bold;">$</span>' + '&nbsp;<span style="font-size:8pt;">Only participate for amounts over $0.00.</span><br />';
    ...
    //                                                html += '<td class="steprowcell">';
    //                                                html += ' <span id="rolecategory-assign_' + i + '_' + j + '" class="rolecategory" bwStepname="' + car.DraftWorkflow.Steps.Step[i]["@Name"] + '" bwRoleId="' + car.DraftWorkflow.Steps.Step[i].Assign[j]["@Role"] + '" bwOldValue="' + car.DraftWorkflow.Steps.Step[i].Assign[j]["@RoleCategory"] + '" style="cursor:cell;" title="Double-click to edit the role category...">';
    //                                                html += car.DraftWorkflow.Steps.Step[i].Assign[j]["@RoleCategory"];
    //                                                html += ' </span>';
    //                                                html += '</td>';
    ...
    //                                                var tasks = '';
    //                                                //debugger;
    //                                                if (car.DraftWorkflow.Steps.Step[i].Assign[j].Action) {
    //                                                    for (var t = 0; t < car.DraftWorkflow.Steps.Step[i].Assign[j].Action.length; t++) {
    ...
    //                                                            if (car.DraftWorkflow.Steps.Step[i].Assign[j].Action[t]["@RequireComments"]) tmpRequireComments = car.DraftWorkflow.Steps.Step[i].Assign[j].Action[t]["@RequireComments"];
    //                                                            oldValue += car.DraftWorkflow.Steps.Step[i].Assign[j].Action[t]["@Name"] + '~' + tmpRequireComments + '|'; // Using these for a delimiter since there are several values: "~', "|"


    //                                                            tasks += '•&nbsp;<span style="white-space:nowrap;">' + car.DraftWorkflow.Steps.Step[i].Assign[j].Action[t]["@Name"];
    //                                                            tasks += '<br />';
    ...
    //                                                }
    //                                                //debugger;
    //                                                html += '<td class="tasks steprowcell" bwStepname="' + car.DraftWorkflow.Steps.Step[i]["@Name"] + '" bwRoleId="' + car.DraftWorkflow.Steps.Step[i].Assign[j]["@Role"] + '" bwOldValue="' + oldValue + '">' + tasks + '</td>';

    //                                                var cond = '';
    //                                                if (car.DraftWorkflow.Steps.Step[i].Assign[j]["@Cond"]) {
    //                                                    cond += car.DraftWorkflow.Steps.Step[i].Assign[j]["@Cond"];
    //                                                }
    //                                                //html += '<td class="cond steprowcell" bwStepname = "' + car.DraftWorkflow.Steps.Step[i]["@Name"] + '" bwRoleId = "' + car.DraftWorkflow.Steps.Step[i].Assign[j]["@Role"] + '" bwOldValue = "' + cond + '">' + cond + '</td>';
    //                                                html += '<td class="cond steprowcell" bwStepname = "' + car.DraftWorkflow.Steps.Step[i]["@Name"] + '" bwRoleId = "' + car.DraftWorkflow.Steps.Step[i].Assign[j]["@Role"] + '" bwOldValue = "' + cond + '">' + thiz.renderTheCondReadOnly(cond) + '</td>';
    ...
    //                                } else if (key == 'movestepdown') {
    //                                    thiz.moveStepDown();
    //                                } else if (key == 'workflowjson') {
    //                                    // THIS WORKS JUST COMMENTING OUT FOR NOW
    //                                    thiz.downloadWorkflowJson();
    //                                }
    //                            },
    //                            items: {
    //                                "workflowjson": {
    //                                    name: "Workflow JSON", icon: "edit"
    //                                },
    //                                "sep0": "---------",
    //                                "addaroleparticipant": {
    //                                    name: "Add a Role/Participant", icon: "edit"
    ...
    //                                },
    //                                "addastepbelow": {
    //                                    name: "Add a Step Below", icon: "edit"
    //                                },
    //                                "sep2": "---------",
    //                                "movestepup": {
    //                                    name: "Move Step Up", icon: "edit"
    ...
    //                                },
    //                                "sep3": "---------",
    //                                "viewextendedinformation3": { name: "Edit Timeframe Preferences", icon: "edit" }
    ...
    //        displayAlertDialog('EXCEPTION IN render1: ' + e.message + ', ' + e.stack);
    //    }
    //},
    ...
    deleteSelectedWorkflows: function () {
        try {
            ...
            alert('In deleteSelectedWorkflows(). Old note, leaving for now: >>>>>>CANNOT DELETE. WE NEED TO VERIFY THAT THE WORKFLOW HAS NO RELATED REQUESTS WHICH MAY BE ORPHANED. COMING SOON!'); //Getting ready to delete ' + selectedWorkflowsToDelete.length + ' workflows. This action cannot be undone. This functionality is incomplete and needs further testing!!!!');
            $.ajax({
                url: this.options.operationUriPrefix + "_bw/deleteworkflows_NewTenant",
                dataType: "json",
                contentType: "application/json",
                type: "Post",
    ...
    moveStepDown: function (step) {
        try {
            ...
            for (var i = 0; i < this.options.CurrentWorkflow.RaciSteps.length; i++) {
                if (this.options.CurrentWorkflow.RaciSteps[i].StepName == step) {
                    selectedIndex = i;
                    stepJson = this.options.CurrentWorkflow.RaciSteps[i]; // Save the step
            ...
            alert('In moveStepDown(). this.options.CurrentWorkflow: ' + JSON.stringify(this.options.CurrentWorkflow));
            this._create();
        } catch (e) {
        ...
        try {
            ...
            $("#divEditStepNameDialog").dialog({
                ...
                open: function () {
                    ...
                    var html = '';
                    html += '  <input type="button" value="Save/Update step" style="height:30pt;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'editStepName\', \'' + stepIndex + '\');" />';
    ...
    displayEditEmailInstructionTextDialog: function (stepName) {
        try {
            ...
            var html = '';
            html += '  <input type="button" value="Save/Update step" style="height:30pt;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'editEmailInstructionText\', \'' + stepName + '\');" />';
    ...
    addARoleParticipant: function (elementId) {
        try {
            ...
            ////html += '  <tr>';
            ////html += '    <td>';
            ////html += '      <input type="button" style="width:100%;" value="Save" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'saveAssignmentRow\', \'' + elementId + '\', \'' + 'xx' + '\');" />';
            ////html += '    </td>';
            ////html += '  </tr>';
            ////html += '  <tr>';
            ////html += '    <td>';
            ////html += '      <input type="button" style="width:100%;" value="Cancel" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'cancelStepRowEditing\', \'' + elementId + '\', \'' + 'xx' + '\');" />';
            ////html += '    </td>';
            ////html += '  </tr>';
            ////html += '  </table>';
            ...
            //html += '  <tr>';
            //html += '    <td>';
            //html += '      <input type="button" style="width:100%;" value="Save" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'saveAssignmentRow\', \'' + elementId + '\');" />';
            //html += '    </td>';
            //html += '  </tr>';
            ////html += '  <tr>';
            ////html += '    <td>';
            ////html += '      <input type="button" style="width:100%;" value="Delete" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'deleteRoleCategory\', \'' + elementId + '\');" />';
            ////html += '    </td>';
            ////html += '  </tr>';
            //html += '  <tr>';
            //html += '    <td>';
            //html += '      <input type="button" style="width:100%;" value="Cancel" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'cancelStepRowEditing\', \'' + elementId + '\');" />';
            //html += '    </td>';
            //html += '  </tr>';
            //html += '  <tr>';
            //html += '    <td>';
            //html += '      <input type="button" style="width:100px;white-space:normal;overflow-wrap:break-word;" value="Configure Email Notifications" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayConfigureEmailNotificationsDialog\', \'' + elementId + '\');" />';
            //html += '    </td>';
            //html += '  </tr>';
            //html += '  </table>';
            ...
            $.ajax({
                ...
                try {
                    ...
                    if (thiz.options.displayRoleIdColumn) {
                        ...
                        //html += '<br />';
                        html += '<select id="selectRoleId"  onchange="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'RoleDropDown_Onchange\', \'' + 'selectRoleId' + '\');">';
                        html += '  <option value=""></option>';
                        for (var i = 0; i < roles.length; i++) {
                            html += '  <option value="' + roles[i].RoleId + '">' + roles[i].RoleId + '</option>';
                        }
                        html += '</select>';
                        //html += '<br />or create a new role:'; //<br /><input id="textNewRoleId" type="text" style="width:75pt;" onkeyup="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'NewRoleTextBox_Onkeyup\', \'' + 'textNewRoleId' + '\');" />';
                        html += '<br /><input style="padding:5px 10px 5px 10px;" id="btnCreateRole1" type="button" value="Create a Role..." onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayCreateANewRoleDialog\');" />';

                        html += '<br />or select a person:'; //<br /><input id="textNewRoleId" type="text" style="width:75pt;" onkeyup="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'NewRoleTextBox_Onkeyup\', \'' + 'textNewRoleId' + '\');" />';
                        html += '<br /><input style="padding:5px 10px 5px 10px;" id="btnCreateRole1" type="button" value="Select Person..." onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayCreateANewRoleDialog\');" />';

                        //html += '<br />or add a new person:'; //<br /><input id="textNewRoleId" type="text" style="width:75pt;" onkeyup="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'NewRoleTextBox_Onkeyup\', \'' + 'textNewRoleId' + '\');" />';
                        html += '<br /><input style="padding:5px 10px 5px 10px;" id="btnCreateRole1" type="button" value="Add a Person..." onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayCreateANewRoleDialog\');" />';
                    ...
                    ////html += ' <img style="width:18px;height:18px;cursor:pointer;vertical-align:text-bottom;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayPeoplePickerDialog\', \'' + 'txtApprover2FriendlyName' + '\', \'' + 'txtApprover2Id' + '\', \'' + 'txtApprover2Email' + '\');" src="images/addressbook-icon18x18.png">';
                    ////html == '&nbsp;&nbsp;';
                    //html += '<select id="selectRoleName"  onchange="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'RoleDropDown_Onchange\', \'' + 'selectRoleName' + '\');">';
                    //html += '  <option value=""></option>';
                    //for (var i = 0; i < roles.length; i++) {
                    //    html += '  <option value="' + roles[i].RoleId + '">' + roles[i].RoleName + '</option>';
                    //}
                    //html += '</select>';
                    ////html += '<br />or create a new role:'; //<br /><input id="textNewRoleName" type="text" style="width:210pt;" onkeyup="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'NewRoleTextBox_Onkeyup\', \'' + 'textNewRoleName' + '\');" />';
                    //html += '<br /><input id="btnCreateRole2" type="button" value="Create a Role..." onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayCreateANewRoleDialog\');" />';

                    //html += '<br />or select a person:'; //<br /><input id="textNewRoleId" type="text" style="width:75pt;" onkeyup="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'NewRoleTextBox_Onkeyup\', \'' + 'textNewRoleId' + '\');" />';
                    //html += '<br /><img style="width:18px;height:18px;cursor:pointer;vertical-align:text-bottom;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayPeoplePickerDialog\', \'' + 'txtApprover2FriendlyName' + '\', \'' + 'txtApprover2Id' + '\', \'' + 'txtApprover2Email' + '\');" src="images/addressbook-icon18x18.png">';
                    //html == '&nbsp;&nbsp;';
                    //html += '<input id="btnCreateRole1" type="button" value="Select Person..." onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayCreateANewRoleDialog\');" />';

                    ////html += '<br />or add a new person:'; //<br /><input id="textNewRoleId" type="text" style="width:75pt;" onkeyup="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'NewRoleTextBox_Onkeyup\', \'' + 'textNewRoleId' + '\');" />';
                    //html += '<br /><input id="btnCreateRole1" type="button" value="Add a Person..." onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayCreateANewRoleDialog\');" />';
                    ...
                    html += '<td class="steprowcell">';
                    html += '<select style="padding:5px 5px 5px 5px;" id="selectRoleCategory" class="rolecategory-dropdown" onchange="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'RoleCategoryDropDown_Onchange\', \'' + thiz.id + '\');">';
                    ...
                    html += '  <tr>';
                    html += '    <td>';
                    html += '      <input type="button" style="width:100%;" value="Save" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'saveAssignmentRow\', \'' + elementId + '\');" />xcx1671';
                    html += '    </td>';
                    html += '  </tr>';
                    //html += '  <tr>';
                    //html += '    <td>';
    ...
    addNewWorkflowStep: function (elementId) {
        try {
            ...
            alert('In addNewWorkflowStep(). elementId: ' + elementId);
            //debugger;
            //this.disableButton('buttonAddNewAssignmentRow');
            var thiz = this;
            //var x = elementId.split('_')[0];
            ...
            $.ajax({
                ...
                try {
                    ...
                    html += '<select style="padding:5px 5px 5px 5px;" id="selectRoleCategory" class="rolecategory-dropdown" onchange="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'RoleCategoryDropDown_Onchange\', \'' + thiz.id + '\');">';
                    //debugger;
                    var stepName = 'stepName'; //thiz.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex]["@Name"];
                    var rcs;
                    var rcs2 = ['Inform (Informed)', 'Collaborator (Consulted)', 'Approver (Accountable)'];
                    if (stepName == 'Done') {
                        rcs = ['Inform']; // The "Completed" step can only have the "Inform" role category.
                    ...
                    html += '  <tr>';
                    html += '    <td>';
                    html += '      <input type="button" style="width:100%;" value="Save" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'saveAssignmentRow\', \'' + elementId + '\');" />xcx1672';
                    html += '    </td>';
                    html += '  </tr>';
                    //html += '  <tr>';
                    //html += '    <td>';
    ...
    expandOrCollapseSliderPanel: function () {
        ...
        if (document.getElementsByClassName('rightSliderPanel')[0].innerHTML == '') {
            ...
            html += '       </td>';
            html += '       <td>';
            html += '           <span class="dialogXButton" style="font-family: \'Segoe UI Light\',\'Segoe UI\',\'Segoe\',Tahoma,Helvetica,Arial,sans-serif;font-size:30pt;font-weight:bold;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'expandOrCollapseSliderPanel\');">X</span>';
            html += '       </td>';
            html += '   </tr>';
            html += '</table>';
            html += '<span style="font-size:8pt;white-space:normal;">Customize the emails that get sent automatically by the system.</span>';
            html += '       </td>';
            html += '   </tr>';
            ...
            html += '           <span class="spanButton" title="Edit email template(s)..." onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'editStepEmails\', \'' + 'Overdue' + '\', \'' + 'Overdue Task(s)' + '\', \'' + instructionText + '\');"><span style="font-size:15pt;display:inline-block;">✉</span> "' + 'Overdue Task(s)' + '" &nbsp;&nbsp;</span>';
            html += '           <br />';
            ...
            //html += '           <span class="spanButton" title="Edit email template(s)..." onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'editStepEmails\', \'' + 'RequestReceived' + '\', \'' + 'Request Received' + '\', \'' + instructionText + '\');"><span style="font-size:15pt;display:inline-block;">✉</span> "' + 'Request Received' + '" &nbsp;&nbsp;</span>';
            //html += '           <br />';
            //html += '           <div style="font-size:8pt;margin-top:10px;">' + instructionText + '</div>';
            //html += '       </td>';
            //html += '   </tr>';
            ...
            //html += '           <span class="spanButton" title="Edit email template..." onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'editStepEmails\',\'Create\');"><span style="font-size:15pt;display:inline-block;">✉</span> "Your request has been received" &nbsp;</span><br /><span style="font-size:8pt;padding:0 25px 0 25px;">These emails get sent to people when they first create a request.</span>';
            //html += '       </td>';
            //html += '   </tr>';
            ...
            //    //html += '           <span class="spanButton" title="Edit email template(s)..." style="padding:5px 10px 5px 10px;margin:0 0 0 20px;white-space:nowrap;vertical-align:top;border:1px solid lightblue;cursor:pointer;font-weight:normal;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'editStepEmails\', \'' + stepName + '\');"><span style="font-size:15pt;display:inline-block;">✉</span> "' + buttonText + '" Email&nbsp;&nbsp;</span>';
            //    html += '           <span class="spanButton" title="Edit email template(s)..." onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'editStepEmails\', \'' + stepName + '\');"><span style="font-size:15pt;display:inline-block;">✉</span> "' + buttonText + '" Email&nbsp;&nbsp;</span>';
            //    html += '       </td>';
            //    html += '   </tr>';
            ...
            //html += '           <span class="spanButton" title="Edit email template(s)..." onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'editStepEmails\', \'all\');"><span style="font-size:15pt;display:inline-block;">✉</span> Task Notification Emails&nbsp;&nbsp;-itworx</span>';
            //html += '       </td>';
            //html += '   </tr>';
            ...
            var html2 = '';
            //html2 += '<input style="padding:5px 10px 5px 10px;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'addARaciStep\', \'' + 'all' + '\');" type="button" value="Add a Step" />';
            var tooltip = 'Edit email template(s)...';
        ...
        try {
            ...
            for (var i = 0; i < car.DraftWorkflow.Steps.Step.length; i++) {
                ...
                //html += '           <span class="spanButton" title="Edit email template(s)..." style="padding:5px 10px 5px 10px;margin:0 0 0 20px;white-space:nowrap;vertical-align:top;border:1px solid lightblue;cursor:pointer;font-weight:normal;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'editStepEmails\', \'' + stepName + '\');"><span style="font-size:15pt;display:inline-block;">✉</span> "' + buttonText + '" Email&nbsp;&nbsp;</span>';
                html += '           <span class="spanButton" title="Edit email template(s)..." onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'editStepEmails\', \'' + stepName + '\', \'' + stepFriendlyName + '\', \'' + instructionText + '\');"><span style="font-size:15pt;display:inline-block;">✉</span> "' + buttonText + '" &nbsp;&nbsp;</span>';
                html += '           <br />';
                html += '           <div style="font-size:8pt;margin-top:10px;">' + instructionText + '</div>';
                html += '       </td>';
                html += '   </tr>';
    ...
    expandOrCollapseAllWorkflowSteps: function () {
        try {
            ...
            var car = this.options.CurrentWorkflow;
            for (var i = 0; i < car.DraftWorkflow.Steps.Step.length; i++) {
                // $('.bwWorkflowEditor_NewTenant').bwWorkflowEditor_NewTenant('expandOrCollapseWorkflowStep', 'stepname_0', 'stepimage_0', 'steprow-create_0_0');
                var stepName = car.DraftWorkflow.Steps.Step[i]["@Name"];
    ...
    downloadWorkflowJson: function () {
        try {
            ...
            alert('In downloadWorkflowJson().');
            var thiz = this;

            var bwWorkflowId = this.options.CurrentWorkflow.bwWorkflowId;
            alert('xcx213213 bwWorkflowId: ' + bwWorkflowId);
            ...
            $("#bwWorkflowEditor_NewTenant_divDisplayJsonDialog").dialog({
                ...
                open: function (event, ui) {
                    try {
                        ...
                        document.getElementById('bwWorkflowEditor_NewTenant_spanDisplayJsonDialogTitle').innerHTML = 'Workflow JSON';

                        $('#bwWorkflowEditor_NewTenant_spanDisplayJsonDialogContent').html('');

                        $('#bwWorkflowEditor_NewTenant_txtDisplayJsonDialogJSON').empty();

                        var json = JSON.stringify(thiz.options.CurrentWorkflow.DraftWorkflow, null, 2);
                        $('#bwWorkflowEditor_NewTenant_txtDisplayJsonDialogJSON').val(json); // pretty print

                        document.getElementById('bwWorkflowEditor_NewTenant_btnRunDiagnostics').innerHTML = '<span style="font-size:30pt;">⚠ Save/Update Workflow JSON</span>';

                        $('#bwWorkflowEditor_NewTenant_btnRunDiagnostics').bind('click', function () {
                            console.log('In downloadWorkflowJson.bwWorkflowEditor_NewTenant_btnRunDiagnostics.click().');
                            alert('In downloadWorkflowJson.bwWorkflowEditor_NewTenant_btnRunDiagnostics.click().');
                            ...
                            if (validJson != true) {
                                ...
                                if (confirm("Are you certain you wish to activate this workflow? Save a copy in notepad in case something goes wrong!")) {
                                    ...
                                    alert('xcx1234 url: ' + thiz.options.operationUriPrefix + "_bw/publishworkflow2_newtenant");
                                    //debugger;
                                    //var data = {
                                    //    //bwTenantId: thiz.options.bwTenantId,
                                    //    bwWorkflowAppId: thiz.options.bwWorkflowAppId,
                                    ...
                                    var data = {
                                        ...
                                        bwRequestTypeId: thiz.options.CurrentWorkflow.bwRequestTypeId,
                                        bwRequestType: thiz.options.CurrentWorkflow.bwRequestType, // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                                        bwWorkflowJson: bwWorkflowJson,
                                    ...
                                    //displayAlertDialog('xcx2131241234 DOES THIS LOOK OK? data: ' + JSON.stringify(data));

                                    $.ajax({
                                        url: thiz.options.operationUriPrefix + "_bw/publishworkflow2_newtenant",
                                        type: "Post",
                                        data: data,
                                        headers: {
                                            "Accept": "application/json; odata=verbose"
                                        ...
                                        try {
                                            ...
                                            if (results.status != 'SUCCESS') {
                                                ...
                                                thiz.displayAlertDialog('Error in downloadWorkflowJson(): ' + JSON.stringify(results));

                                            } else {

                                                console.log('In bwWorkflowEditor_NewTenant.js.downloadWorkflowJson().post: Successfully updated DB. result: ' + JSON.stringify(results)); // using (' + JSON.stringify(json) + ').');
                                                alert('In bwWorkflowEditor_NewTenant.js.downloadWorkflowJson().post: Successfully updated DB.');
                                            }
                                            // Display a dialog with an "Undo" button!!!!
                                            //$("#divUndoWorkflowActivationDialog").dialog({
                                            //    modal: true,
                                            ...
                                            console.log('Exception in bwWorkflowEditor_NewTenant.js.downloadWorkflowJson().xx.update: ' + e.message + ', ' + e.stack);
                                            displayAlertDialog('Exception in bwWorkflowEditor_NewTenant.js.downloadWorkflowJson().xx.update: ' + e.message + ', ' + e.stack);
                                        }
                                    }).error(function (data, errorCode, errorMessage) {
                                        debugger;
                                        //thiz.hideProgress();
                                        ...
                                        console.log('Error in bwWorkflowEditor_NewTenant.js.downloadWorkflowJson().xx.update: ' + JSON.stringify(data) + ', json: ' + JSON.stringify(json));
                                        displayAlertDialog('Error in bwWorkflowEditor_NewTenant.js.downloadWorkflowJson().xx.update: ' + msg); //+ error.message.value + ' ' + error.innererror.message);
                                        //console.log('Fail in CarForm3.aspx.populateSpendForecastNotReadOnly().spendForecastItems.update: ' + JSON.stringify(data));
                                        //var error = JSON.parse(data.responseText)["odata.error"];
                                        //alert('Exception in CarForm3.aspx.populateSpendForecastNotReadOnly().spendForecastItems.update: ' + error.message.value + ' ' + error.innererror.message); // + ', EntityValidationErrors: ' + data.EntityValidationErrors);
                                    });
                        ...
                        console.log('Exception in downloadWorkflowJson():2: ' + e.message + ', ' + e.stack);
                        displayAlertDialog('Exception in downloadWorkflowJson():2: ' + e.message + ', ' + e.stack);
                    }
                } // This allows the dialog to close when clicked outside of the dialog. Only works for modal dialogs.
            });
            //$("#bwWorkflowEditor_NewTenant_divDisplayJsonDialog").dialog().parents(".ui-dialog").find(".ui-dialog-titlebar").remove();
            ...
            //var json = JSON.stringify(this.options.CurrentWorkflow.DraftWorkflow, null, 2);
            //$('#bwWorkflowEditor_NewTenant_txtDisplayJsonDialogJSON').val(json); // pretty print

        } catch (e) {
            console.log('Exception in bwWorkflowEditor_NewTenant.js.downloadWorkflowJson(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwWorkflowEditor_NewTenant.js.downloadWorkflowJson(): ' + e.message + ', ' + e.stack);
        }
    },
    ...
    renderEditSteprow: function (elementId) {
        try {
            ...
            //    // Render the "RoleCategory" drop-down.
            //    //html += '<td class="steprowcell">';
            //    ////html += '<select style="padding:5px 5px 5px 5px;" id="selectRoleCategory" class="rolecategory-dropdown" onchange="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'RoleCategoryDropDown_Onchange\', \'' + elementId + '\');">';
            //    ////var rcs = ['Inform', 'Collaborator', 'Approver'];
            //    ////var rcs2 = ['Inform (Informed)', 'Collaborator (Consulted)', 'Approver (Accountable)'];
            //    ////for (var rci = 0; rci < rcs.length; rci++) {
            //    ////    if (rolecategory == rcs[rci]) {
            ...
            html += '  <tr>';
            html += '    <td>';
            html += '       <input style="padding:5px 10px 5px 10px;opacity:.4;width:70px;" type="button" disabled value="⚙ Edit">';
            html += '    </td>';
            html += '  </tr>';
            ...
            html += '  <tr>';
            html += '    <td>';
            html += '      <input type="button" style="padding:5px 10px 5px 10px;width:70px;cursor:pointer;" value="Save" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'saveAssignmentRow\', \'' + elementId + '\');" />';
            html += '    </td>';
            html += '  </tr>';
            if (!isAdmin) { // The ADMIN row/step cannot be deleted.
                html += '  <tr>';
                html += '    <td>';
                html += '      <input type="button" style="padding:5px 10px 5px 10px;width:70px;cursor:pointer;" value="Delete" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'deleteRoleCategory\', \'' + elementId + '\');" />';
                html += '    </td>';
                html += '  </tr>';
            }
            html += '  <tr>';
            html += '    <td>';
            html += '      <input type="button" style="padding:5px 10px 5px 10px;width:70px;cursor:pointer;" value="Cancel" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'cancelStepRowEditing\', \'' + elementId + '\');" />';
            html += '    </td>';
            html += '  </tr>';
            //html += '  <tr>';
            //html += '    <td>';
            ...
            console.log('Exception in bwWorkflowEditor_NewTenant.js.renderEditSteprow(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwWorkflowEditor_NewTenant.js.renderEditSteprow(): ' + e.message + ', ' + e.stack);
        }
    },
        ...
        try {
            ...
            if (stepIndex && sourceRoleCategory && roleIndex > -1) {
                if (sourceRoleCategory == 'inform') {
                    this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].OnStart.Inform.splice(roleIndex, 1); // delete leaves a null, so we have to use splice.
    ...
    displayWorkflowsConfigurationDialog: function () {
        try {
            ...
            //debugger;
            $.ajax({
                url: thiz.options.operationUriPrefix + "odata/WorkflowConfiguration2_NewTenant/" + selectedRequestType, //'all', //thiz.options.LastSelectedRequestType, //'budgetrequest', //
                dataType: "json",
                contentType: "application/json",
                type: "Get",
                timeout: thiz.options.ajaxTimeout
                ...
                try {
                    ...
                    html += '    <td></td>';
                    html += '    <td colspan="9">';
                    html += '       <span style="padding: 4px 4px 4px 4px;border:1px solid lightblue;cursor:pointer;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'cmdDisplayDeleteWorkflowsDialog\');">';
                    ...
                    html += '    <td><input type="checkbox" id="checkboxTogglePendingEmailCheckboxes" onchange="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'toggleWorkflowConfigurationCheckboxes\', this);" /></td>';
                    ...
                    for (var i = 0; i < result.value.length; i++) {
                        ...
                        html += '       <td class="roleIdxxx">' + result.value[i].bwRequestTypeId + '</td>';
                        if (result.value[i].bwWorkflowActive == true) {
                        ...
                        //    html += '   <td></td>';
                        //} else {
                        html += '   <td><input style="padding:5px 10px 5px 10px;width:100px;" id="btnActivateRaciConfiguration" type="button" value="Activate" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayActivateSelectedWorkflowDialog\', \'' + result.value[i].bwWorkflowId + '\');" /></td>'; // thiz.options.LastSelectedRequestType
                        //}

                        html += '   <td><input style="padding:5px 10px 5px 10px;width:100px;" type="button" value="Edit JSON" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayEditWorkflowJsonDialog\', \'' + result.value[i].bwWorkflowId + '\');" /></td>';
                    ...
                    html += '   </span>';
                    html += '   <br /><br /><br />';
                    html += '   <div class="divSignInButton" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'deleteSelectedWorkflows\');" style="width:90%;text-align:center;line-height:1.1em;font-weight:bold;">';
                ...
                alert('Exception in bwWorkflowEditor_NewTenant.js.renderWorkflowsDropDownList().xx.Get: ' + msg); //+ error.message.value + ' ' + error.innererror.message);
                console.log('Exception in bwWorkflowEditor_NewTenant.js.renderWorkflowsDropDownList().xx.Get: ' + JSON.stringify(data));
                //console.log('Fail in CarForm3.aspx.populateSpendForecastNotReadOnly().spendForecastItems.update: ' + JSON.stringify(data));
                //var error = JSON.parse(data.responseText)["odata.error"];
                //alert('Exception in CarForm3.aspx.populateSpendForecastNotReadOnly().spendForecastItems.update: ' + error.message.value + ' ' + error.innererror.message); // + ', EntityValidationErrors: ' + data.EntityValidationErrors);
            });
        ...
        try {
            ...
            $("#divDeleteAWorkflowDialog").dialog({
                ...
                open: function () {
                    ...
                    var html = '';
                    html += '        <div id="divDeleteAWorkflowDialogReassignTasks" class="divSignInButton" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'deleteWorkflow\', \'' + bwWorkflowId + '\');" style="width:90%;text-align:center;line-height:0.8em;font-weight:bold;">';
                    html += '            Delete';
                    html += '        </div>';
        ...
        try {
            ...
            $('#divActivateSelectedWorkflowDialog').dialog({
                ...
                open: function () {
                    ...
                    var html = '';
                    html += '        <div id="divActivateSelectedWorkflowDialogReassignTasks" class="divSignInButton" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'activateSelectedWorkflow\', \'' + bwWorkflowId + '\', \'' + bwRequestType + '\');" style="width:90%;text-align:center;line-height:0.8em;font-weight:bold;">';
                    html += '            Activate';
                    html += '        </div>';
            ...
            if (confirm("Are you certain you wish to activate this workflow?")) {
                ...
                if (selectedRequestType == bwRequestType) {
                    ...
                    var json = {
                        ...
                        Description: '',
                        bwWorkflowJson: bwWorkflowJson, //thiz.options.CurrentWorkflow.DraftWorkflow), //JSON.stringify(thiz.options.CurrentWorkflow), // WE KNOW WE ARE NOT GETTING THE CORRECT JSON HERE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                        bwRequestType: selectedRequestType
                    };
                    $.ajax({
                        ...
                        try {
                            //debugger;
                            console.log('In bwWorkflowEditor_NewTenant.js.publishWorkflowConfigurationAndActivate().post: Successfully updated DB. result: ' + JSON.stringify(result)); // using (' + JSON.stringify(json) + ').');
                            // Display a dialog with an "Undo" button!!!!
                            $("#divUndoWorkflowActivationDialog").dialog({
                                modal: true,
                                resizable: false,
                            ...
                            console.log('Exception in bwWorkflowEditor_NewTenant.js.publishWorkflowConfigurationAndActivate().xx.update: ' + e.message + ', ' + e.stack);
                            alert('Exception in bwWorkflowEditor_NewTenant.js.publishWorkflowConfigurationAndActivate().xx.update: ' + e.message + ', ' + e.stack);
                        }
                    }).error(function (data, errorCode, errorMessage) {
                        debugger;
                        //thiz.hideProgress();
                        ...
                        console.log('Fail in bwWorkflowEditor_NewTenant.js.publishWorkflowConfigurationAndActivate().xx.update: ' + JSON.stringify(data) + ', json: ' + JSON.stringify(json));
                        alert('Fail in bwWorkflowEditor_NewTenant.js.publishWorkflowConfigurationAndActivate().xx.update: ' + msg); //+ error.message.value + ' ' + error.innererror.message);
                        //console.log('Fail in CarForm3.aspx.populateSpendForecastNotReadOnly().spendForecastItems.update: ' + JSON.stringify(data));
                        //var error = JSON.parse(data.responseText)["odata.error"];
                        //alert('Exception in CarForm3.aspx.populateSpendForecastNotReadOnly().spendForecastItems.update: ' + error.message.value + ' ' + error.innererror.message); // + ', EntityValidationErrors: ' + data.EntityValidationErrors);
                    });
    ...
    publishWorkflowConfigurationAndActivate: function () {
        try {
            ...
            var bwRequestTypeId, bwRequestType;
            $('#selectRequestTypeDropDown').find('option:selected').each(function (index, element) {
                bwRequestType = element.innerHTML;
                bwRequestTypeId = element.value;
            });
                ...
                $.ajax({
                    ...
                    try {
                        ...
                        if (result.status != 'SUCCESS') {
                            ...
                            alert('xcx324888. result: ' + JSON.stringify(result));
    ...
    //publishWorkflowConfigurationAndActivate: function () {
    //    try {
    //        console.log('In bwWorkflowEditor_NewTenant.js.publishWorkflowConfigurationAndActivate().');
    //        var thiz = this;
    ...
    //        if (!(bwRequestType && bwRequestTypeId)) {

    //            displayAlertDialog('Error in publishWorkflowConfigurationAndActivate(). Invalid values. bwRequestType: ' + bwRequestType + ', bwRequestTypeId: ' + bwRequestTypeId);

    //        } else {

    //            var bwWorkflowId = this.options.CurrentWorkflow.bwWorkflowId;
    //            var bwRequestTypeId2 = this.options.CurrentWorkflow.bwRequestTypeId;

    //            console.log('In bwWorkflowEditor_NewTenant.js.publishWorkflowConfigurationAndActivate(). Publishing request type: ' + bwRequestType + ' (' + bwRequestTypeId + '), Workflow: bwWorkflowId: ' + bwWorkflowId + ', bwRequestTypeId2: ' + bwRequestTypeId2);

    //            // Just a final check to make sure the selected bwRequestTypeId matches this.options.CurrentWorkflow.bwRequestTypeId.
    //            if (bwRequestTypeId != this.options.CurrentWorkflow.bwRequestTypeId) {

    //                displayAlertDialog('Error in publishWorkflowConfigurationAndActivate(). The selected request type does not match for the workflow. bwRequestType: ' + bwRequestType + ', bwRequestTypeId: ' + bwRequestTypeId + ', CurrentWorkflow.bwRequestTypeId: ' + this.options.CurrentWorkflow.bwRequestTypeId);
    ...
    //                    ModifiedByEmail: participantEmail,
    //                    ModifiedByFriendlyName: participantFriendlyName,
    //                    bwWorkflowJson: JSON.stringify(this.options.CurrentWorkflow.DraftWorkflow),
    //                    bwRequestTypeId: bwRequestTypeId,
    //                    bwRequestType: bwRequestType
    //                };

    //                $.ajax({
    //                    url: this.options.operationUriPrefix + "_bw/publishworkflow2_newtenant",
    //                    type: "Post",
    //                    data: json,
    //                    headers: {
    //                        "Accept": "application/json; odata=verbose"
    ...
    //                            displayAlertDialog('In bwWorkflowEditor_NewTenant.js.publishWorkflowConfigurationAndActivate(). ' + result.status + ': ' + result.message);

    //                        } else {

    //                            console.log('In bwWorkflowEditor_NewTenant.js.publishWorkflowConfigurationAndActivate(). Successfully updated the database. result: ' + JSON.stringify(result));

    //                            thiz.options.CurrentWorkflow.DraftWorkflow = JSON.parse(JSON.stringify(thiz.options.CurrentWorkflow.DraftWorkflow)); //JSON.parse(thiz.options.CurrentWorkflow.bwWorkflowJson);
    //                            thiz.options.CurrentWorkflow.bwWorkflowJson = JSON.stringify(thiz.options.CurrentWorkflow.DraftWorkflow);

    //                            //thiz.options.CurrentWorkflow.DraftWorkflow = JSON.parse(thiz.options.CurrentWorkflow.bwWorkflowJson);
    ...
    //                            var promise = thiz.loadWorkflowsAndCurrentWorkflow(bwRequestTypeId);
    //                            promise.then(function (result) {
    //                                try {
    //                                    //alert('xcx213123 Preparing to call checkIfWeHaveToDisplayThePublishChangesButton().');
    //                                    thiz.renderWorkflowEditor2();
    //                                    //thiz.checkIfWeHaveToDisplayThePublishChangesButton();
    //                                } catch (e) {
    //                                    console.log('Exception in bwWorkflowEditor_NewTenant._create().loadWorkflowsAndCurrentWorkflow(): ' + e.message + ', ' + e.stack);
    //                                    alert('Exception in bwWorkflowEditor_NewTenant._create().loadWorkflowsAndCurrentWorkflow(): ' + e.message + ', ' + e.stack);
    //                                }
    //                            });
    //                        }
    ...
    //                        msg = JSON.stringify(data) + ', json: ' + JSON.stringify(json);
    //                    }
    //                    console.log('Error in bwWorkflowEditor_NewTenant.js.publishWorkflowConfigurationAndActivate().xx.update: ' + JSON.stringify(data) + ', json: ' + JSON.stringify(json));
    //                    alert('ror in bwWorkflowEditor_NewTenant.js.publishWorkflowConfigurationAndActivate().xx.update: ' + msg); //+ error.message.value + ' ' + error.innererror.message);
    ...
    //    } catch (e) {
    //        console.log('Exception in bwWorkflowEditor_NewTenant.js.publishWorkflowConfigurationAndActivate(): ' + e.message + ', ' + e.stack);
    //        alert('Exception in bwWorkflowEditor_NewTenant.js.publishWorkflowConfigurationAndActivate(): ' + e.message + ', ' + e.stack);
    //    }
    //},
    ...
    checkIfWeHaveToDisplayThePublishChangesButton: function () {
        try {
            ...
            //alert('In checkIfWeHaveToDisplayThePublishChangesButton(). thereHaveBeenChangesToTheWorkflow: ' + thereHaveBeenChangesToTheWorkflow);
            if (thereHaveBeenChangesToTheWorkflow) {
                ...
                if (this.options.DisplayAsNewTenantUserConfigurationEditor == true) {
                    html += '<input style="padding:5px 10px 5px 10px;width:100px;" id="btnSaveWorkflowConfigurationAndActivate" type="button" value="Publish" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'publishWorkflowConfigurationAndActivate\');" />';
                    html += '&nbsp;<input style="padding:5px 10px 5px 10px;" id="btnCancelChangesInDraftWorkflowConfiguration" type="button" value="Cancel Changes" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'cancelChangesInDraftWorkflowConfiguration\');" />';
                } else {
                    html += '<input style="padding:5px 10px 5px 10px;width:100px;" id="btnSaveWorkflowConfigurationAndActivate" type="button" value="Publish" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'publishWorkflowConfigurationAndActivate\');" />';
                    html += '&nbsp;<input style="padding:5px 10px 5px 10px;" id="btnCancelChangesInDraftWorkflowConfiguration" type="button" value="Cancel Changes" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'cancelChangesInDraftWorkflowConfiguration\');" />';
                }
    ...
    //                    '@Cond': newConditionString
    //                }
    //                this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].OnStart.Inform.splice(0, 0, row); // Create the row
    //            } else {
    ...
    //                if (sourceRoleCategory == 'inform') {
    //                    this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].OnStart.Inform[roleIndex]["@Cond"] = newConditionString; // The only thing that could change here is "Cond".
    //                    //debugger;
    //                    if (newRoleId) {
    //                        this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].OnStart.Inform[roleIndex]["@Role"] = newRoleId;
    //                        this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].OnStart.Inform[roleIndex]["@RoleName"] = newRoleName;
    //                    }
    //                } else if (sourceRoleCategory == 'assign') {
    //                    if (newRoleCategory == 'Approver' && newActions) {
    ...
    //                                    var javascript = this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].Assign[roleIndex].Action[a]["@JavaScript"];
    ...
    //                    }
    //                    //debugger;
    //                    this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].Assign[roleIndex]["@Cond"] = newConditionString;
    ...
    //                    debugger;
    //                    this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].Assign.Action = newActions;
    ...
    //                    //    var actionIndex;
    //                    //    for (var a = 0; a < this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].Assign.Action.length; a++) {
    //                    //        var x1 = this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].Assign.Action[a]["@Name"];
    //                    //        for (var b = 0; b < newActions.length; b++) {
    //                    //            var x2 = newActions[b]["@Name"];
    //                    //            if (x1 == x2) {
    //                    //                //debugger;
    //                    //                var tooltip = this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].Assign.Action[a].Tooltip;
    //                    //                var javascript = this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].Assign.Action[a]["@JavaScript"];
    //                    //                newActions[b].Tooltip = tooltip;
    //                    //                newActions[b]["@JavaScript"] = javascript;
    ...
    //            //            thiz._create();
    //            //        } catch (e) {
    //            //            console.log('Exception in bwWorkflowEditor_NewTenant.js.saveAssignmentRow().xx.update: ' + e.message + ', ' + e.stack);
    //            //            alert('Exception in bwWorkflowEditor_NewTenant.js.saveAssignmentRow().xx.update: ' + e.message + ', ' + e.stack);
    //            //        }
    //            //    }).error(function (data, errorCode, errorMessage) {
    //            //        //thiz.hideProgress();
    //            //        var msg;
    ...
    //            //            msg = JSON.stringify(data) + ', json: ' + JSON.stringify(json);
    //            //        }
    //            //        console.log('Fail in bwWorkflowEditor_NewTenant.js.saveAssignmentRow().xx.update: ' + JSON.stringify(data) + ', json: ' + JSON.stringify(json));
    //            //        alert('Fail in bwWorkflowEditor_NewTenant.js.saveAssignmentRow().xx.update: ' + msg); //+ error.message.value + ' ' + error.innererror.message);
    //            //        //console.log('Fail in CarForm3.aspx.populateSpendForecastNotReadOnly().spendForecastItems.update: ' + JSON.stringify(data));
    //            //        //var error = JSON.parse(data.responseText)["odata.error"];
    //            //        //alert('Exception in CarForm3.aspx.populateSpendForecastNotReadOnly().spendForecastItems.update: ' + error.message.value + ' ' + error.innererror.message); // + ', EntityValidationErrors: ' + data.EntityValidationErrors);
    //            //    });
    ...
    //    } catch (e) {
    //        console.log('Exception in bwWorkflowEditor_NewTenant.js.saveAssignmentRow(): ' + e.message + ', ' + e.stack);
    //        displayAlertDialog('Exception in bwWorkflowEditor_NewTenant.js.saveAssignmentRow(): ' + e.message + ', ' + e.stack);
    //    }
    //},
        ...
        try {
            ...
            if (stepIndex && sourceRoleCategory && roleIndex > -1) {
                ...
                if (sourceRoleCategory == 'inform' && (newRoleCategory == 'Collaborator' || newRoleCategory == 'Approver')) {
                    ...
                    if (this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].OnStart && this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].OnStart.Inform) {
                        this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].OnStart.Inform.splice(0, 0, row); // Create the row
                    } else {
                        this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex]["OnStart"] = {
                            Inform: [row]
                        };

                        //this.options.CurrentWorkflow.Workflow.Steps.Step[stepIndex].push({
                        //    OnStart: {
                        //        Inform: [row]
                        //    }
                        //});
                    ...
                    if (sourceRoleCategory == 'inform') {
                        ...
                        if (newRoleId) {
                            this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].OnStart.Inform[roleIndex]["@Role"] = newRoleId;
                            this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].OnStart.Inform[roleIndex]["@RoleName"] = newRoleName;
                            this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].OnStart.Inform[roleIndex]["@Instructions"] = instructions;


                            this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].OnStart.Inform[roleIndex]["@BudgetThreshold"] = budgetThreshold;
                        ...
                        //// Here is our problem: The "Assign" always has to be an array. IT MAY NOT BE in the workflow!!! So that is what this code is for, to make sure that it is...
                        //if (this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].Assign && !this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].Assign.length) {
                        //    alert('xcx2131234124 no length for assign in workflow!!!!!!!');

                        //    var oldAssign = this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].Assign;
                        //    this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].Assign = [];
                        //    this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].Assign.push(oldAssign);

                        //} else {

                        //}
                        //this.options.CurrentWorkflow.DraftWorkflow.Steps.Step[stepIndex].Assign[0]["Action"] = newActions; // removed 12-23-2022 <<<<<<<<<<<<
                        ...
                        displayAlertDialog('ERROR: Invalid sourceRoleCategory: ' + sourceRoleCategory);
                    }
                }
                //
                // Now we have to save the user(s) in the BwWorkflowUserRole schema.
    ...
    editStepEmails: function (stepName, stepFriendlyName, instructionText) {
        try {
            ...
            //    //} else {
            //    //html += '<select style="padding:5px 5px 5px 5px;" id="selectEditEmailFor" onchange="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'selectEditEmailFor_OnChange\', \'editing\');">';
            ...
            // Set the "Save" button.
            var html = '';
            html += '<div class="divSignInButton" style="width:90%;text-align:center;line-height:1.1em;font-weight:bold;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'saveEmailTemplateForStep\', \'' + stepName + '\');">';
            ...
            $("#divConfigureEmailNotificationsDialog").dialog({
                ...
                open: function () {
                    ...
                    var html = '';
                    html += '<span title="Edit email instruction text..." style="width:200px;padding:5px 10px 5px 10px;margin:0 0 0 10px;white-space:nowrap;vertical-align:middle;border:1px solid lightblue;cursor:pointer;font-weight:normal;font-size:10pt;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayEditEmailInstructionTextDialog\',\'' + stepName + '\');"><span style="display:inline-block;"> ⚙ </span></span>';
                    ...
                    if (stepName == 'Overdue') {
                        ...
                        //var emailTemplateTitle = document.getElementById('selectEditEmailFor').value;
                        //if (emailTemplateTitle == 'Informed') {
                        //    emailTemplateForSubject = thiz.options.CurrentWorkflow.DraftWorkflow["EmailTemplateForSubject_Informed"];
                        //    emailTemplate = thiz.options.CurrentWorkflow.DraftWorkflow["EmailTemplate_Informed"];
                        //} else if (emailTemplateTitle == 'Task Assigned') {
                        //    emailTemplateForSubject = thiz.options.CurrentWorkflow.DraftWorkflow["EmailTemplateForSubject_TaskAssigned"];
                        //    emailTemplate = thiz.options.CurrentWorkflow.DraftWorkflow["EmailTemplate_TaskAssigned"];
                        //} else if (emailTemplateTitle == 'Task Overdue') {
                        //    emailTemplateForSubject = thiz.options.CurrentWorkflow.DraftWorkflow["EmailTemplateForSubject_TaskOverdue"];
                        //    emailTemplate = thiz.options.CurrentWorkflow.DraftWorkflow["EmailTemplate_TaskOverdue"];
                        //} else if (emailTemplateTitle == 'Task Cancelled') {
                        //    emailTemplateForSubject = thiz.options.CurrentWorkflow.DraftWorkflow["EmailTemplateForSubject_TaskCancelled"];
                        //    emailTemplate = thiz.options.CurrentWorkflow.DraftWorkflow["EmailTemplate_TaskCancelled"];
                        //} else {
    ...
    //        //
    //        var html = '';
    //        html += '<div class="divSignInButton" style="width:90%;text-align:center;line-height:1.1em;font-weight:bold;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'saveEmailTemplateForOverdueTasks\', \'' + 'stepName' + '\');">';
    //        html += '☑ Save this email template';
    //        html += '</div>';
    //        document.getElementById('spanConfigureEmailNotificationsDialogSaveButton').innerHTML = html;
    //        // Display the email editor.
    ...
    displayEmailDataItemPickerDialog: function (emailSection) { // emailSection is either 'subject' or 'body'.
        try {
            ...
            for (var i = 0; i < keys.length; i++) {
                html += '   <option value="3">' + dataConversionJson[keys[i]].TemplateText + '</option>';
            }
            ...
            html += '</select>';
            html += '&nbsp;&nbsp;';
            html += '<input style="padding:5px 10px 5px 10px;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'insertEmailDataItem\', \'' + emailSection + '\');" type="button" value="Insert">';
    ...
    displayWorkflowActionsUnderlyingPropertiesDialog: function (elementId, actionTitle) {
        try {
            ...
            html += '<br />';
            html += '&nbsp;&nbsp;';
            html += '<input style="padding:5px 10px 5px 10px;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'saveWorkflowActionsUnderlyingProperties\', \'' + elementId + '\', \'' + actionTitle + '\');" type="button" value="Save">';
            ...
            this.displayAlertDialog('Exception in displayWorkflowActionsUnderlyingPropertiesDialog(): ' + e.message + ', ' + e.stack);
        }
    },
    ...
    saveEmailTemplateForStep: function (stepName) {
        try {
            console.log('In saveEmailTemplateForStep(). stepName: ' + stepName);
            ...
            alert('Exception in saveEmailTemplateForStep(' + stepName + '): ' + e.message + ', ' + e.stack);
        }
    },
    ...
    //            if (car.DraftWorkflow.Steps.Step[i]["@FriendlyName"]) {
    //                stepFriendlyName = car.DraftWorkflow.Steps.Step[i]["@FriendlyName"];
    //                var buttonText = car.DraftWorkflow.Steps.Step[i]["@FriendlyName"];
    //            } else {
    //                var buttonText = stepName;
    //            }
    ...
    //            //html += '           <span class="spanButton" title="Edit email template(s)..." style="padding:5px 10px 5px 10px;margin:0 0 0 20px;white-space:nowrap;vertical-align:top;border:1px solid lightblue;cursor:pointer;font-weight:normal;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'editStepEmails\', \'' + stepName + '\');"><span style="font-size:15pt;display:inline-block;">✉</span> "' + buttonText + '" Email&nbsp;&nbsp;</span>';
    //            html += '           <span class="spanButton" title="Edit email template(s)..." onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'editStepEmails\', \'' + stepName + '\', \'' + stepFriendlyName + '\', \'' + instructionText + '\');"><span style="font-size:15pt;display:inline-block;">✉</span> "' + buttonText + '" &nbsp;&nbsp;</span><br /><div style="font-size:8pt;margin-top:10px;">' + instructionText + '</div>';
    //            html += '       </td>';
    //            html += '   </tr>';
    //            //}
    //        }
    ...
    //renderWorkflowsDropDownList: function (bwRequestType, elementId) {
    //    try {
    //        console.log('In renderWorkflowsDropDownList(). tenantId: ' + tenantId + ', workflowAppId: ' + workflowAppId + ', bwRequestType: ' + bwRequestType);
    //        var thiz = this;
    //        debugger;
    //        //var spanWorkflowsDropDownList = document.getElementById('spanWorkflowsDropDownList');
    //        //if (spanWorkflowsDropDownList) { // This checks the element is being displayed. 
    //            $.ajax({
    //                //url: this.options.operationUriPrefix + "odata/WorkflowConfiguration/" + tenantId + '/' + workflowAppId + '/all',
    //                url: this.options.operationUriPrefix + "odata/WorkflowConfiguration2/" + this.options.bwTenantId + '/' + this.options.bwWorkflowAppId + '/' + bwRequestType, //'budgetrequest', // this.options.LastSelectedRequestType,
    //                dataType: "json",
    //                contentType: "application/json",
    //                type: "Get",
    //                timeout: this.options.ajaxTimeout
    ...
    //                    //debugger;
    //                    var html = '';
    //                    html += '<select style="padding:5px 5px 5px 5px;font-size:8pt;" id="workflowsDropDown" onchange="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'WorkflowsDropDown_Onchange\');">';

    //                    for (var i = 0; i < result.value.length; i++) {
    //                        //debugger;
    //                        if (!result.value[i].bwRequestType || (result.value[i].bwRequestType && result.value[i].bwRequestType == thiz.options.LastSelectedRequestType)) {
    //                            var isTheActiveWorkflow = Boolean(result.value[i].bwWorkflowActive);
    ...
    //                    document.getElementById(elementId).innerHTML = html; // 'spanWorkflowsDropDownList'
    ...
    renderSelectRoleOrPersonSection: function (elementId) {
        try {
            ...
            //alert('In renderSelectRoleOrPersonSection().');
            var isAdmin = false;
            if (elementId.indexOf('steprow-admin_') > -1) {
                // This is the ADMIN section. Limit what can be done here, just want to be able to change the assigned user.
                isAdmin = true;
            ...
            //    //html += '<select style="padding:5px 5px 5px 5px;" id="selectRoleName"  onchange="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'RoleDropDown_Onchange\', \'' + 'selectRoleName' + '\');">';
            //    //html += '  <option value=""></option>';
            //    //for (var i = 0; i < roles.length; i++) {
            ...
            //            //html += '&nbsp;<img style="width:18px;height:18px;cursor:pointer;vertical-align:text-bottom;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayPeoplePickerDialog\', \'' + 'txtRoleMembersFriendlyName_' + stepIndex + '_' + roleIndex + '' + '\', \'' + 'txtRoleMembersId_' + stepIndex + '_' + roleIndex + '' + '\', \'' + 'txtRoleMembersEmail_' + stepIndex + '_' + roleIndex + '' + '\');" src="images/addressbook-icon18x18.png">';
            ...
            //            //            roleMembersHtml += '<span style="cursor:pointer;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'DeleteRoleMember\', \'' + elementId + '\', \'' + result[i].bwParticipantId + '\');">&nbsp;&#128465;</span></li>';
            //            //        }
            ...
            //            //html += '<br /><img style="width:18px;height:18px;cursor:pointer;vertical-align:text-bottom;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayPeoplePickerDialog\', \'' + 'txtApprover2FriendlyName' + '\', \'' + 'txtApprover2Id' + '\', \'' + 'txtApprover2Email' + '\');" src="images/addressbook-icon18x18.png">';
            //            //html == '&nbsp;&nbsp;';
            //            //html += '<input id="btnCreateRole1" type="button" value="Select Person..." onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayPeoplePickerDialog\', \'' + 'txtApprover2FriendlyName' + '\', \'' + 'txtApprover2Id' + '\', \'' + 'txtApprover2Email' + '\');" />';

            //            //html += '<br />or add a new person:'; //<br /><input id="textNewRoleId" type="text" style="width:75pt;" onkeyup="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'NewRoleTextBox_Onkeyup\', \'' + 'textNewRoleId' + '\');" />';
            //            //html += '<br /><input id="btnCreateRole1" type="button" value="Add a Person..." onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayAddANewPersonDialog\');" />';
            ...
            $.ajax({
                ...
                success: function (results) {
                    ...
                    try {
                        ...
                        if (results) {
                            ...
                            console.log('In renderSelectRoleOrPersonSection().Get[odata/Roles].done: result: ' + JSON.stringify(results));
                            alert('Unexpected condition in renderSelectRoleOrPersonSection(). xx1');
                        }
                        var html = '';
                        var rolename = $('#' + elementId).find('.rolename').attr('bwOldValue');
                        var roleId = $('#' + elementId).find('.rolename').attr('bwRoleId');
                        //alert('xcx123123 roleId: ' + roleId);
                        html += 'Role:&nbsp;<select xcx="xcx1001" style="padding:5px 5px 5px 5px;" id="selectRoleName"  onchange="$(\'.bwWorkflowEditor\').bwWorkflowEditor(\'RoleDropDown_Onchange\', \'' + 'selectRoleName' + '\');">';
                        html += '  <option value=""></option>';
                        for (var i = 0; i < roles.length; i++) {
                        ...
                        var budgetThreshold = $('#' + elementId).find('.rolename').attr('bwbudgetthreshold');
                        ...
                        displayAlertDialog('Exception in renderSelectRoleOrPersonSection()[odata/Roles].done: ' + e.message + ', ' + e.stack);
                    }
                    //try {
                    //    //debugger;
                    //    // We need to get the workflowAppId, as well as roleId in order to get the list of users who belong to the role.
                    ...
                    //        roles = results.data;
                    //    } else {
                    //        console.log('In renderSelectRoleOrPersonSection().Get[odata/Roles].done: results: ' + JSON.stringify(results));
                    //        alert('Unexpected condition in renderSelectRoleOrPersonSection(). xx1');
                    //    }
                    //    var html = '';
                    //    var rolename = $('#' + elementId).find('.rolename').attr('bwOldValue');
                    //    var roleId = $('#' + elementId).find('.rolename').attr('bwRoleId');
                    //    //debugger;
                    //    html += 'Role:&nbsp;<select test="xcx1001" style="padding:5px 5px 5px 5px;" id="selectRoleName"  onchange="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'RoleDropDown_Onchange\', \'' + 'selectRoleName' + '\');">';
                    //    html += '  <option value=""></option>';
                    //    for (var i = 0; i < roles.length; i++) {
                    //        if (rolename == roles[i].RoleName) {
                    //            html += '  <option value="' + roles[i].RoleId + '" selected >' + roles[i].RoleName + '</option>';
                    ...
                    //    html += '<br />';
                    //    html += '<br />';
                    //    //html += '<input type="checkbox" checked id="cbOnlyInvolveRoleForAmountsOver_' + stepIndex + '_' + roleIndex + '">&nbsp;';
                    //    html += '<input type="checkbox" checked id="cbOnlyInvolveRoleForAmountsOver_' + stepIndex + '_' + roleIndex + '">&nbsp;';
                    //    html += 'Only participate for amounts over:';
                    //    html += '<br />';
                    //    html += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" value="$0.00" style="text-align:right;" />';
                    //    html += '<br />';
                    //    html += '<br />';
                ...
                error: function (data, errorCode, errorMessage) {
                    ...
                    console.log('Error in renderBwRoles(' + elementId + '):2:' + errorCode + ', ' + errorMessage);
            ...
            //var json3 = {
            //    bwTenantId: tenantId, //thiz.options.bwTenantId,
            ...
            //        //html += '&nbsp;<input style="padding:5px 10px 5px 10px;" id="btnCreateRole2" type="button" value="New Role..." onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayCreateANewRoleDialog\');" />';
            //        //html += '<br />';
            //        //$.ajax({
            //        //    url: thiz.options.operationUriPrefix + "odata/UserRole2/" + workflowAppId + "/" + roleId, // pass workflowAppId, as well as roleId in order to get the list of users who belong to the role.
            //        //    dataType: "json",
            //        //    contentType: "application/json",
            //        //    type: "Get",
            //        //    timeout: thiz.options.ajaxTimeout
            ...
            //        //        html += '<input id="txtRoleMembersEmail_' + stepIndex + '_' + roleIndex + '" style="display:none;" />';
            //        //        html += '&nbsp;<img style="width:18px;height:18px;cursor:pointer;vertical-align:text-bottom;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayPeoplePickerDialog\', \'' + 'txtRoleMembersFriendlyName_' + stepIndex + '_' + roleIndex + '' + '\', \'' + 'txtRoleMembersId_' + stepIndex + '_' + roleIndex + '' + '\', \'' + 'txtRoleMembersEmail_' + stepIndex + '_' + roleIndex + '' + '\');" src="images/addressbook-icon18x18.png">';
            //        //        html += '&nbsp;<input style="padding:5px 10px 5px 10px;" id="btnCreateRole1" type="button" value="New Person..." onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayAddANewPersonDialog\');" />';
            //        //        if (participantId) { // Only display when logged in. We need to do more work here!!
            //        //            if (result.length > 0) {
            //        //                var roleMembersHtml = '';
            ...
            //        //                roleMembersHtml += '<span style="color:darkgrey;cursor:help;" ';
            //        //                roleMembersHtml += ' title="';
            //        //                roleMembersHtml += 'RACI: ';
            //        //                roleMembersHtml += '\n';
            //        //                roleMembersHtml += 'R = Responsible (also Recommender)  ';
            //        //                roleMembersHtml += '\n';
            //        //                roleMembersHtml += 'A = Accountable (also Approver or final approving authority)  ';
            //        //                roleMembersHtml += '\n';
            //        //                roleMembersHtml += 'C = Consulted (sometimes Consultant or counsel)  ';
            //        //                roleMembersHtml += '\n';
            //        //                roleMembersHtml += 'I = Informed (also Informee)  ';
            //        //                roleMembersHtml += '" ';
            ...
            //        //                roleMembersHtml += '>';
            //        //                roleMembersHtml += 'RACI Role Member4(s):<ul>';
            ...
            //        //                for (var i = 0; i < result.length; i++) {
            //        //                    roleMembersHtml += '<li title="' + result[i].bwParticipantEmail + '">' + result[i].bwParticipantFriendlyName + ' - ' + result[i].OrgId;
            //        //                    roleMembersHtml += '<span style="cursor:pointer;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'DeleteRoleMember\', \'' + elementId + '\', \'' + result[i].bwParticipantId + '\');">&nbsp;&#128465;</span>';
            //        //                    roleMembersHtml += '</li>';
            //        //                }
            //        //                roleMembersHtml += '</ul></span>';
            //        //                html += roleMembersHtml;
            ...
            displayAlertDialog('Exception in renderSelectRoleOrPersonSection(): ' + e.message + ', ' + e.stack);
        }
    },
            ...
            if (selectedRoleCategory == 'Inform') {
                ...
                for (var i = 0; i < actions.length; i++) {
                    ...
                    if (actions[i] == 'Approve' || actions[i] == 'Revise/Hold') {
                        ...
                        html += '  <input type="checkbox" id="' + checkboxId + '" onchange="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'toggleChildCheckbox\', \'' + checkboxId + '\', \'' + childCheckboxId + '\');" />&nbsp;' + actions[i];
                    }
                    html += '  &nbsp;';
                    html += '  <input id="' + childCheckboxId + '" type="checkbox" onchange="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'toggleParentCheckbox\', \'' + childCheckboxId + '\', \'' + checkboxId + '\');" />&nbsp;require comments';
                    html += '  &nbsp;<span style="cursor:pointer;" title="Select to configure underlying properties..." onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayWorkflowActionsUnderlyingPropertiesDialog\', \'' + elementId + '\', \'' + actions[i] + '\');">&#8230;</span>'
                    html += '</span>';
                    html += '<br />';
                }
                actionsCell.html(html);
    ...
    renderTimeoutSection: function (elementId) {
        try {
            ...
            if (selectedRoleCategory == 'Inform') {
                ...
                html += '<span style="white-space:nowrap;">Timeout:&nbsp;<input type="text" id="textTimeout" style="width:25px;padding:5px 5px 5px 5px;" onkeyup="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'TimeoutTextBox_Onkeyup\', \'' + 'textTimeout' + '\');" />&nbsp;days</span>';
                timeoutCell.html(html);
            } else if (selectedRoleCategory == 'Approver') {
                timeoutCell.html(html);
            } else {
    ...
    renderTheCondReadOnly: function (cond) {
        try {
            console.log('In renderTheCondReadOnly().');
            var result = '';
            ...
            if (cond && cond.trim() != '') {
                ...
                if (conditionString.indexOf('$ChecklistsRequired~') > -1) {
                    //html += '<hr>';
                    //debugger;
                    html += '<span style="color:gray;font-style:italic;">Checklist(s):</span><br />';
                    var checklists = [];
    ...
    renderConditionsSection: function (elementId, conditionString) {
        try {
            ...
            //}
            //if (isExec && isExec == 'True') {
            //    html += '<span style="white-space:nowrap;"><input id="cbIsExec" type="checkbox" checked onchange="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'isExecCondition_CheckChanged\');" />&nbsp;' + 'Exec' + '&nbsp;</span>';
            //} else {
            //    html += '<span style="white-space:nowrap;"><input id="cbIsExec" type="checkbox" onchange="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'isExecCondition_CheckChanged\');" />&nbsp;' + 'Exec' + '&nbsp;</span>';
            //}
            ...
            //}
            //if (isLegal && isLegal == 'True') {
            //    html += '<span style="white-space:nowrap;"><input id="cbIsLegal" type="checkbox" checked onchange="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'isLegalCondition_CheckChanged\');" />&nbsp;' + 'Legal' + '&nbsp;</span>';
            //} else {
            //    html += '<span style="white-space:nowrap;"><input id="cbIsLegal" type="checkbox" onchange="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'isLegalCondition_CheckChanged\');" />&nbsp;' + 'Legal' + '&nbsp;</span>';
            //}
            //html += '&nbsp;';
            //// Is Lease
            //if (conditionString) {
            ...
            //}
            //if (isLease && isLease == 'True') {
            //    html += '<span style="white-space:nowrap;"><input id="cbIsLease" type="checkbox" checked onchange="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'isLeaseCondition_CheckChanged\');" />&nbsp;' + 'Lease' + '&nbsp;</span>';
            //} else {
            //    html += '<span style="white-space:nowrap;"><input id="cbIsLease" type="checkbox" onchange="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'isLeaseCondition_CheckChanged\');" />&nbsp;' + 'Lease' + '&nbsp;</span>';
            //}
            //html += '&nbsp;';
            //// Is IT
            //html += '&nbsp;';
            ...
            //html += '<span style="cursor:pointer;color:red;text-decoration:underline;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayOrgMultiPicker\', \'' + this.id + '\');">Location(s)</span>';
            //html += '&nbsp;&nbsp;&nbsp;&nbsp;';
            //html += '<span style="cursor:pointer;color:red;text-decoration:underline;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayProjectTypeMultiPicker\', \'' + this.id + '\');">Functional area(s)</span>';
            //html += '&nbsp;&nbsp;&nbsp;&nbsp;';
            //html += '<span style="cursor:pointer;color:red;text-decoration:underline;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'displayPillarMultiPicker\', \'' + this.id + '\');">Growth</span>';
            ...
            if (selectedRoleCategory == 'Approver' || selectedRoleCategory == 'Collaborator') {
                ...
                if (this.options.Checklists) {
                    ...
                    for (var i = 0; i < checklists.length; i++) {
                        ...
                        // Decide whether to check the checkbox later!!!!! The event is bound once this has been rendered. See code below.
                        if (conditionString) {
                            var checklistCheckboxId = 'cbChecklistRequired_' + elementId + '_' + checklists[i].bwChecklistTemplatesId;
                            if (conditionString.indexOf(checklists[i].bwChecklistTemplatesId) > -1) {
                                html += '<input id="' + checklistCheckboxId + '" checked="" type="checkbox" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'isChecklistRequired_CheckChanged\', \'' + checklists[i].bwChecklistTemplatesId + '\', \'' + checklistCheckboxId + '\');" />';
                                //html += '<input id="cbChecklistRequired_' + checklists[i].bwChecklistTemplatesId + '" type="checkbox" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'isChecklistRequired_CheckChanged\', this);" />';
                                html += '&nbsp;' + checklists[i].Title + '&nbsp;';
                            } else {
                                html += '<input id="' + checklistCheckboxId + '" type="checkbox" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'isChecklistRequired_CheckChanged\', \'' + checklists[i].bwChecklistTemplatesId + '\', \'' + checklistCheckboxId + '\');" />';
                                //html += '<input id="cbChecklistRequired_' + checklists[i].bwChecklistTemplatesId + '" type="checkbox" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'isChecklistRequired_CheckChanged\', this);" />';
                                html += '&nbsp;' + checklists[i].Title + '&nbsp;';
                            }
                        } else {
                        ...
                        //html += '<span style="cursor:pointer;" onclick="$(\'.bwWorkflowEditor_NewTenant\').bwWorkflowEditor_NewTenant(\'DeleteChecklist\', \'' + elementId + '\', \'' + checklists[i].bwChecklistTemplatesId + '\');">&nbsp;&#128465;</span>';
    ...
    WorkflowRequestTypeDropDown_Onchange: function (elementId) {
        try {
            ...
            try {
                ...
                alert('Set CurrentWorkflow xcx111774-13');
                var selectedRequestType;
                $('#' + elementId).find('option:selected').each(function (index, element) {
                    selectedRequestType = element.value;
                    thiz.options.LastSelectedRequestType = selectedRequestType;
                });
                //debugger;
                console.log('In WorkflowRequestTypeDropDown_Onchange(). selectedRequestType: ' + selectedRequestType);
                var workflow;
                for (var i = 0; i < this.options.Workflows.length; i++) {
                    if (this.options.Workflows[i].bwRequestType == selectedRequestType && this.options.Workflows[i].bwWorkflowActive == true) {
                        this.options.CurrentWorkflow = this.options.Workflows[i];
                        alert('Set CurrentWorkflow xcx111774-14');
                        //alert('Set CurrentWorkflow xcx111774');
                        break;
                    }
                }
                if (this.options.CurrentWorkflow == null) {
                    var html = '';
                    html += 'ERROR: NO ACTIVE WORKFLOW FOUND FOR THIS REQUEST TYPE: ' + selectedRequestType + ', so displaying default workflow.';
                    document.getElementById('spanWorkflowEditor_SelectedWorkflow').innerHTML = html;
                    this.options.CurrentWorkflow = this.options.Workflows[0];
                    alert('Set CurrentWorkflow xcx111774-16');
                    try {
                        debugger;
                        this.options.CurrentWorkflow.DraftWorkflow = JSON.parse(this.options.CurrentWorkflow.bwWorkflowJson); //.Workflow;
                        this.options.CurrentWorkflow.DraftWorkflow = JSON.parse(this.options.CurrentWorkflow.bwWorkflowJson); //.Workflow; //DraftWorkflow; // Creating "DraftWorkflow" so we can tell if the workflow has been changed or not, and then inform the user that changes need to be published.
                    } catch (e) { }
                ...
                console.log('Exception in bwWorkflowEditor_NewTenant._create().xx.Get:1: ' + e.message + ', ' + e.stack);
            }
        } catch (e) {
    ...
    WorkflowRequestTypeDropDownInDialog_Onchange: function (elementId) {
        try {
            ...
            //$('#divWorkflowsConfigurationDialog').dialog('close');
            thiz.displayWorkflowsConfigurationDialog();
            //thiz.renderWorkflowsDropDownList(selectedRequestType, elementId);

            //debugger;

            //// Step 1: Get the selected request type.
            ////var selectedRequestType = 'budgetrequest'; // The default.
            //var workflow;
            //// Step 2: Do the following for this request type.
            //for (var i = 0; i < result.value.length; i++) {
            //    if (result.value[i].bwRequestType == selectedRequestType) {
            //        //debugger;
            //        // This is the workflow.
            //        workflow = result.value[i];
            //        break;
            //    }
            //}
            ////debugger;
            //if (workflow) {
            //    workflow["Workflow"] = JSON.parse(workflow["bwWorkflowJson"]).Workflow;
            //    workflow["DraftWorkflow"] = JSON.parse(workflow["bwWorkflowJson"]).Workflow; //DraftWorkflow; // Creating "DraftWorkflow" so we can tell if the workflow has been changed or not, and then inform the user that changes need to be published.
            //    thiz.options.store = workflow;
            //    thiz.options.LastSelectedRequestType = selectedRequestType;
            //    debugger;
            //thiz.options.renderWorkflowsDropDownList(selectedRequestType, elementId);
            //} else {
            //    thiz.options.store = null;
            //}
            //thiz.renderWorkflowEditor(); // Render the Workflow Editor. 


            //} catch (e) {
            //    console.log('Exception in WorkflowRequestTypeDropDownInDialog_Onchange():1: ' + e.message + ', ' + e.stack);
            //}
            //}).fail(function (data) {
            //    //lpSpinner.Hide();
            //    debugger;
            ...
            //        msg = JSON.stringify(data);
            //    }
            //    alert('Exception inWorkflowRequestTypeDropDownInDialog_Onchange():2: ' + msg); //+ error.message.value + ' ' + error.innererror.message);
            //    console.log('Exception in WorkflowRequestTypeDropDownInDialog_Onchange():2: ' + JSON.stringify(data));
            //    //console.log('Fail in CarForm3.aspx.populateSpendForecastNotReadOnly().spendForecastItems.update: ' + JSON.stringify(data));
            //    //var error = JSON.parse(data.responseText)["odata.error"];
            //    //alert('Exception in CarForm3.aspx.populateSpendForecastNotReadOnly().spendForecastItems.update: ' + error.message.value + ' ' + error.innererror.message); // + ', EntityValidationErrors: ' + data.EntityValidationErrors);
            //});
