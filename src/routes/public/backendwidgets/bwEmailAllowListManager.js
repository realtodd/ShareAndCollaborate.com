$.widget("bw.bwEmailAllowListManager", {
    options: {
        ...
        */
        ...
        /*
        ===========================================================
        This is the bwEmailAllowListManager.js jQuery Widget. 
        ===========================================================
    ...
    _create: function (assignmentRowChanged_ElementId) {
        this.element.addClass("bwEmailAllowListManager");
        ...
        try {
            ...
            var html = '';
            html += '<span style="font-size:24pt;color:red;">bwEmailAllowListManager: CANNOT RENDER</span>';
            html += '<br />';
            html += '<span style="">Exception in bwEmailAllowListManager.Create(): ' + e.message + ', ' + e.stack + '</span>';
    ...
    loadDataFromTheDatabaseAndRender: function () {
        try {
            ...
            $.ajax({
                ...
                try {
                    ...
                    if (result.message != 'SUCCESS') {
                        ...
                        html += '<span style="font-size:24pt;color:red;">bwEmailAllowListManager: CANNOT RENDER</span>';
                        html += '<br />';
                        html += '<span style="">Error in bwEmailAllowListManager.loadDataFromTheDatabaseAndRender(): ' + result.message + '</span>';
                        thiz.element.html(html);
                    } else {
                        debugger;
                        ...
                        var timerServices = {
                            //WorkflowTimers_ElectedServerId: data.WorkflowTimers_ElectedServerId,
                            WorkflowTimers_ElectedThreadId: data.WorkflowTimers_ElectedThreadId,
                            //WorkflowTimers_ElectedServerLatestPing: data.WorkflowTimers_ElectedServerLatestPing,
                            TimerServices: [
                                {
                                    TimerServiceId: 'FILE_SERVICES_RESPONDING_TIMER',
                                    Title: 'Services Responding Checker',
                                    Description: 'This gives any server thread the permission to turn on and check that all the other services are running Ok. They will check the database and make sure the Elected Timer Thread is running, as well as other services as we see fit. In development. More to follow...',
                                    TimerInterval: data.FILE_SERVICES_RESPONDING_TIMER_Interval,
                                    isActive: data.FILE_SERVICES_RESPONDING_TIMER_Running,
                                    LastRunTimestamp: data.FILE_SERVICES_RESPONDING_TIMER_LastRunTimestamp,
                                    disabled: true
                                },
                                 {
                                     TimerServiceId: 'NEW_TASKS_TIMER',
                                     Title: 'New Tasks Timer',
                                     Description: 'Checks for new tasks, sets bwHasBeenProcessedByTheWorkflowEngine = true, updates DailyOverdueTaskNotificationDate to tomorrow, and sends emails. If email is turned off, they will go into the Pending Email queue. If this timer is not running, new tasks will stall, not send reminders, etc.',
                    ...
                    console.log('Exception in bwEmailAllowListManager.loadDataFromTheDatabaseAndRender().xx.Get:1: ' + e.message + ', ' + e.stack);
                }

            }).fail(function (data) {

                debugger;
                console.log('In bwEmailAllowListManager.loadDataFromTheDatabaseAndRender.RequestTypes.fail(): ' + JSON.stringify(data));
                var msg;
                ...
                alert('Exception in bwEmailAllowListManager.loadDataFromTheDatabaseAndRender().xx.Get:2: ' + msg); //+ error.message.value + ' ' + error.innererror.message);
                console.log('Exception in bwEmailAllowListManager.loadDataFromTheDatabaseAndRender().xx.Get:2: ' + JSON.stringify(data));
            ...
            html += '<span style="font-size:24pt;color:red;">bwEmailAllowListManager: CANNOT RENDER</span>';
            html += '<br />';
            html += '<span style="">Exception in bwEmailAllowListManager.loadDataFromTheDatabaseAndRender(): ' + e.message + ', ' + e.stack + '</span>';
            thiz.element.html(html);
        }
    },
    ...
    renderTimerServicesManager_ReadOnly: function () {
        try {
            ...
            html += '           Timer Services:&nbsp;';
            html += '           <br />';
            html += '           <input type="text" class="spanButton" value="Refresh" style="" onclick="$(\'#divBwTimerServicesManager\').bwEmailAllowListManager(\'loadDataFromTheDatabaseAndRender\');" />';
            html += '       </span>';
            html += '       <br /><br />';
            ...
            if (!thiz.options.TimerServices.TimerServices) {
                ...
                for (var i = 0; i < thiz.options.TimerServices.TimerServices.length; i++) {
                    ...
                    if (thiz.options.TimerServices.TimerServices[i].disabled == true) {
                        ...
                        var tmpLastRunTimestamp = $('.bwAuthentication').bwAuthentication('getBudgetWorkflowStandardizedDate', thiz.options.TimerServices.TimerServices[i].LastRunTimestamp);
                        html += '    <td>' + tmpLastRunTimestamp + '</td>';

                        //html += '    <td>' + thiz.options.TimerServices.TimerServices[i].TimerServiceId + '</td>';
                        //html += '    <td><button class="BwSmallButton" onclick="$(\'.bwEmailAllowListManager\').bwEmailAllowListManager(\'editATimerService\', \'' + thiz.options.TimerServices.TimerServices[i].TimerServiceId + '\', \'' + thiz.options.TimerServices.TimerServices[i].RequestType + '\', \'' + thiz.options.TimerServices.TimerServices[i].Abbreviation + '\', \'' + thiz.options.TimerServices.TimerServices[i].SingletonName + '\', \'' + thiz.options.TimerServices.TimerServices[i].PluralName + '\');">edit</button></td>';
                        //html += '    <td><img src="images/trash-can.png" onclick="$(\'.bwEmailAllowListManager\').bwEmailAllowListManager(\'deleteARequestType\', \'' + thiz.options.TimerServices.TimerServices[i].TimerServiceId + '\', \'' + thiz.options.TimerServices.TimerServices[i].RequestType + '\', \'' + thiz.options.TimerServices.TimerServices[i].Abbreviation + '\', \'' + thiz.options.TimerServices.TimerServices[i].SingletonName + '\', \'' + thiz.options.TimerServices.TimerServices[i].PluralName + '\');" title="Delete" style="cursor:pointer;" /></td>';
                        //html += '    <td></td>';
                        //html += '    <td></td>';
                        //html += '    <td></td>';
                        html += '  </tr>';
            ...
            html += '</table>';
            html += '<br />';
            //html += '<input style="padding:5px 10px 5px 10px;" id="btnCreateRole2" onclick="$(\'.bwEmailAllowListManager\').bwEmailAllowListManager(\'addARequestType\');" type="button" value="Add a Request type...">';
    ...
    renderTimerServicesManager: function () {
        try {
            console.log('In renderTimerServicesManager().');
            var thiz = this;
            var html = '';
            ...
            html += '       <br /><br />';
            html += '       <span style="font-size:12pt;color:darkgray;">';
            //html += '           ElectedServerId: ' + thiz.options.TimerServices.WorkflowTimers_ElectedServerId;
            //html += '           <br /><br />';
            html += '           ElectedThreadId: ' + thiz.options.TimerServices.WorkflowTimers_ElectedThreadId + '';
            //html += '           <br /><br />';
            //html += '           ElectedThreadLatestPing: ' + thiz.options.TimerServices.WorkflowTimers_ElectedServerLatestPing;
            html += '       </span>';
            html += '       <br /><br />';
            html += '       <div style="font-size:8pt;color:black;width:350px;">';
            html += '           Note that if NodeJS is recycled on the server, these Timer Services will not be running, and need to be restarted (because the Elected Thread disappeared).';
                ...
                if (thiz.options.TimerServices.TimerServices[i].disabled != true) {
                    ...
                    $('#configurationBehaviorEnable' + thiz.options.TimerServices.TimerServices[i].TimerServiceId + 'Slider').change(function () {
                        try {
                            ...
                            //alert('timerServiceId: ' + timerServiceId + ', isActive: ' + isActive);
                            data = {
                                ...
                                success: function (result) {
                                    try {
                                        ...
                                        if (result.message != 'SUCCESS') {
                                            ...
                                            debugger;
                                            // It was a success, do nothing.
                                            //alert('Successfully started ' + result.TimerServiceId + ', ' + JSON.stringify(result.Mod));
                                            thiz.loadDataFromTheDatabaseAndRender();

                                        }
                                    } catch (e) {
                                        console.log('Exception in renderTimerServicesManager.configurationBehaviorEnableRequestTypeSlider.change():2: ' + e.message + ', ' + e.stack);
                                        displayAlertDialog('Exception in renderTimerServicesManager.configurationBehaviorEnableRequestTypeSlider.change():2: ' + e.message + ', ' + e.stack);
                                    }
                                },
                                error: function (data, errorCode, errorMessage) {
                                    displayAlertDialog('Error in bwEmailAllowListManager.renderTimerServicesManager.configurationBehaviorEnableRequestTypeSlider.change(): ' + errorCode + ' ' + errorMessage);
                                }
                            });
                        } catch (e) {
                            console.log('Exception in renderTimerServicesManager.configurationBehaviorEnableRequestTypeSlider.change(): ' + e.message + ', ' + e.stack);
                            displayAlertDialog('Exception in renderTimerServicesManager.configurationBehaviorEnableRequestTypeSlider.change(): ' + e.message + ', ' + e.stack);
                        }
                    });
                }
            ...
            console.log('Exception in renderTimerServicesManager(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in renderTimerServicesManager(): ' + e.message + ', ' + e.stack);
        }
    },
