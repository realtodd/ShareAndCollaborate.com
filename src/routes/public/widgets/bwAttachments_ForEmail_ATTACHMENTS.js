$.widget("bw.bwAttachments_ForEmail_ATTACHMENTS", {
    options: {
        ...
        */
        ...
        /*
        ===========================================================
        This is the bwAttachments_ForEmail_ATTACHMENTS.js jQuery Widget. 
        ===========================================================
        ...
        */
        ...
        ATTACHMENTS: false, // This is here to support including this widget in the bwEmailClient_Haraka.js widget, and others that may be created in the future. This makes this widget very re-usable. 4-19-2024.
        INLINE: false, // This is here to support including this widget in the bwEmailClient_Haraka.js widget, and others that may be created in the future. This makes this widget very re-usable. 4-19-2024.
    ...
    _create: function () {
        console.log('In bwAttachments_ForEmail_ATTACHMENTS.js._create().');
        //alert('In bwAttachments_ForEmail_ATTACHMENTS.js._create().');
        this.element.addClass("bwAttachments_ForEmail_ATTACHMENTS");
        ...
        try {
            ...
            if (this.options.ATTACHMENTS == true) {
                ...
                console.log('In bwAttachments_ForEmail_ATTACHMENTS.js._create().option.STAND_ALONE == true.');
                this.renderAndPopulateAttachments_ATTACHMENTS(); // Need to render, not allowing the user to make modifications.

            } else if (this.options.INLINE == true) {

                console.log('In bwAttachments_ForEmail_ATTACHMENTS.js._create().option.INLINE == true.');
                this.renderAndPopulateAttachments_INLINE(); // Need to render, not allowing the user to make modifications.

            } else {
                var html = '';
                html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwAttachments_ForEmail_ATTACHMENTS</span>';
                html += '<br />';
            ...
            html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwAttachments_ForEmail_ATTACHMENTS</span>';
            html += '<br />';
            html += '<span style="">Exception in bwAttachments_ForEmail_ATTACHMENTS.Create(): ' + e.message + ', ' + e.stack + '</span>';
    ...
    _destroy: function () {
        this.element
            .removeClass("bwAttachments_ForEmail_ATTACHMENTS")
            .text("");
    },
    getDataType: function () {
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.getDataType().');

            return 'attachments';
        } catch (e) {
            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.getDataType(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.getDataType(): ' + e.message + ', ' + e.stack);
        }
    },
    getData: function () {
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.getData().');
            ...
            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.getData(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.getData(): ' + e.message + ', ' + e.stack);
        }
    },
    isARequiredField: function () {
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.isARequiredField().');
            return this.options.renderAsARequiredField;
        } catch (e) {
            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.isARequiredField(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.isARequiredField(): ' + e.message + ', ' + e.stack);
        }
    },
    getfieldTitle: function () {
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.getfieldTitle().');
            return this.options.fieldTitle;
        } catch (e) {
            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.getfieldTitle(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.getfieldTitle(): ' + e.message + ', ' + e.stack);
        }
    },



    dodrop: function (event, bwDraftEmailId) {
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.dodrop().');
            alert('In bwAttachments_ForEmail_ATTACHMENTS.js.dodrop().');
            var thiz = this;

            if (event.clipboardData) {

                console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.dodrop(). THIS IS clipboardData.');
                ...
                if (navigator.userAgent.toLowerCase().indexOf('firefox') > -1) {
                    ...
                    if (event.clipboardData.types.indexOf('Files') > -1) {
                        ...
                        //
                        alert('xcx23123-1');
                        this.uploadAttachment('newrequestattachments', null, bwDraftEmailId, event.clipboardData);
                ...
                console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.dodrop(). THIS IS dataTransfer.');
                ...
                if ((event.dataTransfer.types.indexOf('Files') > -1) && (event.dataTransfer.types.length == 1)) {
                    ...
                    //
                    alert('xcx23123-2');
                    this.uploadAttachment('newrequestattachments', null, bwDraftEmailId, event.dataTransfer);

                } else if ((event.dataTransfer.types.indexOf('text/uri-list') > -1)) {

                    //
                    // These are files dragged within the browser window. For example, from one request bwAttachments_ForEmail_ATTACHMENTS widget to another requests bwAttachments_ForEmail_ATTACHMENTS widget.
                    //
                    ...
                    if ((imageData.indexOf(strUrlWithWorkflowAppId) > -1) && (imageData.indexOf(strActiveStateIdentifierInUrl) > -1)) {
                        ...
                        console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.dodrop(). Yes the user is dragging and dropping an attachment from one request to another. This is a CUT operation... imageData: ' + imageData);
                        ...
                        console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.dodrop(). fileNameToCutAndPaste_Thumbnail: ' + fileNameToCutAndPaste_Thumbnail + ', bwBudgetRequestId_Origin: ' + bwBudgetRequestId_Origin + ', bwBudgetRequestId_Destination: ' + bwBudgetRequestId_Destination);
                        //displayAlertDialog('In bwAttachments_ForEmail_ATTACHMENTS.js.dodrop(). fileNameToCutAndPaste_Thumbnail: ' + fileNameToCutAndPaste_Thumbnail + ', bwBudgetRequestId_Origin: ' + bwBudgetRequestId_Origin + ', bwBudgetRequestId_Destination: ' + bwBudgetRequestId_Destination);
                            ...
                            success: function (results) {
                                try {
                                    ...
                                    if (results.status != 'SUCCESS') {
                                        ...
                                        var msg = 'Error in bwAttachments_ForEmail_ATTACHMENTS.js.dodrop.bwCutAndPaste_Attachment(). ' + results.status + ': ' + results.message;
                                        console.log(msg);
                                    ...
                                    for (var i = 0; i < forms.length; i++) {
                                        ...
                                        if ((bwBudgetRequestId_Temp == bwBudgetRequestId_Origin) || (bwBudgetRequestId_Temp == bwBudgetRequestId_Destination)) {
                                            requestCounter += 1;
                                            $(forms[i]).find('.bwAttachments_ForEmail_ATTACHMENTS').bwAttachments_ForEmail_ATTACHMENTS('renderAndPopulateAttachments'); // Call this method for this specific widget. This will re-generate the gold colored drag-and-drop section... etc...
                                        }
                                    }
                                    ...
                                    console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.dodrop():2: ' + e.message + ', ' + e.stack);
                                    displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.dodrop():2: ' + e.message + ', ' + e.stack);
                                    $(this.element).find('#newrequestattachments')[0].innerHTML = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.dodrop():2: ' + e.message + ', ' + e.stack;
                                }
                            },
                            error: function (data, errorCode, errorMessage) {
                                $('#divWorkingOnItDialog').dialog('close'); // Close the "Working on it..." dialog.
                                console.log('Error in bwAttachments_ForEmail_ATTACHMENTS.js.dodrop(): ' + errorMessage);
                                debugger;
                                if (errorMessage == 'timeout') {
                                    displayAlertDialog('The file server could not be contacted. Please try again in a few minutes.');
                                } else {
                                    displayAlertDialog('Error in bwAttachments_ForEmail_ATTACHMENTS.js.dodrop(): ' + errorMessage);
                                }
                            }
                        });
                        ...
                        fetch(imageData)
                            ...
                            .then(function (blob) {
                                try {
                                    ...
                                    var fileInput = document.getElementById('inputFile_' + bwDraftEmailId);

                                    if (!fileInput) {

                                        var imgSrc2 = $(thiz.element).find('#dropzone1 img')[0].src;

                                        alert('Error in bwAttachments_ForEmail_ATTACHMENTS.js.dodrop(). Cannot find element: inputFile_' + bwDraftEmailId + ', imgSrc2: ' + imgSrc2);
                                    ...
                                    thiz.uploadAttachment('newrequestattachments', null, bwDraftEmailId);

                                } catch (e) {
                                    alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.dodrop(). xcx215343: ' + e.message + ', ' + e.stack);
                ...
                displayAlertDialog('Error in bwAttachments_ForEmail_ATTACHMENTS.js.dodrop(). Could not find a valid DataTransfer object.');
            ...
            //alert('In bwAttachments_ForEmail_ATTACHMENTS.js.dodrop(). dt.types: ' + dt.types);
            ...
            //        console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments.dropzone.drop(). Yes the user is dragging and dropping an attachment from one request to another. This is a CUT operation... imageData: ' + imageData);
            ...
            //                        var msg = 'Error in bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments.dropzone.drop.bwCutAndPaste_Attachment(). ' + results.status + ': ' + results.message;
            //                        console.log(msg);
            //                        displayAlertDialog_Persistent(msg);
            ...
            //                } catch (e) {
            //                    console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments.dropzone.drop():2: ' + e.message + ', ' + e.stack);
            //                    displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments.dropzone.drop():2: ' + e.message + ', ' + e.stack);
            //                    $(this.element).find('#newrequestattachments')[0].innerHTML = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments.dropzone.drop():2: ' + e.message + ', ' + e.stack;
            //                }
            //            },
            //            error: function (data, errorCode, errorMessage) {
            //                $('#divWorkingOnItDialog').dialog('close'); // Close the "Working on it..." dialog.
            //                console.log('Error in bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments.dropzone.drop(): ' + errorMessage);
            //                debugger;
            //                if (errorMessage == 'timeout') {
            //                    displayAlertDialog('The file server could not be contacted. Please try again in a few minutes.');
            //                } else {
            //                    displayAlertDialog('Error in bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments.dropzone.drop(): ' + errorMessage);
            //                }
            //            }
            //        });
            ...
            //                        alert('Error in bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments.dropzone.drop(). Cannot find element: inputFile_' + bwBudgetRequestId + ', imgSrc2: ' + imgSrc2);
            ...
            //                } catch (e) {
            //                    alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments.dropzone.drop(). xcx215343: ' + e.message + ', ' + e.stack);
            ...
            //    console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments.dropzone.drop(). No base64 in the img src or this element. It must be a http reference... imgSrc: ' + imgSrc);
            ...
            //        console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments.dropzone.drop(). Yes the user is dragging and dropping an attachment from one request to another. This is a CUT operation... imgSrc: ' + imgSrc);
            ...
            console.log('Exception in drop xcx123124235: ' + e.message + ', ' + e.stack);
            alert('Exception in drop xcx123124235: ' + e.message + ', ' + e.stack);
        }
    },

    renderAndPopulateAttachments_ATTACHMENTS: function () {
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments_ATTACHMENTS().');
            //alert('In bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments_ATTACHMENTS().');
            var thiz = this;
            ...
            debugger;
            //try {
            //bwDraftEmailId = $('#displayedemaildetails')[0].getAttribute('bwdraftemailid');

            debugger;
            _id = $(thiz.element).closest('.bwEmailClient_Haraka').find('#divEmailClient_CompositionWindow')[0].getAttribute('_id'); // divEmailClient_CompositionWindow


            //alert('>>>In bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments_ATTACHMENTS(). _id: ' + _id);


            console.log('>>>>>>In bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments_ATTACHMENTS(). _id: ' + _id);
            //alert('>>>>>>In bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments_ATTACHMENTS(). _id: ' + _id);
            ...
            if (!_id) {
                ...
                //alert('Error in bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments_ATTACHMENTS(). Unexpected value for bwDraftEmailId: ' + bwDraftEmailId);
                //displayAlertDialog('Error in bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments_ATTACHMENTS(). Unexpected value for bwDraftEmailId: ' + bwDraftEmailId);

                _id = 'NEW_EMAIL';

            }

            var requestDialogId = 'divRequestFormDialog_' + _id; // eg: divRequestFormDialog_d338d498-987f-4fec-872a-3fe6a3cc65c7
            if ($('#' + requestDialogId)) {
            ...
            html += '   <tbody>';
            html += '       <tr xcx="xcx21342526-1" id="xcx21342526-1" class="xdTableOffsetRow" id="toolboxdraggablerow_2" draggable="true" bwwidgetname="bwAttachments_ForEmail_ATTACHMENTS">';
            ...
            html += '<form id="uploadForm_ATTACHMENTS_' + _id + '" action="/" enctype="multipart/form-data" method="post">';
            ...
            html += '<input type="hidden" name="bwDraftEmailId" value="" />';
            ...
            //alert('xcx23123-6 bwDraftEmailId: ' + bwDraftEmailId);
            html += '<input type="file" multiple name="multerFileUploadIdentifier_original" id="inputFile_ATTACHMENTS_' + _id + '" class="upload" style="visibility:hidden;" onchange="$(\'.bwAttachments_ForEmail_ATTACHMENTS:first\').bwAttachments_ForEmail_ATTACHMENTS(\'uploadAttachment\',\'newrequestattachments\', \'' + requestDialogId + '\', \'ATTACHMENTS\');" />';
            //html += '<input type="file" name="multerFileUploadIdentifier_original" id="inputFile_' + bwBudgetRequestId + '" class="upload" style="visibility:hidden;" onchange="$(\'.bwAttachments_ForEmail_ATTACHMENTS:first\').bwAttachments_ForEmail_ATTACHMENTS(\'uploadAttachment\',\'newrequestattachments\', \'' + requestDialogId + '\', \'' + bwBudgetRequestId + '\');" />';





            //alert('Rendering onchange .bwAttachments_ForEmail_ATTACHMENTS:closest uploadAttachment().')
            ...
            html += '           <p id="newrequestattachments_ATTACHMENTS" xcx="xcx448-1-1-6" style="word-break:break-all;white-space:normal;"></p>';
            ...
            //html += '               onclick="$(\'.bwAttachments_ForEmail_ATTACHMENTS:first\').bwAttachments_ForEmail_ATTACHMENTS(\'copyAttachments\',\'newrequestattachments\', \'' + requestDialogId + '\', \'' + bwDraftEmailId + '\', \'' + bwRequestTitle + '\', \'' + bwRequestTypeId + '\');" />';
            ...
            html += '               onclick="$(\'.bwAttachments_ForEmail_ATTACHMENTS:first\').bwAttachments_ForEmail_ATTACHMENTS(\'performOCR_MultipleImages\', \'' + requestDialogId + '\', \'' + _id + '\', \'' + bwRequestTitle + '\', \'' + bwRequestTypeId + '\');" />';
            ...
            if (this.options.inFormsEditor != true) {
                ...
                //debugger;
                //var element = $('.bwAttachments_ForEmail_ATTACHMENTS:first').find('.xdTableOffsetRow');

                $('.bwAttachments_ForEmail_ATTACHMENTS:first').find('.xdTableOffsetRow').removeAttr('draggable'); // 4-19-2024. 
                $('.bwAttachments_ForEmail_ATTACHMENTS:first').find('.xdTableOffsetRow').removeClass('xdTableOffsetRow-editor');
            ...
            $(this.element).find('#newrequestattachments_ATTACHMENTS')[0].innerHTML = '<span style="color:tomato;"></span>';

            var dropzone = document.getElementById('bwAttachments_ForEmail_ATTACHMENTS_dropzone_ATTACHMENTS_' + _id);
            ...
            dropzone.addEventListener('drop', function (e) {
                try {
                    console.log('In renderAndPopulateAttachments_ATTACHMENTS.dropzone.drop().');
                    e.preventDefault();

                    var gotData;
                    if (e.clipboardData) {
                        console.log('In renderAndPopulateAttachments_ATTACHMENTS.dropzone.drop(). Object is event.clipboardData.');
                        gotData = true;
                    } else if (e.dataTransfer) {
                        console.log('In renderAndPopulateAttachments_ATTACHMENTS.dropzone.drop(). Object is event.dataTransfer.');
                        gotData = true;
                    } else if (window.clipboardData) {
                        console.log('In renderAndPopulateAttachments_ATTACHMENTS.dropzone.drop(). Object is window.clipboardData.');
                        gotData = true;
                    } else {
                        ...
                        if (navigator.userAgent.toLowerCase().indexOf('firefox') > -1) {
                            ...
                            alert('Error in bwAttachments_ForEmail_ATTACHMENTS.js.dropzone.drop(). Could not find a valid DataTransfer object. e: ' + JSON.stringify(e) + ', navigator.userAgent: ' + navigator.userAgent);
                        }
                    ...
                    if (gotData == true) {
                        ...
                        alert('Error in bwAttachments_ForEmail_ATTACHMENTS.js.dropzone.drop(). Could not find a valid DataTransfer object.');
                    }

                } catch (e) {
                    var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.dropzone.drop(): ' + e.message + ', ' + e.stack;
                    console.log(msg);
                    alert(msg);
                }
            });
            ...
            dropzone.addEventListener('paste', function (e) {
                try {
                    console.log('In renderAndPopulateAttachments_ATTACHMENTS.dropzone.paste().');
                    e.preventDefault();

                    var gotData;
                    if (e.clipboardData) {
                        console.log('In renderAndPopulateAttachments_ATTACHMENTS.dropzone.paste(). Object is event.clipboardData.');
                        gotData = true;
                    } else if (e.dataTransfer) {
                        console.log('In renderAndPopulateAttachments_ATTACHMENTS.dropzone.paste(). Object is event.dataTransfer.');
                        gotData = true;
                    } else if (window.clipboardData) {
                        console.log('In renderAndPopulateAttachments_ATTACHMENTS.dropzone.paste(). Object is window.clipboardData.');
                        gotData = true;
                    } else {
                        ...
                        if (navigator.userAgent.toLowerCase().indexOf('firefox') > -1) {
                            ...
                            alert('Error in bwAttachments_ForEmail_ATTACHMENTS.js.dropzone.paste(). Could not find a valid DataTransfer object. e: ' + JSON.stringify(e) + ', navigator.userAgent: ' + navigator.userAgent);
                        }
                    ...
                    if (gotData == true) {
                        ...
                        alert('Error in bwAttachments_ForEmail_ATTACHMENTS.js.dropzone.paste(). Could not find a valid DataTransfer object.');
                    }

                } catch (e) {
                    var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.dropzone.paste(): ' + e.message + ', ' + e.stack;
                    console.log(msg);
                    alert(msg);
                }
            });
            ...
            if (this.options.jsonData) {
                ...
                ////alert('In bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments(). this.options.jsonData.bwBudgetRequestId: ' + this.options.jsonData.bwBudgetRequestId + ', this.options.jsonData: ' + JSON.stringify(this.options.jsonData));
                console.log('Calling bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). xcx13124-1');
                //if (this.options.jsonData.bwWorkflowAppId && this.options.jsonData.bwBudgetRequestId) {

                console.log('xcx213234-19 calling populateAttachments(). _id: ' + _id); // << THIS IS THE INCORRECT bwBudgetRequestId.
                ...
                //this.populateAttachments(workflowAppId, bwDraftEmailId, 'newrequestattachments');
            ...
            console.log('222>>>in renderAndPopulateAttachments_ATTACHMENTS() _id: ' + _id);
            //alert('222>>>in renderAndPopulateAttachments_ATTACHMENTS() _id: ' + _id);

            debugger;
            if (_id != 'NEW_EMAIL') {
                thiz.populateAttachments(null, _id, 'ATTACHMENTS');
            ...
            console.log('Exception in renderAndPopulateAttachments_ATTACHMENTS(): ' + e.message + ', ' + e.stack);
            var html = '';
            html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwAttachments_ForEmail_ATTACHMENTS</span>';
            html += '<br />';
            html += '<span style="">Exception in bwAttachments_ForEmail_ATTACHMENTS.renderAndPopulateAttachments_ATTACHMENTS(): ' + e.message + ', ' + e.stack + '</span>';
            this.element.html(html);
        }
    },

    renderAndPopulateAttachments_INLINE: function () {
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments_INLINE().');
            alert('In bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments_INLINE().');
            var thiz = this;
            ...
            //try {
            //    bwDraftEmailId = $(this.element).closest('#budgetrequestform')[0].getAttribute('bwdraftemailid');
            //    bwRequestTitle = $(this.element).closest('#budgetrequestform')[0].getAttribute('bwrequesttitle');
            ...
            if (!bwDraftEmailId) {
                ...
                //alert('Error in bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments_INLINE(). Unexpected value for bwDraftEmailId: ' + bwDraftEmailId);
                //displayAlertDialog('Error in bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments_INLINE(). Unexpected value for bwDraftEmailId: ' + bwDraftEmailId);

                bwDraftEmailId = 'NEW_EMAIL';

            } //else {

            var requestDialogId = 'divRequestFormDialog_' + bwDraftEmailId; // eg: divRequestFormDialog_d338d498-987f-4fec-872a-3fe6a3cc65c7
            if ($('#' + requestDialogId)) {
                // This is the name of the dialog.
            } else {
                requestDialogId = 'divCreateRequestFormContent'; // This is the name of the dialog.
            ...
            if (/(iPad|iPhone|iPod)/g.test(navigator.userAgent)) {
                ...
                console.log('In renderAndPopulateAttachments_INLINE(). xcx1213-2 RENDERING ELEMENT FOR click(); bwDraftEmailId: ' + bwDraftEmailId);
                var inputFileElementId = 'inputFile_INLINE_' + bwDraftEmailId;


                //
                // allow="clipboard-read;clipboard-write;" MAY BE VERY IMPORTANT!!!!!!!!!! 11-27-2023 3-20pm ast.
                //
                html += '                           <div id="bwAttachments_ForEmail_ATTACHMENTS_dropzone_INLINE_' + bwDraftEmailId + '" allow="clipboard-read;clipboard-write;" onclick="document.getElementById(\'' + inputFileElementId + '\').click();" contenteditable="true" style="height:180px;width:120px;border:1px solid goldenrod;background-color:#f6f6f6;color:gray;font-size:10pt;cursor:pointer;background-image:url(\'/images/upload_arrow_icon.png\');background-repeat: no-repeat;background-size: 130px;text-align: center;padding: 5px; background-position: center bottom;" >';
                html += '                               <div style="color:goldenrod;">Click, right-click, or drag-and-drop to add files inline in the email body.</div>';
                html += '                           </div>';
            ...
            html += '<form id="uploadForm_INLINE_' + bwDraftEmailId + '" action="/" enctype="multipart/form-data" method="post">';
            ...
            //
            console.log('The "multiple" attribute in the input/file element below turns on the ability to select multiple items for the upload. THIS IS TURNED OFF FOR THE MOMENT UNTILL WE UPGRADE THE WEB SERVICE ETC. 4-15-2023.');
            //alert('xcx23123-7 bwDraftEmailId: ' + bwDraftEmailId);
            html += '<input type="file" multiple name="multerFileUploadIdentifier_original" id="inputFile_INLINE_' + bwDraftEmailId + '" class="upload" style="visibility:hidden;" onchange="$(\'.bwAttachments_ForEmail_ATTACHMENTS:first\').bwAttachments_ForEmail_ATTACHMENTS(\'uploadAttachment\',\'newrequestattachments\', \'' + requestDialogId + '\', \'INLINE\');" />';
            //html += '<input type="file" name="multerFileUploadIdentifier_original" id="inputFile_' + bwBudgetRequestId + '" class="upload" style="visibility:hidden;" onchange="$(\'.bwAttachments_ForEmail_ATTACHMENTS:first\').bwAttachments_ForEmail_ATTACHMENTS(\'uploadAttachment\',\'newrequestattachments\', \'' + requestDialogId + '\', \'' + bwDraftEmailId + '\');" />';
            ...
            html += '           <p id="newrequestattachments_INLINE" xcx="xcx448-1-1-7" style="word-break:break-all;white-space:normal;"></p>';
            ...
            // "Email attachments externally..." button.
            html += '           <input id="' + requestDialogId + '_buttonEmailAttachmentsExternally" type="button" style="display:none;padding:5px 10px 5px 10px;" value="Convert to files as attachments..." title="Email attachments to a customer using your internet facing url, which is configured in the organization settings." class="BwButton350" ';
            html += '               onclick="$(\'.bwAttachments_ForEmail_ATTACHMENTS:first\').bwAttachments_ForEmail_ATTACHMENTS(\'copyAttachments\',\'newrequestattachmentsxcx2334\', \'' + requestDialogId + '\', \'' + bwDraftEmailId + '\', \'' + bwRequestTitle + '\', \'' + bwRequestTypeId + '\');" />';
            ...
            // "OCR attachments..." button.
            html += '           <input id="' + requestDialogId + '_buttonOCRAttachments" type="button" style="display:none;padding:5px 10px 5px 10px;" value="OCR attachment(s)..." title="Perform OCR on multiple attachments at one time." class="BwButton350" ';
            html += '               onclick="$(\'.bwAttachments_ForEmail_ATTACHMENTS:first\').bwAttachments_ForEmail_ATTACHMENTS(\'performOCR_MultipleImages\', \'' + requestDialogId + '\', \'' + bwDraftEmailId + '\', \'' + bwRequestTitle + '\', \'' + bwRequestTypeId + '\');" />';
            ...
            $(this.element).find('#newrequestattachments_INLINE')[0].innerHTML = '<span style="color:tomato;"></span>';

            var dropzone = document.getElementById('bwAttachments_ForEmail_ATTACHMENTS_dropzone_INLINE_' + bwDraftEmailId);
            ...
            dropzone.addEventListener('drop', function (e) {
                try {
                    console.log('In renderAndPopulateAttachments_INLINE.dropzone.drop().');
                    e.preventDefault();

                    var gotData;
                    if (e.clipboardData) {
                        console.log('In renderAndPopulateAttachments_INLINE.dropzone.drop(). Object is event.clipboardData.');
                        gotData = true;
                    } else if (e.dataTransfer) {
                        console.log('In renderAndPopulateAttachments_INLINE.dropzone.drop(). Object is event.dataTransfer.');
                        gotData = true;
                    } else if (window.clipboardData) {
                        console.log('In renderAndPopulateAttachments_INLINE.dropzone.drop(). Object is window.clipboardData.');
                        gotData = true;
                    } else {
            ...
            dropzone.addEventListener('paste', function (e) {
                try {
                    console.log('In renderAndPopulateAttachments_INLINE.dropzone.paste().');
                    e.preventDefault();

                    var gotData;
                    if (e.clipboardData) {
                        console.log('In renderAndPopulateAttachments_INLINE.dropzone.paste(). Object is event.clipboardData.');
                        gotData = true;
                    } else if (e.dataTransfer) {
                        console.log('In renderAndPopulateAttachments_INLINE.dropzone.paste(). Object is event.dataTransfer.');
                        gotData = true;
                    } else if (window.clipboardData) {
                        console.log('In renderAndPopulateAttachments_INLINE.dropzone.paste(). Object is window.clipboardData.');
                        gotData = true;
                    } else {
            ...
            if (this.options.jsonData) {
                ...
                console.log('xcx213234-19 calling populateAttachments(). bwDraftEmailId: ' + bwDraftEmailId); // << THIS IS THE INCORRECT bwBudgetRequestId.

                //    this.populateAttachments(this.options.jsonData.bwWorkflowAppId, this.options.jsonData.bwBudgetRequestId, 'newrequestattachments');
                //}
                var workflowAppId = $('.bwAuthentication').bwAuthentication('option', 'workflowAppId');

                //this.populateAttachments(workflowAppId, bwDraftEmailId, 'INLINE');
            ...
            //    var rootFilePath_email = 'public/_files/_draftEmailAttachments/'; // 4-23-2024.
            //    //var xx = rootFilePath_email + bwParticipantId + '/' + bwDraftEmailId; // 'bwDraftEmailId'
            //    var x = 'xcx2314234: <img src="cid:' + rootFilePath_email + 'pancake.jpg" />';
            ...
            //    $('#newrequestattachments').html(x); // newrequestattachments

            //    //thiz.populateAttachments(null, bwDraftEmailId, 'newrequestattachments');
            ...
            //}
            debugger;
            if (bwDraftEmailId != 'NEW_EMAIL') {
                thiz.populateAttachments(null, bwDraftEmailId, 'INLINE');
            }

        } catch (e) {
            console.log('Exception in renderAndPopulateAttachments_INLINE(): ' + e.message + ', ' + e.stack);
            var html = '';
            html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwAttachments_ForEmail_ATTACHMENTS</span>';
            html += '<br />';
            html += '<span style="">Exception in bwAttachments_ForEmail_ATTACHMENTS.renderAndPopulateAttachments_INLINE(): ' + e.message + ', ' + e.stack + '</span>';
            this.element.html(html);
        }
    },
    ...
    ConvertMediaFileToBaselineShareable: function (bwBudgetRequestId, filename) {
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.ConvertMediaFileToBaselineShareable().');
            alert('In bwAttachments_ForEmail_ATTACHMENTS.js.ConvertMediaFileToBaselineShareable().');
            var thiz = this;
            ...
            //      - subsequently, the bwAttachments_ForEmail_ATTACHMENTS.js widget gets progress/status updates by calling /_fileconversion/GetMediaFileConversionProgress.
            //
            //
            //
                ...
                success: function (results) {
                    try {
                        ...
                        if (results.status != 'SUCCESS') {
                            ...
                            displayAlertDialog('In bwAttachments_ForEmail_ATTACHMENTS.js.ConvertMediaFileToBaselineShareable.success(): ' + results.status + ', ' + results.message);
                        ...
                        console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.ConvertMediaFileToBaselineShareable.success(): ' + e.message + ', ' + e.stack);
                        displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.ConvertMediaFileToBaselineShareable.success(): ' + e.message + ', ' + e.stack);
                    }
                },
                error: function (data, errorCode, errorMessage) {
                    ...
                    console.log('Error in bwAttachments_ForEmail_ATTACHMENTS.js.ConvertMediaFileToBaselineShareable.error(): ' + errorMessage);
                    displayAlertDialog('Error in bwAttachments_ForEmail_ATTACHMENTS.js.ConvertMediaFileToBaselineShareable.error(): ' + errorMessage);
                }
            });
            ...
            var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.ConvertMediaFileToBaselineShareable(): ' + e.message + ', ' + e.stack;
            console.log(msg);
    ...
    launchFileConversion: function (bwBudgetRequestId, filename) {
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.launchFileConversion().');
            var thiz = this;
            ...
            $.ajax({
                ...
                success: function (data) {
                    try {
                        ...
                        console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.launchFileConversion.success(): ' + e.message + ', ' + e.stack);
                        displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.launchFileConversion.success(): ' + e.message + ', ' + e.stack);
                    }
                },
                error: function (data, errorCode, errorMessage) {
                    console.log('Error in bwAttachments_ForEmail_ATTACHMENTS.js.launchFileConversion.error(): ' + e.message + ', ' + e.stack);
                    displayAlertDialog('Error in bwAttachments_ForEmail_ATTACHMENTS.js.launchFileConversion.error(): bwBudgetRequestId: ' + bwBudgetRequestId + ', filename: ' + filename + ', errorMessage: ' + errorMessage);
                }
            });

        } catch (e) {
            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.launchFileConversion(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.launchFileConversion(): ' + e.message + ', ' + e.stack);
        }
    },
    getExifData: function (bwBudgetRequestId, filename) {
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.getExifData().');
            var thiz = this;
            ...
            $.ajax({
                ...
                success: function (results) {
                    try {
                        ...
                        console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.getExifData.success(): ' + e.message + ', ' + e.stack);
                        displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.getExifData.success(): ' + e.message + ', ' + e.stack);
                    }
                },
                error: function (data, errorCode, errorMessage) {
                    console.log('Error in bwAttachments_ForEmail_ATTACHMENTS.js.getExifData.error(): ' + e.message + ', ' + e.stack);
                    displayAlertDialog('Error in bwAttachments_ForEmail_ATTACHMENTS.js.getExifData.error(): bwBudgetRequestId: ' + bwBudgetRequestId + ', filename: ' + filename + ', errorMessage: ' + errorMessage);
                }
            });

        } catch (e) {
            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.getExifData(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.getExifData(): ' + e.message + ', ' + e.stack);
        }
    },

    performOCR: function (bwBudgetRequestId, filename) {
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.performOCR().');
            ...
            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.performOCR(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.performOCR(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.performOCR(): ' + e.message + ', ' + e.stack);
        }
    },

    performOCR_MultipleImages: function (requestDialogId, bwBudgetRequestId, bwRequestTitle, bwRequestTypeId) { // filename) {
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.performOCR_MultipleImages().');
            var thiz = this;
            //alert('In bwAttachments_ForEmail_ATTACHMENTS.js.performOCR_MultipleImages(). bwBudgetRequestId: ' + bwBudgetRequestId + ', filename: ' + filename);
            ...
            alert('Calling getlistofattachments_foremail(). xcx2343266-2');
            //var operationUri = globalUrlPrefix + globalUrlForWebServices + '/_files/' + 'getlistofattachmentsforbudgetrequest';
            var operationUri = globalUrlPrefix + globalUrlForWebServices + '/_files/' + 'getlistofattachments_foremail';
            $.ajax({
                url: operationUri,
                type: 'POST',
                data: data,
                ...
                success: function (results) {
                    try {
                        ...
                        if (results.status != 'SUCCESS') {
                            ...
                            //                console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.copyAttachments.displayImageThumbnail(): ' + e.message + ', ' + e.stack);
                            //                displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.copyAttachments.displayImageThumbnail(): ' + e.message + ', ' + e.stack);
                            ...
                            //        console.log('Fail in bwAttachments_ForEmail_ATTACHMENTS.js.copyAttachments.displayImageThumbnail(): ' + JSON.stringify(result));
                            //        displayAlertDialog('Fail in bwAttachments_ForEmail_ATTACHMENTS.js.copyAttachments.displayImageThumbnail(): ' + JSON.stringify(result));
                            //        document.getElementById(imgId).src = globalUrlPrefix + globalUrl + '/images/mp4.jfif'; // There is no thumbnail, so display the icon.
                            //    });
                            //}
                            ...
                            //            var imgId = 'img_bwAttachments_ForEmail_ATTACHMENTS_' + thiz.options.elementIdSuffix + '_' + bwBudgetRequestId + '_' + i;
                        ...
                        console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.copyAttachments.dialog.open.ajax.success(): ' + e.message + ', ' + e.stack);
                        displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.copyAttachments.dialog.open.ajax.success(): ' + e.message + ', ' + e.stack);
                    }

                },
                error: function (data, errorCode, errorMessage) {
                    console.log('Error in bwAttachments_ForEmail_ATTACHMENTS.js.copyAttachments.dialog.open.ajax.error(): ' + errorMessage + ', workflowAppId: ' + workflowAppId + ', bwBudgetRequestId: ' + bwBudgetRequestId + ', participantId: ' + participantId);
                    displayAlertDialog('Error in bwAttachments_ForEmail_ATTACHMENTS.js.copyAttachments.dialog.open.ajax.error(): ' + errorMessage + ', workflowAppId: ' + workflowAppId + ', bwBudgetRequestId: ' + bwBudgetRequestId + ', participantId: ' + participantId);
                }
            });

        } catch (e) {
            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.performOCR_MultipleImages(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.performOCR_MultipleImages(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.performOCR_MultipleImages(): ' + e.message + ', ' + e.stack);
        }
    },

    repairFileType: function (bwBudgetRequestId, originalFilename) {
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.repairFileType().');
            var thiz = this;
            ...
            if (!div) {
                ...
                console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.repairFileType(). adding element to the dom: ' + 'divRepairFileTypeDialog');
                    ...
                    $.ajax({
                        ...
                        success: function (results) {
                            try {
                                ...
                                console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.repairFileType.post.removeattachment.success(): ' + e.message + ', ' + e.stack);
                                displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.repairFileType.post.removeattachment.success(): ' + e.message + ', ' + e.stack);
                            }
                        },
                        error: function (data, errorCode, errorMessage) {
                            console.log('Error in bwAttachments_ForEmail_ATTACHMENTS.js.repairFileType.post.removeattachment.error(): ' + e.message + ', ' + e.stack);
                            displayAlertDialog('Error in bwAttachments_ForEmail_ATTACHMENTS.js.repairFileType.post.removeattachment.error(): bwBudgetRequestId: ' + bwBudgetRequestId + ', filename: ' + filename + ', errorMessage: ' + errorMessage);
                        }
                    });
                } catch (e) {
                    console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.repairFileType.deletebutton.click(): ' + e.message + ', ' + e.stack);
                    displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.repairFileType.deletebutton.click(): ' + e.message + ', ' + e.stack);
                }
            });


        } catch (e) {
            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.repairFileType(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.repairFileType(): ' + e.message + ', ' + e.stack);
        }
    },
    ...
    copyAttachments: function (attachmentsElementId, requestDialogId, bwBudgetRequestId, bwRequestTitle, bwRequestTypeId) { // bwrequesttitle, bwrequesttypeid
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.copyAttachments().');
            //alert('In bwAttachments_ForEmail_ATTACHMENTS.js.copyAttachments().');
            var thiz = this;
            ...
            if (!externallyFacingUrlForAttachments) {
                ...
                var dialogId = 'divEmailAttachmentsDialog_bwAttachments_ForEmail_ATTACHMENTS_' + this.options.elementIdSuffix;
                var div = document.getElementById(dialogId); // 4-1-2020 12-28pm adt.
                if (!div) {
                    //alert('In bwAttachments_ForEmail_ATTACHMENTS.js.copyAttachments(). Adding ' + dialogId + ' to the DOM.');
                    div = document.createElement('div');
                    div.id = dialogId;
                    div.style.display = 'none';
                ...
                //var divEmailRecipients = 'divEmailAttachmentsDialog_bwAttachments_ForEmail_ATTACHMENTS_bwPeoplePicker_EmailRecipients_' + this.options.elementIdSuffix;
                ...
                //html += '<div class="divSignInButton" style="width:90%;text-align:center;line-height:1.1em;font-weight:bold;" onclick="$(\'.bwAttachments_ForEmail_ATTACHMENTS\').bwAttachments_ForEmail_ATTACHMENTS(\'emailAttachments\',\'newrequestattachments\', \'' + requestDialogId + '\', \'' + bwBudgetRequestId + '\', \'' + bwRequestTitle + '\', \'' + bwRequestTypeId + '\');">';

                html += '<div class="divSignInButton" style="width:90%;text-align:center;line-height:1.1em;font-weight:bold;" onclick="$(\'.bwAttachments_ForEmail_ATTACHMENTS\').bwAttachments_ForEmail_ATTACHMENTS(\'copyFiles\',\'newrequestattachments\', \'' + requestDialogId + '\', \'' + bwBudgetRequestId + '\', \'' + bwRequestTitle + '\', \'' + bwRequestTypeId + '\');">';


                html += 'Copy Selected Files to ' + externallyFacingUrlForAttachments;
                html += '</div>'; // ☑ 
                //html += '&nbsp;&nbsp;';
                //html += '<div class="divSignInButton_Disabled" style="width:90%;text-align:center;line-height:1.1em;font-weight:bold;" onclick="$(\'.bwAttachments_ForEmail_ATTACHMENTS\').bwAttachments_ForEmail_ATTACHMENTS(\'sendEmail\', \'' + requestDialogId + '\', \'' + bwBudgetRequestId + '\');">';
                //html += 'Send Email';
                //html += '</div>'; // ☑ 
                html += '</div>';
                ...
                $('#' + dialogId).dialog({
                    ...
                    open: function () {
                        try {
                                            ...
                                            for (var i = 0; i < data.length; i++) {
                                                ...
                                                if (filename.indexOf('_thumbnail_') > -1) {
                                                    ...
                                                    if (activeStateIdentifier.status != 'SUCCESS') {
                                                        ...
                                                        //  var filename = data[i].Display_Filename;
                                                        //fileUrl = "_files/_draftEmailAttachments/" + bwParticipantId + "/" + bwDraftEmailId + "/" + data[i].Actual_Filename + '?ActiveStateIdentifier=' + activeStateIdentifier.ActiveStateIdentifier; // 6-2-2023
                                                        // 7-15-2024.
                                                        fileUrl = "_files/_emailAttachments/" + bwParticipantId + "/" + bwDraftEmailId + "/" + data[i].Actual_Filename + '?ActiveStateIdentifier=' + activeStateIdentifier.ActiveStateIdentifier; // 6-2-2023
                                                    ...
                                                    if (fileExtension == 'png' || fileExtension == 'jpg' || fileExtension == 'jpeg' || fileExtension == 'jfif' || fileExtension == 'webp' || fileExtension == 'gif') {
                                                        ...
                                                        html += '<img id="attachmentstest1" xcx="xcx2312-1" src="' + thiz.options.operationUriPrefix + fileUrl + '" style="height:120px;display:block;margin-left:auto;margin-right:auto;cursor:pointer;" alt="" ';
                                                        if (Platform == 'IOS8') {
                                                            html += 'onclick="$(\'.bwAttachmentDialog_ForEmail\').bwAttachmentDialog_ForEmail(\'displayAttachmentInDialog\', \'' + thiz.options.operationUriPrefix + fileUrl + '\', \'' + filename + '\', \'' + description + '\', \'' + bwBudgetRequestId + '\');"';
                                                        } else {
                                                            html += 'onclick="$(\'.bwAttachmentDialog_ForEmail\').bwAttachmentDialog_ForEmail(\'displayAttachmentInDialog\', \'' + thiz.options.operationUriPrefix + fileUrl + '\', \'' + filename + '\', \'' + description + '\', \'' + bwBudgetRequestId + '\');" ';
                                                        }
                                                        html += ' />';
                                                        ...
                                                        // We need an if statement here to choose between iOS and Windows.
                                                        if (Platform == 'IOS8') {
                                                            html += '<img src="images/excelicon.png" style="width:100px;cursor:pointer;" onclick="$(\'.bwAttachmentDialog_ForEmail\').bwAttachmentDialog_ForEmail(\'displayAttachmentInDialog\', \'' + thiz.options.operationUriPrefix + fileUrl + '\', \'' + filename + '\', \'' + description + '\', \'' + bwBudgetRequestId + '\');" />';
                                                            //html += '<div class="attachmentsSectionFileLink" style="height:50px;border:1px thin red;" onclick="displayAttachmentInDialog(\'' + thiz.options.operationUriPrefix + fileUrl + '\', \'' + filename + '\', \'' + description + '\', \'' + bwBudgetRequestId + '\');">';
                                                        } else {
                                                            html += '<img src="images/excelicon.png" style="width:100px;cursor:pointer;" onclick="$(\'.bwAttachmentDialog_ForEmail\').bwAttachmentDialog_ForEmail(\'displayAttachmentInDialog\', \'' + thiz.options.operationUriPrefix + fileUrl + '\', \'' + filename + '\', \'' + description + '\', \'' + bwBudgetRequestId + '\');" />';
                                                            //html += '<div style="cursor:pointer;" class="attachmentsSectionFileLink" onclick="displayAttachmentInDialog(\'' + thiz.options.operationUriPrefix + fileUrl + '\', \'' + filename + '\', \'' + description + '\', \'' + bwBudgetRequestId + '\');">';
                                                    ...
                                                    // We need an if statement here to choose between iOS and Windows.
                                                    if (Platform == 'IOS8') {
                                                        html += '<div xcx="xcx21342346-1" class="attachmentsSectionFileLink" style="height:50px;border:1px thin red;" onclick="$(\'.bwAttachmentDialog_ForEmail\').bwAttachmentDialog_ForEmail(\'displayAttachmentInDialog\', \'' + thiz.options.operationUriPrefix + fileUrl + '\', \'' + filename + '\', \'' + description + '\', \'' + bwBudgetRequestId + '\');">';
                                                    } else {
                                                        html += '<div xcx="xcx21342346-2" class="attachmentsSectionFileLink" style="cursor:pointer;white-space:nowrap;width:250px;" onclick="$(\'.bwAttachmentDialog_ForEmail\').bwAttachmentDialog_ForEmail(\'displayAttachmentInDialog\', \'' + thiz.options.operationUriPrefix + fileUrl + '\', \'' + filename + '\', \'' + description + '\', \'' + bwBudgetRequestId + '\');">';
                                                    }
                            ...
                            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.copyAttachments.dialog.open(): ' + e.message + ', ' + e.stack);
                            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.copyAttachments.dialog.open(): ' + e.message + ', ' + e.stack);
                        }
                    }
                });
            ...
            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.copyAttachments(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.copyAttachments(): ' + e.message + ', ' + e.stack);
        }
    },
    copyFiles: function (attachmentsElementId, requestDialogId, bwBudgetRequestId, bwRequestTitle, bwRequestTypeId) {
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.copyFiles().');
            var thiz = this;
            ...
            if (checkboxes && checkboxes.length) {
                for (var i = 0; i < checkboxes.length; i++) {
                    if (checkboxes[i].checked) {
                        ...
                        // img_bwAttachments_ForEmail_ATTACHMENTS_4143c511-8afa-4023-87fc-cac61c765957_8cad99af-1797-4d1b-9b9e-e98bb173b058_0
                        ...
                        //    displayAlertDialog('Error in bwAttachments_ForEmail_ATTACHMENTS.js.copyFiles(). Could not locate the img element. IS THIS A ZIP FILE?????????????????????? xcx21345677');
                ...
                $.ajax({
                    ...
                    success: function (results) {
                        try {
                            ...
                            HideActivitySpinner();
                            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.copyFiles():1: ' + e.message + ', ' + e.stack);
                            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.copyFiles():1: ' + e.message + ', ' + e.stack);
                        }
                    },
                    error: function (data, errorCode, errorMessage) {
                        HideActivitySpinner();
                        console.log('Error in bwAttachments_ForEmail_ATTACHMENTS.js.copyFiles(): ' + errorCode + ' ' + errorMessage + ', data: ' + JSON.stringify(data));
                        displayAlertDialog('Error in bwAttachments_ForEmail_ATTACHMENTS.js.copyFiles(): ' + errorCode + ' ' + errorMessage + ', data: ' + JSON.stringify(data));
                    }
                });
            ...
            HideActivitySpinner();
            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.copyFiles(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.copyFiles(): ' + e.message + ', ' + e.stack);
        }
    },
    emailAttachments: function (attachmentsElementId, requestDialogId, bwBudgetRequestId, bwRequestTitle, bwRequestTypeId, attachments) { // bwrequesttitle, bwrequesttypeid
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.emailAttachments().');
            //alert('In bwAttachments_ForEmail_ATTACHMENTS.js.emailAttachments().');
            var thiz = this;
            ...
            if (!externallyFacingUrlForAttachments) {
                ...
                html += '<div style="white-space:nowrap;">';
                //html += '<div class="divSignInButton" style="width:90%;text-align:center;line-height:1.1em;font-weight:bold;" onclick="$(\'.bwAttachments_ForEmail_ATTACHMENTS\').bwAttachments_ForEmail_ATTACHMENTS(\'sendEmail\', \'' + requestDialogId + '\', \'' + bwBudgetRequestId + '\');">';
                ...
                $('#' + dialogId).dialog({
                    ...
                    open: function () {
                        try {
                            ...
                            var divEmailRecipients = 'divEmailAttachmentsDialog_bwAttachments_ForEmail_ATTACHMENTS_bwPeoplePicker_EmailRecipients_' + thiz.options.elementIdSuffix;
                            ...
                            if (!bwBudgetRequestId) {
                                console.log('ERROR IN bwAttachments_ForEmail_ATTACHMENTS.js.emailAttachments(). Unexpected value for bwBudgetRequestId: ' + bwBudgetRequestId);
                                displayAlertDialog('ERROR IN bwAttachments_ForEmail_ATTACHMENTS.js.emailAttachments(). Unexpected value for bwBudgetRequestId: ' + bwBudgetRequestId);
                            } else {
                                ...
                                for (var i = 0; i < attachments.length; i++) {
                                    ...
                                    if (filename.indexOf('_thumbnail_') > -1) {
                                        ...
                                        //html += '       <img xcx="xcx1232567" src="' + fileUrl_thumbnail + '" alt="' + description + '" title="' + description + '" style="height:120px;display:block;cursor:pointer;max-width:250px;border:1px solid gray;" />';
                            ...
                            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.emailAttachments():2: ' + e.message + ', ' + e.stack);
                            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.emailAttachments():2: ' + e.message + ', ' + e.stack);
                        }
            ...
            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.emailAttachments(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.emailAttachments(): ' + e.message + ', ' + e.stack);
        }
    },
    sendEmail: function (attachmentsElementId, requestDialogId, bwBudgetRequestId, bwRequestTitle, bwRequestTypeId) {
        try {
            ...
            var widgetId = 'divEmailAttachmentsDialog_bwAttachments_ForEmail_ATTACHMENTS_bwPeoplePicker_EmailRecipients_' + this.options.elementIdSuffix;
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        try {
                            ...
                            HideActivitySpinner();
                            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.sendEmail():1: ' + e.message + ', ' + e.stack);
                            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.sendEmail():1: ' + e.message + ', ' + e.stack);
                        }
                    },
                    error: function (data, errorCode, errorMessage) {
                        HideActivitySpinner();
                        console.log('Error in bwAttachments_ForEmail_ATTACHMENTS.js.sendEmail(): ' + errorCode + ' ' + errorMessage);
                        displayAlertDialog('Error in bwAttachments_ForEmail_ATTACHMENTS.js.sendEmail(): ' + errorCode + ' ' + errorMessage);
                    }
                });
            }
    ...
    uploadAttachment: function (displayAttachmentsTagName, requestDialogId, attachmentOrInline, objDataTransfer) {
        try {
            //var bwDraftEmailId = $('#displayedemaildetails')[0].getAttribute('bwdraftemailid'); // Changed to _id 7-3-2024.
            var _id = $('#divEmailClient_CompositionWindow')[0].getAttribute('_id');

            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.uploadAttachment(). _id: ' + _id + ', attachmentOrInline: ' + attachmentOrInline + ', elementIdSuffix: ' + this.options.elementIdSuffix + ', displayAttachmentsTagName: ' + displayAttachmentsTagName + ', requestDialogId: ' + requestDialogId + ', attachmentsAreStoredInIndexDb: ' + this.options.attachmentsAreStoredInIndexDb);
            //alert('In bwAttachments_ForEmail_ATTACHMENTS.js.uploadAttachment(). _id: ' + _id + ', attachmentOrInline: ' + attachmentOrInline + ', elementIdSuffix: ' + this.options.elementIdSuffix + ', displayAttachmentsTagName: ' + displayAttachmentsTagName + ', requestDialogId: ' + requestDialogId + ', attachmentsAreStoredInIndexDb: ' + this.options.attachmentsAreStoredInIndexDb);

            if (attachmentOrInline == 'INLINE') {

                if (objDataTransfer) {

                    console.log('In uploadAttachment(). Got a DataTransfer object with ' + objDataTransfer.files.length + ' files.');

                    this.cmdDisplayMultiFileUploadDialog('INLINE', _id, objDataTransfer.files); // Changed from cmdDisplayConfirmFileUploadDialog_zip_wasm() <<< DONT FORGET THIS CODE!

                } else {

                    var inputFileAttachmentsElement = $('#inputFile_INLINE_' + _id)[0];
                    ...
                    if (!(inputFileAttachmentsElement && inputFileAttachmentsElement.files && inputFileAttachmentsElement.files.length && (inputFileAttachmentsElement.files.length > 0))) {
                        ...
                        console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.uploadAttachment(). NO FILES ARE ATTACHED. INLINE.');
                        alert('In bwAttachments_ForEmail_ATTACHMENTS.js.uploadAttachment(). NO FILES ARE ATTACHED. INLINE.');
                        ...
                        //
                        console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.uploadAttachment(). THERE ARE MULTIPLE FILES TO UPLOAD. Number of files attached: ' + inputFileAttachmentsElement.files.length);

                        this.cmdDisplayMultiFileUploadDialog('INLINE', _id, inputFileAttachmentsElement.files); // Changed from cmdDisplayConfirmFileUploadDialog_zip_wasm() <<< DONT FORGET THIS CODE!
                ...
                if (objDataTransfer) {
                    ...
                    this.cmdDisplayMultiFileUploadDialog('ATTACHMENTS', _id, objDataTransfer.files); // Changed from cmdDisplayConfirmFileUploadDialog_zip_wasm() <<< DONT FORGET THIS CODE!

                } else {

                    var inputFileAttachmentsElement = $('#inputFile_ATTACHMENTS_' + _id)[0];

                    if (!(inputFileAttachmentsElement && inputFileAttachmentsElement.files && inputFileAttachmentsElement.files.length && (inputFileAttachmentsElement.files.length > 0))) {

                        console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.uploadAttachment(). NO FILES ARE ATTACHED. Could not find element: ' + 'inputFile_ATTACHMENTS_' + _id);
                        alert('In bwAttachments_ForEmail_ATTACHMENTS.js.uploadAttachment(). NO FILES ARE ATTACHED. Could not find element: ' + 'inputFile_ATTACHMENTS_' + _id);
                        ...
                        this.cmdDisplayMultiFileUploadDialog('ATTACHMENTS', _id, inputFileAttachmentsElement.files); // Changed from cmdDisplayConfirmFileUploadDialog_zip_wasm() <<< DONT FORGET THIS CODE!
            ...
            console.log('Error in bwAttachments_ForEmail_ATTACHMENTS.js.uploadAttachment(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Error in bwAttachments_ForEmail_ATTACHMENTS.js.uploadAttachment(): ' + e.message + ', ' + e.stack);
            $(this.element).find('#newrequestattachments')[0].innerHTML = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.uploadAttachment(): ' + e.message + ', ' + e.stack;
        }
    ...
    uploadPastedAttachment: function (imgSrc) {
        try {
            //console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.uploadPastedAttachment(). attachmentsAreStoredInIndexDb: ' + this.options.attachmentsAreStoredInIndexDb + ', bwBudgetRequestId: ' + bwBudgetRequestId);
            //console.log('In uploadPastedAttachment().  Using .bwAttachments_ForEmail_ATTACHMENTS:closest.  bwBudgetRequestId: ' + bwBudgetRequestId + ', elementIdSuffix: ' + this.options.elementIdSuffix + ', displayAttachmentsTagName: ' + displayAttachmentsTagName + ', requestDialogId: ' + requestDialogId);
    ...
    cmdDisplayConfirmFileUploadDialog: function (displayAttachmentsTagName, bwBudgetRequestId, requestDialogId, element) { // element is >>> var x = $('#inputFile_' + bwBudgetRequestId)[0]; // $('#' + requestDialogId).find('#inputFile')[0];
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog(). bwBudgetRequestId: ' + bwBudgetRequestId);
            alert('>>>>>>>>>>>>>>> In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog(). WE NEED TO FIGURE OUT IF THIS IS ATTACHMENTS OR INLINE<<<<<<<<<<inputFile_xxx_bwDraftEmailId <<<<<<<<<<<<<  bwBudgetRequestId: ' + bwBudgetRequestId);
            ...
            if (!(x && x.files && x.files.length && (x.files.length > 0))) {
                ...
                alert('xcx3124 Fatal error in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog(). NO FILES FOR ELEMENT [' + 'inputFile_' + bwBudgetRequestId + '].')
                ...
                if (!file) { // Don't like doing this but sometimes file is undefined... is this a race condition or a duplicate DOM element?? 6-23-2020
                    ...
                    if (file.size > 100000000) {
                        ...
                        console.log('>>>> In bwAttachments_ForEmail_ATTACHMENTS.js.uploadAttachment(). Calling .dialog(). bwBudgetRequestId: ' + bwBudgetRequestId + ', file.length: ' + file.length);
                                    ...
                                    if (!uploadButton) {
                                        ...
                                        $(this).ready(function () {
                                            ...
                                            $('#' + dialogId + '_btnUploadTheFileNow').off('click').click(function () { // divConfirmFileUploadDialog_ForAttachments_78ebbdca-32d7-4019-9c53-cdc55e11a775_btnUploadTheFileNow
                                                try {
                                                    console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.UploadButton_OnClick(). displayAttachmentsTagName: ' + displayAttachmentsTagName + ', bwBudgetRequestId: ' + bwBudgetRequestId);
                                                    alert('THIS IS THE ONE... xcx11 In bwAttachments_ForEmail_ATTACHMENTS.js.UploadButton_OnClick(). displayAttachmentsTagName: ' + displayAttachmentsTagName + ', bwBudgetRequestId: ' + bwBudgetRequestId);
                                                    ...
                                                    console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.UploadButton_OnClick(). Getting ready to call processUpload(). filename: ' + filename + ', file.name: ' + file.name); //              displayAttachmentsTagName: ' + displayAttachmentsTagName + ', bwBudgetRequestId: ' + bwBudgetRequestId);
                                                    //alert('In bwAttachments_ForEmail_ATTACHMENTS.js.UploadButton_OnClick(). Getting ready to call processUpload(). filename: ' + filename + ', file.name: ' + file.name); //              displayAttachmentsTagName: ' + displayAttachmentsTagName + ', bwBudgetRequestId: ' + bwBudgetRequestId);

                                                    thiz.processUpload(displayAttachmentsTagName, bwBudgetRequestId, file, filename, convertToAPdf);

                                                } catch (e) {
                                                    console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.btnUploadTheFileNow.click(): ' + e.message + ', ' + e.stack);
                                                    alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.btnUploadTheFileNow.click(): ' + e.message + ', ' + e.stack);
                                                }
                                            });
                                        });
            ...
            //            //        console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.UploadButton_OnClick(). displayAttachmentsTagName: ' + displayAttachmentsTagName + ', bwBudgetRequestId: ' + bwBudgetRequestId);
            //            //        alert('In bwAttachments_ForEmail_ATTACHMENTS.js.UploadButton_OnClick(). displayAttachmentsTagName: ' + displayAttachmentsTagName + ', bwBudgetRequestId: ' + bwBudgetRequestId);
            ...
            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog(): ' + e.message + ', ' + e.stack);
        }
    },
    ...
    cmdDisplayConfirmFileUploadDialog_ffmpeg_wasm: async function (displayAttachmentsTagName, bwBudgetRequestId, requestDialogId, element, file) { // element is >>> var x = $('#inputFile_' + bwBudgetRequestId)[0]; // $('#' + requestDialogId).find('#inputFile')[0];
        try {
            ...
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_ffmpeg_wasm(). bwBudgetRequestId: ' + bwBudgetRequestId);
            alert('In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_ffmpeg_wasm(). ARE WE USING THIS? IF SO GET inputFile_xxx_bwDraftEmailId  ATTACHMENTS OR INLINE WORKING<<<<<<<<<<<<<<<<< bwBudgetRequestId: ' + bwBudgetRequestId);
            //alert('###########In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_ffmpeg_wasm(). bwBudgetRequestId: ' + bwBudgetRequestId);
                    ...
                    try {
                        ...
                        if (!uploadButton) {
                            ...
                            $('#' + dialogId + '_btnUploadTheFileNow').off('click').click(function () { // divConfirmFileUploadDialog_ForAttachments_78ebbdca-32d7-4019-9c53-cdc55e11a775_btnUploadTheFileNow
                                ...
                                alert('#>>>>In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_ffmpeg_wasm.dialog.open(). xcx23123142. file.name: ' + file.name + '. Element id: ' + 'inputFile_' + bwBudgetRequestId);
                                ...
                                //alert('xcx21321312. In bwAttachments_ForEmail_ATTACHMENTS.js.UploadButton_OnClick(). displayAttachmentsTagName: ' + displayAttachmentsTagName + ', bwBudgetRequestId: ' + bwBudgetRequestId);
            ...
            $('#' + dialogId + '_btnInitializeFfmpegWasm').off('click').click(function (x) {
                try {
                    ...
                    alert('#>>>>In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_ffmpeg_wasm.xxxxxxxxxx(). x.length: ' + x2.length + '. Element id: ' + 'inputFile_' + bwBudgetRequestId);



                    //alert('#>>>>$$$$$$$$$$$In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_ffmpeg_wasm.dialog.open(). files.length: ' + files.length + '. Element id: ' + 'inputFile_' + bwBudgetRequestId);



                    //alert('###$$$$$$In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_ffmpeg_wasm._btnInitializeFfmpegWasm.click(). files.length: ' + files.length + '. Element id: ' + 'inputFile_' + bwBudgetRequestId);
                    ...
                    if (!(x && x.files && x.files.length && (x.files.length > 0))) {
                        ...
                        alert('xcx3124 Fatal error in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_ffmpeg_wasm(). NO FILES FOR ELEMENT xcx12345364 [' + 'inputFile_' + bwBudgetRequestId + '].')
                                                    ...
                                                    try {
                                                        ...
                                                        canvas.toBlob(async (blob) => {
                                                            try {
                                                                ...
                                                                console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.UploadButton_OnClick(). xcx124677'); // displayAttachmentsTagName: ' + displayAttachmentsTagName + ', bwBudgetRequestId: ' + bwBudgetRequestId);
                    ...
                    alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_ffmpeg_wasm.button.click(): ' + e.message + ', ' + e.stack);
                    console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_ffmpeg_wasm.button.click(): ' + e.message + ', ' + e.stack);
                    displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_ffmpeg_wasm.button.click(): ' + e.message + ', ' + e.stack);

                }
            });

        } catch (e) {
            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_ffmpeg_wasm(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_ffmpeg_wasm(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_ffmpeg_wasm(): ' + e.message + ', ' + e.stack);
        }
    },
    ...
    downloadLargeFile: function (bwBudgetRequestId, filename) {
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.downloadLargeFile().');
            alert('In bwAttachments_ForEmail_ATTACHMENTS.js.downloadLargeFile().');
            ...
            $.ajax({
                ...
                success: function (results) {
                    try {
                        ...
                        if (results.status != 'SUCCESS') {
                            ...
                            var msg = 'Error in bwAttachments_ForEmail_ATTACHMENTS.js.downloadLargeFile(). ' + results.status + ': ' + results.message;
                            console.log(msg);
                            displayAlertDialog(msg);
                        ...
                        var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.downloadLargeFile.download(): ' + e.message + ', ' + e.stack;
                        console.log(msg);
                        displayAlertDialog(msg);
                ...
                error: function (data, errorCode, errorMessage) {
                    ...
                    var msg = 'Error in bwAttachments_ForEmail_ATTACHMENTS.js.downloadLargeFile.download(): errorMessage: ' + errorMessage + ', errorCode: ' + errorCode + ', data: ' + JSON.stringify(data) + ', operationUri: ' + operationUri;
                    console.log(msg);
                    displayAlertDialog(msg);
            ...
            var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.downloadLargeFile(): ' + e.message + ', ' + e.stack;
            console.log(msg);
            displayAlertDialog(msg);
        }
    },
    ...
    cmdDisplayConfirmFileUploadDialog_zip_wasm: async function (displayAttachmentsTagName, bwBudgetRequestId, requestDialogId, element) { // element is >>> var x = $('#inputFile_' + bwBudgetRequestId)[0]; // $('#' + requestDialogId).find('#inputFile')[0];
        try {
            ...
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_zip_wasm(). bwBudgetRequestId: ' + bwBudgetRequestId);
            alert('In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_zip_wasm(). IF WE ARE USING THIS GET inputFile_xxx__id  FIGURED OUT FOR ATTACHMENTS OR INLINE. bwBudgetRequestId: ' + bwBudgetRequestId);
            ...
            //    } catch (e) {
            //        console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.uploadZipFile(): ' + e.message + ', ' + e.stack);
            //        alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.uploadZipFile(): ' + e.message + ', ' + e.stack);
            //        displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.uploadZipFile(): ' + e.message + ', ' + e.stack);
            //    }
            //}
            ...
            $('#' + dialogId + '_btnInitializeZipWasm').off('click').click(function (error) {
                try {
                    ...
                    console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_zip_wasm.button.click(). PREPARING TO ZIP THESE FILES AND UPLOAD TO THE SERVER: ' + JSON.stringify(trackingArray));
                        ...
                        reader.onload = function (event) {
                            try {
                                ...
                                if (trackingArray.length == 0) {
                                    ...
                                    alert('Calling performUpload2_email(). xcx1312341-1.');

                                    thiz.performUpload2_email(file, archiveFileName, _id, null, false); // data: Int8Array >>> convert to input type="file" element....

                                }

                            } catch (e) {
                                console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_zip_wasm.button.click.reader.load(): ' + e.message + ', ' + e.stack);
                                displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_zip_wasm.button.click.reader.load(): ' + e.message + ', ' + e.stack);
                                alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_zip_wasm.button.click.reader.load(): ' + e.message + ', ' + e.stack);
                            }
                        };
                    ...
                    console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_zip_wasm.button.click(): ' + e.message + ', ' + e.stack);
                    displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_zip_wasm.button.click(): ' + e.message + ', ' + e.stack);
                    alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_zip_wasm.button.click(): ' + e.message + ', ' + e.stack);
                }
            });
                        ...
                        if (!uploadButton) {
                            ...
                            $(this).ready(function () {
                                ...
                                $('#' + dialogId + '_btnUploadTheFileNow').off('click').click(function () { // divConfirmFileUploadDialog_ForAttachments_78ebbdca-32d7-4019-9c53-cdc55e11a775_btnUploadTheFileNow
                                    ...
                                    alert('xcx999994-1. In bwAttachments_ForEmail_ATTACHMENTS.js.UploadButton_OnClick(). displayAttachmentsTagName: ' + displayAttachmentsTagName + ', bwBudgetRequestId: ' + bwBudgetRequestId);
            ...
            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_zip_wasm(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_zip_wasm(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayConfirmFileUploadDialog_zip_wasm(): ' + e.message + ', ' + e.stack);
        }
    },

    ffmpegMultiFileMergeAndUpload: async function (bwBudgetRequestId, inputFileAttachmentsElement) {
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.ffmpegMultiFileMergeAndUpload(). bwBudgetRequestId: ' + bwBudgetRequestId);
            alert('In bwAttachments_ForEmail_ATTACHMENTS.js.ffmpegMultiFileMergeAndUpload(). bwBudgetRequestId: ' + bwBudgetRequestId + ', files.length: ' + inputFileAttachmentsElement.files.length);
            ...
            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.ffmpegMultiFileMergeAndUpload(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.ffmpegMultiFileMergeAndUpload(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.ffmpegMultiFileMergeAndUpload(): ' + e.message + ', ' + e.stack);
        }
    },
    ...
    FFMPEGConvertAndUploadMultipleVideoFiles_Batch: function (x, bwBudgetRequestId, dialogId) {
        ...
        return new Promise(function (resolve, reject) {
            try {
                console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadMultipleVideoFiles_Batch().');
                ...
                for (var i = 0; i < thiz.options.smallFileBatchArray[x].length; i++) {
                    ...
                    var promise = thiz.FFMPEGConvertAndUploadVideoFile(thiz.options.smallFileBatchArray[x][i], bwBudgetRequestId, dialogId).then(function (results) {
                        ...
                        alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadMultipleVideoFiles_Batch(). xcx4567. e.FailedFileName: ' + e.FailedFileName + ', JSON.stringify(e): ' + JSON.stringify(e));
                        var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadMultipleVideoFiles_Batch():3: e.FailedFileName: ' + e.FailedFileName + ', JSON.stringify(e): ' + JSON.stringify(e);
                ...
                Promise.all(promiseArray)
                    .then(function (results) {
                        ...
                        var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadMultipleVideoFiles_Batch():2: ' + JSON.stringify(e);
                        alert(msg);
                        var result = {
                            status: 'EXCEPTION',
                            message: msg
                ...
                var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadMultipleVideoFiles_Batch():1: ' + e.message + ', ' + e.stack;
    ...
    FFMPEGConvertAndUploadVideoFile: function (file, bwBudgetRequestId, dialogId) {
        ...
        return new Promise(function (resolve, reject) {
            try {
                console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadVideoFile().');
                //alert('In bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadVideoFile(). file.name: ' + file.name);
                ...
                var loadTheFFMPEGWebAssembly = async function (file) {
                    try {
                        ...
                        debugger;
                        await ffmpeg.load({
                            coreURL: baseURL + '/ffmpeg-core.js' + JSON.parse(activeStateIdentifier).ActiveStateIdentifier,
                            wasmURL: baseURL + '/ffmpeg-core.wasm' + JSON.parse(activeStateIdentifier).ActiveStateIdentifier
                        });
                    ...
                    try {
                        ...
                        function load_file(file) {
                            try {
                                ...
                                HideActivitySpinner();
                                var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadVideoFile.transcode.load_file(): ' + e.message + ', ' + e.stack;
                                console.log(msg);
                                alert(msg);
                        ...
                        async function print_file(files) {
                            try {
                                ...
                                if (!result) {
                                    ...
                                    displayAlertDialog_Persistent('Error in bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadVideoFile.transcode.print_file(). files.currentTarget.result: ' + files.currentTarget.result + ', inputFilename: ' + inputFilename + ', bwBudgetRequestId: ' + bwBudgetRequestId);

                                    var msg = 'Error in bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadVideoFile.transcode.print_file(). FATAL ERROR! NO result in print_file(). result: ' + result;
                                    console.log(msg);
                                    alert(msg);
                                    ...
                                    ffmpeg.writeFile(inputFilename, uint8_view).then(function (data) {
                                        try {
                                            ...
                                            ffmpeg.exec(executionString).then(function (data) {
                                                try {
                                                    ...
                                                    ffmpeg.deleteFile(inputFilename).then(function (data) {
                                                            ...
                                                            if (data != true) {
                                                                ...
                                                                ffmpeg.readFile(outputFilename).then(function (data) {
                                                                    try {
                                                                        ...
                                                                        thiz.performUpload2_email(outFile, outputFilename, bwBudgetRequestId, null, false, true).then(function (data) {
                                                                            ...
                                                                            ffmpeg.deleteFile(inputFilename).then(function (data) {
                                                                                    ...
                                                                                    if (data != true) {
                                                                                        ...
                                                                                        ffmpeg.deleteFile(outputFilename).then(function (data) {
                                                                                            try {
                                                                                                ...
                                                                                                var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadVideoFile.transcode.print_file.ffmpeg.writeFile.exec.readFile():2-xcx1: ' + e.message + ', ' + e.stack;
                                                                                                console.log(msg);
                                                                                                alert(msg);
                                                                                            }
                                                                                            ...
                                                                                            var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadVideoFile.transcode.print_file.ffmpeg.writeFile.exec.readFile.catch():1-2-1-xcx1: ' + JSON.stringify(e);
                                                                                            console.log(msg);
                                                                                            alert(msg);
                                                                                ...
                                                                                var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadVideoFile.transcode.print_file.ffmpeg.writeFile.exec.readFile.catch():1-2-2-xcx1: ' + JSON.stringify(e);
                                                                                console.log(msg);
                                                                                alert(msg);
                                                                        ...
                                                                        var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadVideoFile.transcode.print_file.ffmpeg.writeFile.exec.readFile():2: ' + e.message + ', ' + e.stack;
                                                                        console.log(msg);
                                                                        alert(msg);
                                                                    ...
                                                                    var msg = 'WILL THIS BUBBLE TO THE TOP1? Exception in bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadVideoFile.transcode.print_file.ffmpeg.writeFile.exec.readFile.catch():1: FATAL EXCEPTION. PUTTING THE FILE IN THE RETRY LIST1. JSON.stringify(e): ' + JSON.stringify(e);
                                                                    console.log(msg);
                                                                    alert(msg);
                                                                    displayAlertDialog_Persistent(msg);
                                                        ...
                                                        var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadVideoFile.transcode.print_file.ffmpeg.writeFile.exec.readFile.catch():1-2-2: ' + JSON.stringify(e);
                                                        console.log(msg);
                                                        alert(msg);
                                                    ...
                                                    var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadVideoFile.transcode.print_file.ffmpeg.writeFile.exec():2: ' + e.message + ', ' + e.stack;
                                                    console.log(msg);
                                                    alert(msg);
                                                ...
                                                try {
                                                    ...
                                                    var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadVideoFile.transcode.print_file.ffmpeg.writeFile.catch(): ' + e.message + ', ' + e.stack;
                                                    console.log(msg);
                                                    alert(msg);
                                                    displayAlertDialog_Persistent(msg);
                                            ...
                                            var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadVideoFile.transcode.print_file.ffmpeg.writeFile():2: ' + e.message + ', ' + e.stack;
                                            console.log(msg);
                                            alert(msg);
                                        ...
                                        var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadVideoFile.transcode.print_file.ffmpeg.writeFile():1: ' + JSON.stringify(e);
                                        console.log(msg);
                                        alert(msg);
                                ...
                                var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadVideoFile.transcode.print_file(): ' + e.message + ', ' + e.stack;
                                console.log(msg);
                                alert(msg);
                                displayAlertDialog(msg);
                        ...
                        var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadVideoFile.transcode(): ' + e.message + ', ' + e.stack;
                        console.log(msg);
                        alert(msg);
                ...
                var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.FFMPEGConvertAndUploadVideoFile(): ' + e.message + ', ' + e.stack;
                console.log(msg);
    ...
    CloseDialog_btnCancelUploadTheFileNow: function (bwBudgetRequestId, dialogId) {
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.CloseDialog_btnCancelUploadTheFileNow().');
            alert('In bwAttachments_ForEmail_ATTACHMENTS.js.CloseDialog_btnCancelUploadTheFileNow(). IF WE ARE USING THIS GET UIT SORTED OUT FOR inputFile_xxx__id  WITH ATTACMENTS OR INLINE<<<<<<<<<<<<<<<<');
            ...
            var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.CloseDialog_btnCancelUploadTheFileNow(): ' + e.message + ', ' + e.stack;
            console.log(msg);
            displayAlertDialog(msg);
        }
    },

    cmdDisplayMultiFileUploadDialog: function (attachmentOrInline, _id, files) {
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog(). _id: ' + _id + ', attachmentOrInline: ' + attachmentOrInline);
            //alert('In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog(). IF WE ARE USING THIS GET IT SORTED OUT FOR inputFile_xxx__id  ATTACHMENTS OR INLINE<<<<<<<<<<<<<<<<<<  _id: ' + _id + ', attachmentOrInline: ' + attachmentOrInline);
            var thiz = this;

            var dialogId = 'divConfirmFileUploadDialog_MultiFile_' + _id;
            ...
            //
            // bwAttachments_ForEmail_ATTACHMENTS.js.uploadAttachment() has the line which resets/clears the input file element which has the attachments. THIS MAY NEED TO GO SOMEWHERE IN THE NEAR FUTURE S THAT A USER CAN RETRY TO LOAD THE SAME FILE. 11-10-2023.
            //
            //
            ...
            var CompressAndUploadImages = function (files) {
                try {
                    ...
                    console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.button.click(). PREPARING TO ZIP THESE FILES AND UPLOAD TO THE SERVER: ' + JSON.stringify(trackingArray));
                        ...
                        reader.onload = function (event) {
                            try {
                                ...
                                if (trackingArray.length == 0) {
                                    ...
                                    //alert('Calling performUpload2_email(). xcx1312341-2.');

                                    thiz.performUpload2_email(file, archiveFileName, _id, attachmentOrInline, false); // data: Int8Array >>> convert to input type="file" element....

                                }

                            } catch (e) {
                                console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.CompressAndUploadImages(): ' + e.message + ', ' + e.stack);
                                displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.CompressAndUploadImages(): ' + e.message + ', ' + e.stack);
                                alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.CompressAndUploadImages(): ' + e.message + ', ' + e.stack);
                            }
                        };
            ...
            var UploadMultipleFiles = async function (files) {
                try {
                    ...
                    for (var i = 0; i < files.length; i++) {
                        ...
                        await thiz.performUpload2_email(file, file.name, _id, 'ATTACHMENTS', false, true);

                    }

                } catch (e) {
                    alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.UploadMultipleFiles(): ' + e.message + ', ' + e.stack);
                    console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.UploadMultipleFiles(): ' + e.message + ', ' + e.stack);
                    displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.UploadMultipleFiles(): ' + e.message + ', ' + e.stack);
                }
            }
            ...
            if (thereAreVideoFiles == false) {
                ...
                if (!(files && files.length)) {
                    ...
                    alert('xcx3124 Fatal error in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog(). NO FILES. files.length: ' + files.length); // FOR ELEMENT xcx12345364-2 [' + 'inputFile_' + bwBudgetRequestId + '].')
                    ...
                    $('#divConfirmFileUploadDialog_MultiFile_' + _id).dialog('close'); // Close the dialog window.
                ...
                html += '           <td style="width:9%;"></td>';
                html += '           <td style="width:1%;cursor:pointer;vertical-align:top;">';
                html += '               <span class="dialogXButton" style="font-family: \'Segoe UI Light\',\'Segoe UI\',\'Segoe\',Tahoma,Helvetica,Arial,sans-serif;font-size: 120pt;font-weight:bold;" onclick="$(\'.bwAttachments_ForEmail_ATTACHMENTS\').bwAttachments_ForEmail_ATTACHMENTS(\'CloseDialog_btnCancelUploadTheFileNow\', \'' + _id + '\', \'' + dialogId + '\');">X</span>';
                html += '           </td>';
                html += '       </tr>';
                html += '   </table>';
                html += '   <input type="text" autofocus="true" style="display:none;" /> <!-- This is here to prevent the first visible field from getting the cursor and showing the keyboard. -->';
                ...
                ////html += '   <div id="' + dialogId + '_ffmpegMultiFileMergeAndUpload" class="divDialogButton" title="Click here to initialize ffmpeg.wasm...." >'; // onclick="$(\'bwAttachments_ForEmail_ATTACHMENTS\').bwAttachments_ForEmail_ATTACHMENTS(\'ffmpegMultiFileMergeAndUpload\', bwBudgetRequestId, files);">';
                ...
                html += '   <div id="btnCancelUploadTheFileNow" class="divDialogButton" onclick="$(\'.bwAttachments_ForEmail_ATTACHMENTS\').bwAttachments_ForEmail_ATTACHMENTS(\'CloseDialog_btnCancelUploadTheFileNow\', \'' + _id + '\', \'' + dialogId + '\');" >';
                html += '       Close';
                html += '   </div>';
            ...
            var FFMPEGConvertAndUploadMultipleVideoFiles_CreateAndRunBatches = async function (smallFileArray) {
                try {
                    ...
                    for (var i = 0; i < thiz.options.smallFileBatchArray.length; i++) {
                        ...
                        await thiz.FFMPEGConvertAndUploadMultipleVideoFiles_Batch(i, _id, dialogId);
                    ...
                    console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.FFMPEGConvertAndUploadMultipleVideoFiles_CreateAndRunBatches(): ' + e.message + ', ' + e.stack);
                    alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.FFMPEGConvertAndUploadMultipleVideoFiles_CreateAndRunBatches(): ' + e.message + ', ' + e.stack);
                }
            }
                ...
                try {
                    ...
                    if (largeFileArray.length > 0) {
                        ...
                        for (var i = 0; i < largeFileArray.length; i++) {
                            var promise = thiz.performUpload3(largeFileArray[i], largeFileArray[i].name, _id, null, false, true);
                            promiseArray.push(promise);
                        }
                        Promise.all(promiseArray).then(function (results) {
                            try {
                                ...
                                console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.promise.all.catch():1: ' + e.message + ', ' + e.stack);
                                alert('In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.promise.all.catch():1: ' + e.message + ', ' + e.stack);
                            }

                        }).catch(function (e) {

                            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.promise.all.catch(). e: ' + JSON.stringify(e));
                            displayAlertDialog('In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.promise.all.catch(). e: ' + JSON.stringify(e));
                    ...
                    alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.ConvertAndUploadVideoFiles(): ' + e.message + ', ' + e.stack);
                    console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.ConvertAndUploadVideoFiles(): ' + e.message + ', ' + e.stack);
                    displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.ConvertAndUploadVideoFiles(): ' + e.message + ', ' + e.stack);
                }
            }
            ...
            $('#' + dialogId + '_btnSmartUpload').off('click').click(function (error) {
                try {
                    ...
                    if (!(files && files.length)) {
                        ...
                        alert('xcx3124 Fatal error in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog._btnSmartUpload(). NO FILES. files.length: ' + files.length); // FOR ELEMENT xcx12345364-2 [' + 'inputFile_' + bwBudgetRequestId + '].')
                    ...
                    console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js._btnSmartUpload.button.click(): ' + e.message + ', ' + e.stack);
                    displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js._btnSmartUpload.button.click(): ' + e.message + ', ' + e.stack);
                    alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js._btnSmartUpload.button.click(): ' + e.message + ', ' + e.stack);
                }
            });
            ...
            $('#' + dialogId + '_btnUploadNoProcessing').off('click').click(function (error) {
                try {
                    ...
                    if (!(files && files.length)) {
                        ...
                        alert('xcx3124 Fatal error in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog._btnUploadNoProcessing(). NO FILES FOR ELEMENT xcx12345364-2 [' + 'xcx2131inputFile_' + _id + '].')
                    ...
                    console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js._btnUploadNoProcessing.button.click(): ' + e.message + ', ' + e.stack);
                    displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js._btnUploadNoProcessing.button.click(): ' + e.message + ', ' + e.stack);
                    alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js._btnUploadNoProcessing.button.click(): ' + e.message + ', ' + e.stack);
                }
            });
            ...
            $('#' + dialogId).dialog({
                ...
                open: function (event, ui) {
                    try {
                        ...
                        //        //    alert('xcx999994-2. In bwAttachments_ForEmail_ATTACHMENTS.js.UploadButton_OnClick(). displayAttachmentsTagName: ' + displayAttachmentsTagName + ', bwBudgetRequestId: ' + bwBudgetRequestId);
            ...
            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog(): ' + e.message + ', ' + e.stack);
        }
    },
    ...
    processUpload: function (displayAttachmentsTagName, _budgetRequestId, file, filename, convertToAPdf) { // convertToAPdf means that we will attempt to put the attachment into a pdf and save it as a pdf file.
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.processUpload().');
            //alert('In bwAttachments_ForEmail_ATTACHMENTS.js.processUpload(). filename: ' + filename);
            var thiz = this;
            ...
            if (!file || file.length == 0) {
                ...
                console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.processUpload(). filename: ' + filename);
                //alert('In bwAttachments_ForEmail_ATTACHMENTS.js.processUpload(). filename: ' + filename);
                ...
                if (filename.indexOf('.xml') > 0) {
                    ...
                    console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.processUpload(). Calling performUpload().');
    ...
    performUpload2_email: function (outFile, fileName, _id, attachmentOrInline, convertToAPdf) { // convertToAPdf means that we will attempt to put the attachment into a pdf and save it as a pdf file.
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2_email(). _id: ' + _id + ', fileName: ' + fileName);
            //alert('>>>In bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2_email(). IF WE ARE USING THIS GET IT SORTED OUT FOR inputFile_xxx__id  USING ATTACHMENTS OR INLINE<<<<<<<<<<<<<<<<<<<< attachmentOrInline: ' + attachmentOrInline + ', _id: ' + _id + ', fileName: ' + fileName);
            var thiz = this;
            ...
            var dialogId = 'divConfirmFileUploadDialog_ForAttachments_' + _id;
            ...
            //    alert('Error in performUpload2_email(). Invalid value for bwDraftEmailId: ' + bwDraftEmailId);

            //}
            var elementId;
            if (attachmentOrInline == 'INLINE') {
                elementId = 'uploadForm_INLINE_' + _id;
            } else if (attachmentOrInline == 'ATTACHMENTS') {
                elementId = 'uploadForm_ATTACHMENTS_' + _id;
            } else {

                alert('ERROR xcx213124: Invalid value for attachmentOrInline: ' + attachmentOrInline);

            }
            //var elementId = 'uploadForm_' + bwDraftEmailId;
            var element = $('#' + elementId)[0];
            var formData = new FormData(element);

            var activeStateIdentifier = $('.bwAuthentication:first').bwAuthentication('getActiveStateIdentifier');

            //if (formData && formData.set) { // SOME BROWSERS MAY NOT SUPPORT THE formData object?? 

            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2_email(). formData is an object with a set function.');
            //alert('In bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2_email(). formData is an object with a set function. bwDraftEmailId: ' + bwDraftEmailId);
            ...
            formData.set('bwAttachmentOrInline', 'ATTACHMENTS'); // Added 5-27-2024.
            ...
            if ((largeFileUploadEnabled != true) && (outFile.size > largeFileSizeMinBytes)) {
                ...
                console.log('Calling uploadattachment_large_email(). xcx8790980-2-2.');

                var operationUri = this.options.operationUriPrefix + '_files/' + 'uploadattachment_large_email';
                $.ajax({
                    xhr: function () {
                        try {
                            ...
                            xhr.upload.addEventListener("progress", function (evt) {
                                if (evt.lengthComputable) {
                                    ...
                                    console.log('');
                                    console.log('progress ' + Math.round(percentComplete) + '% workflowAppId: ' + workflowAppId + ', _id: ' + _id);
                                    console.log('');
                                    ...
                                    if (percentComplete == 100) {
                                        ...
                                        ShowActivitySpinner_FileUpload('Completing the save operations for these email file attachments...');
                                    }
                            ...
                            xhr.upload.addEventListener("loadstart", function (evt) {
                                console.log('');
                                console.log('loadstart workflowAppId: ' + workflowAppId + ', _id: ' + _id);
                                console.log('');
                            ...
                            xhr.upload.addEventListener("abort", function (evt) {
                                ...
                                console.log('');
                                console.log('abort message: ' + xhrAbortMessage + ',  workflowAppId: ' + workflowAppId + ', _id: ' + _id);
                                console.log('');
                            ...
                            xhr.upload.addEventListener("error", function (evt) {
                                try {
                                    ...
                                    var msg = 'Error in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload.xhr.upload.error():xcx1-2-1: workflowAppId: ' + workflowAppId + ', _id: ' + _id + ', evt: ' + JSON.stringify(evt);
                                    ...
                                    console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload.xhr.upload.error():xcx1-2: ' + e.message + ', ' + e.stack);
                                    alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload.xhr.upload.error():xcx1-2: ' + e.message + ', ' + e.stack);
                                }
                            }, false);

                            xhr.upload.addEventListener("load", function (evt) {
                                console.log('');
                                console.log('load workflowAppId: ' + workflowAppId + ', _id: ' + _id);
                                console.log('');




                                //$('.bwAuthentication').bwAuthentication('checkIfWeNeedToUpdateTheHomepage', bwDraftEmailId);
                            ...
                            xhr.upload.addEventListener("timeout", function (evt) {
                                ...
                                console.log('');
                                console.log('timeout workflowAppId: ' + workflowAppId + ', _id: ' + _id);
                                console.log('');
                            ...
                            xhr.upload.addEventListener("loadend", function (evt) {
                                console.log('');
                                console.log('loadend workflowAppId: ' + workflowAppId + ', _id: ' + _id);
                                console.log('');

                                //alert('loadend workflowAppId: ' + workflowAppId + ', bwDraftEmailId: ' + bwDraftEmailId);
                            ...
                            console.log('');
                            console.log('Exception in POST uploadattachment_large_email.ajax.xhr(). workflowAppId: ' + workflowAppId + ', _id: ' + _id + ', ' + e.message + ', ' + e.stack);
                            console.log('');

                            alert('Exception in POST uploadattachment_large_email.ajax.xhr(). workflowAppId: ' + workflowAppId + ', _id: ' + _id + ', ' + e.message + ', ' + e.stack);
                    ...
                    success: function (data) {
                        try {
                            ...
                            if (data.status != 'SUCCESS') {
                                ...
                                console.log('Error in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2_email(). Unexpected response. data: ' + JSON.stringify(data));
                                alert('Error in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2_email(). Unexpected response. data: ' + JSON.stringify(data));

                            } else {

                                displayAlertDialog_QuickNotice('The file(s) have uploaded successfully.');

                                var msg = 'In bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2_email(). _id: ' + data._id; // + ' AND HAS BEEN ASSIGNED A NEW VALUE OF ' + data._id;
                                console.log(msg);
                                //displayAlertDialog(msg);

                                //var bwDraftEmailId = data._id;

                                //var form = $('.bwEmailForm')[0]; // There is only 1. Right? 5-11-2024.
                                //form.setAttribute('bwdraftemailid', bwDraftEmailId);



                                //alert('xcx121 setting dom element _id to: ' + data._id);
                                $(thiz.element).closest('.bwEmailClient_Haraka').find('#divEmailClient_CompositionWindow')[0].setAttribute('_id', data._id);
                                ...
                                var x = $('#inputFile_ATTACHMENTS_' + data._id)[0];
                                if (x && x.files) {
                                    x.files = null; //fileList; // Clear the file upload input element.
                                }
                                debugger;
                                thiz.renderAndPopulateAttachments_ATTACHMENTS(data._id);


                                // Need to update the other widget just in case it is still using the bwDraftEmailId of 'NEW_EMAIL'. Added 6-7-2024.
                                $('.bwAttachments_ForEmail_INLINE').bwAttachments_ForEmail_INLINE('renderAndPopulateAttachments_INLINE');
                                ...
                                //    // This resets the file input for uploading, so the onchange/change event fires as expected.
                                //    var x = $('#inputFile_INLINE_' + bwDraftEmailId)[0];
                                //    if (x && x.files) {
                                //        x.files = null; //fileList; // Clear the file upload input element.
                                //    }

                                //    thiz.renderAndPopulateAttachments_INLINE();

                                //} else if (attachmentOrInline == 'ATTACHMENTS') {

                                //    // This resets the file input for uploading, so the onchange/change event fires as expected.
                                //    var x = $('#inputFile_ATTACHMENTS_' + bwDraftEmailId)[0];
                                //    if (x && x.files) {
                                //        x.files = null; //fileList; // Clear the file upload input element.
                                //    }

                                //    thiz.renderAndPopulateAttachments_ATTACHMENTS();

                                //} else {
                                //    alert('xcx23214312 FATAL ERROR UNEXPECTED VALUE FOR attachmentOrInline: ' + attachmentOrInline);
                                //}
                            ...
                            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2_email():xcx33: ' + e.message + ', ' + e.stack);
                            alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2_email():xcx33: ' + e.message + ', ' + e.stack);
                            //$(this.element).find('#newrequestattachments')[0].innerHTML = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.performUpload2_email:xcx33: ' + e.message + ', ' + e.stack;
                            //displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2_email():xcx33: ' + e.message + ', ' + e.stack);
                        }
                    },
                    error: function (e) {
                        ...
                        if (xhrAbortMessage && xhrAbortMessage != '') {
                            ...
                            displayAlertDialog('POST error. xcx1242525 Intentional abort: ' + xhrAbortMessage + ', workflowAppId: ' + workflowAppId + ', _id: ' + _id);
                            ...
                            displayAlertDialog('POST unexpected error. xcx1242525-2-3 workflowAppId: ' + workflowAppId + ', _id: ' + _id + ', Unexpected error: ' + JSON.stringify(e)); //e.responseText)
            ...
            //} else if (!(formData && formData.set)) { // SOME BROWSERS MAY NOT SUPPORT THE formData object?? 

            //    alert('xcx11222233 UNFINISHED CODE HERE.');

            //    // This is for the iPad 2. 4-21-2023.
            //    ShowActivitySpinner_FileUpload('Uploading the file...');

            //    var x = document.getElementById('inputFile_' + bwDraftEmailId); // This element is on the request form with the "Choose File" button, not the attachments dialog.
            ...
            //                        //$('#inputFile_' + bwDraftEmailId).replaceWith($('#inputFile_' + bwDraftEmailId).clone());
            ...
            //                        ////debugger;
            //                        //thiz.populateAttachments(workflowAppId, bwDraftEmailId, 'newrequestattachments', true); //'attachments'); // This lists the attachments in the <p> tag with id='attachments'.

            //                        console.log('');
            //                        console.log('POST success workflowAppId: ' + workflowAppId + ', bwDraftEmailId: ' + bwDraftEmailId);
            //                        console.log('');
            ...
            //                        // This resets the file input for uploading, so the onchange/change event fires as expected.
            //                        console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.xx(). This resets the file input for uploading, so the onchange/change event fires as expected. xcx86984.');
            //                        //alert('render bwAttachments_ForEmail_ATTACHMENTS and maybe xxxxxx');
            //                        var x = $('#inputFile_' + bwDraftEmailId)[0];
            ...
            //                            console.log('Calling bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). xcx13124-3');
            //                            alert('xcx213234-17 calling populateAttachments().');
            //                            thiz.populateAttachments(workflowAppId, bwDraftEmailId, displayAttachmentsTagName, true); // This lists the attachments in the <p> tag with id='attachments'.
            ...
            //                            console.log('8-23-2022 FIXED THIS, remove this log entry once the bwAttachments_ForEmail_ATTACHMENTS.js widget works 100%. xcx1236899 bwDraftEmailId: ' + bwDraftEmailId);
            //                            // 8-23-2022 FIX THIS <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            //                            //var bwRequestTypeId = $('#' + 'divPageContent1').find('#budgetrequestform')[0].getAttribute('bwrequesttypeid'); // THIS IS WRONG, WE SHOULD BE GOING THROUGH ALL THE budgetrequestform collection and matching up the bwDraftEmailId.
            ...
            //                                var _budgetRequestId = $(budgetrequestforms[i])[0].getAttribute('bwDraftEmailId');
            //                                if (bwDraftEmailId == _budgetRequestId) {
            //                                    // We found the form.
            ...
            //                                displayAlertDialog('In bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2_email(). Could not find budgetrequestform with bwDraftEmailId: ' + bwDraftEmailId);

            //                            } else {

            //                                //$('.bwRequest').bwRequest('saveAndPublishTheRequest', 'divPageContent1', true, bwDraftEmailId, bwRequestTypeId); // This saves and publishes the New Request for the first time. 9-8-2022
            ...
            //                                            var msg = 'Error in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2_email(). xcx21345-1: ' + results.status + ', ' + results.message;
            //                                            displayAlertDialog(msg);

            //                                        } else {

            //                                            console.log('Calling bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). xcx13124-4');
            ...
            //                                        console.log('Calling bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). xcx13124-5');
            //                                        alert('xcx213234-15 calling populateAttachments().');
            //                                        thiz.populateAttachments(workflowAppId, bwDraftEmailId, displayAttachmentsTagName, true); // This lists the attachments in the <p> tag with id='attachments'.
            //                                        console.log('Exception from Promise in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2_email(). xcx21345-2: ' + JSON.stringify(e));
            ...
            //                                console.log('Calling bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). xcx13124-6');
            //                                alert('xcx213234-14 calling populateAttachments().');
            //                                thiz.populateAttachments(workflowAppId, bwDraftEmailId, displayAttachmentsTagName, true); // This lists the attachments in the <p> tag with id='attachments'.

            //                            } else {

            //                                var msg = data.status + ' in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2_email() xcx02856-1: ' + data.message;
            //                                console.log(msg);
            //                                displayAlertDialog(msg);
            ...
            //                            var msg = data.status + ' in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2_email() xcx02856-2-1: ' + data.message;
            //                            console.log(msg);
            //                            displayAlertDialog(msg);

            //                        } else {

            //                            //$('.bwAttachments_ForEmail_ATTACHMENTS').bwAttachments_ForEmail_ATTACHMENTS('renderAndPopulateAttachments'); // displayAttachmentsTagName
            ...
            //                    } catch (e) {
            //                        console.log('Exception in performUpload2_email():2: ' + e.message + ', ' + e.stack);
            //                        displayAlertDialog('Exception in performUpload2_email():2: ' + e.message + ', ' + e.stack);
            //                        $(this.element).find('#newrequestattachments')[0].innerHTML = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.performUpload2_email:2: ' + e.message + ', ' + e.stack;
            //                    }
            //                },
            //                error: function (data, errorCode, errorMessage) {
            //                    $('#divWorkingOnItDialog').dialog('close'); // Close the "Working on it..." dialog.
            //                    console.log('Error in performUpload2_email(): ' + errorMessage);
            //                    debugger;
            //                    if (errorMessage == 'timeout') {
            //                        displayAlertDialog('The file server could not be contacted. Please try again in a few minutes.');
            //                    } else {
            //                        displayAlertDialog('Error in performUpload2_email(): ' + errorMessage);
            //                    }
            //                }
            //            });
            //        } catch (e) {

            //            console.log('Exception in performUpload2_email():2:xcx3243246 ' + e.message + ', ' + e.stack);
            //            displayAlertDialog('Exception in performUpload2_email():2:xcx3243246 ' + e.message + ', ' + e.stack);
            //            $(this.element).find('#newrequestattachments')[0].innerHTML = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.performUpload2_email:2: ' + e.message + ', ' + e.stack;
            ...
            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2_email(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2_email(): ' + e.message + ', ' + e.stack);
            //$(this.element).find('#newrequestattachments')[0].innerHTML = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.performUpload2_email: ' + e.message + ', ' + e.stack;
        }
    },
    ...
    cmdDeleteAttachment: function (bwDraftEmailId, filename) {
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDeleteAttachment(' + bwDraftEmailId + ', ' + filename + ').');
            alert('In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDeleteAttachment(' + bwDraftEmailId + ', ' + filename + ').');
            var thiz = this;
            ...
            $('#divDeleteAnAttachmentOfflineDialogDeleteRequestButton').off('click').click(function (error) {
                try {
                    ...
                    console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.cmdDeleteAttachment.divDeleteAnAttachmentOfflineDialogDeleteRequestButton.click(). bwDraftEmailId: ' + bwDraftEmailId + ', filename: ' + filename);
                    ...
                    //var bwParticipantId = brfResult[0].bwParticipantId;
                    //var bwDraftEmailId = brfResult[0].bwDraftEmailId;


                    var fileserviceurl = globalUrlPrefix + globalUrlForWebServices + '/_files';
                    //var operationUri = fileserviceurl + "/removeattachment";
                    var operationUri = fileserviceurl + "/removeattachment_foremail";
                    $.ajax({
                        url: operationUri,
                        type: 'POST',
                        data: data,
                        ...
                        success: function (data) {
                            try {
                                ...
                                if (JSON.stringify(data).toUpperCase().indexOf('THE FILE WAS DELETED') > -1) {
                                    ...
                                    console.log('Calling bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). xcx13124-9');
                                    console.log('xcx213234-6 calling populateAttachments().');
                                    thiz.populateAttachments(workflowAppId, bwDraftEmailId, 'attachmentsInXslForm', true);
                                } else {
                                    ...
                                    console.log('Calling bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). xcx13124-10');
                                ...
                                debugger;
                                console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDeleteAttachment.post.removeattachment.success(): ' + e.message + ', ' + e.stack);
                                displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDeleteAttachment.post.removeattachment.success(): ' + e.message + ', ' + e.stack);
                            }
                        },
                        error: function (data, errorCode, errorMessage) {
                            debugger;
                            console.log('Error in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDeleteAttachment.post.removeattachment.error(): ' + e.message + ', ' + e.stack);
                            displayAlertDialog('Error in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDeleteAttachment.post.removeattachment.error(): bwDraftEmailId: ' + bwDraftEmailId + ', filename: ' + filename + ', errorMessage: ' + errorMessage);
                        }
                    });
                } catch (e) {
                    debugger;
                    console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDeleteAttachment.deletebutton.click(): ' + e.message + ', ' + e.stack);
                    displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDeleteAttachment.deletebutton.click(): ' + e.message + ', ' + e.stack);
                }
            });

        } catch (e) {
            debugger;
            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDeleteAttachment():2: ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDeleteAttachment():2: ' + e.message + ', ' + e.stack);
        }
    },

    populateAttachments: function (bwWorkflowAppId, _id, attachmentOrInline, showRemoveAttachmentButton, requestDialogId) {
        try {
            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). _id: ' + _id + ', attachmentOrInline: ' + attachmentOrInline);
            //alert('>>>>>>>>>>>>>>>In bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). It would be nice to know where this was called from. _id: ' + _id + ', attachmentOrInline: ' + attachmentOrInline);
            var thiz = this;
            debugger;
            if (!_id) {

                var msg = 'Error in bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments. Unexpected value for _id: ' + _id;
                console.log(msg);
                displayAlertDialog(msg);
                ...
                //    var msg = 'Error in bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments. Unexpected value for attachmentOrInline: ' + attachmentOrInline;
                //    console.log(msg);
                //    alert(msg);
                ...
                //
                //console.log('This resets the value of the input file element so that if the same file is selected a second time, it behaves as expected. This should only happen in populateAttachments() and in CloseDialog_btnCancelUploadTheFileNow().');
                //var inputFileAttachmentsElement = $('#inputFile_' + _id)[0];
                //if (!inputFileAttachmentsElement) {
                //    alert('Error in bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). This error is most likely because this was rendered with renderAndPopulateAttachments_ReadOnly(). It is readonly. DEV: This needs some work. Proceed from here.')
                //} else {
                //    inputFileAttachmentsElement.value = '';
                //}
                //
                ...
                // commented out 7-17-2024.
                //var _id = $(thiz.element).closest('.bwEmailClient_Haraka').find('#divEmailClient_CompositionWindow')[0].getAttribute('_id');



                //debugger;
                //var forms = $('.bwEmailForm');
                //var form;
                //for (var i = 0; i < forms.length; i++) {
                //    if (forms[i].getAttribute('bwdraftemailid') == _id) {
                //        console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). FOUND THE FORM');
                //        //alert('In bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). FOUND THE FORM');
                //        form = forms[i];
                //    }
                //}
                //if (_id != _id) {

                //    console.log('Error in bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). NO FORM FOUND for bwDraftEmailId: ' + bwDraftEmailId);
                //    alert('#### Error in bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). NO FORM FOUND for bwDraftEmailId: ' + bwDraftEmailId);

                //} else {
                debugger;
                //var attachmentWidget = $(form).find('.bwAttachments_ForEmail_ATTACHMENTS')[0];
                var attachmentWidget = $('#divBwEmailClient_Haraka_BwAttachments');

                if (!attachmentWidget) {

                    console.log('Error in bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). Could not locate the bwAttachment widget. xcx32425263. _id: ' + _id);
                    displayAlertDialog('Error in bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). Could not locate the bwAttachment widget. xcx32425263. _id: ' + _id);

                } else {

                    console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). workflowAppId: ' + workflowAppId + ', _id: ' + _id + ', showRemoveAttachmentButton: ' + showRemoveAttachmentButton + ').');
                    ...
                    var data = {
                        ...
                        bwWorkflowAppId: workflowAppId,
                        _id: _id,
                        bwAttachmentOrInline: 'ATTACHMENTS'
                    };
                    console.log('Calling getlistofattachments_foremail(). xcx2343266-1');
                    //var operationUri = globalUrlPrefix + globalUrlForWebServices + '/_files/' + 'getlistofattachmentsforbudgetrequest';
                    var operationUri = globalUrlPrefix + globalUrlForWebServices + '/_files/' + 'getlistofattachments_foremail';
                    $.ajax({
                        url: operationUri,
                        ...
                        success: function (results) {
                            try {
                                debugger;
                                console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). RETURNED FILELIST xcx23234352 results: ' + JSON.stringify(results));
                                //alert('In bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). RETURNED FILELIST xcx23234352 results: ' + JSON.stringify(results));

                                //var html = $('.bwCoreComponent:first').bwCoreComponent('createHtmlToDisplayTheListOfAttachments_ForbwAttachments_ForEmail_ATTACHMENTSWidget', thiz.options.elementIdSuffix, workflowAppId, bwDraftEmailId, results);
                                var html = $('.bwCoreComponent:first').bwCoreComponent('createHtmlToDisplayTheListOfAttachments_ForbwAttachments_ForEmailWidget', 'ATTACHMENTS', workflowAppId, _id, results);
                                ...
                                //$(attachmentWidget).find('#newrequestattachments_ATTACHMENTS')[0].innerHTML = ''; // It worked, and here were attachments, so get rid of the message.

                                //$(attachmentWidget).find('#newrequestattachments_' + attachmentOrInline).append(html);

                                $(thiz.element).find('#newrequestattachments_ATTACHMENTS').html(html);

                            } catch (e) {

                                alert('xcx23123. CAUGHT EXCEPTION: ' + e.message + ', ' + e.stack);

                                $(attachmentWidget).find('#newrequestattachments')[0].innerHTML = '<span style="color:tomato;">Exception in bwAttachments_ForEmail_ATTACHMENTS.populateAttachments():1: ' + e.message + ', ' + e.stack + '</span>';
                                if (e.number) {
                                    console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments():1: ' + e.message + ', ' + e.stack);
                                    displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments():1: ' + e.message + ', ' + e.stack);
                                } else {
                        ...
                        error: function (data, errorCode, errorMessage) {
                            ...
                            if (errorCode === 'timeout' && errorMessage === 'timeout') {
                                displayAlertDialog('In bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). SERVICE UNAVAILABLE. File services is not responding. communication timeout is set at ' + ajaxTimeout / 1000 + ' seconds: ' + errorCode + ', ' + errorMessage);
                                ...
                                var msg = 'Error in bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(): ' + errorMessage + '.'; // A common error at the moment is "Backend fetch failed". 10-13-2022
                                ...
                                //displayAlertDialog('Error in bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments():2:xcx2 ' + errorCode + ', ' + errorMessage + ', ' + JSON.stringify(data));
            ...
            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(): ' + e.message + ', ' + e.stack);

            //renderAndPopulateAttachments
            //$(thiz.element).find('#newrequestattachments')[0].innerHTML = '<span style="color:tomato;">Exception in bwAttachments_ForEmail_ATTACHMENTS.populateAttachments(): ' + e.message + ', ' + e.stack + '</span>';
            //$(attachmentWidget).find('#newrequestattachments')[0].innerHTML = '<span style="color:tomato;">Exception in bwAttachments_ForEmail_ATTACHMENTS.populateAttachments(): ' + e.message + ', ' + e.stack + '</span>';
    ...
    //performUpload3: function (outFile, fileName, bwBudgetRequestId, displayAttachmentsTagName, convertToAPdf) { // convertToAPdf means that we will attempt to put the attachment into a pdf and save it as a pdf file.
    //performUpload3: function (outFile, fileName, bwDraftEmailId, attachmentOrInline, convertToAPdf) { // convertToAPdf means that we will attempt to put the attachment into a pdf and save it as a pdf file.
    //    var thiz = this;
    //    return new Promise(function (resolve, reject) {
    //        //
    //        // data: Int8Array
    //        //
    //        try {
    //            console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.performUpload3(). fileName: ' + fileName);
    //            alert('In bwAttachments_ForEmail_ATTACHMENTS.js.performUpload3(). IF WE ARE USING THIS METHOD GET IT SORTED OUT FOR inputFile_xxx_bwDraftEmailId  ATTACHMENTS OR INLINE<<<<<<<<<<<<<<<  fileName: ' + fileName);
    ...
    //            // Description
    //            var dialogId = 'divConfirmFileUploadDialog_ForAttachments_' + bwDraftEmailId;
    //            var description = $('#' + dialogId).find('#txtConfirmFileUploadDialogFileDescription').val();
    ...
    //                                        console.log('8-23-2022 FIXED THIS, remove this log entry once the bwAttachments_ForEmail_ATTACHMENTS.js widget works 100%. xcx1236899 bwBudgetRequestId: ' + bwBudgetRequestId);
    //                                        // 8-23-2022 FIX THIS <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    //                                            displayAlertDialog('In bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2(). Could not find budgetrequestform with bwBudgetRequestId: ' + bwBudgetRequestId);
    ...
    //                                                        var msg = 'Error in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2(). xcx21345-1: ' + results.status + ', ' + results.message;
    //                                                        displayAlertDialog(msg);
    ...
    //                                                    alert('xcx213234-15 calling populateAttachments().');
    //                                                    thiz.populateAttachments(workflowAppId, bwBudgetRequestId, displayAttachmentsTagName, true); // This lists the attachments in the <p> tag with id='attachments'.
    //                                                    console.log('Exception from Promise in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2(). xcx21345-2: ' + JSON.stringify(e));
    ...
    //                                            var msg = data.status + ' in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2() xcx02856-1-2: ' + data.message;
    //                                            console.log(msg);
    //                                            displayAlertDialog(msg);
    ...
    //                                        var msg = data.status + ' in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2() xcx02856-2-3: ' + data.message;
    //                                        console.log(msg);
    //                                        displayAlertDialog(msg);
    ...
    //                                    $(thiz.element).find('#newrequestattachments')[0].innerHTML = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.performUpload2:2: ' + e.message + ', ' + e.stack;
    //                                }
    //                            },
    //                            error: function (data, errorCode, errorMessage) {
    //                                $('#divWorkingOnItDialog').dialog('close'); // Close the "Working on it..." dialog.
    ...
    //                // It is important to prepend these values, so that they get read immediately and not be undefined in the web service.
    //                formData.set('bwWorkflowAppId', workflowAppId);
    //                formData.set('bwAttachmentOrInline', 'ATTACHMENTS');
    //                formData.set('bwDraftEmailId', bwDraftEmailId);
    //                formData.set('bwParticipantId', participantId);
    //                formData.set('bwParticipantEmail', participantEmail);
    //                formData.set('bwParticipantFriendlyName', participantFriendlyName);
    //                formData.set('bwActiveStateIdentifier', activeStateIdentifier);
    ...
    //                    alert('Calling uploadattachment_large_email(). xcx8790980-3.');

    //                    var operationUri = thiz.options.operationUriPrefix + '_files/' + 'uploadattachment_large_email';
    //                    $.ajax({
    //                        xhr: function () {
    //                            try {
    //                                xhr = new window.XMLHttpRequest();
    ...
    //                                        console.log('');
    //                                        console.log('progress ' + Math.round(percentComplete) + '% workflowAppId: ' + workflowAppId + ', bwDraftEmailId: ' + bwDraftEmailId);
    //                                        console.log('');

    //                                        if (percentComplete == 100) {
    //                                            // It looks like the file stalls at 100%, but it must be the server saving the file, doing final operations... so let the user know that.
    //                                            ShowActivitySpinner_FileUpload('Completing the save operations... xcx1-3');
    //                                        }
    ...
    //                                xhr.upload.addEventListener("loadstart", function (evt) {
    //                                    console.log('');
    //                                    console.log('loadstart workflowAppId: ' + workflowAppId + ', bwDraftEmailId: ' + bwDraftEmailId);
    //                                    console.log('');
    ...
    //                                    console.log('');
    //                                    console.log('abort message: ' + xhrAbortMessage + ',  workflowAppId: ' + workflowAppId + ', bwDraftEmailId: ' + bwDraftEmailId);
    //                                    console.log('');
    ...
    //                                        var msg = 'Error in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload.xhr.upload.error():xcx1-2-2: workflowAppId: ' + workflowAppId + ', bwDraftEmailId: ' + bwDraftEmailId + ', evt: ' + JSON.stringify(evt) + ', evt.type: ' + evt.type + ', lengthComputable: ' + evt.lengthComputable + ', loaded: ' + evt.loaded + ', total: ' + evt.total;
    ...
    //                                xhr.upload.addEventListener("load", function (evt) {
    //                                    console.log('');
    //                                    console.log('load workflowAppId: ' + workflowAppId + ', bwDraftEmailId: ' + bwDraftEmailId);
    //                                    console.log('');
    ...
    //                                    console.log('');
    //                                    console.log('timeout workflowAppId: ' + workflowAppId + ', bwDraftEmailId: ' + bwDraftEmailId);
    //                                    console.log('');
    ...
    //                                xhr.upload.addEventListener("loadend", function (evt) {
    //                                    console.log('');
    //                                    console.log('loadend workflowAppId: ' + workflowAppId + ', bwDraftEmailId: ' + bwDraftEmailId);
    //                                    console.log('');
    ...
    //                                console.log('');
    //                                console.log('Exception in POST uploadattachment_large_email.ajax.xhr(). workflowAppId: ' + workflowAppId + ', bwDraftEmailId: ' + bwDraftEmailId + ', ' + e.message + ', ' + e.stack);
    //                                console.log('');

    //                                alert('Exception in POST uploadattachment_large_email.ajax.xhr(). workflowAppId: ' + workflowAppId + ', bwDraftEmailId: ' + bwDraftEmailId + ', ' + e.message + ', ' + e.stack);
    ...
    //                                    var dialogId2 = 'divConfirmFileUploadDialog_ForAttachments_zip_wasm_' + bwDraftEmailId;
    ...
    //                                        displayAlertDialog('In bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2(). Could not find budgetrequestform with bwDraftEmailId: ' + bwDraftEmailId);
    ...
    //                                        var msg = data.status + ' in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2() xcx02856-1-3: ' + data.message;
    //                                        console.log(msg);
    //                                        displayAlertDialog(msg);
    ...
    //                                    var msg = data.status + ' in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2() xcx02856-2-4: ' + data.message;
    //                                    console.log(msg);
    //                                    displayAlertDialog(msg);
    ...
    //                                    //$('.bwAttachments_ForEmail_ATTACHMENTS').bwAttachments_ForEmail_ATTACHMENTS('renderAndPopulateAttachments'); // displayAttachmentsTagName
    //                                    alert('xcx2131234-4 Unexpected response. data: ' + JSON.stringify(data));

    //                                    displayAlertDialog(data.message);
    //                                }
    //                            } catch (e) {
    //                                console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2():xcx33: ' + e.message + ', ' + e.stack);
    //                                alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2():xcx33: ' + e.message + ', ' + e.stack);
    //                                //$(thiz.element).find('#newrequestattachments')[0].innerHTML = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.performUpload2:xcx33: ' + e.message + ', ' + e.stack;
    //                                //displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2():xcx33: ' + e.message + ', ' + e.stack);
    //                            }
    //                        },
    //                        error: function (e) {
    ...
    //                                displayAlertDialog('POST error. xcx1242525 Intentional abort: ' + xhrAbortMessage + ', workflowAppId: ' + workflowAppId + ', bwDraftEmailId: ' + bwDraftEmailId);
    ...
    //                                displayAlertDialog('POST unexpected error. xcx1242525-2-4 workflowAppId: ' + workflowAppId + ', bwDraftEmailId: ' + bwDraftEmailId + ', Unexpected error: ' + JSON.stringify(e)); //e.responseText)
    ...
    //        } catch (e) {
    //            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2(): ' + e.message + ', ' + e.stack);
    //            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.performUpload2(): ' + e.message + ', ' + e.stack);
    //            $(thiz.element).find('#newrequestattachments')[0].innerHTML = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.performUpload2: ' + e.message + ', ' + e.stack;
    //        }
    //    });
    //},
    ...
    //    try {
    //        console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.GetStatusUpdateForMediaFileConversion().');
    //        var thiz = this;
    //        var filename = JSON.parse(decodeURI(json)).Display_Filename;

    //        //alert('filename: ' + filename);
    //        //displayAlertDialog_Persistent('In bwAttachments_ForEmail_ATTACHMENTS.js.GetStatusUpdateForMediaFileConversion(). bwWorkflowAppId: ' + bwWorkflowAppId + ', bwBudgetRequestId: ' + bwBudgetRequestId + ', json: ' + decodeURI(json));
    ...
    //                        displayAlertDialog('Error in bwAttachments_ForEmail_ATTACHMENTS.js.GetStatusUpdateForMediaFileConversion.success(): ' + results.status + ', ' + results.message);
    ...
    //                    console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.GetStatusUpdateForMediaFileConversion.success(): ' + e.message + ', ' + e.stack);
    //                    displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.GetStatusUpdateForMediaFileConversion.success(): ' + e.message + ', ' + e.stack);
    //                }
    //            },
    //            error: function (data, errorCode, errorMessage) {
    ...
    //                console.log('Error in bwAttachments_ForEmail_ATTACHMENTS.js.GetStatusUpdateForMediaFileConversion.error(): ' + errorMessage);
    //                displayAlertDialog('Error in bwAttachments_ForEmail_ATTACHMENTS.js.GetStatusUpdateForMediaFileConversion.error(): ' + errorMessage);
    //            }
    //        });

    //    } catch (e) {
    //        var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.GetStatusUpdateForMediaFileConversion(): ' + e.message + ', ' + e.stack;
    //        console.log(msg);
    //        displayAlertDialog(msg);
    //    }
    //},
    ...
    //populateAttachments: function (bwWorkflowAppId, bwDraftEmailId, attachmentsTagId, showRemoveAttachmentButton, requestDialogId) {
    ...
    //    try {
    //        console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.cmdSaveAttachmentMetadata().');
    //        var thiz = this;
    ...
    //                            console.log('Calling bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). xcx13124-7');
    ...
    //                            console.log('Calling bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). xcx13124-8');
    ...
    //                } catch (e) {
    //                    console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdSaveAttachmentMetadata(): ' + e.message + ', ' + e.stack);
    //                    displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdSaveAttachmentMetadata(): ' + e.message + ', ' + e.stack);
    //                }
    //            },
    //            error: function (data, errorCode, errorMessage) {
    ...
    //                displayAlertDialog('Error in bwAttachments_ForEmail_ATTACHMENTS.js.cmdSaveAttachmentMetadata().saveattachmentmetadata: ' + JSON.stringify(data) + ', ' + errorMessage);
    //            }
    //        });
    //    } catch (e) {
    //        console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdSaveAttachmentMetadata():2: ' + e.message + ', ' + e.stack);
    //        displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdSaveAttachmentMetadata():2: ' + e.message + ', ' + e.stack);
    //    }
    //},
    ...
    //    } catch (e) {
    //        console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.button.click.onAllFilesLoaded(): ' + e.message + ', ' + e.stack);
    //        displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.button.click.onAllFilesLoaded(): ' + e.message + ', ' + e.stack);
    //        alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.button.click.onAllFilesLoaded(): ' + e.message + ', ' + e.stack);
    //    }
    //}
    ...
    //        } catch (e) {
    //            console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.button.click.reader.load(): ' + e.message + ', ' + e.stack);
    //            displayAlertDialog('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.button.click.reader.load(): ' + e.message + ', ' + e.stack);
    //            alert('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.cmdDisplayMultiFileUploadDialog.button.click.reader.load(): ' + e.message + ', ' + e.stack);
    //        }
    //    };
    ...
    //    try {
    //        console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.downloadZippedFiles().');
    //        //alert('In bwAttachments_ForEmail_ATTACHMENTS.js.downloadZippedFiles().');
    //        var thiz = this;
    ...
    //                        var msg = 'Error in bwAttachments_ForEmail_ATTACHMENTS.js.downloadZippedFiles(). ' + results.status + ': ' + results.message;
    //                        console.log(msg);
    //                        displayAlertDialog(msg);
    ...
    //                    var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.downloadZippedFiles.DownloadAllFilesZippedForBudgetRequest(): ' + e.message + ', ' + e.stack;
    //                    console.log(msg);
    //                    displayAlertDialog(msg);
    //                    $(this.element).find('#newrequestattachments')[0].innerHTML = msg;
    //                }
    ...
    //                var msg = 'Error in bwAttachments_ForEmail_ATTACHMENTS.js.downloadZippedFiles.DownloadAllFilesZippedForBudgetRequest(): ' + errorMessage;
    //                console.log(msg);
    //                displayAlertDialog(msg);
    ...
    //        var msg = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.js.downloadZippedFiles(): ' + e.message + ', ' + e.stack;
    //        console.log(msg);
    //        alert(msg);
    ...
    //    try {
    //        console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments_ReadOnly().');
    //        alert('In bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments_ReadOnly(). This method may need some work. DO NOT USE THIS METHOD!!!!!!!!!!!!!!!!');
    //        var thiz = this;
    ...
    //        html += '<table style="height:100%;vertical-align:top;">';
    //        html += '   <tbody>';
    //        html += '       <tr xcx="xcx21342526-1" id="xcx21342526-1" class="xdTableOffsetRow xdTableOffsetRow-editor" id="toolboxdraggablerow_2" draggable="true" bwwidgetname="bwAttachments_ForEmail_ATTACHMENTS">';
    ...
    //        //html += '<input type="file" multiple name="multerFileUploadIdentifier" id="inputFile_' + bwBudgetRequestId + '" class="upload" style="visibility:hidden;" onchange="$(\'.bwAttachments_ForEmail_ATTACHMENTS:first\').bwAttachments_ForEmail_ATTACHMENTS(\'uploadAttachment\',\'newrequestattachments\', \'' + requestDialogId + '\', \'' + bwBudgetRequestId + '\');" />';
    ...
    //        // "Email attachments externally..." button.
    //        //html += '           <input id="' + requestDialogId + '_buttonEmailAttachmentsExternally" type="button" style="display:none;padding:5px 10px 5px 10px;" value="Email attachment(s) externally..." title="Email attachments to a customer using your internet facing url, which is configured in the organization settings." class="BwButton350" ';
    //        //html += '               onclick="$(\'.bwAttachments_ForEmail_ATTACHMENTS:first\').bwAttachments_ForEmail_ATTACHMENTS(\'copyAttachments\',\'newrequestattachments\', \'' + requestDialogId + '\', \'' + bwBudgetRequestId + '\', \'' + bwRequestTitle + '\', \'' + bwRequestTypeId + '\');" />';
    ...
    //            console.log('Calling bwAttachments_ForEmail_ATTACHMENTS.js.populateAttachments(). xcx13124-2');
    ...
    //        html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwAttachments_ForEmail_ATTACHMENTS</span>';
    //        html += '<br />';
    //        html += '<span style="">Exception in bwAttachments_ForEmail_ATTACHMENTS.renderAndPopulateAttachments_ReadOnly(): ' + e.message + ', ' + e.stack + '</span>';
    //        this.element.html(html);
    //    }
    //    //try {
    //    //    console.log('In bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments_ReadOnly().');
    //    //    alert('In bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments_ReadOnly().');
    //    //    //var thiz = this;
    //    //    var html = '';

    //    //    html += '<table style="width:100%;">';
    //    //    html += '                        <tbody><tr  xcx="xcx21342526-2" id="xcx21342526-2" class="xdTableOffsetRow xdTableOffsetRow-editor" id="toolboxdraggablerow_2" draggable="true" bwwidgetname="bwAttachments_ForEmail_ATTACHMENTS">';
    ...
    //    //    //html += '               <input type="file" id="inputFile" class="upload" style="visibility:hidden;" onchange="$(\'.bwAttachments_ForEmail_ATTACHMENTS\').bwAttachments_ForEmail_ATTACHMENTS(\'uploadAttachment\',\'newrequestattachments\');" />';
    //    //    //html += '           </div>';
    //    //    //html += '           <!--<br />';
    ...
    //    //} catch (e) {
    //    //    console.log('Exception in bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments_ReadOnly: ' + e.message + ', ' + e.stack);

    //    //    //renderAndPopulateAttachments
    //    //    //$(this.element).find('#newrequestattachments')[0].innerHTML = 'Exception in bwAttachments_ForEmail_ATTACHMENTS.renderAndPopulateAttachments_ReadOnly: ' + e.message + ', ' + e.stack;
    //    //    var html = '';
    //    //    html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwAttachments_ForEmail_ATTACHMENTS</span>';
    //    //    html += '<br />';
    //    //    html += '<span style="">Exception in bwAttachments_ForEmail_ATTACHMENTS.js.renderAndPopulateAttachments_ReadOnly(): ' + e.message + ', ' + e.stack + '</span>';
    //    //    thiz.element.html(html);
    //    //}
    //},
