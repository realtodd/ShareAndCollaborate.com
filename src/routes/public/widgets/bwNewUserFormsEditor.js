$.widget("bw.bwNewUserFormsEditor", {
    options: {
        ...
        */
        ...
        /*
        ===========================================================
        This is the bwNewUserFormsEditor.js jQuery Widget. 
        ===========================================================
    ...
    _create: function (assignmentRowChanged_ElementId) {
        this.element.addClass("bwNewUserFormsEditor");
        //debugger;
        //var thiz = this; // Need this because of the asynchronous operations below.
        try {
            console.log('In bwNewUserFormsEditor._create().');
            ...
            var html = '';
            html += '<span style="font-size:24pt;color:red;">bwNewUserFormsEditor: CANNOT INITIALIZE HOME PAGE</span>';
            html += '<br />';
            html += '<span style="">Exception in bwNewUserFormsEditor.Create(): ' + e.message + ', ' + e.stack + '</span>';
    ...
    _destroy: function () {
        this.element
            .removeClass("bwNewUserFormsEditor")
            .text("");
    },
    ...
    deployNewUserExperience: function () {
        try {
            ...
            var appIdAndAppTitle2 = $('#selectChangeUserRoleDialogOrganizationOwnerDropDown2 option:selected').text();
            console.log('In bwNewUserFormsEditor.deployNewUserExperience(). appIdAndAppTitle: ' + appIdAndAppTitle);
            var proceed = confirm('Are you certain that you want to deploy the "New User" forms to this Tenant?\n[' + appIdAndAppTitle2 + ']\n[' + appIdAndAppTitle + ']\n\nThis action cannot be undone here.\n\n\nClick the OK button to proceed...');
            if (proceed != true) {
                //alert('You have chosen not to proceed.');
            } else {
                //debugger;
                var bwWorkflowAppId = appIdAndAppTitle.split('|')[0];
                var json = {
                    //bwTenantId: tenantId, //  We don't know it here... the web service will look it up.
                ...
                $.ajax({
                    url: thiz.options.operationUriPrefix + "_bw/replacetenantformswithnewuserforms",
                    type: "Post",
                    ...
                    try {
                        //debugger;
                        console.log('In bwNewUserFormsEditor.deployNewUserExperience.replacetenantformswithnewuserforms.post.success(): Successfully updated DB. result: ' + JSON.stringify(result)); // using (' + JSON.stringify(json) + ').');
                        alert('Successfully deployed the new forms!');
                        ...
                        //        } catch (e) {
                        //            console.log('Exception in bwNewUserFormsEditor.deployNewUserExperience.replacetenantformswithnewuserforms.post.success(): ' + e.message + ', ' + e.stack);
                        //        }
                        //        //});
                        //    }
                        //});
                        ...
                        console.log('Exception in bwNewUserFormsEditor.deployNewUserExperience.replacetenantformswithnewuserforms.post.success(): ' + e.message + ', ' + e.stack);
                        alert('Exception in bwNewUserFormsEditor.deployNewUserExperience.replacetenantformswithnewuserforms.post.success(): ' + e.message + ', ' + e.stack);
                    }
                    ...
                    console.log('Error in bwNewUserFormsEditor.deployNewUserExperience.replacetenantformswithnewuserforms.post.error(): ' + JSON.stringify(data) + ', json: ' + JSON.stringify(json));
                    alert('Error in bwNewUserFormsEditor.deployNewUserExperience.replacetenantformswithnewuserforms.post.error(): ' + msg); //+ error.message.value + ' ' + error.innererror.message);
            ...
            console.log('Exception in bwNewUserFormsEditor.deployNewUserExperience(): ' + e.message + ', ' + e.stack);
        }
    },

    renderFormsEditor: function () {
        try {
            //debugger;
            console.log('In renderFormsEditor().');
            var html = '';

            //html += 'In bwNewUserFormsEditor.renderFormsEditor(). Configure new Tenant Organization and Roles... THIS FUNCTIONALITY IS INCOMPLETE. This will allow a forest administrator to edit and choose the business model that will be initially presented to a new user. New users need a default organization!!! eg: Stark Enterprises.';


            html += '<div style="display:none;" id="divConfigureFormsNotificationsDialog">';
            html += '   <table style="width:100%;">';
            html += '       <tr>';
            html += '           <td style="width:90%;">';
            html += '               <span style="font-family: \'Segoe UI Light\',\'Segoe UI\',\'Segoe\',Tahoma,Helvetica,Arial,sans-serif;color: #3f3f3f;font-size:30pt;font-weight:bold;">Configure new tenant "Introductory Forms"...</span>';
            html += '               <br />';
            ...
            html += '               <span class="dialogXButton" style="font-family: \'Segoe UI Light\',\'Segoe UI\',\'Segoe\',Tahoma,Helvetica,Arial,sans-serif;font-size:30pt;font-weight:bold;" onclick="$(\'#divConfigureFormsNotificationsDialog\').dialog(\'close\');">X</span>';
            html += '           </td>';
            html += '       </tr>';
            html += '   </table>';
            ...
            //html += '    <span id="spanThereAreChangesToPublishText7" style="font-style:italic;color:tomato;">[spanThereAreChangesToPublishText7]</span>'; //<input value=" There are unsaved changes. Enter a description here and click Save..." type="text" id="txtNewWorkflowDescription" style="width:450px;color:grey;font-style:italic;padding:5px 5px 5px 5px;" onkeyup="$(\'.bwWorkflowEditor\').bwWorkflowEditor(\'NewWorkflowDescriptionTextBox_Onkeyup\');" />';
            //html += '  </td>';
            //html += '  <td>';
            //html += '    <span id="spanThereAreChangesToPublishButton7">[spanThereAreChangesToPublishButton7]</span>';
            //html += '  </td>';
            //html += '  <td>';
            //html += '    <span onclick="$(\'.bwFormsEditor\').bwFormsEditor(\'renderTreeviewWithNewJson\');" style="cursor:pointer;color:tomato;font-size:smaller;border:1px solid goldenrod;padding:3px 13px 5px 13px;">RE-RENDER</span>';
            //html += '  </td>';
            //html += '</tr>';
            //html += '</table>';
            ...
            html += '   <br />';
            html += '   <span id="spanConfigureFormsConfigurationDialogContent">[spanConfigureFormsConfigurationDialogContent]</span>';
            html += '   <br />';
            html += '   <br />';
            ...
            //html += '<br />';
            //html += '<span title="Edit email template(s)..." class="spanButton" onclick="$(\'.bwNewUserFormsEditor\').bwNewUserFormsEditor(\'editStepEmails\', \'Create\');"><span style="font-size:15pt;display:inline-block;">✉</span> "New User" Business Model&nbsp;&nbsp;</span>';
            ...
            console.log('Exception in renderFormsEditor():3: ' + e.message + ', ' + e.stack);
        }
    },
            ...
            try {
                ...
                $.ajax({
                    ...
                    try {
                        ...
                        $("#divConfigureFormsNotificationsDialog").dialog({
                            modal: true,
                            resizable: false,
                            //closeText: "Cancel",
                            closeOnEscape: false, // Hit the ESC key to hide! Yeah!
                            ...
                            open: function () {
                                $('.ui-widget-overlay').bind('click', function () {
                                    $('#divConfigureFormsNotificationsDialog').dialog('close');
                                });
                            ...
                            close: function () {
                                $('#divConfigureFormsNotificationsDialog').dialog('destroy');
                            }
                        });
                        $('#divConfigureFormsNotificationsDialog').dialog().parents('.ui-dialog').find('.ui-dialog-titlebar').remove();
                    } catch (e) {
                        //lpSpinner.Hide();
    ...
    editStepEmails: function (stepName) {
        try {
            ...
            $("#divConfigureFormsNotificationsDialog").dialog({
                ...
                //closeText: "Cancel",
                closeOnEscape: false, // Hit the ESC key to hide! Yeah!
                //title: 'divConfigureFormsNotificationsDialog',
                width: '800',
                dialogClass: 'no-close', // No close button in the upper right corner.
                hide: false, // This means when hiding just disappear with no effects.
                open: function () {
                    try {
                        ...
                        var html = '';
                        html += '<div id="divBwFormsEditor">[divBwFormsEditor]</div>';

                        document.getElementById('spanConfigureFormsConfigurationDialogContent').innerHTML = html;
                        ...
                        var $bwformseditor = $("#divBwFormsEditor").bwFormsEditor(options);
                        ...
                        console.log('Exception in editStepEmails.divConfigureFormsNotificationsDialog.open(): ' + e.message + ', ' + e.stack);
                    }
                },
                close: function () {
                    $('#divConfigureFormsNotificationsDialog').dialog('destroy');
    ...
    //            document.getElementById('spanThereAreChangesToPublishText7').innerHTML = 'You have changes that won\'t be available until you publish: ';
    //            var html = '';
    //            html += '<input style="padding:5px 10px 5px 10px;width:100px;" id="btnSaveWorkflowConfigurationAndActivate" type="button" value="Publish" onclick="$(\'.bwNewUserFormsEditor\').bwNewUserFormsEditor(\'publishNewUserFormsConfigurationAndActivate\');" />';
    //            html += '&nbsp;<input style="padding:5px 10px 5px 10px;" id="btnCancelChangesInDraftWorkflowConfiguration" type="button" value="Cancel Changes" onclick="$(\'.bwNewUserFormsEditor\').bwNewUserFormsEditor(\'cancelChangesInDraftWorkflowConfiguration\');" />';
    //            document.getElementById('spanThereAreChangesToPublishButton7').innerHTML = html;
    //        } else {
    //            // Do nothing because the user has made no changes to the workflow.
    //            document.getElementById('spanThereAreChangesToPublishText7').innerHTML = '';
    //            document.getElementById('spanThereAreChangesToPublishButton7').innerHTML = '';
    //        }
    //    } catch (e) {
    //        console.log('Exception in checkIfWeHaveToDisplayThePublishChangesButton(): ' + e.message + ', ' + e.stack);
    //    }
    //},
    //publishNewUserFormsConfigurationAndActivate: function () {
    //    var thiz = this;
    //    try {
    //        console.log('In publishNewUserFormsConfigurationAndActivate().');
    //        debugger;
