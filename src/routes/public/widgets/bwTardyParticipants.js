$.widget("bw.bwTardyParticipants", {
    options: {
        ...
        */
        ...
        /*
        ===========================================================
        This is the bwTardyParticipants.js jQuery Widget. 
        ===========================================================
    ...
    _create: function (assignmentRowChanged_ElementId) {
        this.element.addClass("bwTardyParticipants");
        ...
        try {
                ...
                $.ajax({
                    ...
                    success: function (pdata) {
                        ...
                        try {
                            ...
                            //
                            // This will get all outstanding tasks for the organization.
                            //
                            var endpointUrl = webserviceurl + "/bwworkflowtasks/getbyappid/" + workflowAppId;
                            $.ajax({
                                url: endpointUrl,
                                method: "GET",
                                headers: {
                                ...
                                success: function (tasks1) {
                                    try {

                                        var workflowUsers = JSON.parse(JSON.stringify(thiz.options.store.BwWorkflowUsers));
                                        var tasks = tasks1.d.results;

                                        for (var i = 0; i < workflowUsers.length; i++) {
                                            //
                                            // Find all the tasks for a participant and add to the JSON here so that it is easy when it comes time to display it.
                                            //
                                            for (var j = 0; j < tasks.length; j++) {
                                                if (workflowUsers[i].bwParticipantId == tasks[j].bwAssignedToId) {
                                                    //debugger;
                                                    var task = {
                                                        bwWorkflowTaskItemId: tasks[j].bwWorkflowTaskItemId,
                                                        bwTaskTitle: tasks[j].bwTaskTitle,
                                                        TaskType: tasks[j].TaskType,
                                                        WorkflowStepId: tasks[j].WorkflowStepId, // Not currently used, but we need to move this towards using a GUID.
                                                        WorkflowStepName: tasks[j].WorkflowStepName, // eg: 'Collaboration'
                                                        RoleCategory: tasks[j].RoleCategory, // Approver, Collaborator.
                                                        bwStatus: tasks[j].bwStatus,
                                                        bwRelatedItemId: tasks[j].bwRelatedItemId, // eg: guid id of the budget request
                                                        bwAssignedToRaciRoleAbbreviation: tasks[j].bwAssignedToRaciRoleAbbreviation, // eg: PM
                                                        bwAssignedToRaciRoleName: tasks[j].bwAssignedToRaciRoleName // eg: Project Manager
                                                    }

                                                    if (workflowUsers[i] && workflowUsers[i].OutstandingTasks) {
                                                        // do nothing.
                                                    } else {
                                                        workflowUsers[i]["OutstandingTasks"] = []; // instantiate the array.
                                                    }
                                                    workflowUsers[i]["OutstandingTasks"].push(task);
                                        ...
                                        //
                                        // Now we will order the list so that the users with the most tasks show up at the top. Bubble-sort!
                                        //
                                        for (var i = 0; i < workflowUsers.length; i++) {
                                            for (var j = 0; j < workflowUsers.length; j++) {
                                                var lengthI;
                                                var lengthJ;
                                                if (workflowUsers[i].OutstandingTasks && workflowUsers[i].OutstandingTasks.length) {
                                                    lengthI = workflowUsers[i].OutstandingTasks.length;
                                                } else {
                                                    lengthI = 0;
                                                }
                                                if (workflowUsers[j].OutstandingTasks && workflowUsers[j].OutstandingTasks.length) {
                                                    lengthJ = workflowUsers[j].OutstandingTasks.length;
                                                } else {
                                                    lengthJ = 0;
                                                }
                                                ...
                                                if (lengthJ > lengthI) {
                                                    ...
                                                    //debugger;
                                                    var tmpWorkflowUserJ = JSON.parse(JSON.stringify(workflowUsers[j])); // save it
                                                    workflowUsers.splice(j, 1); // Delete it.
                                                    workflowUsers.splice(0, 0, tmpWorkflowUserJ); // Put it here (move it up!) bubble sort.
                                                }
                                            }
                                        }

                                        thiz.options.store.BwWorkflowUsers = workflowUsers;

                                        thiz.renderTardyParticipants();

                                    } catch (e) {
                                        console.log('Exception in bwTardyParticipants.js._create():3: ' + e.message + ', ' + e.stack);
                                        displayAlertDialog('Exception in bwTardyParticipants.js._create():3: ' + e.message + ', ' + e.stack);
                                    }
                                },
                                fail: function (data) {
                                    //lpSpinner.Hide();
                                    debugger;
                                    console.log('In xx.fail():3: ' + JSON.stringify(data));
                                    var msg;
                                    ...
                                    alert('Exception in bwTardyParticipants._create().xx.Get:3: ' + msg); //+ error.message.value + ' ' + error.innererror.message);
                                    console.log('Exception in bwTardyParticipants._create().xx.Get:3: ' + JSON.stringify(data));
                                }
                            });

                        } catch (e) {
                            console.log('Exception in bwTardyParticipants._create().xx.Get:1: ' + e.message + ', ' + e.stack);
                            displayAlertDialog('Exception in bwTardyParticipants._create().xx.Get:1: ' + e.message + ', ' + e.stack);
                        }
                    },
                }).fail(function (data) {
                    //lpSpinner.Hide();
                    debugger;
                    console.log('In bwTardyParticipants.js._create.fail(): ' + JSON.stringify(data));
                    var msg;
                    if (JSON.stringify(data).indexOf('The specified URL cannot be found.') > -1) {
                        msg = 'There has been an error contacting the server. A firewall or network appliance may be interrupting this traffic.';
                    } else {
                    ...
                    alert('Exception in bwTardyParticipants.js._create().xx.Get:2: ' + msg); //+ error.message.value + ' ' + error.innererror.message);
                    console.log('Exception in bwTardyParticipants._create().xx.Get:2: ' + JSON.stringify(data));
            ...
            var html = '';
            html += '<span style="font-size:24pt;color:red;">bwTardyParticipants: CANNOT RENDER</span>';
            html += '<br />';
            html += '<span style="">Exception in bwTardyParticipants.Create(): ' + e.message + ', ' + e.stack + '</span>';
    ...
    expandOrCollapseMonitoringParticipantTasksSection: function (rowId, imageId, collapsibleRowId) { // collapsibleRowId = steprow-' + stepName.toLowerCase() + '_' + i + '_' + '0'
        try {


            //
            // 1-16-2022
            // THIS HAS AN ACCORDIAN collapse when other expaned behavior we need to get rid of
            //
            ...
            console.log('In expandOrCollapseMonitoringParticipantTasksSection(' + rowId + ', ' + imageId + ', ' + collapsibleRowId + ').');
            var elementIds = ['', 'participantParticipantTasksSectionRow_1', 'participantParticipantTasksSectionRow_1', 'participantParticipantTasksSectionRow_1'];
            var imageElementIds = ['', 'participantParticipantTasksSectionImage_1', 'participantParticipantTasksSectionImage_1', 'participantParticipantTasksSectionImage_1'];
            ...
            console.log('Exception in expandOrCollapseMonitoringParticipantTasksSection(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in expandOrCollapseMonitoringParticipantTasksSection(): ' + e.message + ', ' + e.stack);
        }
    },


    renderTardyParticipants: function () {
        try {
            console.log('In renderTardyParticipants().');
            var thiz = this;
            ...
            html += '   <div class="divSignInButton" style="width:90%;text-align:center;line-height:1.1em;font-weight:bold;" onclick="$(\'.bwTardyParticipants\').bwTardyParticipants(\'publishEmailTemplateConfigurationAndActivate\');">';
            ...
            //html += '   <tr>';
            //html += '       <td>';
            //html += '           <span style="color:black;font-family: \'Segoe UI Light\',\'Segoe UI\',\'Segoe\',Tahoma,Helvetica,Arial,sans-serif;color: #262626;font-size: 20pt;font-weight:normal;">';
            //html += '               Participant Tasks (identify tardy users)...&nbsp;';
            ...
            //html += '   </tr>';
            //html += '</table>';
            html += '           <span style="font-size:20pt;font-weight:normal;">Participant Tasks (identify tardy users).&nbsp;</span>';

            //// Configure Introductory Email button.
            //html += '<span class="spanButton" onclick="$(\'.bwTardyParticipants\').bwTardyParticipants(\'configureEmailTemplate_IntroductoryEmail\', \'Create\');"><span style="font-size:15pt;display:inline-block;">✉</span> Configure "Introductory" email&nbsp;&nbsp;</span>';
            ...
            //html += '<span class="spanButton" onclick="$(\'.bwTardyParticipants\').bwTardyParticipants(\'configureEmailTemplate_ChangedSecurityRole\', \'Createxx\');"><span style="font-size:15pt;display:inline-block;">✉</span> Configure "Changed security role" email&nbsp;&nbsp;</span>';
            ...
            //html += '<span class="spanButton" onclick="$(\'.bwTardyParticipants\').bwTardyParticipants(\'configureEmailTemplate_ReassignedResponsibilities\', \'Createxx\');"><span style="font-size:15pt;display:inline-block;">✉</span> Configure "Reassigned responsibilities" email&nbsp;&nbsp;</span>';
            ...
            html += '    <td>Joined the Organization</td>';
            html += '    <td>Personal Email Settings</td>';
            //html += '    <td></td>';
            //html += '    <td></td>';
            //html += '    <td></td>';
            html += '  </tr>';
            ...
            for (var i = 0; i < pdata.length; i++) {
                if (pdata[i].bwParticipantRole == 'emailrecipient') {
                    ...
                    if (alternatingRow == 'light') {
                        ...
                        html += '  <tr class="alternatingRowLight" style="cursor:pointer;" onclick="$(\'.bwCircleDialog2\').bwCircleDialog2(\'displayParticipantRoleMultiPickerInACircle\', true, \'' + 'btnEditRaciRoles_' + pdata[i].bwParticipantId + '\', \'' + pdata[i].bwParticipantId + '\', \'' + pdata[i].bwParticipantFriendlyName + '\', \'' + pdata[i].bwParticipantEmail + '\');">';
                        //html += '  <tr id="participantParticipantTasksSectionRow_' + i + '" class="alternatingRowLight" style="cursor:pointer;" >';
                        ...
                        html += '           <img id="participantParticipantTasksSectionImage_1" src="images/drawer-open.png" title="collapse" style="cursor:pointer;width:45px;height:45px;vertical-align:middle;" '; // xcx12342-1


                        // to do 1-16-2022 xcx446998
                        // was (\'.bwCircleDialog\').bwCircleDialog(\'expandOrCollapseParticipantCircleDialogSection
                        html += ' onclick="$(\'.bwTardyParticipants\').bwTardyParticipants(\'expandOrCollapseMonitoringParticipantTasksSection\', \'participantParticipantTasksSection_1\', \'participantParticipantTasksSectionImage_1\', \'participantParticipantTasksSectionRow_1\');" ';
                        html += ' >';

                        if (pdata[i].OutstandingTasks && pdata[i].OutstandingTasks.length) {
                            html += '               &nbsp;<span id="participantCircleDialogSectionTitle_1">(' + pdata[i].OutstandingTasks.length + ' pending tasks)</span>';
                        } else {
                            html += '               &nbsp;<span id="participantCircleDialogSectionTitle_1">(' + '0' + ' pending tasks)</span>';
                        }

                        //html += '               &nbsp;<span id="participantCircleDialogSectionTitle_1">Manage Todd\'s role in the organization:</span>';
                        ...
                        html += '    <td><img id="bwTardyParticipants_participantsEditorParticipantsListItem_' + i + '" style="width:50px;height:50px;" src="' + thiz.options.operationUriPrefix + 'images/businesswoman2.png' + '" /></td>';
                        alternatingRow = 'dark';
                    } else {
                        html += '  <tr class="alternatingRowDark" style="cursor:pointer;" onclick="$(\'.bwCircleDialog2\').bwCircleDialog2(\'displayParticipantRoleMultiPickerInACircle\', true, \'' + 'btnEditRaciRoles_' + pdata[i].bwParticipantId + '\', \'' + pdata[i].bwParticipantId + '\', \'' + pdata[i].bwParticipantFriendlyName + '\', \'' + pdata[i].bwParticipantEmail + '\');">';
                        //html += '  <tr id="participantParticipantTasksSectionRow_' + i + '" class="alternatingRowDark" style="cursor:pointer;" >';
                        ...
                        html += '   <span style="white-space:nowrap;width:100%;">';
                        html += '       <span class="stepname" style="cursor:cell;vertical-align:middle;">              ';
                        html += '           <img id="participantCircleDialogSectionImage_1" src="images/drawer-open.png" title="collapse" style="cursor:pointer;width:45px;height:45px;vertical-align:middle;" ';
                        ...
                        //html += '               &nbsp;<span id="participantCircleDialogSectionTitle_1">(xx2 pending tasks)</span>';
                        if (pdata[i].OutstandingTasks && pdata[i].OutstandingTasks.length) {
                            html += '               &nbsp;<span id="participantCircleDialogSectionTitle_1">(' + pdata[i].OutstandingTasks.length + ' pending tasks)</span>';
                        } else {
                            html += '               &nbsp;<span id="participantCircleDialogSectionTitle_1">(' + '0' + ' pending tasks)</span>';
                        ...
                        html += '    <td><img id="bwTardyParticipants_participantsEditorParticipantsListItem_' + i + '" style="width:50px;height:50px;" src="' + thiz.options.operationUriPrefix + 'images/userimage.png' + '" /></td>';
                    ...
                    var participantImageFetchingInformation = {
                        imageId: 'bwTardyParticipants_participantsEditorParticipantsListItem_' + i,
                    ...
                    //html += '    <td>' + 'Dec 11, 2021 xcx798r24' + '</td>';
                    ...
                    //debugger;
                    if (alternatingRow == 'light') {
                        html += '    <td>' + 'Aggregated [xxAM, xxPM] Atlantic Time xcx23410054' + '</td>';
                    } else {
                        html += '    <td>' + 'Immediate xcx23410054' + '</td>';
                    }
                    ...
                    if ((pdata[i].bwParticipantId == participantId) || (pdata[i].bwParticipantRole == 'owner')) {
                        ...
                        //html += '    <td><button class="BwSmallButton" onclick="cmdDisplayChangeUserRoleDialog(\'' + pdata[i].bwParticipantId + '\', \'' + pdata[i].bwParticipantFriendlyName + '\', \'' + pdata[i].bwParticipantEmail + '\', \'' + pdata[i].bwParticipantRole + '\', \'' + pdata[i].bwParticipantLogonType + '\');">change<br />security role</button></td>';
                        //html += '    <td><button class="BwSmallButton" onclick="cmdDisplayReassignUserTasksDialog(\'' + pdata[i].bwParticipantId + '\', \'' + pdata[i].bwParticipantFriendlyName + '\', \'' + pdata[i].bwParticipantEmail + '\', \'' + pdata[i].bwParticipantRole + '\', \'' + pdata[i].bwParticipantLogonType + '\');">reassign<br />responsibilities (#)</button></td>';
            ...
            //html += '<input style="padding:5px 10px 5px 10px;" id="btnCreateRole2" onclick="$(\'.bwCircleDialog\').bwCircleDialog(\'displayAddANewPersonInACircle\', true);" type="button" value="Add a Person/Participant/Vendor...">';
                            ...
                            $.get(imagePath).done(function () {
                                ...
                                img.onload = function (e) {
                                    try {
                                        ...
                                        console.log('Exception in renderTardyParticipants().img.onload(). The code gets here if the user switches pages quickly: ' + e.message + ', ' + e.stack);
                                        //alert('Exception in renderTardyParticipants().img.onload(): ' + e.message + ', ' + e.stack);
                                        reject();
                                    }
                                }
                            }).fail(function () {
            ...
            if (!(assignToId && assignToFriendlyName && assignToEmail && modifiedById && modifiedByFriendlyName && modifiedByEmail)) {
                ...
                $.ajax({
                    ...
                    try {
                        ...
                        // CLEAN UP and reload bwTardyParticipants.
    ...
    //                html += '      <td style="width:9%;"></td>';
    //                html += '      <td style="width:1%;cursor:pointer;vertical-align:top;">';
    //                html += '        <span class="dialogXButton" style="font-family: \'Segoe UI Light\',\'Segoe UI\',\'Segoe\',Tahoma,Helvetica,Arial,sans-serif;font-size:30pt;font-weight:bold;" onclick="$(\'.bwTardyParticipants\').bwTardyParticipants(\'hideParticipantRoleMultiPickerInACircle\');">X</span>';
    //                html += '      </td>';
    //                html += '    </tr>';
    //                html += '  </table>';
    //                html += '  <input type="text" autofocus="true" style="display:none;" /> <!-- This is here to prevent the first visible field from getting the cursor and showing the keyboard. -->';
    ...
    //                    console.log('CLEAN UP bwTardyParticipants! Figure out where the user clicked and respond accordingly. :D');
    ...
    //                        }
    //                        if (isSelected) {
    //                            html += '<td>xcx55<input id="' + 'orgRoleCheckbox_' + w + '" type="checkbox" checked onclick="$(\'.bwTardyParticipants\').bwTardyParticipants(\'OrgRoleCheckbox_Onchange\', \'' + 'orgRoleCheckbox_' + w + '\', \'' + workflowRoles[w].RoleId + '\', \'' + workflowRoles[w].RoleName + '\', \'' + divisionIndex + '\', \'' + groupIndex + '\', \'' + entityIndex + '\', \'' + locationIndex + '\');" /></td>';
    //                        } else {
    //                            html += '<td>xcx56<input id="' + 'orgRoleCheckbox_' + w + '" type="checkbox" onclick="$(\'.bwTardyParticipants\').bwTardyParticipants(\'OrgRoleCheckbox_Onchange\', \'' + 'orgRoleCheckbox_' + w + '\', \'' + workflowRoles[w].RoleId + '\', \'' + workflowRoles[w].RoleName + '\', \'' + divisionIndex + '\', \'' + groupIndex + '\', \'' + entityIndex + '\', \'' + locationIndex + '\');" /></td>';
    //                        }
    ...
    displayADMINParticipantInCircleDialog: function () {
        try {
            ...
            if (this.options.store.BwWorkflowUserRoleAdmin && this.options.store.BwWorkflowUserRoleAdmin.length == 1) {
                ...
                $().bwCircleDialog2('displayParticipantRoleMultiPickerInACircle', true, '', '' + this.options.store.BwWorkflowUserRoleAdmin[0].bwParticipantId + '', '' + this.options.store.BwWorkflowUserRoleAdmin[0].bwParticipantFriendlyName + '', '' + this.options.store.BwWorkflowUserRoleAdmin[0].bwParticipantEmail + '');
            }
    ...
    OrgRoleCheckbox_Onchange: function (checkboxId, roleId, roleName, divisionIndex, groupIndex, entityIndex, locationIndex, bwParticipantId, bwParticipantFriendlyName, bwParticipantEmail) {
        try {
            ...
            var thiz = this;
            //alert('In bwTardyParticipants.OrgRoleCheckbox_Onchange(). This functionality is IN PROGRESS. Expect unexpected results until testing is complete.');
    ...
    manageRaciRoles: function (divisionIndex, groupIndex, entityIndex) {
        try {
            ...
            for (var i = 0; i < availableRoles.length; i++) {
                ...
                //} else {
                html += '<td>xcx60<input id="' + 'roleCheckbox_' + i + '" type="checkbox" /></td>';
                //}
    ...
    displayEmailDataItemPickerDialog: function () {
        try {
            ...
            html += '</select>';
            html += '&nbsp;&nbsp;';
            html += '<input style="padding:5px 10px 5px 10px;" onclick="$(\'.bwTardyParticipants\').bwTardyParticipants(\'insertEmailDataItem\');" type="button" value="Insert">';
