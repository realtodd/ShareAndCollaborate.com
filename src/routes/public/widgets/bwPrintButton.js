$.widget("bw.bwPrintButton", {
    options: {
        ...
        */
        ...
        /*
        ===========================================================
        This is the bwPrintButton.js jQuery Widget. 
        ===========================================================
    ...
    _create: function (assignmentRowChanged_ElementId) {
        var thiz = this;
        this.element.addClass("bwPrintButton");
        //debugger;
        ...
        try {
            ...
            console.log('In bwPrintButton._create(). The widget has been initialized.');

        } catch (e) {
            var html = '';
            html += '<span style="font-size:24pt;color:red;">bwPrintButton: CANNOT INITIALIZE PRINTING</span>';
            html += '<br />';
            html += '<span style="">Exception in bwPrintButton.Create(): ' + e.message + ', ' + e.stack + '</span>';
    ...
    renderPrintButton: function () {
        try {
            ...
            if (reportType == 'CurrentYearBudgetRequestsReport') {
                html += '<span title="print" class="printButton" onclick="$(\'.bwPrintButton\').bwPrintButton(\'PrintCurrentYearBudgetRequestsReport\');">🖨</span>';
            } else if (reportType == 'MyPendingTasksReport') {
                html += '<span title="print" class="printButton" onclick="$(\'.bwPrintButton\').bwPrintButton(\'PrintMyPendingTasksReport\');">🖨</span>';
            } else if (reportType == 'InProcessBudgetRequestsReport') {
                html += '<span title="print" class="printButton" onclick="$(\'.bwPrintButton\').bwPrintButton(\'PrintInProcessBudgetRequestsReport\');">🖨</span>';
            } else if (reportType == 'IndividualRequestReport') {
                html += '<span title="print" class="printButton" onclick="$(\'.bwPrintButton\').bwPrintButton(\'PrintIndividualRequestReport\');" style="font-size:22pt;">🖨&nbsp;</span>'; // Different size because this one shows up in the dialog draggable top bar header.
            } else {
    ...
    CreateIFrameAndPrintThis: function (html) {
        try {
            ...
            if (isAndroid == true) {
                ...
                window1.document.body.innerHTML = html;

                window1.print();

                //
                // I have given up for the moment on trying to close this window. A difficult challenge.
                //
                //window1.onafterprint = function () {
                //    //displayAlertDialog("In onafterprint(). xcx3246777. MAYBE PUT A BIG CLOSE THIS WINDOW button here?? Hmmm.....");

                //    //window1.close(); // DO NOT DO THIS HERE, IT BREAKS THE PRINTING PROCESS SOMEHOW.
                //    // MAYBE PUT A BIG "CLOSE THIS WINDOW" button here?? Hmmm.....

                //    setTimeout(function () {
                //        window1.close();
                //    }, 5000); // Wait 5 seconds before trying to close the window.
                ...
                //
                // Put the html into an iFrame, so that we can print it.
                //

                html += '<style>';
                html += '   .errorsuggestionxcx3432 {';
                html += '       width:1600px;';
                ...
                if (!iframe) {
                    ...
                    iframe.width = '1px';
                    iframe.height = '1px';
                    iframe.style = 'transform:scale(1.75);'; // 4-16-2022 THIS IS HOW WE SCALED UP THE HTML TO PRINT OK!!!!!!!!!!!!!!!!!!!!!! WOOP!!!!!!!!!!!!!!!!!!!!!!!! :) :)
                    iframe.class = 'printf'; // Used for Android work-around.
                ...
                //iframeDocument.body.style.width = '4000px';
                //iframeDocument.body.style.height = '10000px';
                //iframeDocument.body.style.transform = 'scale(2)';
                //iframeDocument.body.style.height = '10000px'; // This works, but is not the answer...yet 4-16-2022

                //
                // This keeps checking until iframeDocument.readyState == 'complete', then it calls PrintNow(). Doesn't seem to work right in Firefox (see note and try-catch below).
                //
                var hasThePageLoaded = new function () {
                    setTimeout(function () {
                        try {
                            if (iframeDocument.readyState == 'complete') {
                                thiz.PrintNow();
                                console.log('in bwPrintButton.js.CreateIFrameAndPrintThis.hasThePageLoaded(). iframeDocument.readyState == "complete". xcx23312-1');
                            } else {
                                //alert('xcx22294 iframeDocument.readyState: ' + iframeDocument.readyState);
                                try {
                                    hasThePageLoaded();
                                } catch (e) {

                                    //
                                    // This happens in Firefox. For some reason, it doesn't know what "hasThePageLoaded" is. It says, "hasThePageLoaded" is not a function. 3-22-2024.
                                    //
                                    var msg = 'There was an error initializing the printer dialog. Please click the Print button a second time.';
                                    console.log(msg);
                                    displayAlertDialog(msg);

                                }
                                console.log('in bwPrintButton.js.CreateIFrameAndPrintThis.hasThePageLoaded(). iframeDocument.readyState == "complete". xcx23312-2');
                            }
                        } catch (e) {
                            console.log('Exception in bwPrintButton.js.CreateIFrameAndPrintThis.hasThePageLoaded(): ' + e.message + ', ' + e.stack);
                            displayAlertDialog('Exception in bwPrintButton.js.CreateIFrameAndPrintThis.hasThePageLoaded(): ' + e.message + ', ' + e.stack);
                        }
                    }, 250);
                }
            ...
            console.log('Exception in bwPrintButton.js.CreateIFrameAndPrintThis(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwPrintButton.js.CreateIFrameAndPrintThis(): ' + e.message + ', ' + e.stack);
        }
    },
    CreateIFrameAndPrintThis2: function (html) {
        var thiz = this;
        return new Promise(function (resolve, reject) {
            try {
                ...
                if (isAndroid == true) {
                    ...
                    //
                    // This keeps checking until iframeDocument.readyState == 'complete', then it calls PrintNow().
                    //
                    var hasThePageLoaded = setTimeout(function () {
                        try {
                            if (iframeDocument.readyState == 'complete') {
                                ...
                                console.log('in bwPrintButton.js.CreateIFrameAndPrintThis2.hasThePageLoaded(). iframeDocument.readyState == "complete". xcx23312-1');

                            } else {
                                hasThePageLoaded();
                                console.log('in bwPrintButton.js.CreateIFrameAndPrintThis2.hasThePageLoaded(). iframeDocument.readyState == "complete". xcx23312-2');
                            }
                        } catch (e) {

                            var msg = 'Exception in bwPrintButton.js.CreateIFrameAndPrintThis2.hasThePageLoaded(): ' + e.message + ', ' + e.stack;
                            console.log(msg);
                ...
                var msg ='Exception in bwPrintButton.js.CreateIFrameAndPrintThis2(): ' + e.message + ', ' + e.stack;
                console.log(msg);
    ...
    PrintNow: function () {
        try {
            ...
            setTimeout(function () {
                try {
                    try {
                        window.frames['printf'].contentWindow.print(); // Works in Edge.
                    } catch (e) {
                        try {
                            window.frames['printf'].print(); // Chrome.
                        } catch (e) {
                            console.log('Exception in PrintNow(): ' + e.message + ', ' + e.stack);
                        }
                    }
    ...
    PrintMyPendingTasksReport: function () {
        try {
            ...
            html += '                   <span><img id="imgParticipantImage" src="' + imagePath2 + '" style="width:200px;height:200px;"/>'; // This gets lazy loaded below.
            html += '                   </span>';
            html += '               </td>';
            html += '           </tr>';
            html += '           <tr>';
                                            ...
                                            $.ajax({
                                                ...
                                                success: function (brData) {
                                                    try {
                                                        ...
                                                        // Do the printing.
                                                        thiz.CreateIFrameAndPrintThis(html);
                                                        ...
                                                        //        //window.frames["printf"].print();
                                                        //        try {
                                                        //            console.log('bwPrintButton.PrintMyPendingTasksReport(): Try #1.');
                                                        //            window.frames["printf"].contentWindow.print(); // Confirmed 5-14-2020. Works in Edge.
                                                        //        } catch (e) {
                                                        //            try {
                                                        //                console.log('bwPrintButton.PrintMyPendingTasksReport(): Try #2.'); 
                                                        ...
                                                        //            } catch (e) {
                                                        //                console.log('bwPrintButton.PrintMyPendingTasksReport(): Try #3.');
                                                        //                window.print(); // In Safari on MacBook, and Safari & Chrome on iOS, this is a synchronous call!!! (unlike most other situations where it is asynchronous).
                                                        //            }
        ...
        try {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    alert('xcx213121243214234-11 fix the image url here....');
    ...
    PrintErrorAndSuggestion: function (bodyElementId, subjectInputElementId) {
        try {
            // Then we iterate throgh all of the html elements, and display set to inline.
            console.log('');
            console.log('----- PRINT ---- PRINT ---- PRINT ----');
            console.log('In bwPrintButton.js.PrintIndividualRequestReport();');
            console.log('----- PRINT ---- PRINT ---- PRINT ----');
            console.log('');
            ...
            // First, we have to extract the img element src attribute from the bodyHtml. We do this so that we can size it as we see fit.
            var bodyElement = $('#' + bodyElementId)[0].outerHTML;
            var imgIndex1 = bodyElement.indexOf('<img src="');
            var imgElement1 = bodyElement.substring(imgIndex1);
            var quoteIndex1 = imgElement1.indexOf('"'); // Find the opening quote for the img element src attribute.
            var imgElement2 = imgElement1.substring((quoteIndex1 + 1));
            var quoteIndex2 = imgElement2.indexOf('"'); // Find the closing quote for the img element src attribute.
            var imgPath = imgElement2.substring(0, quoteIndex2);

            //
            var subject = $('#' + subjectInputElementId).val();
            ...
            html += subject;
            html += '<br />';
            html += '<img src="' + imgPath + '" class="errorsuggestionxcx3432" />';
            ...
            console.log('');
            console.log('--- PRINT EXCEPTION ---');
            console.log('Exception in PrintIndividualRequestReport(): ' + e.message + ', ' + e.stack);
            console.log('--- PRINT EXCEPTION ---');
            console.log('');

            alert('Exception in PrintErrorAndSuggestion(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in PrintErrorAndSuggestion(): ' + e.message + ', ' + e.stack);
        }
    ...
    PrintIndividualRequestReport: function (requestDialogId) {
        try {
            // Then we iterate throgh all of the html elements, and display set to inline.
            console.log('');
            console.log('----- PRINT ---- PRINT ---- PRINT ----');
            ...
            if (window.opener) {
                ...
                // This is coming from a popped-out request window.
                //var rectElement = $('#budgetrequestform').find('.xdFormLayout')[0];

                //if ($('#budgetrequestform').find('.xdFormLayout')[0]) {
                //html = $('#budgetrequestform').find('.xdFormLayout')[0].innerHTML;
                //} 
                ...
                // Get the html.
                //if ($('#' + requestDialogId).find('#budgetrequestform')[0] && $('#' + requestDialogId).find('#budgetrequestform')[0]) {
                //if ($('#' + requestDialogId).find('#budgetrequestform').find('.xdFormLayout')[0]) {
                //    //html = $('#' + requestDialogId).find('#budgetrequestform')[0].innerHTML;
                //    html = $('#' + requestDialogId).find('#budgetrequestform').find('.xdFormLayout')[0].innerHTML;
                //} //else {
                //    html = $('#' + requestDialogId)[0].outerHTML;
                //}

                //displayAlertDialog(html);


                //alert('xcx879087098-1 In bwPrintButton.js.PrintInvoice(). getInvoiceHtml (old: renderAndPopulateCostsGrid) should be called to render our formatted invoice.');


                // Do the printing.
                //if ($('#' + requestDialogId).find('.bwInvoiceGrid')[0]) {
                //    //html += $('#' + requestDialogId).find('.bwInvoiceGrid')[0].innerHTML;
                //    html += $('#' + requestDialogId).find('.bwInvoiceGrid')[0].getInvoiceHtml(); // 8-12-2023.
                //}
                if ($('#' + requestDialogId).find('#budgetrequestform')[0]) {
                    html += $('#' + requestDialogId).find('#budgetrequestform')[0].innerHTML;
                }
                thiz.CreateIFrameAndPrintThis(html);
            ...
            alert('Exception in PrintIndividualRequestReport(): ' + e.message + ', ' + e.stack);
    ...
    PrintInvoice: function (requestDialogId, requestTitle) {
        try {
            // Then we iterate throgh all of the html elements, and display set to inline.
            console.log('');
            console.log('----- PRINT ---- PRINT ---- PRINT ----');
            console.log('In bwPrintButton.js.PrintInvoice();');
            console.log('----- PRINT ---- PRINT ---- PRINT ----');
            console.log('');

            //alert('In bwPrintButton.js.PrintInvoice();');

            var thiz = this;

            var documentTitle = document.title; // Save this so we can put it back sometime????

            var requestTitle = requestTitle; // This means when printing to PDF, the file name will be something like "INVOICE-2300050".
            ...
            if (window.opener) {
                ...
                //alert('xcx879087098-0 In bwPrintButton.js.PrintInvoice(). renderAndPopulateCostsGrid should be called to render our formatted invoice.');


                if ($('#' + requestDialogId).find('.bwInvoiceGrid')[0]) {
                    html += $('#' + requestDialogId).find('.bwInvoiceGrid')[0].innerHTML;
                    //html += $('#' + requestDialogId).find('.bwInvoiceGrid')[0].getInvoiceHtml();
                }
                thiz.CreateIFrameAndPrintThis(html);
            ...
            console.log('');
            console.log('--- PRINT EXCEPTION ---');
            console.log('Exception in PrintInvoice(): ' + e.message + ', ' + e.stack);
            console.log('--- PRINT EXCEPTION ---');
            console.log('');

            alert('Exception in PrintInvoice(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in PrintInvoice(): ' + e.message + ', ' + e.stack);
        }
    ...
    PrintReceipts: function (requestDialogId, requestTitle) {
        try {
            // Then we iterate throgh all of the html elements, and display set to inline.
            console.log('');
            console.log('----- PRINT ---- PRINT ---- PRINT ----');
            console.log('In bwPrintButton.js.PrintReceipts();');
            console.log('----- PRINT ---- PRINT ---- PRINT ----');
            console.log('');

            var documentTitle = document.title; // Save this so we can put it back sometime????

            var requestTitle = 'Receipts for ' + requestTitle;

            document.title = requestTitle;

            var html = '';

            html += $('#span_PrintReceiptsDialog_ReceiptsHtml').html();

           
            this.CreateIFrameAndPrintThis2(html).then(function () {

                //alert('xcx21123132 SUCCESS DID THE TTITLE WORK?.');
                //document.title = documentTitle;

            }).catch(function (e) {
                alert('exception xcx21312312312312');
            });
            ...
            //    //alert('xcx879087098-0 In bwPrintButton.js.PrintReceipts(). renderAndPopulateCostsGrid should be called to render our formatted invoice.');
            ...
            console.log('');
            console.log('--- PRINT EXCEPTION ---');
            console.log('Exception in PrintReceipts(): ' + e.message + ', ' + e.stack);
            console.log('--- PRINT EXCEPTION ---');
            console.log('');

            alert('Exception in PrintReceipts(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in PrintReceipts(): ' + e.message + ', ' + e.stack);
        }
    ...
    PrintHtml: function (requestDialogId, requestTitle) {
        try {
            // Then we iterate throgh all of the html elements, and display set to inline.
            console.log('');
            console.log('----- PRINT ---- PRINT ---- PRINT ----');
            ...
            if (window.opener) {
                ...
                if ($('#' + requestDialogId).find('.bwJustificationDetailsField')[0]) {
                    //html += $('#' + requestDialogId).find('.bwInvoiceGrid')[0].innerHTML;
                    html += $('#' + requestDialogId).find('.bwJustificationDetailsField')[0].innerHTML;
    ...
    PrintIndividualAttachment: function (requestDialogId) {
        try {
            // Then we iterate throgh all of the html elements, and display set to inline.
            console.log('');
            console.log('----- PRINT ---- PRINT ---- PRINT ----');
            console.log('In bwPrintButton.js.PrintIndividualAttachment();');
            console.log('----- PRINT ---- PRINT ---- PRINT ----');
            console.log('');
            ...
            if (window.opener) {
                ...
                // Do the printing.
                var html = $('#' + requestDialogId)[0].innerHTML;
                thiz.CreateIFrameAndPrintThis(html);
                ...
                if ($('#' + requestDialogId)[0]) {
                    ...
                    var image = $('#' + requestDialogId).find('img')[0];
                    image.style.width = '100%'; // This makes sure that the image takes the entire page when printed. It works when switching between portrait and landscape. Woop! 4-1-2023.

                    var html = $('#' + requestDialogId)[0].innerHTML;
                    thiz.CreateIFrameAndPrintThis(html);

                } else {
                    displayAlertDialog('xcx2343251 Could not locate the element in the DOM for printing: "' + requestDialogId + '".');
                }
            ...
            console.log('');
            console.log('--- PRINT EXCEPTION ---');
            console.log('Exception in PrintIndividualAttachment(): ' + e.message + ', ' + e.stack);
            console.log('--- PRINT EXCEPTION ---');
            console.log('');

            alert('Exception in PrintIndividualAttachment(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in PrintIndividualAttachment(): ' + e.message + ', ' + e.stack);
        }
    ...
    PrintIndividualRequestReport_old_newer: function (requestDialogId) {
        try {
            // Then we iterate throgh all of the html elements, and display set to inline.
            console.log('');
            console.log('----- PRINT ---- PRINT ---- PRINT ----');
            ...
            // Get the html.
            if ($('#' + requestDialogId).find('#budgetrequestform')[0] && $('#' + requestDialogId).find('#budgetrequestform')[0]) {
                html = $('#' + requestDialogId).find('#budgetrequestform')[0].outerHTML;
            } else {
                html = $('#' + requestDialogId)[0].outerHTML; // 
            }
    ...
    PrintIndividualRequestReport_old: function (orgId) {
        //debugger;
        //$('#PrintDialog2').dialog('close');
        ...
        try {
            ...
            var printNow = function () {
                try {
                    setTimeout(function () { // Only needs to happen for Chrome.
                        try {
                            ...
                            try {
                                ...
                                try {
                                    console.log('bwPrintButton.PrintMyPendingTasksReport(): Try #2.');
                                    alert('bwPrintButton.PrintMyPendingTasksReport(): Try #2.');
                                    window.frames["printf"].print(); // Confirmed 5-14-2020. Works in Chrome. The print dialog gets invoked for Firefox, but only a blank page gets printed.
                                } catch (e) {
                                    console.log('bwPrintButton.PrintMyPendingTasksReport(): Try #3.');
                                    alert('bwPrintButton.PrintMyPendingTasksReport(): Try #3.');
                                    window.print(); // In Safari on MacBook, and Safari & Chrome on iOS, this is a synchronous call!!! (unlike most other situations where it is asynchronous).
                                }
                    ...
                    console.log('Exception in PrintIndividualRequestReport():xcx445: ' + e.message + ', ' + e.stack);
                    alert('Exception in PrintIndividualRequestReport():xcx445: ' + e.message + ', ' + e.stack);
                }
            }



            alert('xcx213121243214234-12 fix the image url here....');
                ...
                try {
                    setTimeout(function () { // Only needs to happen for Chrome.
                        try {
                            ...
                            console.log('Exception in xcx2141: ' + e.message + ', ' + e.stack);
                            alert('Exception in xcx2141: ' + e.message + ', ' + e.stack);
                        }
                    }, 500);
                } catch (e) {
                    console.log('Exception in xcx2142: ' + e.message + ', ' + e.stack);
                    alert('Exception in xcx2142: ' + e.message + ', ' + e.stack);
                }
            }).fail(function () {
                //alert("This org has no image."); // do nothing 
                console.log('Fail in xcx2142.');
                alert('Fail in xcx2142.');
            });
            ...
            window.onafterprint = function (e) {
                try {
                    ...
                    //};
                    //var $printbutton = $('#spanRequestPrintButton').bwPrintButton(printButtonOptions);
                    ...
                    console.log('Exception in window.onafterprint: ' + e.message);
                    alert('Exception in window.onafterprint: ' + e.message);
                }
            };
    ...
    expandHomePageNotificationAndRenderAlerts_Printed: function (notificationType, deferredIndex, functionalAreaRowId) {
        ...
        try {
            console.log('In bwPrintButton.expandHomePageNotificationAndRenderAlerts_Printed(' + notificationType + ', ' + deferredIndex + ', ' + functionalAreaRowId + ').');
            //debugger;
            //displayAlertDialog('type:' + notifications);
            ////displayAlertDialog('text:' + notifications);
            //displayAlertDialog('notifications[2].length:' + notifications.length);
            ...
            //handleExceptionWithAlert('Error in my.js.expandHomePageNotificationAndRenderAlerts()', '2:' + e.message);
            console.log('Exception in bwPrintButton.expandHomePageNotificationAndRenderAlerts_Printed() 2:' + e.message + ' ' + e.stack + ' This is not necessarily a service unavailable error but we dont want to bother the user!!');
            displayAlertDialog('Exception in bwPrintButton.expandHomePageNotificationAndRenderAlerts_Printed() 2:' + e.message + ' ' + e.stack + ' This is not necessarily a service unavailable error but we dont want to bother the user!!');
