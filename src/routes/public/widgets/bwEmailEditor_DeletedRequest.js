$.widget("bw.bwEmailEditor_DeletedRequest", {
    options: {
        ...
        */
        ...
        /*
        ===========================================================
        This is the bwEmailEditor_DeletedRequest.js jQuery Widget. 
        ===========================================================
    ...
    _create: function (assignmentRowChanged_ElementId) {
        this.element.addClass("bwEmailEditor_DeletedRequest");
        var thiz = this; // Need this because of the asynchronous operations below.
        try {
            console.log('In bwEmailEditor_DeletedRequest._create().');
            ...
            var html = '';
            html += '<span style="font-size:24pt;color:red;">bwEmailEditor_DeletedRequest: CANNOT RENDER</span>';
            html += '<br />';
            html += '<span style="">Exception in bwEmailEditor_DeletedRequest.Create(): ' + e.message + ', ' + e.stack + '</span>';
    ...
    _destroy: function () {
        this.element
            .removeClass("bwEmailEditor_DeletedRequest")
            .text("");
    },
    ...
    displayConfigureIntroductoryEmailDialog: function () {
        try {
            ...
            debugger;
            $.ajax({
                url: this.options.operationUriPrefix + "_bw/WorkflowDeletedRequestNotificationEmail",
                type: "Post",
                data: data,
                headers: {
                ...
                success: function (results) {
                    try {
                        if (results.message != 'SUCCESS') {
                            ...
                            var html = '';
                            html += 'This email gets sent to participants when a Request is Deleted.';
                            ...
                            $("#divConfigureNewUserEmailsDialog").dialog({
                                ...
                                open: function () {
                                    try {
                                        ...
                                        if (results.bwDeletedRequestNotificationEmailHtml) {
                                            try {
                                                debugger;
                                                // Yay, we got the email template from the database for this organization/workflowApp.
                                                emailTemplate = JSON.parse(results.bwDeletedRequestNotificationEmailHtml);
                        ...
                        var msg = 'Exception in bwEmailEditor_DeletedRequest.js.displayConfigureIntroductoryEmailDialog.WorkflowDeletedRequestNotificationEmail.success(): ' + e.message + ', ' + e.stack;
                        console.log(msg);
                ...
                error: function (data, errorCode, errorMessage) {
                    ...
                    var msg = 'Exception in bwEmailEditor_DeletedRequest.js.displayConfigureIntroductoryEmailDialog.WorkflowDeletedRequestNotificationEmail.error(): ' + JSON.stringify(data) + ', errorCode: ' + errorCode + ', errorMessage: ' + errorMessage;
                    console.log(msg);
                    displayAlertDialog(msg);
    ...
    publishEmailTemplateConfigurationAndActivate: function () {
        try {
            ...
            var bwDeletedRequestNotificationEmailHtml = {
                Subject: subject,
                Body: body
            };
            ...
            var data = {
                ...
                bwWorkflowAppId: workflowAppId,
                bwDeletedRequestNotificationEmailHtml: JSON.stringify(bwDeletedRequestNotificationEmailHtml)
            };

            $.ajax({
                url: thiz.options.operationUriPrefix + "_bw/PublishWorkflowDeletedRequestNotificationEmailTemplate",
                type: "Post",
                data: data,
                headers: {
                    "Accept": "application/json; odata=verbose"
                        ...
                        $("#divUndoNewUserEmailTemplateActivationDialog").dialog({
                            ...
                            open: function () {
                                try {
                                    ...
                                    thiz.options.CurrentEmailTemplate.EmailTemplate = JSON.parse(JSON.stringify(bwDeletedRequestNotificationEmailHtml));
                                    thiz.options.CurrentEmailTemplate.DraftEmailTemplate = JSON.parse(JSON.stringify(bwDeletedRequestNotificationEmailHtml));
    ...
    renderEmailEditor: function () {
        try {
            ...
            html += '           <tr>';
            html += '               <td style="width:90%;">';
            html += '                   <span id="spanUndoWorkflowActivationTitle" style="color: #3f3f3f;font-size:30pt;font-weight:bold;">Deleted Request Email ACTIVATED</span>';
            html += '               </td>';
            html += '               <td style="width:9%;"></td>';
            html += '               <td style="width:1%;cursor:pointer;vertical-align:top;">';
            ...
            html += '       This Deleted Request email has been activated and will be available immediately. ';
            html += '       <br />';
            html += '       <br />';
            html += '       <br />';
            ...
            html += '       <tr>';
            html += '           <td style="width:90%;">';
            html += '               <span style="font-family: \'Segoe UI Light\',\'Segoe UI\',\'Segoe\',Tahoma,Helvetica,Arial,sans-serif;color: #3f3f3f;font-size:30pt;font-weight:bold;">Configure "Deleted Request Email"</span>';
            html += '<span title="Introductory Email maintenance..." style="width:200px;padding:5px 10px 5px 10px;margin:0 0 0 10px;white-space:nowrap;vertical-align:middle;border:1px solid lightblue;cursor:pointer;font-weight:normal;font-size:10pt;" onclick="$(\'.bwEmailEditor_DeletedRequest\').bwEmailEditor_DeletedRequest(\'displayWorkflowsConfigurationDialog\');"><span style="display:inline-block;"> ⚙ </span></span>';
            html += '               <br />';
            ...
            html += '   <div class="divSignInButton" style="width:90%;text-align:center;line-height:1.1em;font-weight:bold;" onclick="$(\'.bwEmailEditor_DeletedRequest\').bwEmailEditor_DeletedRequest(\'publishEmailTemplateConfigurationAndActivate\');">';
