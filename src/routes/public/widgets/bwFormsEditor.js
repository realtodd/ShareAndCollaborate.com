$.widget("bw.bwFormsEditor", {
    options: {
        ...
        */
        ...
        /*
        ===========================================================
        This is the bwFormsEditor.js jQuery Widget. 
        ===========================================================
       ...
       */
        ...
        ActiveRequestForms: null, // THIS IS WHERE THE FORM (with HTML) IS STORED WHEN THE WIDGET IS INSTANTIATED! THIS COMES FROM THE BwForm TABLE.

        CurrentCloseoutForm: {
            Form: null,
            DraftForm: null
        },
        ActiveCloseoutForms: null, // THIS IS WHERE THE FORM (with HTML) IS STORED WHEN THE WIDGET IS INSTANTIATED! THIS COMES FROM THE BwForm TABLE.
    ...
    loadAndRenderFormsEditor: function () {
        try {
            ...
            console.log('In bwFormsEditor.js.loadAndRenderFormsEditor(). Getting forms configuration or organization: ' + workflowAppId);
                ...
                success: function (results) {
                    try {
                        ...
                        if (results.status != 'SUCCESS') {
                            var msg = 'Error in bwFormsEditor.js.loadAndRenderFormsEditor(). ' + results.status + ': ' + results.message;
                            console.log(msg);
                            alert(msg);
                        } else {

                            //
                            // The results include the field bwFormType, which is either 'request', or 'closeout'. Loop through to populate ActiveRequestForms and ActiveCloseoutForms.
                            //
                            thiz.options.ActiveRequestForms = [];
                            thiz.options.ActiveCloseoutForms = [];
                            for (var i = 0; i < results.data.length; i++) {
                                if (!results.data[i].bwFormType || (results.data[i].bwFormType == 'request')) { // Old records in the database may not have the bwFormType field. 3-25-2024.
                                    thiz.options.ActiveRequestForms.push(results.data[i]);
                                } else if (results.data[i].bwFormType == 'closeout') {
                                    thiz.options.ActiveCloseoutForms.push(results.data[i]);
                                } else {

                                    var msg = 'Error in bwFormsEditor.js.loadAndRenderFormsEditor(). Unexpected value for field bwFormType: ' + results.data[i].bwFormType;
                                    console.log(msg);
                        ...
                        var msg = 'Exception in bwFormsEditor.js.loadAndRenderFormsEditor.success(): ' + e.message + ', ' + e.stack;
                        console.log(msg);
                        displayAlertDialog(msg);
                    }
                },
                error: function (data, errorCode, errorMessage) {
                    var msg = 'Error in bwFormsEditor.js.loadAndRenderFormsEditor(). errorMessage: ' + errorMessage + ', errorCode: ' + errorCode + ', data: ' + JSON.stringify(data);
                    console.log(msg);
                    displayAlertDialog(msg);
                }
            });
            ...
            var msg = 'Exception in bwFormsEditor.js.loadAndRenderFormsEditor(): ' + e.message + ', ' + e.stack;
            console.log(msg);
            displayAlertDialog(msg);
        }
    },
    renderFormsEditor: function (checklistIndex) {
        try {
            console.log('In bwFormsEditor.js.renderFormsEditor().');
            var thiz = this;
            ...
            html += '                </td>';
            html += '                <td>';
            html += '<div id="divToolbox_BwAttachments_MusicPlaylist"></div>';
            html += '                </td>';
            html += '            </tr>';
            ...
            html += '                </td>';
            html += '                <td>';
            html += '<div id="divToolbox_BwCloseoutAssets"></div>';
            html += '                </td>';
            html += '            </tr>';
            ...
            if (this.options.BackendGlobalNewTenantFormsEditing == true) {
                ...
                html += 'Configure the <select id="selectFormTypeDropDown" onchange="$(\'.bwFormsEditor\').bwFormsEditor(\'formTypeOrRequestTypeDropdown_Onchange\');" class="selectHomePageWorkflowAppDropDown" style=\'display:inline;border-color: whitesmoke; color: grey; font-family: "Segoe UI Light","Segoe UI","Segoe",Tahoma,Helvetica,Arial,sans-serif; font-size: 1em; font-weight: bold; cursor: pointer; margin-bottom:15px; \'>'; // was .5em
                ...
                html += '<option value="executivesummary" >Executive Summary</option>';
                ...
                html += '   <select id="selectRequestTypeDropDown" onchange="$(\'.bwFormsEditor\').bwFormsEditor(\'formTypeOrRequestTypeDropdown_Onchange\');" class="selectHomePageWorkflowAppDropDown" style=\'display:inline;border-color: whitesmoke; color: grey; font-family: "Segoe UI Light","Segoe UI","Segoe",Tahoma,Helvetica,Arial,sans-serif; font-size: 1em; font-weight: bold; cursor: pointer; margin-bottom:15px; \'>'; // was .5em
                for (var i = 0; i < requestTypes.length; i++) {
                    var selected = '';
                    html += '<option value="' + requestTypes[i].bwRequestTypeId + '" ' + selected + ' >' + requestTypes[i].PluralName + '</option>';
                }
                ...
                //} else { // There is more than 1, so we have to display as a drop down.
                //    html += '<span style="font_weight:bold;color:grey;"><strong>';
                //    html += '   <select id="selectRequestTypeDropDown" onchange="$(\'.bwFormsEditor\').bwFormsEditor(\'formTypeOrRequestTypeDropdown_Onchange\');" style=\'display:inline;border-color: whitesmoke; color: grey; font-family: "Segoe UI Light","Segoe UI","Segoe",Tahoma,Helvetica,Arial,sans-serif; font-size: 14pt; font-weight: bold; cursor: pointer;\'>'; // was .5em
                //    for (var i = 0; i < requestTypes.length; i++) {
                //        var selected = '';
            ...
            //html += this.options.CurrentRequestForm.DraftForm; // This is the form! Make sure we get the right one in the _create() method.
            html += '<div id="budgetrequestform"></div>';
            ...
            this.formTypeOrRequestTypeDropdown_Onchange(); // This checks the values in the drop downs and displays the correct form.
            ...
            $('#divToolbox_BwAttachments_MusicPlaylist').bwAttachments_MusicPlaylist({ inFormsEditor: true });
            ...
            $('#divToolbox_BwCloseoutAssets').bwCloseoutAssets({ inFormsEditor: true });
    ...
    renderRequestForm: function () {
        try {
            console.log('In bwFormsEditor.js.renderRequestForm().');
            ...
            if (!bwRequestTypeId) {
                ...
                var msg = 'Error in bwFormsEditor.js.renderRequestForm(). Unexpected value for bwRequestTypeId: ' + bwRequestTypeId;
                console.log(msg);
                displayAlertDialog(msg);
                ...
                var bwFormHtml;
                for (var i = 0; i < this.options.ActiveRequestForms.length; i++) {
                    if (this.options.ActiveRequestForms[i].bwRequestTypeId == bwRequestTypeId) {
                        bwFormHtml = this.options.ActiveRequestForms[i].bwFormHtml;
                    }
                }
                ...
                if (!bwFormHtml) {
                    ...
                    bwFormHtml = $('.bwAuthentication').bwAuthentication('option', 'bwFormHtml_Default'); // Request form.

                    html += '<span style="color:tomato;">Error: No request form exists for this Request Type. Create one by customizing this default form, and clicking the Publish button.</span>';
                    html += '<br /><br />';
                    ...
                    //html += '<div id="budgetrequestform" align="left" bwrequesttitle="" bwbudgetrequestid="" bwworkflowtaskitemid="" bwworkflowid="" bwrequesttype="' + thiz.options.ActiveRequestForms[selectedFormIndex].bwRequestType + '" bwformid="' + thiz.options.ActiveRequestForms[selectedFormIndex].bwFormId + '" bworgid="" bworgname="">';
                    ...
                    this.options.CurrentRequestForm.Form = bwFormHtml; // Save these so that when checkIfWeHaveToDisplayThePublishChangesButton() is called, it can tell if there were any changes.
                    this.options.CurrentRequestForm.DraftForm = bwFormHtml;

                } else {

                    console.log('');
                    console.log('******************************************');
                    console.log('FORM>>: In bwFormsEditor.js.renderRequestForm(). Rendering the top of the form div tag with the request attributes.xcx464367-2');
                    console.log('******************************************');
                    console.log('');
                ...
                console.log('In bwFormsEditor.js.renderRequestForm(). Setting form element xdFormLayout width to 750px.'); // Added this 12-27-2023. This also exists elsewhere...
                document.getElementById('divPageContent3').getElementsByClassName('xdFormLayout')[0].style.width = '750px'; // 3-21-2023 This works for a New Request as well.

                this.replaceSpanElementTextWithGrayBar(); // This just pretties it up for the user. // This replaces some of the text on the form with gray areas which roughly reflect the current size and shape of the element displayed.

                var form = $(this.element).find('#budgetrequestform')[0];
                var draggableRows = $(form).find('.xdTableOffsetRow');
                console.log('In bwFormsEditor.js.renderRequestForm(). draggableRows (xdTableOffsetRow): ' + draggableRows.length);
                for (var i = 0; i < draggableRows.length; i++) {
                    if (draggableRows[i].style.display != 'none') {
                        draggableRows[i].id = 'draggablerow_' + i; // Our approach requires id's.
                        draggableRows[i].classList.add('xdTableOffsetRow-editor'); // This gives us the hover style etc. which help out with the drag and drop.
            ...
            var msg = 'Exception in bwFormsEditor.js.renderRequestForm(): ' + e.message + ', ' + e.stack;
            console.log(msg);
            displayAlertDialog(msg);
            $(this.element).find('#budgetrequestform')[0].innerHTML = msg;
        }
    },
    renderCloseoutForm: function () {
        try {
            console.log('In bwFormsEditor.js.renderCloseoutForm().');
            ...
            if (!bwRequestTypeId) {
                ...
                var msg = 'Error in bwFormsEditor.js.renderCloseoutForm(). Unexpected value for bwRequestTypeId: ' + bwRequestTypeId;
                console.log(msg);
                displayAlertDialog(msg);
                ...
                var bwFormHtml;
                for (var i = 0; i < this.options.ActiveCloseoutForms.length; i++) {
                    if (this.options.ActiveCloseoutForms[i].bwRequestTypeId == bwRequestTypeId) {
                        bwFormHtml = this.options.ActiveCloseoutForms[i].bwFormHtml;
                    }
                }
                ...
                if (!bwFormHtml) {
                    ...
                    bwFormHtml = $('.bwAuthentication').bwAuthentication('option', 'bwCloseoutFormHtml_Default'); // Closeout form.

                    html += '<span style="color:tomato;">Error: No closeout form exists for this Request Type. Create one by customizing this default form, and clicking the Publish button.</span>';
                    html += '<br /><br />';
                    ...
                    //html += '<div id="budgetrequestform" align="left" bwrequesttitle="" bwbudgetrequestid="" bwworkflowtaskitemid="" bwworkflowid="" bwrequesttype="' + thiz.options.ActiveCloseoutForms[selectedFormIndex].bwRequestType + '" bwformid="' + thiz.options.ActiveRequestForms[selectedFormIndex].bwFormId + '" bworgid="" bworgname="">';
                    html += '<div id="budgetrequestform" class="context-menu-newrequesteditor" align="left" bwbudgetrequestid="" bwrequesttypeid="' + bwRequestTypeId + '" bwformid="">';

                    html += bwFormHtml;

                    this.options.CurrentCloseoutForm.Form = bwFormHtml; // Save these so that when checkIfWeHaveToDisplayThePublishChangesButton() is called, it can tell if there were any changes.
                    this.options.CurrentCloseoutForm.DraftForm = bwFormHtml;
                ...
                console.log('In bwFormsEditor.js.renderCloseoutForm(). Setting form element xdFormLayout width to 750px.'); // Added this 12-27-2023. This also exists elsewhere...
                document.getElementById('divPageContent3').getElementsByClassName('xdFormLayout')[0].style.width = '750px'; // 3-21-2023 This works for a New Request as well.
                ...
                var form = $(this.element).find('#budgetrequestform')[0];
                var draggableRows = $(form).find('.xdTableOffsetRow');
                console.log('In bwFormsEditor.js.renderCloseoutForm(). draggableRows (xdTableOffsetRow): ' + draggableRows.length);
                for (var i = 0; i < draggableRows.length; i++) {
                    if (draggableRows[i].style.display != 'none') {
                        draggableRows[i].id = 'draggablerow_' + i; // Our approach requires id's.
                        draggableRows[i].classList.add('xdTableOffsetRow-editor'); // This gives us the hover style etc. which help out with the drag and drop.
            ...
            var msg = 'Exception in bwFormsEditor.js.renderCloseoutForm(): ' + e.message + ', ' + e.stack;
            console.log(msg);
            displayAlertDialog(msg);
            $(this.element).find('#budgetrequestform')[0].innerHTML = msg;
        }
    ...
    formTypeOrRequestTypeDropdown_Onchange: function () {
        try {
            console.log('In bwFormsEditor.js.formTypeOrRequestTypeDropdown_Onchange().');

            //var bwFormHtml1 = $('.bwAuthentication').bwAuthentication('option', 'bwCloseoutFormHtml_Default'); // Closeout form.

            //displayAlertDialog_Persistent('In bwFormsEditor.js.formTypeOrRequestTypeDropdown_Onchange(). bwFormHtml1: ' + bwFormHtml1);

            var formType;
            var formTypeDropdown = $(this.element).find('#selectFormTypeDropDown')[0];
            $(formTypeDropdown).find('option:selected').each(function (index, element) {
                formType = element.value;
            });
            ...
            } else if (formType == 'executivesummary') {

                alert('DEV NOTE: WE NEED TO ADD THIS HERE. xcx1231234123.');
               // this.renderCloseoutForm();

            } else if (formType == 'closeout') {

                this.renderCloseoutForm();

            } else {

                var msg = 'Error in bwFormsEditor.js.formTypeOrRequestTypeDropdown_Onchange(). Unexpected value for formType: ' + formType;
                console.log(msg);
                displayAlertDialog(msg);
            ...
            var msg = 'Exception in bwFormsEditor.js.formTypeOrRequestTypeDropdown_Onchange(): ' + e.message + ', ' + e.stack;
            console.log(msg);
            $(this.element).find('#budgetrequestform')[0].innerHTML = msg;
        }
    },
    ...
    publishFormConfigurationAndActivate: function () {
        try {
            ...
            if (!((formType == 'request') || (formType == 'closeout'))) {

                var msg = 'Error in bwFormsEditor.js.publishFormConfigurationAndActivate(). Unexpected value for formType: ' + formType;
                console.log(msg);
                displayAlertDialog(msg);
                ...
                //alert('xcx221312 bwFormId: ' + bwFormId);
                ...
                $.ajax({
                    url: this.options.operationUriPrefix + '_bw/publishform',
                    type: 'POST',
                    data: json,
                    headers: {
                    ...
                    success: function (result) {
                        try {
                            ...
                            if (result.message != 'SUCCESS') {
                                ...
                                console.log('Error in publishFormConfigurationAndActivate(), calling publishform xcx1: ' + result.message);
                                displayAlertDialog('Error in publishFormConfigurationAndActivate(), calling publishform xcx1: ' + result.message);
                                ...
                                displayAlertDialog_QuickNotice(msg).then(function () {
                                    try {
                                        ...
                                        // Todd: I removed this 1-22-2024. I hope it is OK!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                                        console.log('Todd: I commented this out (bwRequest.loadActiveRequestForms()) 1-22-2024. I hope it is OK!! I cant find the method loadActiveRequestForms().!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
                                        //var promise = $('.bwRequest').bwRequest('loadActiveRequestForms'); // replaced with loadActiveRequestForms() 4-15-2022
    ...
    cancelChangesInDraftFormConfiguration: function () {
        try {
            ...
            // Doing it for both request and closeout forms. Changes are deleted because we overwrite "Form" with "DraftForm".
            this.options.CurrentRequestForm["Form"] = this.options.CurrentRequestForm["DraftForm"]; // Creating "DraftWorkflow" so we can tell if the workflow has been changed or not, and then inform the user that changes need to be published.
            this.options.CurrentCloseoutForm["Form"] = this.options.CurrentCloseoutForm["DraftForm"];

            this.formTypeOrRequestTypeDropdown_Onchange();
    ...
    checkIfWeHaveToDisplayThePublishChangesButton: function () {
        try {
            ...
            var oldJsonString = this.options.CurrentRequestForm.Form;
            var newJsonString = this.options.CurrentRequestForm.DraftForm;
    ...
    toggleFormRequiredField: function (element) {
        try {
            ...
            this.options.CurrentRequestForm.DraftForm = $(this.element).find('#budgetrequestform').innerHTML;
    ...
    onDrop: function (event) {
        try {
            ...
            //debugger;
            thiz.options.CurrentRequestForm.DraftForm = $(thiz.element).find('#budgetrequestform').innerHTML;
    ...
    activateSelectedForm: function (bwFormId, bwRequestType) {
        try {
            ...
            alert('In activateSelectedForm(). Is this method still used?');
            var thiz = this;
    ...
    deactivateSelectedForm: function (bwFormId) {
        try {
            ...
            alert('In deactivateSelectedForm(). Is this method still used?');
            var thiz = this;
    ...
    deleteSelectedForms: function () {
        try {
            ...
            alert('In deleteSelectedForms(). Is this method still used?');
            var thiz = this;
    ...
    displayFormsConfigurationDialog: function () {
        try {
            ...
            alert('In displayFormsConfigurationDialog(). Is this method still used?');
            var thiz = this;
