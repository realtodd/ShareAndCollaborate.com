$.widget("bw.bwOrganizationPicker2", {
    options: {
        ...
        */
        ...
        /*
        ===========================================================
        This is the bwOrganizationPicker2.js jQuery Widget. 
        ===========================================================
        ...
        */
        ...
        bwLastSelectedOrgUnitId: null, // Added to table and here 12-18-2023. THIS field is required for this widget in order for it to operate. It should return an error if this is not present.
    ...
    _create: function (assignmentRowChanged_ElementId) {
        this.element.addClass("bwOrganizationPicker2");
        var thiz = this; // Need this because of the asynchronous operations below.
        try {

            if (!this.options.bwLastSelectedOrgUnitId) {

                var html = '';
                html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwOrganizationPicker2</span>';
                html += '<br />';
                html += '<span style="">Invalid value for bwLastSelectedOrgUnitId: ' + this.options.bwLastSelectedOrgUnitId + '</span>';
                ...
                if (this.options.bwOrgId && this.options.bwOrgName) {
                ...
                console.log('In bwOrganizationPicker2.js(). ALL OF THE FORM WIDGETS NEED TO CHECK IF THIS IS A CUSTOMER, VENDOR, OR [participant, archiveviewer, reportviewer, configurationmanager, owner].');
                ...
                if (!(participantRole && ((participantRole == 'customer') || (participantRole == 'participant') || (participantRole == 'archiveviewer') || (participantRole == 'reportviewer') || (participantRole == 'configurationmanager') || (participantRole == 'owner')))) {
                    ...
                    var html = '';
                    html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwOrganizationPicker2xcx1-2</span>';
                    html += '<br />';
                    ...
                    this.element.html(this.renderLocationPicker(assignmentRowChanged_ElementId)); // Need to render, not allowing the user to make modifications.

                } else if (participantRole && ((participantRole == 'participant') || (participantRole == 'archiveviewer') || (participantRole == 'reportviewer') || (participantRole == 'configurationmanager') || (participantRole == 'owner'))) {
                    ...
                    if (this.options.inVisualizations == true) {
                        $.ajax({
                            ...
                            try {
                                ...
                                console.log('Exception in bwOrganizationPicker2._create().xx.Get:1-1: ' + e.message + ', ' + e.stack);
                                displayAlertDialog('Exception in bwOrganizationPicker2._create().xx.Get:1-1: ' + e.message + ', ' + e.stack);
                            }
                        }).fail(function (data) {
                            console.log('In bwOrganizationPicker2._create.fail(): ' + JSON.stringify(data));
                            var msg;
                            ...
                            alert('Exception in bwOrganizationPicker2._create().xx.Get:2-1: ' + msg); //+ error.message.value + ' ' + error.innererror.message);
                            console.log('Exception in bwOrganizationPicker2._create().xx.Get:2-1: ' + JSON.stringify(data));
                        ...
                        if (this.options.inFormsEditor == true) {
                            ...
                            if (this.options.allowRequestModifications == false) {
                                ...
                                var html = '';
                                html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwOrganizationPicker2xcx1</span>';
                                html += '<br />';
                            ...
                            $.ajax({
                                ...
                                success: function (orcResult) {
                                    try {
                                        ...
                                        if (!(thiz.options.bwOrgId && thiz.options.bwOrgName)) {
                                            ...
                                            console.log('');
                                            console.log('***************');
                                            console.log('In bwOrganizationPicker2.js._create(). Another crazy fix here, come back sometime to revisit. xcx3245347-2');
                                            console.log('***************');
                                            console.log('');

                                            //alert('In bwOrganizationPicker2.js._create(). Another crazy fix here, come back sometime to revisit. xcx3245347-2');
                                        ...
                                        console.log('Exception in bwOrganizationPicker2._create().xx.Get:1-2: ' + e.message + ', ' + e.stack);
                                        displayAlertDialog('Exception in bwOrganizationPicker2._create().xx.Get:1-2: ' + e.message + ', ' + e.stack);
                                    }
                                },
                                error: function (jqXHR, settings, errorThrown) {
                                    ...
                                    if (jqXHR.status == 401) { // HTTP 401 "Unauthorized".
                                        ...
                                        console.log('Error in bwOrganizationPicker2.js._create.orgrolesconfiguration(). xcx213124-1 Unauthorized. Please refresh your browser and log in again. jqXHR.status: ' + jqXHR.status + ', settings: ' + settings + ', errorThrown: ' + errorThrown);

                                    } else {
                                        console.log('Error in bwOrganizationPicker2.js._create.orgrolesconfiguration():1: ' + settings + ', ' + errorThrown + ' I suspect this may be a service unavailable error but not sure by any means! More investigation needed!' + JSON.stringify(jqXHR));
                                        alert('Error in bwOrganizationPicker2.js._create.orgrolesconfiguration():1: ' + settings + ', ' + errorThrown + ' I suspect this may be a service unavailable error but not sure by any means! More investigation needed!' + JSON.stringify(jqXHR));
                                    }
                ...
                console.log('In bwOrganizationPicker2._create(). The widget has been initialized.');
            ...
            var html = '';
            html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwOrganizationPicker2xcx2</span>';
            html += '<br />';
            html += '<span style="">Exception in bwOrganizationPicker2.Create(): ' + e.message + ', ' + e.stack + '</span>';
            thiz.element.html(html);
        }
    },
    ...
    getData: function () {
        try {
            console.log('In bwOrganizationPicker2.getData().');
            var value = $(this.element).find('#txtOrganizationPickerFilter')[0].value;
            return value;
        } catch (e) {
            console.log('Exception in bwOrganizationPicker2.getData(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwOrganizationPicker2.getData(): ' + e.message + ', ' + e.stack);
        }
    },
    instantiated: function () {
        try {
            console.log('In bwOrganizationPicker2.instantiated().');
            return this.options.instantiated;
        } catch (e) {
            console.log('Exception in bwOrganizationPicker2.instantiated(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwOrganizationPicker2.instantiated(): ' + e.message + ', ' + e.stack);
        }
    },
    isARequiredField: function () {
        try {
            console.log('In bwOrganizationPicker2.isARequiredField().');
            return this.options.renderAsARequiredField;
        } catch (e) {
            console.log('Exception in bwOrganizationPicker2.isARequiredField(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwOrganizationPicker2.isARequiredField(): ' + e.message + ', ' + e.stack);
        }
    },
    getfieldTitle: function () {
        try {
            console.log('In bwOrganizationPicker2.getfieldTitle().');
            return this.options.fieldTitle;
        } catch (e) {
            console.log('Exception in bwOrganizationPicker2.getfieldTitle(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwOrganizationPicker2.getfieldTitle(): ' + e.message + ', ' + e.stack);
        }
    },
    renderLocationPicker: function (assignmentRowChanged_ElementId) {
        try {
            console.log('In bwOrganizationPicker2.js.renderLocationPicker().');
            ...
            html += '           <input  id="txtOrganizationPickerFilter" type="text" class="selectHomePageWorkflowAppDropDown" style="width:750px;border-color: whitesmoke; color: grey;font-size: 1em; font-weight: bold; cursor: pointer; font-family: \'Segoe UI Light\',\'Segoe UI\',\'Segoe\',Tahoma,Helvetica,Arial,sans-serif;" onkeyup="$(\'#' + parentElementId + '\').find(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'txtOrganizationPickerFilter_OnKeyup\', \'txtOrganizationPickerFilter\');" onmouseup="$(\'#' + parentElementId + '\').find(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'txtOrganizationPickerFilter_OnMouseup\', \'txtOrganizationPickerFilter\');" />';
            ...
            if (developerModeEnabled == true) {
                //
                //
                // 12-18-2023. Checkbox: Include Sub-Units in results
                //
                //
                html += '<br />';
                html += '<input type="checkbox" style="transform: scale(3);" />&nbsp;<span style="font-size:20px;color:cornflowerblue;font-weight:bold;">Include Sub-Units in results</span>';
            }
            ...
            var html = '';
            html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwOrganizationPicker2xcx3</span>';
            html += '<br />';
            html += '<span style="">Exception in bwOrganizationPicker2.renderLocationPicker(): ' + e.message + ', ' + e.stack + '</span>';
            thiz.element.html(html);
        }
    },
    ...
    //        html += '                        <tbody><tr class="xdTableOffsetRow" id="toolboxdraggablerow_2554" bwwidgetname="bwOrganizationPicker2">';
    ...
    //        html += '<input id="txtOrganizationPickerFilter" class="bwRequestJson" bwdatarequired="true" bwdatatype="String" bwfieldname="bwOrganizationPicker2" contentEditable="false" disabled type="text" style="width:450px;font-family: \'Segoe UI Light\',\'Segoe UI\',\'Segoe\',Tahoma,Helvetica,Arial,sans-serif;color: #262626;font-size: 20pt;"></input>';
    ...
    //        var html = '';
    //        html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwOrganizationPicker2xcx4</span>';
    //        html += '<br />';
    //        html += '<span style="">Exception in bwOrganizationPicker2.renderLocationPicker_ReadOnly(): ' + e.message + ', ' + e.stack + '</span>';
    //        thiz.element.html(html);
    //    }
    //},
    ...
    //        html += '<table style="width:100%;">';
    //        html += '                        <tbody><tr class="xdTableOffsetRow" bwwidgetname="bwOrganizationPicker2">';
    //        html += '                            <td class="xdTableOffsetCellLabel" style="text-align:left; VERTICAL-ALIGN: top; PADDING-BOTTOM: 4px; PADDING-TOP: 4px; PADDING-LEFT: 22px; BORDER-LEFT: #d8d8d8 1pt; PADDING-RIGHT: 5px">';
    //        html += '                                <span class="xdlabel">';
    //        html += '                                    Locationxcx3:';
    //        html += '                                </span>';
    ...
    //        html += ' onkeyup="$(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'txtOrganizationPickerFilter_OnKeyup\', \'txtOrganizationPickerFilter\');" ';
    //        html += ' onmouseup="$(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'txtOrganizationPickerFilter_OnMouseup\', \'txtOrganizationPickerFilter\');">';
    //        html += '</input>';
    //        //html == '<input id="txtOrganizationPickerFilterSelectedLocationId" type="hidden" value="" />';
    ...
    //        var html = '';
    //        html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwOrganizationPicker2xcx5</span>';
    //        html += '<br />';
    //        html += '<span style="">Exception in bwOrganizationPicker2.renderLocationPicker_Visualizations(): ' + e.message + ', ' + e.stack + '</span>';
    //        thiz.element.html(html);
    //    }
    //},
    ...
    renderLocationPickerTreeview1: function (assignmentRowChanged_ElementId) {
        try {
            ...
            html += '               <tr>';
            html += '                   <td>';
            html += '                       <span style="cursor:pointer;font-weight:bold;" title="Collapse..." onclick="$(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'collapseTree\', \'root\');">';
            //debugger;
            ...
            for (var d = 0; d < json.Global.Divisions.Items.length; d++) {
                ...
                if (json.Global.Divisions.Items[d].Groups.Style == 'display:none;') {
                    html += '                       <tr>';
                    html += '                           <td>';
                    html += '                               <span style="cursor:pointer;font-weight:bold;" title="Collapse..." onclick="$(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'expandTree\', \'' + d + '\');">';
                    html += '✚ ✣ Groups ✣  </span>';
                    html += '                           </td>';
                    html += '                       </tr>';
                } else {
                    html += '                       <tr>';
                    html += '                           <td>';
                    html += '                               <span style="cursor:pointer;font-weight:bold;" title="Collapse..." onclick="$(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'collapseTree\', \'' + d + '\');">';
                    html += '⚊ ✣ Groups ✣  </span>';
                    html += '                           </td>';
                    html += '                       </tr>';
                    for (var g = 0; g < json.Global.Divisions.Items[d].Groups.Items.length; g++) {
                        ...
                        if (json.Global.Divisions.Items[d].Groups.Items[g].LegalEntities.Style == 'display:none;') {
                            html += '                               <tr>';
                            html += '                                   <td>';
                            html += '                               <span style="cursor:pointer;font-weight:bold;" title="Collapse..." onclick="$(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'expandTree\', \'' + d + '\', \'' + g + '\');">';
                            ...
                            html += '                               <tr>';
                            html += '                                   <td>';
                            html += '                               <span style="cursor:pointer;font-weight:bold;" title="Collapse..." onclick="$(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'collapseTree\', \'' + d + '\', \'' + g + '\');">';
                            html += '⚊ ⚖ Legal Entities ⚖  </span>';
                            html += '                                   </td>';
                            html += '                               </tr>';
                            for (var e = 0; e < json.Global.Divisions.Items[d].Groups.Items[g].LegalEntities.Items.length; e++) {
                                ...
                                if (json.Global.Divisions.Items[d].Groups.Items[g].LegalEntities.Items[e].Locations.Style == 'display:none;') {
                                    html += '                                   <tr>';
                                    html += '                                       <td>';
                                    html += '                               <span style="cursor:pointer;font-weight:bold;" title="Expand Locations" onclick="$(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'expandTree\', \'' + d + '\', \'' + g + '\', \'' + e + '\');">';
                                    html += '✚ 🏠 Locations 🏠</span>';
                                    html += '                                       </td>';
                                    html += '                                   </tr>';
                                } else {
                                    html += '                                   <tr>';
                                    html += '                                       <td>';
                                    html += '                               <span style="cursor:pointer;font-weight:bold;" title="Collapse Locations" onclick="$(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'collapseTree\', \'' + d + '\', \'' + g + '\', \'' + e + '\');">';
                                    html += '⚊ 🏠 Locations 🏠</span>';
                                    html += '                                       </td>';
                                    html += '                                   </tr>';
                                    for (var l = 0; l < json.Global.Divisions.Items[d].Groups.Items[g].LegalEntities.Items[e].Locations.Items.length; l++) {
                                        ...
                                        html += '<span style="cursor:pointer;" title="Manage Roles..." onclick="$(\'#' + requestFormId + '\').find(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'selectLocation\', \'' + json.Global.Divisions.Items[d].Groups.Items[g].LegalEntities.Items[e].Locations.Items[l].Id + '\', \'' + tmpName + '\');">';
            ...
            console.log('Exception in bwOrganizationPicker2.js.renderLocationPickerTreeview1: ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwOrganizationPicker2.js.renderLocationPickerTreeview1: ' + e.message + ', ' + e.stack);
        }
    },
    ...
    renderLocationPickerTreeview2: function (assignmentRowChanged_ElementId) {
        try {
            ...
            if (this.options.store && this.options.store.Global) {
                json = this.options.store.Global;
                if (!(json && json.Global)) {
                    json = this.options.store;
                }
            }
            ...
            var tmpName = 'xcx99187245';

            if (json && json.Global && json.Global.Name) {
            ...
            //var tmpName = String(json.Global.Name).replace(/[']/g, ''); // encodeURIComponent does not do apostrophe's, which we need here, because we are passing this as defined method calls in the code, preformulated.
            html += '                                   <span class="orgTreeNode2" xcx="xcx12314-10" style="cursor:pointer;" onclick="$(\'#' + requestFormId + '\').find(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'selectLocation\', \'root\', \'' + tmpName + '\');" >';
            ...
            html += '&nbsp;&nbsp;</span><span style="font-size:15pt;">' + name + '</span> ';
            html += '                                   </span> ';
            ...
            html += '<td>';
            // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            html += '                               <span id="spanDivisionsTreeExpandCollapseButton" style="cursor:pointer;" onclick="$(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'collapseOrExpandTree\', \'\', \'spanDivisionsTreeExpandCollapseButton\', \'root\');">';
            //html += '                               ⚊ ';
            ...
            if (json && json.Global && json.Global.Divisions && json.Global.Divisions.Items && json.Global.Divisions.Items.length && (json.Global.Divisions.Items.length > 0)) {
                ...
                for (var d = 0; d < json.Global.Divisions.Items.length; d++) {
                    ...
                    html += '<span id="spanDivisionTreeExpandCollapseButton_' + d + '" style="cursor:pointer;" onclick="$(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'collapseOrExpandTree\', \'\', \'spanDivisionTreeExpandCollapseButton_' + d + '\', \'' + d + '\');">';
                    //html += '⚊';
                    html += '<img title="collapse" style="cursor:pointer;width:23px;height:23px;vertical-align:middle;" src="images/drawer-close.png">';
                    html += '</span>';
                    ...
                    html += ' <span class="orgTreeNode2" xcx="xcx12314-12" style="cursor:pointer;" onclick="$(\'#' + requestFormId + '\').find(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'selectLocation\', \'' + json.Global.Divisions.Items[d].Id + '\', \'' + tmpName + '\');" >';
                    ...
                    if (json.Global.Divisions.Items[d].Groups.Style == 'display:none;') {
                        ...
                        // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                        html += '                               <span id="spanGroupsTreeExpandCollapseButton_' + d + '" style="cursor:pointer;" onclick="$(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'collapseOrExpandTree\', \'groupsnode\', \'spanGroupsTreeExpandCollapseButton_' + d + '\', \'' + d + '\');">';
                        //html += '                               ⚊ ';
                        html += '<img title="collapse" style="cursor:pointer;width:23px;height:23px;vertical-align:middle;" src="images/drawer-close.png" />';
                        html += '                               </span>';
                        ...
                        for (var g = 0; g < json.Global.Divisions.Items[d].Groups.Items.length; g++) {
                            ...
                            html += '                               <span id="spanGroupTreeExpandCollapseButton_' + d + '_' + g + '" style="cursor:pointer;" onclick="$(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'collapseOrExpandTree\', \'\', \'spanGroupTreeExpandCollapseButton_' + d + '_' + g + '\', \'' + d + '\', \'' + g + '\');">';
                            html += '<img title="collapse" style="cursor:pointer;width:23px;height:23px;vertical-align:middle;" src="images/drawer-close.png" />';
                            html += '                               </span>';
                            ...
                            html += '                                   <span class="orgTreeNode2" xcx="xcx12314-14" style="cursor:pointer;" onclick="$(\'#' + requestFormId + '\').find(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'selectLocation\', \'' + json.Global.Divisions.Items[d].Groups.Items[g].Id + '\', \'' + tmpName + '\');" >';
                            ...
                            if (json.Global.Divisions.Items[d].Groups.Items[g].LegalEntities.Style == 'display:none;') {
                                ...
                                html += '                                       <span id="spanLegalEntitiesTreeExpandCollapseButton_' + d + '_' + g + '" style="cursor:pointer;font-weight:bold;white-space:nowrap;" title="Collapse..." onclick="$(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'collapseOrExpandTree\', \'legalentitiesnode\', \'spanLegalEntitiesTreeExpandCollapseButton_' + d + '_' + g + '\', \'' + d + '\', \'' + g + '\');">';
                                //html += '                                       ⚊ ';
                                html += '<img title="collapse" style="cursor:pointer;width:23px;height:23px;vertical-align:middle;" src="images/drawer-close.png">';
                                html += '                                       </span>';
                                html += 'Legal Entities &nbsp;&nbsp; ';
                                ...
                                for (var e = 0; e < json.Global.Divisions.Items[d].Groups.Items[g].LegalEntities.Items.length; e++) {
                                    ...
                                    // new 3-7-2020
                                    html += '                               <span id="spanGroupsTreeExpandCollapseButton_' + d + '_' + g + '_' + e + '" style="cursor:pointer;" onclick="$(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'collapseOrExpandTree\', \'\', \'spanGroupsTreeExpandCollapseButton_' + d + '_' + g + '_' + e + '\', \'' + d + '\', \'' + g + '\', \'' + e + '\');">';
                                    //html += '                               ⚊ ';
                                    html += '<img title="collapse" style="cursor:pointer;width:23px;height:23px;vertical-align:middle;" src="images/drawer-close.png" />';
                                    html += '                               </span>';
                                    ...
                                    html += '   <span class="orgTreeNode2" xcx="xcx12314-15" style="cursor:pointer;color:#29685F;white-space:nowrap;" onclick="$(\'#' + requestFormId + '\').find(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'selectLocation\', \'' + json.Global.Divisions.Items[d].Groups.Items[g].LegalEntities.Items[e].Id + '\', \'' + tmpName + '\');" >';
                                    ...
                                    if (json.Global.Divisions.Items[d].Groups.Items[g].LegalEntities.Items[e].Locations.Style == 'display:none;') {
                                        ...
                                        for (var l = 0; l < json.Global.Divisions.Items[d].Groups.Items[g].LegalEntities.Items[e].Locations.Items.length; l++) {
                                            ...
                                            var tmpName = String(json.Global.Divisions.Items[d].Groups.Items[g].LegalEntities.Items[e].Locations.Items[l].Name).replace(/[']/g, ''); // encodeURIComponent does not do apostrophe's, which we need here, because we are passing this as defined method calls in the code, preformulated.
                                            html += '                                   <span class="orgTreeNode2" xcx="xcx12314-16" style="cursor:pointer;white-space:nowrap;" onclick="$(\'#' + requestFormId + '\').find(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'selectLocation\', \'' + json.Global.Divisions.Items[d].Groups.Items[g].LegalEntities.Items[e].Locations.Items[l].Id + '\', \'' + tmpName + '\');">';
            ...
            console.log('Exception in renderLocationPickerTreeview2: ' + e.message + ', ' + e.stack);
            var html = '';
            html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwOrganizationPicker2xcx6</span>';
            html += '<br />';
            html += '<span style="">Exception in bwOrganizationPicker2.renderLocationPickerTreeview2(): ' + e.message + ', ' + e.stack + '</span>';
            this.element.html(html);
        }
    },
                            ...
                            for (var e = 0; e < json.Global.Divisions.Items[d].Groups.Items[g].LegalEntities.Items.length; e++) {
                                ...
                                if (json.Global.Divisions.Items[d].Groups.Items[g].LegalEntities.Items[e].Locations.Style == 'display:none;') {
                                    ...
                                    for (var l = 0; l < json.Global.Divisions.Items[d].Groups.Items[g].LegalEntities.Items[e].Locations.Items.length; l++) {
                                        ...
                                        var tmpName = String(json.Global.Divisions.Items[d].Groups.Items[g].LegalEntities.Items[e].Locations.Items[l].Name).replace(/[']/g, ''); // encodeURIComponent does not do apostrophe's, which we need here, because we are passing this as defined method calls in the code, preformulated.
                                        html += '<span style="cursor:pointer;" title="Manage Roles..." onclick="$(\'.bwTrackSpending\').find(\'.bwOrganizationPicker2\').bwOrganizationPicker2(\'selectLocation\', \'' + json.Global.Divisions.Items[d].Groups.Items[g].LegalEntities.Items[e].Locations.Items[l].Id + '\', \'' + tmpName + '\');">';
                                        html += ' 🏠 ' + json.Global.Divisions.Items[d].Groups.Items[g].LegalEntities.Items[e].Locations.Items[l].Name + ' 🏠 ';
                                        html += '</span>';
                                        html += '                                   </td>';
                                        html += '                               </tr>';
    ...
    selectLocation: function (bwOrgId, bwOrgName) {
        try {
            ...
            console.log('In bwOrganizationPicker2.js.selectLocation(). bwOrgId: ' + bwOrgId + ' xcx1, bwOrgName: ' + bwOrgName);
            //alert('In bwOrganizationPicker2.js.selectLocation(). bwOrgId: ' + bwOrgId + ' xcx1, bwOrgName: ' + bwOrgName);
            var thiz = this;
            ...
            console.log('In bwOrganizationPicker2.js.selectLocation(). bwOrgId: ' + bwOrgId + ' xcx2, bwOrgName: ' + bwOrgName);
            //debugger;
            ...
            try {
                ...
                console.log('Caught exception in bwOrganizationPicker2.selectLocation.dialog.close():xcx1: ' + e.message + ', ' + e.stack);
                //debugger;
            ...
            //    console.log('');
            //    console.log('***************');
            //    console.log('In bwOrganizationPicker2.js.selectLocation(). Another crazy fix here, come back sometime to revisit. xcx3245347-1');
            //    console.log('***************');
            //    console.log('');
            ...
            //    } else {
            //        debugger;
            //        console.log('Error in bwOrganizationPicker2.selectLocation(): Unexpected parameter in displayOrgRoleEditorInACircle():2:.');
            //    }
            //    //debugger;
            ...
            if (parentElementId == thiz.options.parentElementId) { //'divPageContent1') { // 3-3-2022
                ...
                var txtOrganizationPickerFilter = $(this.element).find('#txtOrganizationPickerFilter');
                var spanNewRequestFormLocationPickerBreadcrumb = $(this.element).find('#spanNewRequestFormLocationPickerBreadcrumb');

                if (!(txtOrganizationPickerFilter && txtOrganizationPickerFilter.length && (txtOrganizationPickerFilter.length > 0) && txtOrganizationPickerFilter[0])) {

                    console.log('xcx88295 Could not find element #txtOrganizationPickerFilter.');
                    alert('xcx88295 Could not find element #txtOrganizationPickerFilter.');
                ...
                if (bwDataGrid) {
                    ...
                    ////alert('In bwOrganizationPicker2.js.selectLocation(). Its bwDataGrid.');
                    ...
                    //    //alert('In bwOrganizationPicker2.js.selectLocation(). Its bwDataGrid. It is the same, so do not re-render. bwOrgId: ' + bwOrgId);
                    //} else {
                    //    // The user has selected a different location/org, so re-render.
                    //    console.log('In bwOrganizationPicker2.js.selectLocation(). bwOrgId: ' + bwOrgId + ', bwOrgName: ' + bwOrgName + ', and calling bwDataGrid.renderDataGrid().');
                    //    //alert('In bwOrganizationPicker2.js.selectLocation(). A new location/org has been selected for the bwDataGrid widget.');
                    //    thiz.options.bwOrgId = bwOrgId;
                    //    thiz.options.bwOrgName = bwOrgName;
                    ...
                    //    //    console.log('In bwOrganizationPicker2.js.selectLocation(). Unexpected value for bwDisplayFormat: ' + bwDisplayFormat);
                    ...
                    //alert('In bwOrganizationPicker2.js.selectLocation(). Its bwTrackSpending.');
                    //var divBwDataGrid_Content_Html = $(thiz.element).find('#divBwDataGrid_Content').html(); // If this is empty, it means the DataGrid has never been rendered... it is the first time here.
                    if ((thiz.options.bwOrgId == bwOrgId) && (thiz.options.bwOrgName == bwOrgName)) {
                        // It is the same, so do not re-render.
                    } else {
                        // The user has selected a different location/org, so re-render.
                        console.log('In bwOrganizationPicker2.js.selectLocation(). bwOrgId: ' + bwOrgId + ', bwOrgName: ' + bwOrgName + ', and calling bwTrackSpending.renderTrackSpending().');
                        //alert('In bwOrganizationPicker2.js.selectLocation(). A new location/org has been selected for the bwTrackSpending widget.');
                        thiz.options.bwOrgId = bwOrgId;
                        thiz.options.bwOrgName = bwOrgName;
                    ...
                    var participantOrgRolePickerDialog_OrgPicker = $('#divParticipantOrgRolePickerDialog_OrgPicker'); //.find('.bwOrganizationPicker2')[0];   // bwOrganizationPicker2
                    ...
                    console.log('Error in bwOrganizationPicker2.js.selectLocation(). Could not locate widgets bwDataGrid, bwTrackSpending.');
                    //displayAlertDialog('Error in bwOrganizationPicker2.js.selectLocation(). Could not locate widgets bwDataGrid, bwTrackSpending, or bwParticipantsEditor.');
                ...
                $.get(imagePath2).done(function () {
                    setTimeout(function () { // Only needs to happen for Chrome.
                        ...
                        console.log('In bwOrganizationPicker2.js.selectLocation.get.imagePath2.done(). imagePath2: ' + imagePath2);
                        console.log('00000000000000000000000000000000');
                        console.log('');
                    ...
                    console.log('');
                    console.log('00000000000000000000000000000000');
                    console.log('In bwOrganizationPicker2.js.selectLocation.get.imagePath2.fail(). imagePath2: ' + imagePath2);
                    console.log('00000000000000000000000000000000');
                    console.log('');
                });
                // End: Getting the custom image
                ...
                alert('Error in bwOrganizationPicker2.js.selectLocation(). Unexpected value for parentElementId: ' + parentElementId);
            ...
            //
            //console.log('In bwOrganizationPicker2.selectLocation(). This is where we check if the workflow dialog is displayed. If so, update the thing to correspond to this org id.');
            ...
            //    console.log('In bwOrganizationPicker2.selectLocation(). Need to redraw chart.xcx1');
            ...
            try {
                ...
                //console.log('Exception in bwOrganizationPicker2.selectLocation.dialog.close():xcx1: ' + e.message + ', ' + e.stack);
                ////debugger;
                //var x = 'why are we here?';
            }

            console.log('Exception in bwOrganizationPicker2.js.selectLocation(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwOrganizationPicker2.js.selectLocation(): ' + e.message + ', ' + e.stack);
        }
    },
    txtOrganizationPickerFilter_OnKeyup: function (elementId) {
        try {
            console.log('In bwOrganizationPicker2.js.txtOrganizationPickerFilter_OnKeyup(). elementId: ' + elementId);
            //var formElement = this.element.closest("#budgetrequestform");
            //if (formElement) {
    ...
    txtOrganizationPickerFilter_OnMouseup: function () {
        try {
            //debugger;
            console.log('In bwOrganizationPicker2.js.txtOrganizationPickerFilter_OnMouseup().');
            var thiz = this;
