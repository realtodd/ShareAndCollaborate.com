...
function cmdAdminDeleteSelectedTenant() {
    ...
    try {
        console.log('In admin2.js.cmdAdminDeleteSelectedTenant().');
...
function renderConfigurationSettings() {
    try {
        ...
        html += '  <tr>';
        html += '    <td style="text-align:left;" class="bwSliderTitleCell">';
        html += '       Organization theme/colors:&nbsp;';
        html += '    </td>';
        html += '    <td class="bwChartCalculatorLightCurrencyTableCell">';
...
function cmdListAllExceptionLogs() {
    try {
        console.log('In cmdListAllExceptionLogs().');

        // Use ajax.
        $('#txtBwExceptionLog').empty();
        var data = {
            //"bwTenantId": "null",
            //"bwTenantOwnerFacebookUserId": "test1",
            //"bwTenantOwnerLinkedInUserId": "test1",
        ...
        debugger;
        $.ajax({
            url: webserviceurl + "/bwexceptionlogs",
            type: "DELETE",
            contentType: 'application/json',
            data: JSON.stringify(data),
            success: function (data) {
                try {
                    ...
                    html += 'DONE';
                    $('#txtBwExceptionLog').append(html);
                } catch (e) {
                    console.log('Exception in cmdListAllExceptionLogs(): ' + e.message + ', ' + e.stack);
                }
            },
            error: function (data, errorCode, errorMessage) {
                //window.waitDialog.close();
                //handleExceptionWithAlert('Error in index.js.cmdCreateANewTenant()', errorCode + ', ' + errorMessage);
                displayAlertDialog('Error in admin.js.cmdListAllExceptionLogs():' + errorCode + ', ' + errorMessage);
            }
        });
    } catch (e) {
        console.log('Exception in cmdListAllExceptionLogs(): ' + e.message + ', ' + e.stack);
        displayAlertDialog('Exception in cmdListAllExceptionLogs(): ' + e.message + ', ' + e.stack);
    }
}
...
function populateSelectViewBwWorkflowTaskSelectWorkflowApp() {
    ...
    $.ajax({
        ...
        error: function (data, errorCode, errorMessage) {
            console.log('Error in index.js.populateSelectViewBwWorkflowTaskSelectWorkflowApp():xcx8709:' + errorCode + ', ' + errorMessage);
            displayAlertDialog('Error in index.js.populateSelectViewBwWorkflowTaskSelectWorkflowApp():xcx8709:' + errorCode + ', ' + errorMessage);
        }
    });
}
