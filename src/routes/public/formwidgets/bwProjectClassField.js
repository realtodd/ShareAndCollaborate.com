$.widget("bw.bwProjectClassField", {
    options: {
        ...
        */
        ...
        /*
        ===========================================================
        This is the bwProjectClassField.js jQuery Widget. 
        ===========================================================
    ...
    _create: function () {
        this.element.addClass("bwProjectClassField");
        ...
        try {
            ...
            if (this.options.inFormsEditor == true) {
                ...
                this.renderAndPopulateProjectClassField_ReadOnly(); // Need to render, not allowing the user to make modifications.
            } else if (this.options.allowRequestModifications == false) {
                this.renderAndPopulateProjectClassField_ReadOnly(); // Need to render, not allowing the user to make modifications.
            } else if (this.options.allowRequestModifications == true) {
                this.renderAndPopulateProjectClassField();
            } else {
                var html = '';
                html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwProjectClassField</span>';
                html += '<br />';
                html += '<span style="">Invalid value for allowRequestModifications: ' + this.options.allowRequestModifications + '</span>';
                this.element.html(html);
            }
            
            console.log('In bwProjectClassField._create(). The widget has been initialized.');

        } catch (e) {
            var html = '';
            html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwProjectClassField</span>';
            html += '<br />';
            html += '<span style="">Exception in bwProjectClassField.Create(): ' + e.message + ', ' + e.stack + '</span>';
    ...
    _destroy: function () {
        this.element
            .removeClass("bwProjectClassField")
            .text("");
    },
    getData: function () {
        try {
            console.log('In bwProjectClassField.getData().');
            //return 'This getData method is incomplete.';
            return null;
        } catch (e) {
            console.log('Exception in bwProjectClassField.getData(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwProjectClassField.getData(): ' + e.message + ', ' + e.stack);
        }
    },
    isARequiredField: function () {
        try {
            console.log('In bwProjectClassField.isARequiredField().');
            return this.options.renderAsARequiredField;
        } catch (e) {
            console.log('Exception in bwProjectClassField.isARequiredField(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwProjectClassField.isARequiredField(): ' + e.message + ', ' + e.stack);
        }
    },
    getfieldTitle: function () {
        try {
            console.log('In bwProjectClassField.getfieldTitle().');
            return this.options.fieldTitle;
        } catch (e) {
            console.log('Exception in bwProjectClassField.getfieldTitle(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwProjectClassField.getfieldTitle(): ' + e.message + ', ' + e.stack);
        }
    },
    renderAndPopulateProjectClassField: function (requestDialogId) {
        try {
            console.log('In renderAndPopulateProjectClassField().');
            var thiz = this;
            var html = '';
            ...
            html += '                        <tbody><tr class="xdTableOffsetRow xdTableOffsetRow-editor" id="toolboxdraggablerow_2" draggable="true" bwwidgetname="bwProjectClassField">';
            ...
            console.log('Exception in renderAndPopulateProjectClassField: ' + e.message + ', ' + e.stack);
            var html = '';
            html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwProjectClassField</span>';
            html += '<br />';
            html += '<span style="">Exception in bwProjectClassField.renderAndPopulateProjectClassField(): ' + e.message + ', ' + e.stack + '</span>';
            thiz.element.html(html);
        }
    },

    renderAndPopulateProjectClassField_ReadOnly: function (requestDialogId) {
        try {
            console.log('In renderAndPopulateProjectClassField_ReadOnly().');
            var thiz = this;
            var html = '';
            //html += '<div style="white-space:nowrap;">';
            //html += '   <input type="text" class="bwRequestJson" bwDataRequired="true" bwDataType="string" bwFieldname="ProjectTitle" id="strProjectTitle" onkeyup="$(\'.bwRequest\').bwRequest(\'editTextbox_OnKeyUp\', this);" style="WIDTH: 100%;font-family: \'Segoe UI Light\',\'Segoe UI\',\'Segoe\',Tahoma,Helvetica,Arial,sans-serif;color: #262626;font-size: 20pt;" /> <!-- was 2.77em -->';
            ...
            console.log('Exception in renderAndPopulateProjectClassField_ReadOnly: ' + e.message + ', ' + e.stack);
            var html = '';
            html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwProjectClassField</span>';
            html += '<br />';
            html += '<span style="">Exception in bwProjectClassField.renderAndPopulateProjectClassField_ReadOnly(): ' + e.message + ', ' + e.stack + '</span>';
            thiz.element.html(html);
        }
    }
