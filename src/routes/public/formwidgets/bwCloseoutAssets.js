$.widget("bw.bwCloseoutAssets", { 
    options: {
        ...
        */
        ...
        /*
        ===========================================================
        This is the bwCloseoutAssets.js jQuery Widget. 
        ===========================================================
    ...
    _create: function () {
        this.element.addClass("bwCloseoutAssets");
        ...
        try {
            ...
            if (this.options.jsonData && this.options.jsonData.bwCloseoutAssets && this.options.jsonData.bwCloseoutAssets.value && (this.options.jsonData.bwCloseoutAssets.value.length == 0)) {
                // No inventory items so just create the json envelope.
                this.options.jsonData['bwCloseoutAssets'] = {}; // Instantiate the data object. bwCloseoutAssets
            } else if (this.options.jsonData && this.options.jsonData.bwCloseoutAssets && this.options.jsonData.bwCloseoutAssets.value && this.options.jsonData.bwCloseoutAssets.value.length && (this.options.jsonData.bwCloseoutAssets.value.length > 0)) {
                // There inventory items, so put it in the correct place.
                this.options.InventoryItems = this.options.jsonData.bwCloseoutAssets.value;
            } else if (this.options.jsonData) { // 3-1-2022 // && (this.options.jsonData.length == 0)) {
                // No inventory items so just create the json envelope.
                this.options.jsonData['bwCloseoutAssets'] = {}; // Instantiate the data object. bwCloseoutAssets
            } else {
                displayAlertDialog('Error in bwCloseoutAssets.js._create(). Unexpected value for jsonData: ' + JSON.stringify(this.options.jsonData));
            }
            ...
            if (!div) {
                ...
                console.log('In bwCloseoutAssets.js._create(). Instantiating bwAdvancedProductSearch.js. xcx23142425')
                var elementId = 'divBwAdvancedProductSearch_For_bwCloseoutAssets_widgets';
                console.log('In bwCloseoutAssets.js._create(). Adding element to the dom: ' + elementId);
            ...
            console.log('In bwCloseoutAssets._create(). The dialog has been initialized.');

        } catch (e) {
            var html = '';
            html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwCloseoutAssets</span>';
            html += '<br />';
            html += '<span style="">Exception in bwCloseoutAssets.Create(): ' + e.message + ', ' + e.stack + '</span>';
    ...
    _destroy: function () {
        this.element
            .removeClass("bwCloseoutAssets")
            .text("");
    },
    getDataType: function () {
        try {
            console.log('In bwCloseoutAssets.js.getDataType().');

            return 'spreadsheet';
        } catch (e) {
            console.log('Exception in bwCloseoutAssets.js.getDataType(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwCloseoutAssets.js.getDataType(): ' + e.message + ', ' + e.stack);
        }
    },
    getData: function () {
        try {
            console.log('In bwCloseoutAssets.js.getData().');
            ...
            console.log('Exception in bwCloseoutAssets.getData(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwCloseoutAssets.getData(): ' + e.message + ', ' + e.stack);
        }
    },
    isARequiredField: function () {
        try {
            console.log('In bwCloseoutAssets.isARequiredField().');
            return this.options.renderAsARequiredField;
        } catch (e) {
            console.log('Exception in bwCloseoutAssets.isARequiredField(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwCloseoutAssets.isARequiredField(): ' + e.message + ', ' + e.stack);
        }
    },
    getfieldTitle: function () {
        try {
            console.log('In bwCloseoutAssets.getfieldTitle().');
            return this.options.fieldTitle;
        } catch (e) {
            console.log('Exception in bwCloseoutAssets.getfieldTitle(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwCloseoutAssets.getfieldTitle(): ' + e.message + ', ' + e.stack);
        }
    },
    ...
    renderInventoryItems: function (bwBudgetRequestId) {
        try {
            ...
            if (bwBudgetRequestId != this.options.bwBudgetRequestId) {
                ...
                console.log('In bwCloseoutAssets.renderInventoryItems(). bwBudgetRequestId: ' + bwBudgetRequestId);
                var thiz = this;
                ...
                for (var i = 0; i < thiz.options.InventoryItems.length; i++) {
                    ...
                    // Quantity.
                    html += '   <td style="padding:5px;" >';
                    html += '       <input class="InventoryItemQuantity" id="inputInventoryItemQuantity_' + thiz.options.InventoryItems[i].bwInventoryItemId + '" style="WIDTH: 40px;font-family: \'Segoe UI Light\',\'Segoe UI\',\'Segoe\',Tahoma,Helvetica,Arial,sans-serif;color: #262626;font-size: 18pt;TEXT-ALIGN: right;" onchange="$(\'.bwCloseoutAssets\').bwCloseoutAssets(\'calculateSelectedInventoryItems\', this, \'' + thiz.options.InventoryItems[i].bwInventoryItemId + '\');" type="text">'; // formatRequestedCapital_InitBudgetRequest
                    html += '   </td>';
                    ...
                    html += '  <img src="images/trash-can.png" title="Delete" style="cursor:pointer;" onclick="$(\'.bwCloseoutAssets\').bwCloseoutAssets(\'removeInventoryItemFromThisRequest\', \'' + thiz.options.InventoryItems[i].bwInventoryItemId + '\', \'' + bwBudgetRequestId + '\');" />';
                    html += '</td>';
            ...
            console.log('Exception in bwCloseoutAssets.renderInventoryItems(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwCloseoutAssets.renderInventoryItems(): ' + e.message + ', ' + e.stack);
        }
    },
    addInventoryItemToThisForm: function (inventoryItemJson, bwBudgetRequestId) {
        try {
            ...
            if (bwBudgetRequestId != this.options.bwBudgetRequestId) {
                ...
                console.log('In bwCloseoutAssets.addInventoryItemToThisForm(). bwBudgetRequestId: ' + bwBudgetRequestId + ', inventoryItemJson: ' + JSON.stringify(inventoryItemJson));
                //alert('In bwCloseoutAssets.addInventoryItemToThisForm(). inventoryItemJson: ' + JSON.stringify(inventoryItemJson));
                var thiz = this;
            ...
            console.log('Exception in bwCloseoutAssets.addInventoryItemToThisForm(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwCloseoutAssets.addInventoryItemToThisForm(): ' + e.message + ', ' + e.stack);
        }
    },
    removeInventoryItemFromThisRequest: function (bwInventoryItemId, bwBudgetRequestId) {
        try {
            ...
            if (bwBudgetRequestId != this.options.bwBudgetRequestId) {
                ...
                console.log('In bwCloseoutAssets.removeInventoryItemFromThisRequest(). bwBudgetRequestId: ' + bwBudgetRequestId + ', bwInventoryItemId: ' + bwInventoryItemId);
                var thiz = this;
            ...
            console.log('Exception in bwCloseoutAssets.removeInventoryItemFromThisRequest(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwCloseoutAssets.removeInventoryItemFromThisRequest(): ' + e.message + ', ' + e.stack);
        }
    },
    ...
    renderAndPopulateSelectedInventoryItemsGrid: function (requestDialogId) {
        try {
            ...
            //alert('In bwCloseoutAssets.js.renderAndPopulateSelectedInventoryItemsGrid(). bwBudgetRequestId: ' + this.options.bwBudgetRequestId);
            ...
            html += '                        <tbody><tr class="xdTableOffsetRow xdTableOffsetRow-editor" id="toolboxdraggablerow_2" draggable="true" bwwidgetname="bwCloseoutAssets" bwfieldname="bwCloseoutAssets">';
            ...
            //html += '                               <span style="font-size:20pt;cursor:zoom-in;" onclick="$(\'.bwCloseoutAssets\').bwCloseoutAssets(\'viewInLargeWindow\');"><img src="/images/zoom.jpg" style="width:20px;height:20px;" /></span>';
            html += '                                <span class="xdlabel">';
            html += '                                    Closeout Assets:';
            html += '                                </span>';
            ...
            html += '   <input type="button" style="cursor:pointer;font-size: 14pt;" value="View inventory..." onclick="$(\'.bwCloseoutAssets\').bwCloseoutAssets(\'displayProductAdvancedSearchModalDialog_ForInventoryFormWidget\', \'' + thiz.options.bwBudgetRequestId + '\');" />';
            html += '</div>';
            ...
            html += '                               <div style="font-size:20pt;cursor:zoom-in;" onclick="$(\'#' + this.options.jsonData.requestDialogId + '\').find(\'.bwCloseoutAssets\').bwCloseoutAssets(\'viewInLargeWindow\');"><img class="gridMagnifyingGlass" src="/images/zoom.jpg" style="width:50px;height:50px;" /></div>';
            //}
            html += '</td>';
            ...
            html += '<tr>';
            html += '   <td>';
            html += '       <span class="xdlabel" id="spanbwCloseoutAssets_Error" style="color:tomato;white-space:normal;" hidefocus="1"></span>'; // Errors and exceptions get displayed here.
            html += '   </td>';
            html += '</tr>';
            ...
            //    //var bwFieldName = 'bwCloseoutAssets';
            //    //var json = this.options.jsonData['bwCloseoutAssets'].value;
            ...
            //    //if (thiz.options.jsonData["bwCloseoutAssets"]) {
            //    //    // already have data
            //    //    //var value = this.options.jsonData['bwCostsGrid'].value;
            //    //    //dataElement.value = value;
            //    //    //debugger;
            //    //    if (thiz.options.jsonData["bwCloseoutAssets"].value) {

            //    //    } else {
            //    //        thiz.options.jsonData["bwCloseoutAssets"]["value"] = [];
            //    //    }
            //    //} else {
            //    //    // no data yet
            //    //    thiz.options.jsonData["bwCloseoutAssets"] = {
            //    //        value: []
            //    //    }
            //    //}
            ...
            html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwCloseoutAssets</span>';
            html += '<br />';
            html += '<span style="">Exception in bwCloseoutAssets.renderAndPopulateSelectedInventoryItemsGrid(): ' + e.message + ', ' + e.stack + '</span>';
            thiz.element.html(html);
        }
    },
    viewInLargeWindow: function () {
        try {
            ...
            var requestDialogId = 'divZoomedbwCloseoutAssetsDialog_' + thiz.options.bwBudgetRequestId;
            ...
            $('#' + requestDialogId).dialog({
                ...
                open: function () {
                    try {
                        ...
                        html += '       <span class="xdlabel" id="spanbwCloseoutAssets_Error" style="color:tomato;" hidefocus="1"></span>'; // Errors and exceptions get displayed here.
                        html += '   </td>';
                        html += '</tr>';
                        ...
                        $('#' + requestDialogParentId + '_Content').find("#jsGridSpend_large").jsGrid({
                            ...
                            data: thiz.options.jsonData["bwCloseoutAssets"].value, // json,
                            ...
                            onRefreshed: function (args) { // jsGridCosts_small
                                try {
                                    ...
                                    thiz.options.jsonData["bwCloseoutAssets"].value = this.data; // Store in the widget.
                                    ...
                                    console.log('Exception in bwCloseoutAssets.onRefreshed: ' + e.message + ', ' + e.stack);
                                    $('#spanbwCloseoutAssets_Error')[0].innerHTML = 'Exception in bwCloseoutAssets.onRefreshed: ' + e.message + ', ' + e.stack;
                                }
                            },
                            onItemInserted: function () {
                                try {
                                    ...
                                    thiz.options.jsonData["bwCloseoutAssets"].value = this.data; // Store in the widget.
                                    $(thiz.element).closest('#budgetrequestform').find("#jsGridSpend_small").jsGrid("option", "data", thiz.options.jsonData["bwCloseoutAssets"].value); // Update the smaller grid.
                                } catch (e) {
                        ...
                        console.log('Exception in bwCloseoutAssets.viewInLargeWindow().dialog.open(): ' + e.message + ', ' + e.stack);
                    }
                }
            });
            try {
            ...
            console.log('Exception in bwCloseoutAssets.viewInLargeWindow(): ' + e.message + ', ' + e.stack);
        }
    },
    ...
    renderFilterAndProductsSections_ForInventoryFormWidget: function (productSegment, productSubClass, highlightThisProductCode, bwBudgetRequestId) {
        try {
            ...
            for (var i = 0; i < products.length; i++) {
                ...
                html += '   <td>';
                //debugger;
                html += '<input type="button" value="Add" class="button-blue" onclick="$(\'.bwCloseoutAssets\').bwCloseoutAssets(\'btnAdd_OnClick\', \'' + products[i].ProductId + '\', \'' + bwBudgetRequestId + '\', \'' + sessionId + '\');" />'; // 
                ...
                for (var i = 0; i < data.length; i++) {
                    if (data[i].bwInventoryItemId == productId) {
                        ...
                        if (!product.ProductId) {
                            ...
                            //var formwidget = $(this.element).find('.bwCloseoutAssets')[0]; // Find the formwidget, then we will invoke the method to add the inventory item to the form json.

                            //var inventoryItemJson = {
                            //    ProductId
                            //}

                            $(this.element).bwCloseoutAssets('addInventoryItemToThisForm', product, bwBudgetRequestId);
