$.widget("bw.bwSelectQuoteItems", {
    options: {
        ...
        */
        ...
        /*
        ===========================================================
        This is the bwSelectQuoteItems.js jQuery Widget. 
        ===========================================================
        ...
        */
        ...
        fieldTitle: 'Select Quote Items', // This is the title, which shows up on the form in the left column.
    ...
    _create: function () {
        this.element.addClass("bwSelectQuoteItems");
        ...
        try {
            ...
            if (this.options.jsonData.length == 0) {
                this.options.jsonData['bwSelectQuoteItems'] = {}; // Instantiate the data object. bwSelectQuoteItems
            }

            this.renderAndPopulateSelectedQuoteItemsGrid();

            console.log('In bwSelectQuoteItems._create(). The dialog has been initialized.');

        } catch (e) {
            var html = '';
            html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwSelectQuoteItems</span>';
            html += '<br />';
            html += '<span style="">Exception in bwSelectQuoteItems.Create(): ' + e.message + ', ' + e.stack + '</span>';
    ...
    _destroy: function () {
        this.element
            .removeClass("bwSelectQuoteItems")
            .text("");
    },
    getDataType: function () {
        try {
            console.log('In bwSelectQuoteItems.js.getDataType().');

            return 'spreadsheet';
        } catch (e) {
            console.log('Exception in bwSelectQuoteItems.js.getDataType(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwSelectQuoteItems.js.getDataType(): ' + e.message + ', ' + e.stack);
        }
    },
    getData: function () {
        try {
            console.log('In bwSelectQuoteItems.js.getData().');
            ...
            console.log('Exception in bwSelectQuoteItems.getData(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwSelectQuoteItems.getData(): ' + e.message + ', ' + e.stack);
        }
    },
    isARequiredField: function () {
        try {
            console.log('In bwSelectQuoteItems.isARequiredField().');
            return this.options.renderAsARequiredField;
        } catch (e) {
            console.log('Exception in bwSelectQuoteItems.isARequiredField(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwSelectQuoteItems.isARequiredField(): ' + e.message + ', ' + e.stack);
        }
    },
    getfieldTitle: function () {
        try {
            console.log('In bwSelectQuoteItems.getfieldTitle().');
            return this.options.fieldTitle;
        } catch (e) {
            console.log('Exception in bwSelectQuoteItems.getfieldTitle(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwSelectQuoteItems.getfieldTitle(): ' + e.message + ', ' + e.stack);
        }
    },
    ...
    renderAndPopulateSelectedQuoteItemsGrid: function (requestDialogId) {
        try {
            console.log('In renderAndPopulateSelectedQuoteItemsGrid().');
            var thiz = this;
            var html = '';
            ...
            html += '                        <tbody><tr class="xdTableOffsetRow xdTableOffsetRow-editor" id="toolboxdraggablerow_2" draggable="true" bwwidgetname="bwSelectQuoteItems" bwfieldname="bwSelectQuoteItems">';
            ...
            //html += '                               <span style="font-size:20pt;cursor:zoom-in;" onclick="$(\'.bwSelectQuoteItems\').bwSelectQuoteItems(\'viewInLargeWindow\');"><img src="/images/zoom.jpg" style="width:20px;height:20px;" /></span>';
            html += '                                <span class="xdlabel">';
            html += '                                    Select Quote Items:';
            html += '                                </span>';
            ...
            if (this.options.inFormsEditor != true) {
                ...
                html += '                               <span style="font-size:20pt;cursor:zoom-in;" onclick="$(\'#' + this.options.jsonData.requestDialogId + '\').find(\'.bwSelectQuoteItems\').bwSelectQuoteItems(\'viewInLargeWindow\');"><img class="gridMagnifyingGlass" src="/images/zoom.jpg" style="width:50px;height:50px;" /></span>';
            }
            html += '</td></tr>';

            html += '<tr>';
            html += '   <td>';
            html += '       <span class="xdlabel" id="spanbwSelectQuoteItems_Error" style="color:tomato;white-space:normal;" hidefocus="1"></span>'; // Errors and exceptions get displayed here.
            html += '   </td>';
            html += '</tr>';
            ...
            if ((!startDate || !endDate) || (startDate == null || endDate == null)) {
                // do nothing.
                var html = '';
                html += '<span style="color:tomato;font-size:12pt;">Select a new quote itemxcx1 <input type="button" style="cursor:pointer;" value="Search" onclick="$(\'.bwAdvancedProductSearch\').bwAdvancedProductSearch(\'displayProductAdvancedSearchModalDialog\', \'BW1\');" /></select>';
                ...
                //var bwFieldName = 'bwSelectQuoteItems';
                //var json = this.options.jsonData['bwSelectQuoteItems'].value;
                ...
                if (thiz.options.jsonData["bwSelectQuoteItems"]) {
                    // already have data
                    //var value = this.options.jsonData['bwCostsGrid'].value;
                    //dataElement.value = value;
                    //debugger;
                    if (thiz.options.jsonData["bwSelectQuoteItems"].value) {

                    } else {
                        thiz.options.jsonData["bwSelectQuoteItems"]["value"] = [];
                    }
                } else {
                    // no data yet
                    thiz.options.jsonData["bwSelectQuoteItems"] = {
                        value: []
                    }
                }
                ...
                $(this.element).find("#jsGridSpend_small").jsGrid({
                    ...
                    data: thiz.options.jsonData["bwSelectQuoteItems"].value, // json,
                    noDataContent: html, //"Click the \"Plus\" (+) sign to add a Spend item...",

                    onInit: function () {
                        try {
                            console.log('In bwSelectQuoteItems.onInit-small().');
                            ...
                            console.log('Exception in bwSelectQuoteItems.onInit: ' + e.message + ', ' + e.stack);
                        }
                    },
                    ...
                    onRefreshed: function (args) { // jsGridCosts_small
                        try {
                            if (thiz.options.inFormsEditor != true) {
                                ...
                                thiz.options.jsonData["bwSelectQuoteItems"].value = this.data; // Store in the widget.
                            ...
                            console.log('Exception in bwSelectQuoteItems.onRefreshed: ' + e.message + ', ' + e.stack);
                            $('#spanbwSelectQuoteItems_Error')[0].innerHTML = 'Exception in bwSelectQuoteItems.onRefreshed: ' + e.message + ', ' + e.stack;
                        }
                    },
                    ...
                    onItemInserted: function () {
                        try {
                            ...
                            var bwBudgetRequestId = $(thiz.element).closest('#budgetrequestform')[0].getAttribute('bwbudgetrequestid');
                            var requestDialogId = 'divZoomedbwSelectQuoteItemsDialog_' + bwBudgetRequestId;
                            if ($('#' + requestDialogId).find('#jsGridSpend_large')) { // If the large one is displayed, update it!
                                $('#' + requestDialogId).find("#jsGridSpend_large").jsGrid("option", "data", thiz.options.jsonData["bwSelectQuoteItems"].value); // Update the larger grid.
                            }

                        } catch (e) {
                            console.log('Exception in bwSelectQuoteItems.onItemInserted: ' + e.message + ', ' + e.stack);
                            $('#spanbwSelectQuoteItems_Error')[0].innerHTML = 'Exception in bwSelectQuoteItems.onItemInserted: ' + e.message + ', ' + e.stack;
                        }
                    },
                    onItemUpdated: function () {
                        try {
                            ...
                            console.log('Exception in bwSelectQuoteItems.onItemInserted: ' + e.message + ', ' + e.stack);
                            $('#spanbwSelectQuoteItems_Error')[0].innerHTML = 'Exception in bwSelectQuoteItems.onItemUpdated: ' + e.message + ', ' + e.stack;
                        }
                    },
                    onItemDeleted: function () {
                        try {
                            ...
                            console.log('Exception in bwSelectQuoteItems.onItemInserted: ' + e.message + ', ' + e.stack);
                            $('#spanbwSelectQuoteItems_Error')[0].innerHTML = 'Exception in bwSelectQuoteItems.onItemDeleted: ' + e.message + ', ' + e.stack;
                        }
                    },
            ...
            console.log('Exception in renderAndPopulateSelectedQuoteItemsGrid: ' + e.message + ', ' + e.stack);
            var html = '';
            html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwSelectQuoteItems</span>';
            html += '<br />';
            html += '<span style="">Exception in bwSelectQuoteItems.renderAndPopulateSelectedQuoteItemsGrid(): ' + e.message + ', ' + e.stack + '</span>';
            thiz.element.html(html);
        }
    },
    ...
    viewInLargeWindow: function () {
        try {
            ...
            $('#' + requestDialogId).dialog({
                ...
                open: function () {
                    try {
                        ...
                        //html += '                               [._BwRequestType]';
                        //html += '                           </span>';
                        html += '<span style="font-family: \'Segoe UI Light\',\'Segoe UI\',\'Segoe\',Tahoma,Helvetica,Arial,sans-serif;color: #3f3f3f;font-size: 40pt;font-weight:bold;">Select Quote Items</span>';
                        html += '                           <br />';
                        ...
                        html += '       <span class="xdlabel" id="spanbwSelectQuoteItems_Error" style="color:tomato;" hidefocus="1"></span>'; // Errors and exceptions get displayed here.
                        html += '   </td>';
                        html += '</tr>';
                            ...
                            $('#' + requestDialogParentId + '_Content').find("#jsGridSpend_large").jsGrid({
                                ...
                                onItemInserted: function () {
                                    try {
                                        //console.log('In bwCostsGrid.onItemInserted().');
                                        thiz.options.jsonData["bwSelectQuoteItems"].value = this.data; // Store in the widget.
                                        $(thiz.element).closest('#budgetrequestform').find("#jsGridSpend_small").jsGrid("option", "data", thiz.options.jsonData["bwSelectQuoteItems"].value); // Update the smaller grid.
                                    } catch (e) {
                        ...
                        console.log('Exception in bwSelectQuoteItems.viewInLargeWindow().dialog.open(): ' + e.message + ', ' + e.stack);
                    }
                }
            });
            try {
            ...
            console.log('Exception in bwSelectQuoteItems.viewInLargeWindow(): ' + e.message + ', ' + e.stack);
        }
    },
    ...
    displayAlertDialog: function (errorMessage) {
        try {
            ...
            console.log('Exception in bwSelectQuoteItems.displayAlertDialog(): ' + e.message + ', ' + e.stack);
        }
    }
