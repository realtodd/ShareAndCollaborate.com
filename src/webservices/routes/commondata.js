...
module.exports = function () {
    ...
    var bwExceptionLogSchema = new mongoose.Schema({
        ...
        ClientException: Boolean, // This is true when it is a client side error. It is not necessary to populate this when a server side error. Added 6-5-2024. 
                                                                ...
                                                                for (var j = 0; j < brResult.length; j++) { // doesn't need wrap.
                                                                    if (brResult[j]._doc.bwBudgetRequestId == taskResult[i]._doc.bwRelatedItemId) {
                                                                        var task = {
                                                                            ...
                                                                            bwRequestJson: brResult[j]._doc.bwRequestJson,
    ...
    bwtasksoutstanding_GETCOUNTS = function (bwWorkflowAppId, bwParticipantId) {
        return new Promise(function (resolve, reject) {
            try {
                ...
                BwWorkflowTask.countDocuments({ bwWorkflowAppId: bwWorkflowAppId, bwPercentComplete: 0, bwAssignedToId: bwParticipantId, TrashBin: { $ne: true } }, function (wtcError, wtcResult_MY_PENDING_TASKS) { // bwRelatedItemId
                    try {
                        if (wtcError) {

                            var msg = 'Error in commondata.js.bwtasksoutstanding_GETCOUNTS(). wtcError: ' + wtcError;
                            console.log(msg);
                            var result = {
                                status: 'ERROR',
                                message: msg,
                            ...
                            BwBudgetRequest.countDocuments({ bwWorkflowAppId: bwWorkflowAppId, CreatedById: bwParticipantId, BudgetWorkflowStatus: 'NOT_SUBMITTED', TrashBin: { $ne: true } }, function (brcError1, brcResult_MY_UNSUBMITTED_REQUESTS) {
                                try {
                                    if (brcError1) {

                                        var msg = 'Error in commondata.js.bwtasksoutstanding_GETCOUNTS(). brcError1: ' + brcError1;
                                        console.log(msg);
                                        var result = {
                                            status: 'ERROR',
                                            message: msg,
                                        ...
                                        BwBudgetRequest.countDocuments({ bwWorkflowAppId: bwWorkflowAppId, CreatedById: bwParticipantId, BudgetWorkflowStatus: { $ne: 'NOT_SUBMITTED' }, TrashBin: { $ne: true } }, function (brcError2, brcResult_MY_SUBMITTED_REQUESTS) {
                                            try {
                                                if (brcError2) {

                                                    var msg = 'Error in commondata.js.bwtasksoutstanding_GETCOUNTS(). brcError2: ' + brcError2;
                                                    console.log(msg);
                                                    var result = {
                                                        status: 'ERROR',
                                                        message: msg,
                                                    ...
                                                    BwBudgetRequest.countDocuments({ bwWorkflowAppId: bwWorkflowAppId, TrashBin: { $ne: true } }, function (brcError2, brcResult_ACTIVE_REQUESTS) {
                                                        try {
                                                            ...
                                                            if (brcError2) {
                                                                ...
                                                                BwBudgetRequest.countDocuments({ bwWorkflowAppId: bwWorkflowAppId, TrashBin: { $ne: true } }, function (brcError2, brcResult_SOCIAL_NETWORK) {
                                                                    try {
                                                                        ...
                                                                        if (brcError2) {
                                                                            ...
                                                                            var result = {
                                                                                ...
                                                                                SOCIAL_NETWORK: brcResult_SOCIAL_NETWORK
                                                                            }
                                                                            resolve(result);
                                                                        ...
                                                                        var msg = 'Exception in commondata.js.bwtasksoutstanding_GETCOUNTS(). bwWorkflowAppId: ' + bwWorkflowAppId + ':3: ' + e.message + ', ' + e.stack;
                                                                        var threatLevel = 'severe'; // severe, high, elevated, guarded, low.
                                                                        var source = 'commondata.js.bwtasksoutstanding_GETCOUNTS()';
                                                                        var errorCode = null;
                                    ...
                                    var msg = 'Exception in commondata.js.bwtasksoutstanding_GETCOUNTS(). bwWorkflowAppId: ' + bwWorkflowAppId + ':2: ' + e.message + ', ' + e.stack;
                                    var threatLevel = 'severe'; // severe, high, elevated, guarded, low.
                                    var source = 'commondata.js.bwtasksoutstanding_GETCOUNTS()';
                                    var errorCode = null;
                                    WriteToErrorLog(threatLevel, source, errorCode, msg);
                        ...
                        var msg = 'Exception in commondata.js.bwtasksoutstanding_GETCOUNTS(). bwWorkflowAppId: ' + bwWorkflowAppId + ':1: ' + e.message + ', ' + e.stack;
                        var threatLevel = 'severe'; // severe, high, elevated, guarded, low.
                        var source = 'commondata.js.bwtasksoutstanding_GETCOUNTS()';
                        var errorCode = null;
                        WriteToErrorLog(threatLevel, source, errorCode, msg);
    ...
    getPagedDataFor_PINNED_REQUESTS = function (bwWorkflowAppId, bwParticipantId, pageNumber, pageSize, bwBudgetRequestId_Array) { // bwBudgetRequestId_Array is used for Pinned Requests.
        return new Promise(function (resolve, reject) {
            try {
                //
                // This file exists on both the web and file services servers. Be careful not to lose the latest version!!!
                ...
                console.log('--------------');
                console.log('=============================================');
                console.log('In commondata.getPagedDataFor_PINNED_REQUESTS(). bwWorkflowAppId: ' + bwWorkflowAppId + ', pageSize: ' + pageSize + ', bwBudgetRequestId_Array: ' + bwBudgetRequestId_Array);
                console.log('=============================================');
                console.log('--------------');
                console.log('');

                //var msg = 'THIS IS A TEST 2';
                //var result = {
                //    status: 'SUCCESS',
                //    message: msg
                //}
                ...
                BwBudgetRequest.find({ bwWorkflowAppId: bwWorkflowAppId, bwBudgetRequestId: { $in: bwBudgetRequestId_Array }, TrashBin: { $ne: true } }, null, { sort: { Created: 'descending' }, limit: pageSize }, function (brError, brResult) {
                    try {
                        if (brError) {

                            var msg = 'Error in commondata.js.getPagedDataFor_PINNED_REQUESTS(). brError: ' + brError;
                            var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                            var source = 'commondata.js.getPagedDataFor_PINNED_REQUESTS()';
                            var errorCode = null;
                            WriteToErrorLog(threatLevel, source, errorCode, msg);
                        ...
                        var msg = 'Exception in commondata.js.getPagedDataFor_PINNED_REQUESTS():2: ' + e.message + ', ' + e.stack;
                        var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                        var source = 'commondata.js.getPagedDataFor_PINNED_REQUESTS()';
                        var errorCode = null;
                        WriteToErrorLog(threatLevel, source, errorCode, msg);
                ...
                var msg = 'Exception in commondata.js.getPagedDataFor_PINNED_REQUESTS():1: ' + e.message + ', ' + e.stack;
                var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                var source = 'commondata.js.getPagedDataFor_PINNED_REQUESTS()';
                var errorCode = null;
                WriteToErrorLog(threatLevel, source, errorCode, msg);
    ...
    getPagedDataFor_MY_UNSUBMITTED_REQUESTS = function (bwWorkflowAppId, bwParticipantId, pageNumber, pageSize) {
        return new Promise(function (resolve, reject) {
            try {
                ...
                console.log('--------------');
                console.log('=============================================');
                console.log('In commondata.getPagedDataFor_MY_UNSUBMITTED_REQUESTS(). bwWorkflowAppId: ' + bwWorkflowAppId + ', pageSize: ' + pageSize);
                console.log('=============================================');
                console.log('--------------');
                console.log('');
                ...
                BwBudgetRequest.find({ bwWorkflowAppId: bwWorkflowAppId, CreatedById: bwParticipantId, BudgetWorkflowStatus: 'NOT_SUBMITTED', TrashBin: { $ne: true } }, null, { sort: { Created: 'descending' }, limit: pageSize }, function (brError, brResult) {
                    try {

                        if (brError) {

                            var msg = 'Error in commondata.js.getPagedDataFor_MY_UNSUBMITTED_REQUESTS(). brError: ' + brError;
                            var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                            var source = 'commondata.js.getPagedDataFor_MY_UNSUBMITTED_REQUESTS()';
                            var errorCode = null;
                            WriteToErrorLog(threatLevel, source, errorCode, msg);
                        ...
                        var msg = 'Exception in commondata.js.getPagedDataFor_MY_UNSUBMITTED_REQUESTS():2: ' + e.message + ', ' + e.stack;
                        var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                        var source = 'commondata.js.getPagedDataFor_MY_UNSUBMITTED_REQUESTS()';
                        var errorCode = null;
                        WriteToErrorLog(threatLevel, source, errorCode, msg);
                ...
                var msg = 'Exception in commondata.js.getPagedDataFor_MY_UNSUBMITTED_REQUESTS():1: ' + e.message + ', ' + e.stack;
                var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                var source = 'commondata.js.getPagedDataFor_MY_UNSUBMITTED_REQUESTS()';
                var errorCode = null;
                WriteToErrorLog(threatLevel, source, errorCode, msg);
    ...
    getPagedDataFor_MY_SUBMITTED_REQUESTS = function (bwWorkflowAppId, bwParticipantId, pageNumber, pageSize) {
        return new Promise(function (resolve, reject) {
            try {
                ...
                console.log('--------------');
                console.log('=============================================');
                console.log('In commondata.getPagedDataFor_MY_SUBMITTED_REQUESTS(). bwWorkflowAppId: ' + bwWorkflowAppId + ', pageSize: ' + pageSize);
                console.log('=============================================');
                console.log('--------------');
                console.log('');
                ...
                BwBudgetRequest.find({ bwWorkflowAppId: bwWorkflowAppId, CreatedById: bwParticipantId, BudgetWorkflowStatus: { $ne: 'NOT_SUBMITTED' }, TrashBin: { $ne: true } }, null, { sort: { Created: 'descending' }, limit: pageSize }, function (brError, brResult) {
                    try {

                        if (brError) {

                            var msg = 'Error in commondata.js.getPagedDataFor_MY_SUBMITTED_REQUESTS(). brError: ' + brError;
                            var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                            var source = 'commondata.js.getPagedDataFor_MY_SUBMITTED_REQUESTS()';
                            var errorCode = null;
                            WriteToErrorLog(threatLevel, source, errorCode, msg);
                        ...
                        var msg = 'Exception in commondata.js.getPagedDataFor_MY_SUBMITTED_REQUESTS():2: ' + e.message + ', ' + e.stack;
                        var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                        var source = 'commondata.js.getPagedDataFor_MY_SUBMITTED_REQUESTS()';
                        var errorCode = null;
                        WriteToErrorLog(threatLevel, source, errorCode, msg);
                ...
                var msg = 'Exception in commondata.js.getPagedDataFor_MY_SUBMITTED_REQUESTS():1: ' + e.message + ', ' + e.stack;
                var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                var source = 'commondata.js.getPagedDataFor_MY_SUBMITTED_REQUESTS()';
                var errorCode = null;
                WriteToErrorLog(threatLevel, source, errorCode, msg);
    ...
    getPagedDataFor_ACTIVE_REQUESTS = function (bwWorkflowAppId, bwParticipantId, displaySupplementals, pageNumber, pageSize) {
        return new Promise(function (resolve, reject) {
            try {
                ...
                console.log('--------------');
                console.log('=============================================');
                console.log('In commondata.getPagedDataFor_ACTIVE_REQUESTS(). bwWorkflowAppId: ' + bwWorkflowAppId + ', pageSize: ' + pageSize);
                console.log('=============================================');
                console.log('--------------');
                console.log('');
                ...
                if (displaySupplementals == true) {
                    ...
                    BwBudgetRequest.find({ bwWorkflowAppId: bwWorkflowAppId, CreatedById: bwParticipantId, BudgetWorkflowStatus: { $ne: 'NOT_SUBMITTED' }, TrashBin: { $ne: true } }, null, { sort: { Created: 'descending' }, limit: pageSize }, function (brError, brResult) {
                        try {
                            if (brError) {
                                ...
                                var msg = 'Error in commondata.js.getPagedDataFor_ACTIVE_REQUESTS(). brError: ' + brError;
                                var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                                var source = 'commondata.js.getPagedDataFor_ACTIVE_REQUESTS()';
                                var errorCode = null;
                                WriteToErrorLog(threatLevel, source, errorCode, msg);
                            ...
                            var msg = 'Exception in commondata.js.getPagedDataFor_ACTIVE_REQUESTS():2: ' + e.message + ', ' + e.stack;
                            var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                            var source = 'commondata.js.getPagedDataFor_ACTIVE_REQUESTS()';
                            var errorCode = null;
                            WriteToErrorLog(threatLevel, source, errorCode, msg);
                    ...
                    BwBudgetRequest.find({ bwWorkflowAppId: bwWorkflowAppId, CreatedById: bwParticipantId, IsSupplementalRequest: { $ne: true }, BudgetWorkflowStatus: { $ne: 'NOT_SUBMITTED' }, TrashBin: { $ne: true } }, null, { sort: { Created: 'descending' }, limit: pageSize }, function (brError, brResult) {
                        try {
                            if (brError) {
                ...
                var msg = 'Exception in commondata.js.getPagedDataFor_ACTIVE_REQUESTS():1: ' + e.message + ', ' + e.stack;
                var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                var source = 'commondata.js.getPagedDataFor_ACTIVE_REQUESTS()';
                var errorCode = null;
                WriteToErrorLog(threatLevel, source, errorCode, msg);
    ...
    getPagedDataFor_MY_PENDING_TASKS = function (bwWorkflowAppId, bwParticipantId, pageNumber, pageSize) {
        return new Promise(function (resolve, reject) {
            try {
                //
                // This file exists on both the web and file services servers. Be careful not to lose the latest version!!!
                ...
                console.log('--------------');
                console.log('=============================================');
                console.log('In commondata.getPagedDataFor_MY_PENDING_TASKS(). bwWorkflowAppId: ' + bwWorkflowAppId);
                console.log('=============================================');
                console.log('--------------');
                console.log('');
                ...
                BwWorkflowTask.find({ bwWorkflowAppId: bwWorkflowAppId, bwPercentComplete: 0, bwAssignedToId: bwParticipantId, TrashBin: { $ne: true } }, null, { sort: { Created: 'descending' }, limit: pageSize }, function (wtError, wtResult) {
                    try {

                        if (wtError) {

                            var msg = 'Error in commondata.js.getPagedDataFor_MY_PENDING_TASKS(). wtError: ' + wtError;
                            var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                            var source = 'commondata.js.getPagedDataFor_MY_PENDING_TASKS()';
                            var errorCode = null;
                            WriteToErrorLog(threatLevel, source, errorCode, msg);
                            ...
                            var findBudgetRequestsQuery = [];
                            for (var i = 0; i < wtResult.length; i++) {
                                if (wtResult[i].TaskType == 'BUDGET_REQUEST_WORKFLOW_TASK') {
                                    findBudgetRequestsQuery.push(wtResult[i].bwRelatedItemId);
                                }
                            }

                            BwBudgetRequest.find({ bwBudgetRequestId: { $in: findBudgetRequestsQuery }, TrashBin: { $ne: true } }, function (brError, brResult) {
                                try {

                                    if (brError) {

                                        var msg = 'Error in commondata.js.getPagedDataFor_MY_PENDING_TASKS(). brError: ' + brError;
                                        var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                                        var source = 'commondata.js.getPagedDataFor_MY_PENDING_TASKS()';
                                        var errorCode = null;
                                        WriteToErrorLog(threatLevel, source, errorCode, msg);
                                    ...
                                    var msg = 'Exception in commondata.js.getPagedDataFor_MY_PENDING_TASKS():3: ' + e.message + ', ' + e.stack;
                                    var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                                    var source = 'commondata.js.getPagedDataFor_MY_PENDING_TASKS()';
                                    var errorCode = null;
                                    WriteToErrorLog(threatLevel, source, errorCode, msg);
                        ...
                        var msg = 'Exception in commondata.js.getPagedDataFor_MY_PENDING_TASKS():2: ' + e.message + ', ' + e.stack;
                        var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                        var source = 'commondata.js.getPagedDataFor_MY_PENDING_TASKS()';
                        var errorCode = null;
                        WriteToErrorLog(threatLevel, source, errorCode, msg);
                ...
                var msg = 'Error in commondata.js.getPagedDataFor_PINNED_REQUESTS():1: ' + e.message + ', ' + e.stack;
                var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                var source = 'commondata.js.getPagedDataFor_PINNED_REQUESTS()';
                var errorCode = null;
                WriteToErrorLog(threatLevel, source, errorCode, msg);
    ...
    getPagedDataFor_SOCIAL_NETWORK = function (bwWorkflowAppId, bwParticipantId, pageNumber, pageSize) {
        return new Promise(function (resolve, reject) {
            try {
                ...
                console.log('--------------');
                console.log('=============================================');
                console.log('In commondata.getPagedDataFor_SOCIAL_NETWORK(). bwWorkflowAppId: ' + bwWorkflowAppId + ', pageSize: ' + pageSize);
                console.log('=============================================');
                console.log('--------------');
                console.log('');
                ...
                //BwBudgetRequest.find({ bwWorkflowAppId: bwWorkflowAppId, CreatedById: bwParticipantId, BudgetWorkflowStatus: { $ne: 'NOT_SUBMITTED' }, TrashBin: { $ne: true } }, null, { sort: { Created: 'descending' }, limit: pageSize }, function (brError, brResult) {
                BwBudgetRequest.find({ bwWorkflowAppId: bwWorkflowAppId, BudgetWorkflowStatus: { $ne: 'NOT_SUBMITTED' }, TrashBin: { $ne: true } }, null, { sort: { Created: 'descending' }, limit: pageSize }, function (brError, brResult) {
                    try {

                        if (brError) {

                            var msg = 'Error in commondata.js.getPagedDataFor_SOCIAL_NETWORK(): ' + brError;
                            var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                            var source = 'commondata.js.getPagedDataFor_SOCIAL_NETWORK()';
                            var errorCode = null;
                            WriteToErrorLog(threatLevel, source, errorCode, msg);
                        ...
                        var msg = 'Exception in commondata.js.getPagedDataFor_SOCIAL_NETWORK():2: ' + e.message + ', ' + e.stack;
                        var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                        var source = 'commondata.js.getPagedDataFor_SOCIAL_NETWORK()';
                        var errorCode = null;
                        WriteToErrorLog(threatLevel, source, errorCode, msg);
                ...
                var msg = 'Exception in commondata.js.getPagedDataFor_SOCIAL_NETWORK():1: ' + e.message + ', ' + e.stack;
                var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                var source = 'commondata.js.getPagedDataFor_SOCIAL_NETWORK()';
                var errorCode = null;
                WriteToErrorLog(threatLevel, source, errorCode, msg);
    ...
    bwtasksoutstanding_FASTSECONDPASS = function (bwWorkflowAppId, bwParticipantId, bwParticipantEmail) {
        return new Promise(function (resolve, reject) {
            try {
                //
                // This file exists on both the web and file services servers. Be careful not to lose the latest version!!!
                ...
                console.log('--------------');
                console.log('=============================================');
                console.log('In commondata.bwtasksoutstanding_FASTSECONDPASS(). bwWorkflowAppId: ' + bwWorkflowAppId);
                console.log('=============================================');
                console.log('--------------');
                console.log('');
                ...
                //console.log('In POST1 bwtasksoutstanding_FASTSECONDPASS(). bwWorkflowAppId: ' + bwWorkflowAppId + ' bwParticipantId: ' + bwParticipantId);
                //console.log('REQUEST: ')
                //console.log(JSON.stringify(request.body))
                ...
                //    bwWorkflowAppId: bwWorkflowAppId, bwPercentComplete: 0, bwAssignedToId: bwParticipantId, TrashBin: { $ne: true }
                //}, function (error, taskResult) {
                BwWorkflowTask.find({ bwWorkflowAppId: bwWorkflowAppId, bwPercentComplete: 0, bwAssignedToId: bwParticipantId, TrashBin: { $ne: true } }, null, { sort: { Created: 'descending' } }, function (error, taskResult) {
                    try {
                        if (error) {
                            var msg = 'Error in /bwtasksoutstanding_FASTSECONDPASS:1: ' + error;
                            // Exception section:
                            var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                            var source = 'start.js.DELETE.bwtasksoutstanding_FASTFIRSTPASS()';
                            var errorCode = null;
                            var message = msg;
                            WriteToErrorLog(threatLevel, source, errorCode, message);
                            //
                            ...
                            BwBudgetRequest.find({
                                ...
                                if (error2) {
                                    var msg = 'Error in /bwtasksoutstanding_FASTSECONDPASS:2: ' + error2;
                                    // Exception section:
                                    var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                                    var source = 'start.js.bwtasksoutstanding_FASTSECONDPASS()';
                                    var errorCode = null;
                                    var message = msg;
                                    WriteToErrorLog(threatLevel, source, errorCode, message);
                                    //
                                    ...
                                    //}, function (refError, refResult) {
                                    //    if (refError) {
                                    //        var msg = 'Error in /bwtasksoutstanding_FASTSECONDPASS:2-1: ' + refError;
                                    //        // Exception section:
                                    //        var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                                    //        var source = 'start.js.DELETE.bwtasksoutstanding_FASTSECONDPASS()';
                                    //        var errorCode = null;
                                    //        var message = msg;
                                    //        WriteToErrorLog(threatLevel, source, errorCode, message);
                                    //        //
                        ...
                        console.log('Exception in post bwtasksoutstanding_FASTSECONDPASS(): GGGGGGGGGGG: ' + e.message + ', ' + e.stack);
                        reject();
                    }
                });
                //        }
                //    } catch (e) {
                //        var msg = 'Error in /bwtasksoutstanding_FASTSECONDPASS:xcx43557: bwWorkflowAppId: ' + bwWorkflowAppId + ':' + e.message;
                //        // Exception section:
                //        var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                //        var source = 'start.js.DELETE.bwtasksoutstanding_FASTSECONDPASS()';
                //        var errorCode = null;
                ...
                var msg = 'Error in /bwtasksoutstanding_FASTSECONDPASS:3: ' + bwParticipantId + ':' + e.message;
                // Exception section:
                var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                var source = 'start.js.DELETE.bwtasksoutstanding_FASTSECONDPASS()';
                var errorCode = null;
    ...
    bwbudgetrequestspending_FASTSECONDPASS = function (bwWorkflowAppId) {
        return new Promise(function (resolve, reject) {
            try {
                console.log('In commondata.js.bwbudgetrequestspending_FASTSECONDPASS().');
                ...
                //        $nin: ['Completed', 'Approved', 'Rejected', 'Active']
                //    }, BudgetWorkflowStatus: { $nin: ['NOT_SUBMITTED'] }, TrashBin: { $ne: true } // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                //}, null, { sort: { Created: 'descending' }, limit: 5 }, function (br1Error, br1Result) {
                ...
                BwBudgetRequest.find({
                    ...
                    try {
                        if (!br1Error) {
                            ...
                            var msg = 'Exception in bwbudgetrequestspending_FASTSECONDPASS():2: ' + br1Error;
                            // Exception section:
                            var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                            var source = 'Exception in bwbudgetrequestspending_FASTSECONDPASS()()';
                            var errorCode = null;
                            var message = msg;
                            WriteToErrorLog(threatLevel, source, errorCode, message);
                            //
                        ...
                        var msg = 'Exception in bwbudgetrequestspending_FASTSECONDPASS():3: ' + e.message;
                        // Exception section:
                        var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                        var source = 'bwbudgetrequestspending_FASTSECONDPASS()';
                        var errorCode = null;
                        var message = msg;
                        WriteToErrorLog(threatLevel, source, errorCode, message);
                        //
    ...
    dataForHomePageNotificationScreen_FASTSECONDPASS = function (bwWorkflowAppId, bwParticipantId, bwParticipantEmail) {
        return new Promise(function (resolve, reject) {
            try {
                //
                // This file exists on both the web and file services servers. Be careful not to lose the latest version!!!
                ...
                console.log('--------------');
                console.log('=============================================');
                console.log('In commondata.dataForHomePageNotificationScreen_FASTSECONDPASS(). bwWorkflowAppId: ' + bwWorkflowAppId + ', bwParticipantId: ' + bwParticipantId);
                console.log('=============================================');
                console.log('--------------');
                console.log('');
                ...
                BwWorkflowTask.find({ bwWorkflowAppId: bwWorkflowAppId, bwPercentComplete: 0, bwAssignedToId: bwParticipantId, TrashBin: { $ne: true } }, null, { sort: { Created: 'descending' } }, function (error, taskResult) {
                    try {
                        if (error) {
                            ...
                            var msg = 'Error in commondata.js.dataForHomePageNotificationScreen_FASTSECONDPASS(). error: ' + error;
                            var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                            var source = 'commondata.js.dataForHomePageNotificationScreen_FASTSECONDPASS()';
                            var errorCode = null;
                            WriteToErrorLog(threatLevel, source, errorCode, msg);
                            ...
                            BwBudgetRequest.find({ bwBudgetRequestId: { $in: findBudgetRequestsQuery }, TrashBin: { $ne: true } }, function (error2, brResult) {
                                try {

                                    if (error2) {

                                        var msg = 'Error in commondata.js.dataForHomePageNotificationScreen_FASTSECONDPASS(). error2: ' + error2;
                                        var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                                        var source = 'commondata.js.dataForHomePageNotificationScreen_FASTSECONDPASS()';
                                        var errorCode = null;
                                        WriteToErrorLog(threatLevel, source, errorCode, msg);
                                        ...
                                        }, function (brError2, brResult2) {
                                            try {
                                                if (brError2) {

                                                    var msg = 'Error in commondata.js.dataForHomePageNotificationScreen_FASTSECONDPASS(). brError2: ' + brError2;
                                                    var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                                                    var source = 'commondata.js.dataForHomePageNotificationScreen_FASTSECONDPASS()';
                                                    var errorCode = null;
                                                    WriteToErrorLog(threatLevel, source, errorCode, msg);
                                                    ...
                                                    BwBudgetRequest.find({ bwWorkflowAppId: bwWorkflowAppId, CreatedById: bwParticipantId, TrashBin: { $ne: true } }, function (brError3, brResult3) {
                                                        try {

                                                            if (brError3) {

                                                                var msg = 'Error in commondata.js.dataForHomePageNotificationScreen_FASTSECONDPASS(). brError3: ' + brError3;
                                                                var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                                                                var source = 'commondata.js.dataForHomePageNotificationScreen_FASTSECONDPASS()';
                                                                var errorCode = null;
                                                                WriteToErrorLog(threatLevel, source, errorCode, msg);
                                                                ...
                                                                var result = {
                                                                    ...
                                                                    taskResult: taskResult,
                                                                    brResult: brResult,
                                                                    PendingBudgetRequests: brResult2,
                                                                    MyRequests: brResult3
                                                                }
                                                                resolve(result);
                                                            ...
                                                            var msg = 'Exception in commondata.js.dataForHomePageNotificationScreen_FASTSECONDPASS(): ' + e.message + ', ' + e.stack;
                                                            var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                                                            var source = 'commondata.js.dataForHomePageNotificationScreen_FASTSECONDPASS()';
                                                            var errorCode = null;
                                                            WriteToErrorLog(threatLevel, source, errorCode, msg);
                ...
                var msg = 'Exception in commondata.js.dataForHomePageNotificationScreen_FASTSECONDPASS(): bwParticipantId: ' + bwParticipantId + ' :: ' + e.message + ', ' + e.stack;
                var threatLevel = 'high'; // severe, high, elevated, guarded, low.
                var source = 'commondata.js.dataForHomePageNotificationScreen_FASTSECONDPASS()';
                var errorCode = null;
                WriteToErrorLog(threatLevel, source, errorCode, msg);
    ...
    WriteToErrorLog = function (threatLevel, source, errorCode, message, bwWorkflowAppId, bwParticipantId, request) {
        try {
            ...
            console.log('***********************************');
            console.log('***********************************');
            console.log('In webservices.commondata.js.WriteToErrorLog().');
            console.log('***********************************');
            console.log('***********************************');
            console.log('***********************************');
            ...
            if (!message) {
                ...
                message = message.substring(0, 4096); // 1024); // 7-14-2023. Prevents ERR_BUFFER_OUT_OF_BOUNDS: Attempt to write outside buffer bounds. 
