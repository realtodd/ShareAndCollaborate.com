$.widget("bw.bwProjectTypeCheckboxGroup", {
    options: {
        ...
        */
        ...
        /*
        ===========================================================
        This is the bwProjectTypeCheckboxGroup.js jQuery Widget. 
        ===========================================================
    ...
    _create: function () {
        this.element.addClass("bwProjectTypeCheckboxGroup");
        ...
        try {
            ...
            //    this.renderAndPopulateProjectTypeCheckboxGroup();
            //} else {
                $.ajax({
                    ...
                    try {
                        ...
                        if (result.message != 'SUCCESS') {
                            ...
                            //debugger;
                            thiz.options.jsonData = result.ProjectTypes;
                            //thiz.renderProjectTypeEditor();
                            thiz.renderAndPopulateProjectTypeCheckboxGroup();

                            console.log('In bwProjectTypeCheckboxGroup._create(). The dialog has been initialized.');
                        }
                    } catch (e) {
            ...
            var html = '';
            html += '<span style="font-size:24pt;color:red;">bwWorkflowPicker: CANNOT RENDER bwProjectTypeCheckboxGroup</span>';
            html += '<br />';
            html += '<span style="">Exception in bwProjectTypeCheckboxGroup.Create(): ' + e.message + ', ' + e.stack + '</span>';
            thiz.element.html(html);
        }
    },
    ...
    _destroy: function () {
        this.element
            .removeClass("bwProjectTypeCheckboxGroup")
            .text("");
    },
    getData: function () {
        try {
            console.log('In bwProjectTypeCheckboxGroup.getData().');
            //return 'This getData method is incomplete.';
            return null;
        } catch (e) {
            console.log('Exception in bwProjectTypeCheckboxGroup.getData(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwProjectTypeCheckboxGroup.getData(): ' + e.message + ', ' + e.stack);
        }
    },
    isARequiredField: function () {
        try {
            console.log('In bwProjectTypeCheckboxGroup.isARequiredField().');
            return this.options.renderAsARequiredField;
        } catch (e) {
            console.log('Exception in bwProjectTypeCheckboxGroup.isARequiredField(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwProjectTypeCheckboxGroup.isARequiredField(): ' + e.message + ', ' + e.stack);
        }
    },
    getfieldTitle: function () {
        try {
            console.log('In bwProjectTypeCheckboxGroup.getfieldTitle().');
            return this.options.fieldTitle;
        } catch (e) {
            console.log('Exception in bwProjectTypeCheckboxGroup.getfieldTitle(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwProjectTypeCheckboxGroup.getfieldTitle(): ' + e.message + ', ' + e.stack);
        }
    },
    renderAndPopulateProjectTypeCheckboxGroup: function (requestDialogId) {
        try {
            console.log('In renderAndPopulateProjectTypeCheckboxGroup().');
            var thiz = this;
            var html = '';

            html += '<table style="width:100%;">';
            html += '                        <tbody><tr class="xdTableOffsetRow" bwwidgetname="bwProjectTypeCheckboxGroup">';
            ...
            if (this.options.jsonData.length == 0) {
                //html += '   <option>No project types exist in the database.</option>';
                html += '<span style="white-space:nowrap;">' + 'No project types exist in the database.' + '</span>';
            } else {
                ...
                var conditionStringProjectTypes = []; // var conditionStringProjectTypes = ['xx-xxxxx-xx-xxxx-xx-xx,xx-xxxx-xx-xxxx-xx', 'xx-xxxxx-xx-xxxx-xx-xx,xx-xxxx-xx-xxxx-xx'];
                if (conditionString.includes('$ProjectTypes~')) {
                    var conditionStringArray = conditionString.split('&');
                    for (var i = 0; i < conditionStringArray.length; i++) {
                        if (conditionStringArray[i].indexOf('$ProjectTypes~') > -1) {
                            // We found the Project Type section.
                            var tmp = conditionStringArray[i].split('~')[1];
                            var projectTypeGuidsArray = tmp.split(',');
                            for (var j = 0; j < projectTypeGuidsArray.length; j++) {
                                conditionStringProjectTypes.push(String(projectTypeGuidsArray[j]));
                            }
                        } 
                    }
                }
                ...
                for (var i = 0; i < thiz.options.jsonData.length; i++) {
                    if (thiz.options.jsonData[i].isActive == true) {
                        if (conditionStringProjectTypes.includes(String(thiz.options.jsonData[i].bwProjectTypeId))) {
                            // Checked.
                            html += '<span style="white-space:nowrap;"><input bwProjectTypeId="' + thiz.options.jsonData[i].bwProjectTypeId + '" id="' + 'cbProjectType_' + thiz.options.jsonData[i].bwProjectTypeId + '" type="checkbox" checked onchange="$(\'.bwWorkflowEditor\').bwWorkflowEditor(\'cbProjectType_CheckChanged\', this);" />&nbsp;' + thiz.options.jsonData[i].Abbreviation + ' - ' + thiz.options.jsonData[i].ProjectType + '&nbsp;</span>';
                        } else {
                            // Not checked.
                            html += '<span style="white-space:nowrap;"><input bwProjectTypeId="' + thiz.options.jsonData[i].bwProjectTypeId + '" id="' + 'cbProjectType_' + thiz.options.jsonData[i].bwProjectTypeId + '" type="checkbox" onchange="$(\'.bwWorkflowEditor\').bwWorkflowEditor(\'cbProjectType_CheckChanged\', this);" />&nbsp;' + thiz.options.jsonData[i].Abbreviation + ' - ' + thiz.options.jsonData[i].ProjectType + '&nbsp;</span>';
                        }
                        html += '<br />';
                    }
                }
            ...
            ////debugger;
            ////$('.bwCoreComponent').bwCoreComponent('populateTheProjectTypeDropdown', 'selectProjectType', false); // Populate the project type drop-down.
            ...
            console.log('Exception in renderAndPopulateProjectTypeCheckboxGroup: ' + e.message + ', ' + e.stack);
        }
    }
