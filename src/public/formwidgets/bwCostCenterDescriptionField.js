$.widget("bw.bwCostCenterDescriptionField", {
    options: {
        ...
        */
        ...
        /*
        ===========================================================
        This is the bwCostCenterDescriptionField.js jQuery Widget. 
        ===========================================================
    ...
    _create: function () {
        this.element.addClass("bwCostCenterDescriptionField");
        ...
        try {
            ...
            if (this.options.inFormsEditor == true) {
                ...
                this.renderAndPopulateCostCenterDescriptionField_ReadOnly(); // Need to render, not allowing the user to make modifications.
            } else if (this.options.allowRequestModifications == false) {
                this.renderAndPopulateCostCenterDescriptionField_ReadOnly(); // Need to render, not allowing the user to make modifications.
            } else if (this.options.allowRequestModifications == true) {
                this.renderAndPopulateCostCenterDescriptionField();
            } else {
                var html = '';
                html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwCostCenterDescriptionField</span>';
                html += '<br />';
                html += '<span style="">Invalid value for allowRequestModifications: ' + this.options.allowRequestModifications + '</span>';
                this.element.html(html);
            }
            
            console.log('In bwCostCenterDescriptionField._create(). The widget has been initialized.');

        } catch (e) {
            var html = '';
            html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwCostCenterDescriptionField</span>';
            html += '<br />';
            html += '<span style="">Exception in bwCostCenterDescriptionField.Create(): ' + e.message + ', ' + e.stack + '</span>';
    ...
    _destroy: function () {
        this.element
            .removeClass("bwCostCenterDescriptionField")
            .text("");
    },
    getData: function () {
        try {
            console.log('In bwCostCenterDescriptionField.getData().');
            //return 'This getData method is incomplete.';
            return null;
        } catch (e) {
            console.log('Exception in bwCostCenterDescriptionField.getData(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwCostCenterDescriptionField.getData(): ' + e.message + ', ' + e.stack);
        }
    },
    isARequiredField: function () {
        try {
            console.log('In bwCostCenterDescriptionField.isARequiredField().');
            return this.options.renderAsARequiredField;
        } catch (e) {
            console.log('Exception in bwCostCenterDescriptionField.isARequiredField(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwCostCenterDescriptionField.isARequiredField(): ' + e.message + ', ' + e.stack);
        }
    },
    getfieldTitle: function () {
        try {
            console.log('In bwCostCenterDescriptionField.getfieldTitle().');
            return this.options.fieldTitle;
        } catch (e) {
            console.log('Exception in bwCostCenterDescriptionField.getfieldTitle(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwCostCenterDescriptionField.getfieldTitle(): ' + e.message + ', ' + e.stack);
        }
    },
    renderAndPopulateCostCenterDescriptionField: function (requestDialogId) {
        try {
            console.log('In renderAndPopulateCostCenterDescriptionField().');
            var thiz = this;
            var html = '';
            ...
            html += '                        <tbody><tr class="xdTableOffsetRow xdTableOffsetRow-editor" id="toolboxdraggablerow_2" draggable="true" bwwidgetname="bwCostCenterDescriptionField">';
            ...
            console.log('Exception in renderAndPopulateCostCenterDescriptionField: ' + e.message + ', ' + e.stack);
            var html = '';
            html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwCostCenterDescriptionField</span>';
            html += '<br />';
            html += '<span style="">Exception in bwCostCenterDescriptionField.renderAndPopulateCostCenterDescriptionField(): ' + e.message + ', ' + e.stack + '</span>';
            thiz.element.html(html);
        }
    },

    renderAndPopulateCostCenterDescriptionField_ReadOnly: function (requestDialogId) {
        try {
            console.log('In renderAndPopulateCostCenterDescriptionField_ReadOnly().');
            var thiz = this;
            var html = '';
            //html += '<div style="white-space:nowrap;">';
            //html += '   <input type="text" class="bwRequestJson" bwDataRequired="true" bwDataType="string" bwFieldname="ProjectTitle" id="strProjectTitle" onkeyup="$(\'.bwRequest\').bwRequest(\'editTextbox_OnKeyUp\', this);" style="WIDTH: 100%;font-family: \'Segoe UI Light\',\'Segoe UI\',\'Segoe\',Tahoma,Helvetica,Arial,sans-serif;color: #262626;font-size: 20pt;" /> <!-- was 2.77em -->';
            ...
            console.log('Exception in renderAndPopulateCostCenterDescriptionField_ReadOnly: ' + e.message + ', ' + e.stack);
            var html = '';
            html += '<span style="font-size:24pt;color:red;">CANNOT RENDER bwCostCenterDescriptionField</span>';
            html += '<br />';
            html += '<span style="">Exception in bwCostCenterDescriptionField.renderAndPopulateCostCenterDescriptionField_ReadOnly(): ' + e.message + ', ' + e.stack + '</span>';
            thiz.element.html(html);
        }
    }
