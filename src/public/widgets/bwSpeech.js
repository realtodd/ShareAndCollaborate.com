$.widget("bw.bwSpeech", {
    options: {
        ...
        */
        ...
        /*
        ===========================================================
        This is the bwSpeech.js jQuery Widget. 
        ===========================================================
        ...
        */
        ...
        KaldiRecognizer: null, // This is a vosk object. We set this to null to stop listening.
        AudioContext: null,
    ...
    _create: function () {
        //alert('In bwSpeech._create().');
        this.element.addClass("bwSpeech");
        //debugger;
        ...
        try {
            ...
            //html += '<style>';
            //html += 'vosk-recognizer {';
            //html += '    --font-size: 2rem;';
            //html += '    background-color: #fff;';
            //html += '    border-radius: 5px;';
            //html += '    border: 1px solid #aaa;';
            ...
            //html += '}';
            //html += '</style>';
            ////html += '<vosk-recognizer model-url="model.tar.gz" oneshot></vosk-recognizer>';
            //html += '<vosk-recognizer model-url="model.tar.gz" ></vosk-recognizer>';
            //html += '<script src="VoskJS.js"></script>';
            ...
            console.log('In bwSpeech._create(). The widget has been initialized.');

        } catch (e) {
            var html = '';
            html += '<span style="font-size:24pt;color:red;">bwSpeech: CANNOT INITIALIZE widget bwSpeech.js.</span>';
            html += '<br />';
            html += '<span style="">Exception in bwSpeech.Create(): ' + e.message + ', ' + e.stack + '</span>';
    ...
    _destroy: function () {
        this.element
            .removeClass("bwSpeech")
            .text("");
    },

    AquireSpeech: function () {
        try {
            console.log('In bwSpeech.js.AquireSpeech(). PUT THE CURSOR SOMEWHERE, AND START TALKING!!!!!!!!!!!!!!!!!!!!!!!!!!!');
            alert('In bwSpeech.js.AquireSpeech(). PUT THE CURSOR SOMEWHERE, AND START TALKING!!!!!!!!!!!!!!!!!!!!!!!!!!!');
            var thiz = this;

            $('.ai_conversation_icon').css('fontSize', '150pt'); // This is our visual indicator that sherry is listening.

            async function initializeVosk() {
                try {
                    console.log('In bwSpeech.js.AquireSpeech.initializeVosk().');

                    //const resultsContainer = document.getElementById('recognition-result');

                    const channel = new MessageChannel();
                    const model = await Vosk.createModel('https://shareandcollaborate.com/vosk/vanilla/vosk-model-small-en-us-0.15.tar.gz');
                    model.registerPort(channel.port1);

                    const sampleRate = 48000;

                    thiz.options.KaldiRecognizer = new model.KaldiRecognizer(48000); // sampleRate);
                    thiz.options.KaldiRecognizer.setWords(true);

                    //$('.bwNotificationSound').bwNotificationSound('playNotificationSound', 'bloop-starting');

                    thiz.options.SpeechRecognitionIsActive = true;

                    //setTimeout(function () {
                    //    $('.bwNotificationSound').bwNotificationSound('playNotificationSound', 'bloop-working-on-it');
                    //}, 1000);

                    //
                    //
                    // Here we need to determine if any requests are open, or if the user is on the Home screen.
                    // The use case: A user notices they need to buy more honey at the store. They select the app, click the top-left-bluebar-logo, then speak. NO MORE INTERACTION NEEDED, and say, "We need to buy more honey."
                    //     - If there are no request dialogs open, and they are on the home screen, take them to the new request screen, and plop them into the "Justification Details" section.
                    //     - They can click "Save" or "Submit" as soon as they are done, and return to the Home screen.
                    //
                    //

                    // Check if there are any requests dialogs open:
                    var formsOpenInADialog = [];
                    var forms = $(document).find('.budgetrequestform');
                    for (var i = 0; i < forms.length; i++) {
                        var bwrequesttitle = $(forms[i]).attr('bwrequesttitle');
                        if (bwrequesttitle != 'New') { // This makes sure we don't include the NEW REQUEST form, which may be open/displayed.
                            formsOpenInADialog.push(forms[i]);
                        }
                    }

                    console.log('In bwSpeech.js.AquireSpeech.initializeVosk(). formsOpenInADialog.length: ' + formsOpenInADialog.length);
                    if (formsOpenInADialog.length == 0) {

                        console.log('bwSpeech.js.AquireSpeech.initializeVosk(). There are no request dialogs open, and the NEW REQUEST screen isn\'t displayed (it also uses the budgetrequestform class).');

                        // Now check if we are on the Home screen.
                        ...
                        console.log('In bwSpeech.js.AquireSpeech.initializeVosk(). workflowAppTheme_SelectedButton: ' + workflowAppTheme_SelectedButton);
                        if (!$('#divNewRequestButton').hasClass(workflowAppTheme_SelectedButton)) {

                            console.log('In bwSpeech.js.AquireSpeech.initializeVosk(). We are not on the new-request screen, so displaying a New Request, with the cursor in the Justification Details form widget.');
                            //alert('In bwSpeech.js.AquireSpeech.initializeVosk(). We are not on the new-request screen, so displaying a New Request, with the cursor in the Justification Details form widget.');

                            $('.bwActiveMenu').bwActiveMenu('RenderContentForButton', null, 'NEW_REQUEST');

                            setTimeout(function () {
                                $('#JustificationDetails').focus();
                            }, 500);
                            ...
                            console.log('In bwSpeech.js.AquireSpeech.initializeVosk(). We are on the NEW REQUEST screen.');

                            // We are on the New Request screen. We will make sure that the spoken text shows up in the Justification Details widget.
                        ...
                        alert('In bwSpeech.js.AquireSpeech.initializeVosk(). There are form(s) open in a dialog. We need to add new code here to accomodate this use case. xcx555867.');

                    }


                    thiz.options.KaldiRecognizer.on("result", (message) => {
                        try {
                            const result = message.result;

                            if (result && result.text) {
                                console.warn('In bwSpeech.js.initializeVosk.on.result(): ' + JSON.stringify(result, null, 2));

                                if (thiz.options.SpeechRecognitionIsActive == true) {

                                    //
                                    // SPEECH RECOGNITION IS ACTIVE, SO OUTPUT TO THE ELEMENT WITH FOCUS.
                                    //

                                    var isEditable = $(document.activeElement).is("input") || $(document.activeElement).is("textarea") || $(document.activeElement).is("div") || $(document.activeElement).is("span");

                                    if (isEditable == true) {

                                        if ($(document.activeElement).is("input")) {

                                            if (document.activeElement.id && (document.activeElement.id == 'inputBwAuthentication_SearchBox')) {

                                                // This is the search box. This behavior is better for this one.
                                                document.activeElement.value = result.text;

                                            } else {

                                                var oldText = document.activeElement.value;
                                                var newText = oldText + '<br />' + result.text;
                                                document.activeElement.value = newText;

                                            }

                                        } else if ($(document.activeElement).is("textarea")) {

                                            var oldText = document.activeElement.innerHTML;
                                            var newText = oldText + '<br />' + result.text;
                                            document.activeElement.innerHTML = newText;

                                        } else if ($(document.activeElement).is("div")) {

                                            //
                                            // Change code so that it puts it at the cursor location, not at the end of the text. 10-18-2023.
                                            //
                                            ...
                                            //    var startText = document.activeElement.innerHTML.substring(0, startPos);
                                            //    var endText = document.activeElement.innerHTML.substring(0, endPos);

                                            //    var newText = startText + '<br />' + result.text + '<br />' + endText;
                                            //    document.activeElement.innerHTML = newText;

                                            //} else {

                                            // Couldn't get a cursor position, so just p[utting it at the end.
                                            var oldText = document.activeElement.innerHTML;
                                            var newText = oldText + '<br />' + result.text;
                                            document.activeElement.innerHTML = newText;
                                            ...
                                            if (document.activeElement.id && (document.activeElement.id == 'JustificationDetails')) {

                                                // This makes sure that this element is expanded and scrolls so that the user can see the speech recognition text as they speak.
                                                var imgElement = $(document.activeElement).closest('tr').find('#imgExpandJustificationDetails')[0];
                                                if (imgElement.src.indexOf('plus-sign.png') > -1) {

                                                    // This means the element is collapsed. Call this method to expand it.
                                                    $(document.activeElement).closest('.bwJustificationDetailsField').bwJustificationDetailsField('ToggleExpandJustificationDetails');

                                                }

                                            }

                                        } else if ($(document.activeElement).is("span")) {

                                            if (document.activeElement.id && (document.activeElement.id == 'strProjectTitle')) {

                                                document.activeElement.innerHTML = result.text;
                                            ...
                                            alert('xcx21343462-1. Unexpected value for document.activeElement.id: ' + document.activeElement.id);

                                        }

                                    } else {

                                        console.warn('xcx21343462-2. NO ELEMENT HAS BEEN SELECTED FOR TEXT OUTPUT FROM SPEECH RECOGNITION. SELECT AN ELEMENT NOW.');

                                    }

                                } else {

                                    //var wordsArray = ['share it', 'sharey', 'sherry', 'surely', 'cherry', 'terry', 'carrie', 'new request'];

                                    ////
                                    //// THIS OPENS US UP FOR A LOT OF IDEAS. ['new invoice', [New {formwidget name/request type} ], ] // 10-14-2023.
                                    ////

                                    //for (var i = 0; i < wordsArray.length; i++) {
                                    //    if (result.text.toLowerCase().indexOf(wordsArray[i]) > -1) {

                                    //        //$('.bwNotificationSound').bwNotificationSound('playNotificationSound', 'starpulse');
                                    //        $('.bwNotificationSound').bwNotificationSound('playNotificationSound', 'bloop-starting');
                            ...
                            console.log('Exception in bwSpeech.js.initializeVosk(). xcx886754. ' + e.message + ', ' + e.stack);
                            $('.ai_conversation_icon').css('fontSize', '25pt'); // This is our visual indicator that sherry is NOT listening.
                        }
                    });

                    thiz.options.KaldiRecognizer.on("partialresult", (message) => {
                        try {
                            ...
                            if (partial) {
                                console.info('In bwSpeech.js.initializeVosk.on.partialresult(): ' + partial);
                            }
                            //partialContainer.textContent = partial;
                        } catch (e) {
                            console.log('Exception in bwSpeech.js.initializeVosk()). xcx56345. ' + e.message + ', ' + e.stack);
                            $('.ai_conversation_icon').css('fontSize', '25pt'); // This is our visual indicator that sherry is NOT listening.
                        }
                    });
                    ...
                    thiz.options.mediaDevices_getUserMedia_stream = await navigator.mediaDevices.getUserMedia({
                        video: false,
                        audio: {
                    ...
                    //    try {
                    //        console.log('In bwSpeech.js.startup.navigator.mediaDevices.getUserMedia().');

                    //        thiz.options.mediaDevices_getUserMedia_stream = stream;


                    //        thiz.options.AudioContext = new AudioContext();

                    //        await thiz.options.AudioContext.audioWorklet.addModule('https://shareandcollaborate.com/vosk/vanilla/recognizer-processor.js');

                    //        thiz.options.RecognizerProcessor = new AudioWorkletNode(thiz.options.AudioContext, 'recognizer-processor', { channelCount: 1, numberOfInputs: 1, numberOfOutputs: 1 });

                    //        thiz.options.RecognizerProcessor.port.postMessage({ action: 'init', recognizerId: thiz.options.KaldiRecognizer.id }, [channel.port2])
                    //        thiz.options.RecognizerProcessor.connect(thiz.options.AudioContext.destination);

                    //        thiz.options.Source = thiz.options.AudioContext.createMediaStreamSource(mediaStream);

                    //        thiz.options.Source.connect(thiz.options.RecognizerProcessor);

                    //    } catch (e) {

                    //        alert('xcx2123123 exception: ' + e.message + ', ' + e.stack);
                    //    }

                    //}).catch(function (e) {

                    //    alert('exception xcx213211241244 : ' + JSON.stringify(e));
                    ...
                    thiz.options.Source = thiz.options.AudioContext.createMediaStreamSource(thiz.options.mediaDevices_getUserMedia_stream);

                    thiz.options.Source.connect(thiz.options.RecognizerProcessor);

                    //thiz.options.mediaDevices_getUserMedia_stream = thiz.options.AudioContext.createMediaStreamSource(stream);

                    //thiz.options.mediaDevices_getUserMedia_stream.connect(thiz.options.RecognizerProcessor);



                } catch (e) {
                    console.error('Exception in bwSpeech.js.initializeVosk(). xcx231234. ' + e.message + ', ' + e.stack);
                    $('.ai_conversation_icon').css('fontSize', '25pt'); // This is our visual indicator that sherry is NOT listening.
                }
            }
            ...
            console.log('Exception in bwSpeech.js.AquireSpeech(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwSpeech.js.AquireSpeech(): ' + e.message + ', ' + e.stack);
        }
    },
    ...
    StopListening: function () {
        try {
            console.log('In bwSpeech.js.StopListening().');
            var thiz = this;

            thiz.options.SpeechRecognitionIsActive = false;

            //var isEditable = $(document.activeElement).is("input") || $(document.activeElement).is("textarea");

            $('.bwNotificationSound').bwNotificationSound('playNotificationSound', 'bloop-ending');

            $('.ai_conversation_icon').css('fontSize', '25pt'); // This is our visual indicator that sherry is listening.

            thiz.options.AudioContext.close(); // Not sure if this line does anything... 11-7-2023.
            ...
            console.log('Exception in bwSpeech.js.StopListening(): ' + e.message + ', ' + e.stack);
            alert('Exception in bwSpeech.js.StopListening(): ' + e.message + ', ' + e.stack);
        }
    },
    ...
    renderPrintButton: function () {
        try {
            ...
            if (reportType == 'CurrentYearBudgetRequestsReport') {
                html += '<span title="print" class="printButton" onclick="$(\'.bwSpeech\').bwSpeech(\'PrintCurrentYearBudgetRequestsReport\');">🖨</span>';
            } else if (reportType == 'MyPendingTasksReport') {
                html += '<span title="print" class="printButton" onclick="$(\'.bwSpeech\').bwSpeech(\'PrintMyPendingTasksReport\');">🖨</span>';
            } else if (reportType == 'InProcessBudgetRequestsReport') {
                html += '<span title="print" class="printButton" onclick="$(\'.bwSpeech\').bwSpeech(\'PrintInProcessBudgetRequestsReport\');">🖨</span>';
            } else if (reportType == 'IndividualRequestReport') {
                html += '<span title="print" class="printButton" onclick="$(\'.bwSpeech\').bwSpeech(\'PrintIndividualRequestReport\');" style="font-size:22pt;">🖨&nbsp;</span>'; // Different size because this one shows up in the dialog draggable top bar header.
            } else {
                                                    ...
                                                    try {
                                                        ...
                                                        var printNow = function () {
                                                            setTimeout(function () { // Only needs to happen for Chrome.
                                                                ...
                                                                //window.frames["printf"].print();
                                                                try {
                                                                    console.log('bwSpeech.PrintMyPendingTasksReport(): Try #1.');
                                                                    window.frames["printf"].contentWindow.print(); // Confirmed 5-14-2020. Works in Edge.
                                                                } catch (e) {
                                                                    try {
                                                                        console.log('bwSpeech.PrintMyPendingTasksReport(): Try #2.');
                                                                        ...
                                                                        console.log('bwSpeech.PrintMyPendingTasksReport(): Try #3.');
                                                                        window.print(); // In Safari on MacBook, and Safari & Chrome on iOS, this is a synchronous call!!! (unlike most other situations where it is asynchronous).
                                                                    }
                                                                ...
                                                                //};
                                                                //var $printbutton = $('#spanArchivePagePrintButton').bwSpeech(printButtonOptions);
        ...
        try {
            ...
            window.onafterprint = function (e) {
                try {
                    ...
                    //};
                    //var $printbutton = $('#spanRequestPrintButton').bwSpeech(printButtonOptions);
    ...
    expandHomePageNotificationAndRenderAlerts_Printed: function (notificationType, deferredIndex, functionalAreaRowId) {
        ...
        try {
            console.log('In bwSpeech.expandHomePageNotificationAndRenderAlerts_Printed(' + notificationType + ', ' + deferredIndex + ', ' + functionalAreaRowId + ').');
            //debugger;
            //displayAlertDialog('type:' + notifications);
            ////displayAlertDialog('text:' + notifications);
            //displayAlertDialog('notifications[2].length:' + notifications.length);
            ...
            //handleExceptionWithAlert('Error in my.js.expandHomePageNotificationAndRenderAlerts()', '2:' + e.message);
            console.log('Exception in bwSpeech.expandHomePageNotificationAndRenderAlerts_Printed() 2:' + e.message + ' ' + e.stack + ' This is not necessarily a service unavailable error but we dont want to bother the user!!');
            displayAlertDialog('Exception in bwSpeech.expandHomePageNotificationAndRenderAlerts_Printed() 2:' + e.message + ' ' + e.stack + ' This is not necessarily a service unavailable error but we dont want to bother the user!!');
