$.widget("bw.bwNewUserWorkflowEditor", {
    options: {
        ...
        */
        ...
        /*
        ===========================================================
        This is the bwNewUserWorkflowEditor.js jQuery Widget. 
        ===========================================================
    ...
    _create: function (assignmentRowChanged_ElementId) {
        this.element.addClass("bwNewUserWorkflowEditor");
        //debugger;
        //var thiz = this; // Need this because of the asynchronous operations below.
        try {
            console.log('In bwNewUserWorkflowEditor._create().');
            ...
            this.renderBusinessModelEditor();

        } catch (e) {
            var html = '';
            html += '<span style="font-size:24pt;color:red;">bwNewUserWorkflowEditor: CANNOT INITIALIZE HOME PAGE</span>';
            html += '<br />';
            html += '<span style="">Exception in bwNewUserWorkflowEditor.Create(): ' + e.message + ', ' + e.stack + '</span>';
    ...
    _destroy: function () {
        this.element
            .removeClass("bwNewUserWorkflowEditor")
            .text("");
    },
    ...
    deployNewUserExperience: function () {
        try {
            ...
            console.log('In bwNewUserWorkflowEditor.deployNewUserExperience(). appIdAndAppTitle: ' + appIdAndAppTitle);
            ...
            console.log('Exception in bwNewUserWorkflowEditor.deployNewUserExperience(): ' + e.message + ', ' + e.stack);
        }
    },
    ...
    renderBusinessModelEditor: function () {
        try {
            //debugger;
            console.log('In renderBusinessModelEditor().');
            var html = '';

            //html += 'In bwNewUserWorkflowEditor.renderBusinessModelEditor(). Configure new Tenant Organization and Roles... THIS FUNCTIONALITY IS INCOMPLETE. This will allow a forest administrator to edit and choose the business model that will be initially presented to a new user. New users need a default organization!!! eg: Stark Enterprises.';


            html += '<div style="display:none;" id="divConfigureWorkflowNotificationsDialog">';
            html += '   <table style="width:100%;">';
            html += '       <tr>';
            html += '           <td style="width:90%;">';
            html += '               <span style="font-family: \'Segoe UI Light\',\'Segoe UI\',\'Segoe\',Tahoma,Helvetica,Arial,sans-serif;color: #3f3f3f;font-size:30pt;font-weight:bold;">Configure new tenant "Introductory Workflows"...</span>';
            html += '               <br />';
            ...
            html += '               <span class="dialogXButton" style="font-family: \'Segoe UI Light\',\'Segoe UI\',\'Segoe\',Tahoma,Helvetica,Arial,sans-serif;font-size:30pt;font-weight:bold;" onclick="$(\'#divConfigureWorkflowNotificationsDialog\').dialog(\'close\');">X</span>';
            html += '           </td>';
            html += '       </tr>';
            html += '   </table>';
            ...
            //html += '  </td>';
            //html += '  <td>';
            //html += '    <span onclick="$(\'.bwWorkflowEditor\').bwWorkflowEditor(\'renderTreeviewWithNewJson\');" style="cursor:pointer;color:tomato;font-size:smaller;border:1px solid goldenrod;padding:3px 13px 5px 13px;">RE-RENDER</span>';
            //html += '  </td>';
            //html += '</tr>';
            //html += '</table>';
            ...
            html += '   <br />';
            html += '   <span id="spanConfigureWorkflowsConfigurationDialogContent">[spanConfigureWorkflowsConfigurationDialogContent]</span>';
            html += '   <br />';
            html += '   <br />';
            ...
            //html += '<br />';
            //html += '<span title="Edit email template(s)..." class="spanButton" onclick="$(\'.bwNewUserWorkflowEditor\').bwNewUserWorkflowEditor(\'editStepEmails\', \'Create\');"><span style="font-size:15pt;display:inline-block;">✉</span> "New User" Business Model&nbsp;&nbsp;</span>';
            ...
            console.log('Exception in renderBusinessModelEditor(): ' + e.message + ', ' + e.stack);
        }
    },
            ...
            try {
                ...
                $.ajax({
                    ...
                    try {
                        ...
                        $("#divConfigureWorkflowNotificationsDialog").dialog({
                            modal: true,
                            resizable: true,
                            //closeText: "Cancel",
                            ...
                            open: function () {
                                $('.ui-widget-overlay').bind('click', function () {
                                    $('#divConfigureWorkflowNotificationsDialog').dialog('close');
                                });
                            ...
                            close: function () {
                                $('#divConfigureWorkflowNotificationsDialog').dialog('destroy');
                            }
                        });
                        $('#divConfigureWorkflowNotificationsDialog').dialog().parents('.ui-dialog').find('.ui-dialog-titlebar').remove();
                    } catch (e) {
                        //lpSpinner.Hide();
    ...
    editStepEmails: function (stepName) {
        try {
            ...
            $("#divConfigureWorkflowNotificationsDialog").dialog({
                ...
                //closeText: "Cancel",
                closeOnEscape: false, // Hit the ESC key to hide! Yeah!
                //title: 'divConfigureWorkflowNotificationsDialog',
                width: '1200',
                dialogClass: 'no-close', // No close button in the upper right corner.
                hide: false, // This means when hiding just disappear with no effects.
                open: function () {
                    try {
                        ...
                        var html = '';
                        html += '<div id="divBwWorkflowEditor">[divBwWorkflowEditor]</div>';

                        document.getElementById('spanConfigureWorkflowsConfigurationDialogContent').innerHTML = html;
                        ...
                        var $bwworkfloweditor = $("#divBwWorkflowEditor").bwWorkflowEditor(options);
                        ...
                        console.log('Exception in editStepEmails.divConfigureWorkflowNotificationsDialog.open(): ' + e.message + ', ' + e.stack);
                    }
                },
                close: function () {
                    $('#divConfigureWorkflowNotificationsDialog').dialog('destroy');
    ...
    publishNewUserWorkflowConfigurationAndActivate: function () {
        var thiz = this;
        try {
            //debugger;
            console.log('In publishNewUserWorkflowConfigurationAndActivate().');
            ...
            //debugger;
            var bwWorkflowJson = $("#divBwWorkflowEditor").bwWorkflowEditor('getCurrentWorkflowJson');
            //debugger;
            var json = {
                //Created: dtNow,
                ...
                bwWorkflowJson: JSON.stringify(bwWorkflowJson), // JSON.stringify(thiz.options.CurrentWorkflow.Workflow), //, //.bwWorkflowJson, //JSON.stringify(thiz.options.CurrentWorkflow), // WE KNOW WE ARE NOT GETTING THE CORRECT JSON HERE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                bwRequestType: bwRequestType 
            };
            //debugger;
            $.ajax({
                url: thiz.options.operationUriPrefix + "_bw/PublishNewTenantWorkflowDefault",
                type: "Post",
                ...
                try {
                    //debugger;
                    //thiz.hideProgress();
                    console.log('In publishNewUserWorkflowConfigurationAndActivate().post: Successfully updated DB. result: ' + JSON.stringify(result)); // using (' + JSON.stringify(json) + ').');
                    ...
                    //debugger; // 6-1-2020 it works to this point!!
                    ...
                    //debugger; // make sure the request type is correct!!!!!!!!!!!!!!!!!!!!!
                    //var promise = thiz.loadWorkflowsAndCurrentWorkflow(bwRequestType); // This is the default.
                    ...
                    console.log('Exception in publishNewUserWorkflowConfigurationAndActivate().xx.update: ' + e.message + ', ' + e.stack);
                    alert('Exception in publishNewUserWorkflowConfigurationAndActivate().xx.update: ' + e.message + ', ' + e.stack);
                }
                ...
                console.log('Fail in publishNewUserWorkflowConfigurationAndActivate().xx.update: ' + JSON.stringify(data) + ', json: ' + JSON.stringify(json));
                alert('Fail in publishNewUserWorkflowConfigurationAndActivate().xx.update: ' + msg); //+ error.message.value + ' ' + error.innererror.message);
                //console.log('Fail in CarForm3.aspx.populateSpendForecastNotReadOnly().spendForecastItems.update: ' + JSON.stringify(data));
                //var error = JSON.parse(data.responseText)["odata.error"];
                //alert('Exception in CarForm3.aspx.populateSpendForecastNotReadOnly().spendForecastItems.update: ' + error.message.value + ' ' + error.innererror.message); // + ', EntityValidationErrors: ' + data.EntityValidationErrors);
            });
            ...
            debugger;
            //thiz.hideProgress();
            alert('Exception in publishNewUserWorkflowConfigurationAndActivate(): ' + e.message + ', ' + e.stack);
            console.log('Exception in publishNewUserWorkflowConfigurationAndActivate(): ' + e.message + ', ' + e.stack);
        }
