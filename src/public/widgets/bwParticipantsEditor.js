$.widget("bw.bwParticipantsEditor", {
    ...
    loadAndRenderParticipantsEditor: function (force) {
        try {
            console.log('In bwParticipantsEditor.loadAndRenderParticipantsEditor().');
            var thiz = this;
            ...
            if ((!force) && (this.options.store != null)) {
                ...
                $.ajax({
                    ...
                    error: function (data, errorCode, errorMessage) {
                        var msg = 'Error in bwParticipantsEditor.js.loadAndRenderParticipantsEditor(): ' + errorCode + ', ' + errorMessage + ', data: ' + JSON.stringify(data);
                        console.log(msg);
            ...
            console.log('Exception in bwParticipantsEditor.js.loadAndRenderParticipantsEditor(): ' + e.message + ', ' + e.stack);
            displayAlertDialog('Exception in bwParticipantsEditor.js.loadAndRenderParticipantsEditor(): ' + e.message + ', ' + e.stack);
        }
    },
        ...
        try {
            ...
            for (var i = 0; i < pdata.length; i++) {
                ...
                if (alternatingRow == 'light') {
                    ...
                    html += '           onclick="$(\'.bwCircleDialog2\').bwCircleDialog2(\'displayParticipantRoleMultiPickerInACircle\', true, \'' + 'btnEditRaciRoles_' + pdata[i].bwParticipantId + '\', \'' + pdata[i].bwParticipantId + '\', \'' + pdata[i].bwParticipantFriendlyName + '\', \'' + pdata[i].bwParticipantEmail + '\', \'' + pdata[i].bwParticipantLogonType + '\');" ';
                    html += '       />';
                    html += '   </td>';
                    alternatingRow = 'dark';
                } else {
                ...
                if ((pdata[i].bwParticipantRole == 'emailrecipient')) {
                    ...
                    // Joined the organization.
                    var timestamp4 = bwCommonScripts.getBudgetWorkflowStandardizedDate(pdata[i].Created);
                    html += '    <td style="cursor:pointer;" onclick="$(\'.bwCircleDialog2\').bwCircleDialog2(\'displayParticipantRoleMultiPickerInACircle\', true, \'' + 'btnEditRaciRoles_' + pdata[i].bwParticipantId + '\', \'' + pdata[i].bwParticipantId + '\', \'' + pdata[i].bwParticipantFriendlyName + '\', \'' + pdata[i].bwParticipantEmail + '\', \'' + pdata[i].bwParticipantLogonType + '\');">' + timestamp4 + '</td>';
                    ...
                    html += '    <td style="cursor:pointer;" onclick="$(\'.bwCircleDialog2\').bwCircleDialog2(\'displayParticipantRoleMultiPickerInACircle\', true, \'' + 'btnEditRaciRoles_' + pdata[i].bwParticipantId + '\', \'' + pdata[i].bwParticipantId + '\', \'' + pdata[i].bwParticipantFriendlyName + '\', \'' + pdata[i].bwParticipantEmail + '\', \'' + pdata[i].bwParticipantLogonType + '\');">' + _logonType + '</td>';
            ...
            $.ajax({
                ...
                success: function (result) {
                    try {
                        ...
                        // Rebasing. This seems to happen still, so figure it out osmetime 3-28-2024.
                        var json;
                        if (result.value && result.value.Global) {
                            json = result.value;
                        } else {
                            json = {
                                Global: result.value
                        ...
                        if (theRoleAssignmentExistsHereAlready == true) {
                            ...
                            //}, success: function (result) {
                            $.ajax({
                                url: thiz.options.operationUriPrefix + "_bw/SaveBwOrgRoles_With_SynchronizeParticipantOrgRoles",
                                type: "POST",
                                data: data,
                                headers: {
                                    "Accept": "application/json; odata=verbose"
                        ...
                        console.log('Exception in bwParticipantsEditor.js.assignParticipantToOrgRole():3: ' + e.message + ', ' + e.stack);
                        displayAlertDialog('Exception in bwParticipantsEditor.js.assignParticipantToOrgRole():3: ' + e.message + ', ' + e.stack);
                    }
                },
                error: function (data, errorCode, errorMessage) {
                    try {
                        ...
                        //HideActivitySpinner();
                        //debugger;
                        //console.log('In bwParticipantsEditor.js.assignParticipantToOrgRole.Get.OrgRolesConfiguration.fail(): ' + JSON.stringify(data));
                        //var msg;
                        //if (JSON.stringify(data).indexOf('The specified URL cannot be found.') > -1) {
                        //    msg = 'There has been an error contacting the server. A firewall or network appliance may be interrupting this traffic.';
                        //} else {
                        //    msg = JSON.stringify(data);
                        //}
                        //alert('Error in bwParticipantsEditor.js.assignParticipantToOrgRole.Get.OrgRolesConfiguration.fail(): ' + msg); //+ error.message.value + ' ' + error.innererror.message);
                        //console.log('Error in bwParticipantsEditor.js.assignParticipantToOrgRole.Get.OrgRolesConfiguration.fail(): ' + JSON.stringify(data));
                        ////console.log('Fail in CarForm3.aspx.populateSpendForecastNotReadOnly().spendForecastItems.update: ' + JSON.stringify(data));
                        ////var error = JSON.parse(data.responseText)["odata.error"];
                        ////alert('Exception in CarForm3.aspx.populateSpendForecastNotReadOnly().spendForecastItems.update: ' + error.message.value + ' ' + error.innererror.message); // + ', EntityValidationErrors: ' + data.EntityValidationErrors);
                    } catch (e) {
                        console.log('Exception in bwParticipantsEditor.js.assignParticipantToOrgRole():2: ' + e.message + ', ' + e.stack);
                        displayAlertDialog('Exception in bwParticipantsEditor.js.assignParticipantToOrgRole():2: ' + e.message + ', ' + e.stack);
                    }
                }
            });
    ...
    participantOrgRoleCheckbox_OnChange: function (checkboxElement, bwParticipantId, bwParticipantFriendlyName, bwParticipantEmail, bwRoleId, bwRoleName) {
        try {
            ...
            alert('In bwParticipantsEditor.js.participantOrgRoleCheckbox_OnChange(). bwParticipantId: ' + bwParticipantId + ', bwParticipantFriendlyName: ' + bwParticipantFriendlyName + ', bwParticipantEmail: ' + bwParticipantEmail + ', bwRoleId: ' + bwRoleId + ', bwRoleName: ' + bwRoleName);
